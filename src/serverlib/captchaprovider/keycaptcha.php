...
class BMCaptchaProvider_KeyCAPTCHA extends BMAbstractCaptchaProvider {
    ...
    public function getHTML() {
        ...
        $sessionID = uniqid();
        $wSign = md5(
            $sessionID . $_SERVER['REMOTE_ADDR'] . $config['privateKey'],
        );
    ...
    public function check($release = true) {
        ...
        if (
            $vars[0] ===
            md5('accept' . $vars[1] . $config['privateKey'] . $vars[2])
        ) {
            if (strpos($vars[2], 'http://') !== 0) {
                PutLog(
                    'KeyCAPTCHA: Please enable the "Allow outgoing requests" setting at www.keycaptcha.com!',
                    PRIO_WARNING,
                    __FILE__,
                    __LINE__,
                );
                ...
                $http = _new('BMHTTP', [$vars[2]]);
                $resp = $http->DownloadToString();

                PutLog(
                    sprintf('KeyCAPTCHA response data: "%s"', $resp),
                    PRIO_DEBUG,
                    __FILE__,
                    __LINE__,
                );
            ...
            PutLog('KeyCAPTCHA: MD5 mismatch', PRIO_DEBUG, __FILE__, __LINE__);
        }
