...
class BMTodo {
    ...
    function GetTodoList(
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,beginn,faellig,akt_status,titel,priority,erledigt,comments,dav_uri,dav_uid FROM {pre}tasks WHERE user=? AND tasklistid=?' .
                $queryAdd .
                ' ORDER BY ' .
                $sortColumn .
                ' ' .
    ...
    function GetUndoneTaskCount() {
        ...
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}tasks WHERE user=? AND akt_status!=?',
            $this->_userID,
            TASKS_DONE,
        );
        [$taskCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    function Add(
        ...
        $db->Query(
            'INSERT INTO {pre}tasks(user,beginn,faellig,akt_status,titel,priority,erledigt,comments,tasklistid,dav_uri,dav_uid) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
            $this->_userID,
            (int) $beginn,
            (int) $faellig,
            (int) $akt_status,
    ...
    function Change(
        ...
        $db->Query(
            'UPDATE {pre}tasks SET beginn=?,faellig=?,akt_status=?,titel=?,priority=?,erledigt=?,comments=?,tasklistid=? WHERE id=? AND user=?',
            (int) $beginn,
            (int) $faellig,
            (int) $akt_status,
            $titel,
    ...
    function SetStatus($id, $status) {
        ...
        $db->Query(
            'UPDATE {pre}tasks SET akt_status=? WHERE id=? AND user=?',
            (int) $status,
            (int) $id,
            $this->_userID,
        );
    ...
    function Delete($id) {
        ...
        $db->Query(
            'DELETE FROM {pre}tasks WHERE id=? AND user=?',
            (int) $id,
            $this->_userID,
        );
        if ($db->AffectedRows() == 1) {
    ...
    function GetTask($id) {
        ...
        $res = $db->Query(
            'SELECT id,beginn,faellig,akt_status,titel,priority,erledigt,comments,tasklistid,dav_uri,dav_uid FROM {pre}tasks WHERE id=? AND user=?',
            (int) $id,
            $this->_userID,
        );
        if ($res->RowCount() == 0) {
    ...
    function GetTaskLists() {
        ...
        $res = $db->Query(
            'SELECT `tasklistid`,`title`,`dav_uri` FROM {pre}tasklists WHERE `userid`=? ORDER BY `tasklistid` ASC',
            $this->_userID,
        );
    ...
    function AddTaskList($title, $davURI = '') {
        ...
        $db->Query(
            'INSERT INTO {pre}tasklists(`userid`,`title`,`dav_uri`) VALUES(?,?,?)',
            $this->_userID,
            $title,
            $davURI,
        );
    ...
    function ChangeTaskList($taskListID, $title) {
        ...
        $db->Query(
            'UPDATE {pre}tasklists SET `title`=? WHERE `userid`=? AND `tasklistid`=?',
            $title,
            $this->_userID,
            $taskListID,
        );
    ...
    function DeleteTaskList($taskListID, $deleteTasks = true) {
        ...
        if ($deleteTasks) {
            $db->Query(
                'DELETE FROM {pre}tasks WHERE `user`=? AND `tasklistid`=?',
                $this->_userID,
                $taskListID,
            );
        } else {
            $db->Query(
                'UPDATE {pre}tasks SET `tasklistid`=0 WHERE `user`=? AND `tasklistid`=?',
                $this->_userID,
                $taskListID,
            );
        }
        ...
        $db->Query(
            'DELETE FROM {pre}tasklists WHERE `tasklistid`=? AND `userid`=?',
            $taskListID,
            $this->_userID,
        );
    ...
    function MoveTasks($tasks, $taskListID) {
        ...
        $db->Query(
            'UPDATE {pre}tasks SET `tasklistid`=? WHERE `id` IN ? AND `user`=?',
            $taskListID,
            $tasks,
            $this->_userID,
        );
