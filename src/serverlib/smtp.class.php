...
class BMSMTP {
    ...
    function Connect() {
        ...
        if (!is_resource($this->_sock)) {
            PutLog(
                sprintf(
                    'SMTP connection to <%s:%d> failed (%d, %s)',
                    $this->_host,
                    $this->_port,
                    $errNo,
                    $errStr,
            ...
            if (substr($responseLine, 0, 3) != '220') {
                PutLog(
                    sprintf(
                        'SMTP server <%s:%d> did not return +OK',
                        $this->_host,
                        $this->_port,
                    ),
                    PRIO_DEBUG,
            ...
            if ($this->_isb1gMailServer) {
                PutLog(
                    sprintf(
                        'SMTP server <%s:%d> identified as b1gMailServer',
                        $this->_host,
                        $this->_port,
                    ),
                    PRIO_DEBUG,
    ...
    function Login($user, $pass) {
        ...
        if (
            fwrite($this->_sock, 'AUTH LOGIN' . "\r\n") &&
            substr($this->_getResponse(), 0, 3) == '334'
        ) {
            if (
                fwrite(
                    $this->_sock,
                    base64_encode(EncodeEMail($user)) . "\r\n",
                ) &&
                substr($this->_getResponse(), 0, 3) == '334'
            ) {
                if (
                    fwrite($this->_sock, base64_encode($pass) . "\r\n") &&
                    ...
                    PutLog(
                        sprintf(
                            'SMTP server <%s:%d> rejected username or password for user <%s>',
                            $this->_host,
                            $this->_port,
                            EncodeEMail($user),
                        ),
                ...
                PutLog(
                    sprintf(
                        'SMTP server <%s:%d> rejected username <%s>',
                        $this->_host,
                        $this->_port,
                        EncodeEMail($user),
                    ),
            ...
            PutLog(
                sprintf(
                    'SMTP server <%s:%d> does not seem to support LOGIN authentication',
                    $this->_host,
                    $this->_port,
                ),
                PRIO_WARNING,
    ...
    function StartMail($from, $to) {
        ...
        if (
            ...
            PutLog(
                sprintf(
                    'SMTP server <%s:%d> did not accept sender address <%s>',
                    $this->_host,
                    $this->_port,
                    $from,
                ),
