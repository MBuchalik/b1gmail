...
class BMPayment {
    ...
    static function Available() {
        ...
        if (
            ...
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}paymethods WHERE `enabled`=1',
            );
    ...
    static function GetCustomPaymentMethods() {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT * FROM {pre}paymethods WHERE `enabled`=1 ORDER BY `title` ASC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            foreach ($row['fields'] as $fieldID => $field) {
                $row['fields'][$fieldID]['options'] = array_map(
                    'trim',
                    explode(',', $field['options']),
                );
            }

            uasort($row['fields'], ['BMPayment', 'CustomPaymentFieldSort']);
     ...
     * @return int Amount
     */
    function CalcTotal($cart, $vatRate) {
        global $bm_prefs;
    ...
    function ActivateOrderWithVKCode($vkCode, $paidAmount) {
        ...
        $res = $db->Query(
            'SELECT `orderid` FROM {pre}orders WHERE `vkcode`=? AND `status`=?',
            $vkCode,
     ...
     * @return bool Success
     */
    function ActivateOrder($orderID, $paidAmount) {
        global $db, $bm_prefs, $lang_custom, $lang_user;

        // retrieve order
        $res = $db->Query(
            'SELECT `amount`,`status`,`userid`,`cart`,`paymethod`,`txnid` FROM {pre}orders WHERE `orderid`=?',
            $orderID,
        );
        if ($res->RowCount() != 1) {
            PutLog(
                sprintf(
                    'Failed to activate order %d (order not found)',
                    $orderID,
                ),
                PRIO_NOTE,
                __FILE__,
        ...
        if ($row['status'] != ORDER_STATUS_CREATED) {
            PutLog(
                sprintf(
                    'Failed to activate order %d (already activated or cancelled)',
                    $orderID,
                ),
                PRIO_NOTE,
                __FILE__,
        ...
        if ($row['amount'] > $paidAmount) {
            PutLog(
                sprintf(
                    'Failed to activate order %d (paid amount (%.02f) < order amount (%.02f))',
                    $orderID,
                    $paidAmount / 100,
                    $row['amount'] / 100,
                ),
        ...
        foreach ($cart as $cartItemID => $cartItem) {
            if (
                !isset($cartItem['key']) ||
                !isset($cartItem['count']) ||
                $cartItem['count'] <= 0
            ) {
                PutLog(
                    sprintf(
                        'Invalid cart item %d in order %d',
                        $cartItemID,
                        $orderID,
                    ),
                    PRIO_WARNING,
            ...
            if ($cartItem['key'] == 'b1gMail.credits') {
                $user->Debit(
                    ...
                    sprintf(
                        $lang_user['tx_charge'],
                        BMPayment::InvoiceNo($orderID),
                    ),
                );
            } else {
                PutLog(
                    sprintf(
                        'Unknown cart item key: %s - calling plugins',
                        $cartItem['key'],
                    ),
                    PRIO_DEBUG,
                    __FILE__,
        ...
        // mark as activated
        $db->Query(
            'UPDATE {pre}orders SET `status`=?,`activated`=? WHERE `orderid`=?',
        ...
        if ($bm_prefs['sendrg'] == 'yes' && $row['amount'] > 0) {
            if (
                in_array($row['paymethod'], [
                    PAYMENT_METHOD_PAYPAL,
                    PAYMENT_METHOD_SKRILL,
                ...
                if (
                    isset($payMethods[$payMethodID]) &&
                    $payMethods[$payMethodID]['invoice'] == 'at_activation'
                ) {
                    $generateInvoice = true;
                }
            }
        ...
        // log
        PutLog(
            sprintf('Order %d activated', $orderID),
            PRIO_NOTE,
            __FILE__,
            __LINE__,
        );
    ...
    function InitiatePayment(
        ...
        $res = $db->Query(
            'SELECT `paymethod`,`amount`,`status`,`cart`,`vkcode` FROM {pre}orders WHERE `userid`=? AND `orderid`=?',
            $userID,
            $orderID,
        );
        if ($res->RowCount() != 1) {
        ...
        $pf = [
            ...
            'returnURL' =>
                $returnURL != ''
                    ? $returnURL
                    : sprintf(
                        '%sprefs.php?action=paymentReturn&sid=%s',
                        $bm_prefs['selfurl'],
                        session_id(),
                    ),
        ];
        $pf['returnURL_SU'] = preg_replace(
            '/^(http|https)\:\/\//i',
            '',
            $pf['returnURL'],
        );
        ...
        if ($row['paymethod'] == PAYMENT_METHOD_PAYPAL) {
        ...
        } elseif ($row['paymethod'] == PAYMENT_METHOD_BANKTRANSFER) {
            ...
            if (isset($payMethods[$payMethodID])) {
                ...
                $pf['payMethodText'] = sprintf(
                    $lang_user['pn_customtext'],
                    $payMethods[$payMethodID]['title'],
                );
            }
        }
     ...
     */
    function ProcessPaymentForm(&$tpl, $cart, $fail = false, $userID = 0) {
        ...
        if (!is_array($cart) || count($cart) == 0) {
            PutLog(
                'BMPayment::ProcessPaymentForm(): $cart is empty',
                PRIO_WARNING,
                __FILE__,
                __LINE__,
            );
        ...
        if (
            !(
                ($_POST['paymentMethod'] == PAYMENT_METHOD_PAYPAL &&
                    $bm_prefs['enable_paypal'] == 'yes') ||
                ($_POST['paymentMethod'] == PAYMENT_METHOD_BANKTRANSFER &&
                    $bm_prefs['enable_vk'] == 'yes') ||
                ($_POST['paymentMethod'] == PAYMENT_METHOD_SOFORTUEBERWEISUNG &&
                    $bm_prefs['enable_su'] == 'yes') ||
                ($_POST['paymentMethod'] == PAYMENT_METHOD_SKRILL &&
                    $bm_prefs['enable_skrill'] == 'yes')
            )
        ) {
            if (
                $_POST['paymentMethod'] >= 0 ||
            ...
            if (isset($payMethods[$methodID])) {
                ...
                foreach ($payMethod['fields'] as $fieldID => $field) {
                    ...
                    switch ($field['type']) {
                        case FIELD_CHECKBOX:
                            $fieldOK = isset(
                                $_POST['fields'][$methodID][$fieldID],
                            );
                            $payMethodData[$fieldTitle] = isset(
                                $_POST['fields'][$methodID][$fieldID],
                            )
                                ? $lang_user['yes']
                                : $lang_user['no'];
                        ...
                        case FIELD_DATE:
                            ...
                            if ($fieldOK) {
                                $payMethodData[$fieldTitle] = sprintf(
                                    '%04d-%02d-%02d',
                                    $_POST[$fieldName . 'Year'],
                                    $_POST[$fieldName . 'Month'],
                                    $_POST[$fieldName . 'Day'],
                    ...
                    if (
                        (isset($field['oblig']) ||
                            (isset($_POST['fields'][$methodID][$fieldID]) &&
                                strlen($_POST['fields'][$methodID][$fieldID]) >
                                    0)) &&
                        !$fieldOK
                    ) {
        ...
        if (count($invalidFields) > 0 || $fail) {
            ...
            $vatRate = isset($countries[$countryID])
                ? $countries[$countryID]['vat']
                : 0;
            ...
            $db->Query(
                'INSERT INTO {pre}orders(`userid`,`vkcode`,`cart`,`paymethod`,`paymethod_params`,`amount`,`tax`,`inv_firstname`,`inv_lastname`,`inv_street`,`inv_no`,`inv_zip`,`inv_city`,`inv_country`,`created`,`status`) VALUES ' .
                    '(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                $userID,
                $vkCode,
                serialize($cart),
            ...
            if (
                ...
                ($_POST['paymentMethod'] == PAYMENT_METHOD_BANKTRANSFER ||
                    ($_POST['paymentMethod'] < 0 &&
                        $payMethods[abs($_POST['paymentMethod'])]['invoice'] ==
                            'at_order'))
            ) {
                BMPayment::GenerateInvoice($orderId);
            }
     ...
     */
    function GenerateInvoice($orderID, $regenerate = false) {
        ...
        if (!$regenerate) {
            ...
            [$invCount] = $res->FetchArray(MYSQLI_NUM);
            $res->Free();
        ...
        // fetch order row
        $res = $db->Query(
            'SELECT * FROM {pre}orders WHERE `orderid`=?',
            $orderID,
        );
        if ($res->RowCount() == 0) {
            return false;
        ...
        if ($orderRow['status'] != ORDER_STATUS_ACTIVATED) {
            ...
            if ($orderRow['paymethod'] == PAYMENT_METHOD_BANKTRANSFER) {
                $abort = false;
            } elseif ($orderRow['paymethod'] < 0) {
                $payMethodID = abs($orderRow['paymethod']);
                $payMethods = BMPayment::GetCustomPaymentMethods();
                if (
                    isset($payMethods[$payMethodID]) &&
                    $payMethods[$payMethodID]['invoice'] == 'at_order'
                ) {
                    $abort = false;
                }
            }
            ...
            if ($abort) {
                PutLog(
                    sprintf(
                        'Cannot generate invoice for payment method %d in advance for order #%d',
                        $orderRow['paymethod'],
                        $orderID,
                    ),
                    PRIO_WARNING,
        ...
        } elseif ($orderRow['paymethod'] == PAYMENT_METHOD_PAYPAL) {
            $paymentNote = sprintf(
                $lang_user['pn_paypal'],
                $amount,
                $bm_prefs['currency'],
            );
        } elseif ($orderRow['paymethod'] == PAYMENT_METHOD_SKRILL) {
            $paymentNote = sprintf(
                $lang_user['pn_skrill'],
                $amount,
                $bm_prefs['currency'],
            );
        } elseif ($orderRow['paymethod'] == PAYMENT_METHOD_SOFORTUEBERWEISUNG) {
            $paymentNote = sprintf(
        ...
        $rgTpl->assign(
            'land',
            isset($countryList[$orderRow['inv_country']])
        ...
        $rgTpl->register_resource('prefsdb', [
            &$bmPayment,
            '__tpl_getTemplate',
            '__tpl_getTimestamp',
            '__tpl_getSecure',
        ...
        // save invoice
        $db->Query(
            'REPLACE INTO {pre}invoices(`orderid`,`invoice`) VALUES(?,?)',
            $orderID,
            $invoice,
        );
    ...
    function GenerateVKCode() {
        ...
        for ($i = 0; $i < VKCODE_LENGTH; $i++) {
            $result .= substr(
                VKCODE_CHARS,
                mt_rand(0, strlen(VKCODE_CHARS) - 1),
                1,
            );
        }
