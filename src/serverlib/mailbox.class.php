...
class BMMailbox {
    ...
    function __construct($userID, $userMail, $userObject) {
        ...
        $this->_mailboxStructureGeneration =
            $userObject->_row['mailbox_structure_generation'];
    }
    ...
    function IncGeneration() {
        ...
        $db->Query(
            'UPDATE {pre}users SET mailbox_generation=mailbox_generation+1 WHERE id=?',
            $this->_userID,
        );
    ...
    function IncStructureGeneration() {
        ...
        $db->Query(
            'UPDATE {pre}users SET mailbox_structure_generation=mailbox_structure_generation+1 WHERE id=?',
            $this->_userID,
        );
        $this->_mailboxStructureGeneration++;
    }
    ...
    function IsIntelligentFolder($folderID) {
        ...
        if (!is_array($this->_intelligentFolders)) {
            $this->_intelligentFolders = [];
            $res = $db->Query(
                'SELECT id FROM {pre}folders WHERE intelligent=1 AND userid=?',
                $this->_userID,
            );
    ...
    function GroupMailList($mailList, $groupMode) {
        ...
        foreach ($mailList as $key => $value) {
            if ($groupMode == 'gelesen') {
                $groupValue =
                    ($value['row']['flags'] & FLAG_UNREAD) == 0 ? 1 : 0;
            } elseif ($groupMode == 'beantwortet') {
                $groupValue =
                    ($value['row']['flags'] & FLAG_ANSWERED) != 0 ? 1 : 0;
            } elseif ($groupMode == 'weitergeleitet') {
                $groupValue =
                    ($value['row']['flags'] & FLAG_FORWARDED) != 0 ? 1 : 0;
            } elseif ($groupMode == 'attach') {
                $groupValue =
                    ($value['row']['flags'] & FLAG_ATTACHMENT) != 0 ? 1 : 0;
            } elseif ($groupMode == 'flagged') {
                $groupValue =
                    ($value['row']['flags'] & FLAG_FLAGGED) != 0 ? 1 : 0;
            } elseif ($groupMode == 'done') {
                $groupValue = ($value['row']['flags'] & FLAG_DONE) != 0 ? 1 : 0;
            } elseif ($groupMode == 'fetched') {
                $groupValue = $value['row']['datum'];
            } elseif ($groupMode == 'color') {
            ...
            if ($groupMode != 'fetched') {
                if ($groupValue != $lastGroupValue) {
                    ...
                    if ($groupMode == 'gelesen') {
                        ...
                        $text =
                            ...
                            ': ' .
                            $lang_user[$groupValue ? 'yes' : 'no'];
                    } elseif ($groupMode == 'von') {
                        $text = HTMLFormat($groupValue);
                    } elseif ($groupMode == 'color') {
                        $text =
                            $lang_user['color'] .
                            ': ' .
                            $lang_user['color_' . $groupValue];
                    }
                ...
                [$cat1] = categorizeDate($lastGroupValue);
                [$cat2, $cat2Array] = categorizeDate($groupValue);
                ...
                if ($lastGroupValue == -1 || $cat1 != $cat2) {
                    $newList[--$sepID] = [
                        ...
                        'date' => $cat2Array['date'],
                        'groupID' => substr(
                            md5($cat2Array['text'] . $cat2Array['date']),
                            0,
                            8,
                        ),
                    ];
    ...
    function GetMailsPerPage($id) {
        ...
        if ($id <= 0) {
            ...
            if (
                ...
                $res = $db->Query(
                    'SELECT perpage FROM {pre}folders WHERE id=?',
                    $id,
                );
            } else {
                $res = $db->Query(
                    'SELECT perpage FROM {pre}folders WHERE userid=? AND id=?',
                    $this->_userID,
                    $id,
                );
            }
    ...
    function SetMailsPerPage($id, $num) {
        ...
        if ($id <= 0) {
            ...
            if (
                ...
                $db->Query(
                    'UPDATE {pre}folders SET perpage=? WHERE id=?',
                    $num,
                    $id,
                );
            } else {
                $db->Query(
                    'UPDATE {pre}folders SET perpage=? WHERE userid=? AND id=?',
                    $num,
                    $this->_userID,
                    $id,
                );
    ...
    function GetGroupMode($id) {
        ...
        if ($id <= 0) {
            ...
            if (
                ...
                $res = $db->Query(
                    'SELECT group_mode FROM {pre}folders WHERE id=?',
                    $id,
                );
            } else {
                $res = $db->Query(
                    'SELECT group_mode FROM {pre}folders WHERE userid=? AND id=?',
                    $this->_userID,
                    $id,
                );
            }
    ...
    function SetGroupMode($id, $mode) {
        ...
        if ($id <= 0) {
            ...
            if (
                ...
                $db->Query(
                    'UPDATE {pre}folders SET group_mode=? WHERE id=?',
                    $mode,
                    $id,
                );
            } else {
                $db->Query(
                    'UPDATE {pre}folders SET group_mode=? WHERE userid=? AND id=?',
                    $mode,
                    $this->_userID,
                    $id,
                );
    ...
    function GetFolderTitle($folderID) {
        ...
        if (
            ...
            $res = $db->Query(
                'SELECT titel FROM {pre}folders WHERE id=?',
                (int) $folderID,
            );
        } else {
            $res = $db->Query(
                'SELECT titel FROM {pre}folders WHERE id=? AND userid=?',
                (int) $folderID,
                $this->_userID,
            );
        }
        ...
        [$title] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    function DeleteFolder($id) {
        ...
        $cacheManager->Delete(
            'folderList:1:' . $this->_userID . ':' . $this->_mailboxGeneration,
        );
        $cacheManager->Delete(
            'folderList:0:' .
                $this->_userID .
        ...
        $db->Query(
            'UPDATE {pre}folders SET parent=? WHERE parent=? AND userid=?',
            -1,
            (int) $id,
            $this->_userID,
        );
        ...
        $db->Query(
            'DELETE FROM {pre}folders WHERE id=? AND userid=?',
            (int) $id,
            $this->_userID,
        );
        if ($db->AffectedRows() == 1) {
            $db->Query(
                'DELETE FROM {pre}folder_conditions WHERE folder=?',
                (int) $id,
            );

            $db->Query(
                'UPDATE {pre}mails SET folder=?,trashstamp=? WHERE userid=? AND folder=?',
                FOLDER_TRASH,
                time(),
                $this->_userID,
                $id,
    ...
    function AddFolder(
        ...
        $storetime,
        $intelligent,
        $noDefaultCondition = false
    ) {
        global $db, $cacheManager, $bm_prefs;

        $db->Query(
            'INSERT INTO {pre}folders(userid,titel,parent,subscribed,storetime,intelligent,group_mode) VALUES(?,?,?,?,?,?,?)',
            $this->_userID,
            $title,
            (int) $parent,
            $subscribed ? 1 : 0,
    ...
    function UpdateFolder(
        ...
        $db->Query(
            'UPDATE {pre}folders SET titel=?, parent=?, subscribed=?, storetime=?, intelligent_link=? WHERE id=? AND userid=?',
            $title,
            (int) $parent,
            $subscribed ? 1 : 0,
            (int) $storetime,
    ...
    function GetFolder($folderID) {
        ...
        $res = $db->Query(
            'SELECT id,titel,parent,subscribed,perpage,storetime,group_mode,intelligent,intelligent_link,userid FROM {pre}folders WHERE id=?',
            (int) $folderID,
            $this->_userID,
        );
        if ($res->RowCount() == 0) {
    ...
    function GetConditions($folderID) {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,field,op,val FROM {pre}folder_conditions WHERE folder=? ORDER BY id ASC',
            (int) $folderID,
        );
    ...
    function DeleteCondition($conditionID, $folderID) {
        ...
        $db->Query(
            'DELETE FROM {pre}folder_conditions WHERE id=? AND folder=?',
            (int) $conditionID,
            (int) $folderID,
        );
        return $db->AffectedRows() == 1;
    ...
    function AddCondition($folderID) {
        ...
        $db->Query(
            'INSERT INTO {pre}folder_conditions(folder,field,op,val) VALUES(?,?,?,?)',
            (int) $folderID,
            1,
            1,
            '',
    ...
    function UpdateCondition($conditionID, $folderID, $field, $op, $val) {
        ...
        $db->Query(
            'UPDATE {pre}folder_conditions SET field=?,op=?,val=? WHERE id=? AND folder=?',
            (int) $field,
            (int) $op,
            $val,
            (int) $conditionID,
     ...
     * @return array
     */
    function GetUserFolderList(
        $sortColumn = 'titel',
        $sortOrder = 'ASC',
        $withParentTitle = false,
        $withStats = false
    ) {
        global $db;
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,titel,parent,subscribed,perpage,storetime,group_mode,intelligent,intelligent_link FROM {pre}folders WHERE userid=? ORDER BY ' .
                $sortColumn .
                ' ' .
                $sortOrder,
            $this->_userID,
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            if ($withStats) {
                ...
                $row['flaggedMails'] = $this->GetMailCount(
                    $row['id'],
                    false,
                    true,
                );
    ...
    function GetSharedFolderList(
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,titel,parent,subscribed,perpage,storetime,group_mode,intelligent,intelligent_link,writeaccess FROM {pre}folders ' .
                ...
                'WHERE {pre}workgroups_shares.sharetype=' .
                WORKGROUP_TYPE_MAILFOLDER .
                ' AND {pre}workgroups_member.user=? ' .
                'ORDER BY ' .
                $sortColumn .
                ' ' .
                $sortOrder,
    ...
    function GetFolderSize($folderID) {
        ...
        if (
            ...
            $res = $db->Query(
                'SELECT SUM(size) FROM {pre}mails WHERE folder=?',
                $folderID,
            );
        } else {
            $res = $db->Query(
                'SELECT SUM(size) FROM {pre}mails WHERE folder=? AND userid=?',
                $folderID,
                $this->_userID,
            );
        }
        [$folderSize] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    function SubscribeFolder($folderID, $subscribe = true) {
        ...
        $db->Query(
            'UPDATE {pre}folders SET subscribed=? WHERE id=? AND userid=?',
            $subscribe ? 1 : 0,
            (int) $folderID,
            $this->_userID,
        );
    ...
    function GetDropdownFolderList(
        ...
        $folderCount = 0;
        $res = $db->Query(
            'SELECT titel,id FROM {pre}folders WHERE userid=? AND intelligent=0 AND parent=? ORDER BY titel ASC',
            $this->_userID,
            $parent,
        );
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            if (
                $this->GetDropdownFolderList(
                    $row['id'],
                    $result,
                    $level + 1,
                    $lastItem || $level == 0 ? $more + 1 : $more,
                ) > 0
            ) {
                $result[$row['id']] = str_replace(
                    '|-- ',
                    '|-+ ',
                    $result[$row['id']],
                );
        ...
        if (EXTENDED_WORKGROUPS && $parent == -1 && $includeShared) {
            $res = $db->Query(
                'SELECT id,titel FROM {pre}folders ' .
                    'INNER JOIN {pre}workgroups_shares ON {pre}folders.id={pre}workgroups_shares.shareid ' .
                    'INNER JOIN {pre}workgroups_member ON {pre}workgroups_shares.workgroupid={pre}workgroups_member.workgroup ' .
                    'WHERE {pre}workgroups_shares.sharetype=' .
                    WORKGROUP_TYPE_MAILFOLDER .
    ...
    function GetFolderList(
        ...
        if (!($result = $cacheManager->Get($cacheKey))) {
            ...
            // user folders
            $res = $db->Query(
                'SELECT parent,titel,intelligent,id FROM {pre}folders WHERE userid=? AND subscribed=1 ORDER BY titel ASC',
                $this->_userID,
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $result[$row['id']] = [
                    ...
                    'title' => $row['titel'],
                    'type' =>
                        $row['intelligent'] == 1 ? 'intellifolder' : 'folder',
        ...
        if (EXTENDED_WORKGROUPS) {
            $res = $db->Query(
                'SELECT titel,id,writeaccess FROM {pre}folders ' .
                    'INNER JOIN {pre}workgroups_shares ON {pre}folders.id={pre}workgroups_shares.shareid ' .
                    'INNER JOIN {pre}workgroups_member ON {pre}workgroups_shares.workgroupid={pre}workgroups_member.workgroup ' .
                    'WHERE {pre}workgroups_shares.sharetype=' .
                    WORKGROUP_TYPE_MAILFOLDER .
            ...
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                ...
                if ($withUnreadCount) {
                    $result[$row['id']]['unread'] = $this->GetMailCount(
                        $row['id'],
                        true,
                        false,
                    );
                ...
                if ($withAllCount) {
                    $result[$row['id']]['all'] = $this->GetMailCount(
                        $row['id'],
                        false,
                        false,
                    );
    ...
    function RenameFolder($folderID, $newName) {
        ...
        // get folder parent
        $res = $db->Query(
            'SELECT parent FROM {pre}folders WHERE id=? AND userid=?',
            $folderID,
            $this->_userID,
        );
        [$parent] = $res->FetchArray();
        $res->Free();

        // check if name is available
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}folders WHERE parent=? AND titel=? AND userid=?',
            $parent,
            $newName,
            $this->_userID,
        );
        ...
        if ($count == 0) {
            // okay!
            $db->Query(
                'UPDATE {pre}folders SET titel=? WHERE id=? AND userid=?',
                $newName,
                $folderID,
                $this->_userID,
            );
    ...
    function GetMailCount($folderID, $unread = false, $flagged = false) {
        ...
        $cond = $folderID == -1 ? '1' : $this->FolderCondition($folderID);
        if ($unread) {
        ...
        if (
            ...
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}mails WHERE folder=? AND ' . $cond,
                $folderID,
            );
        } elseif ($this->IsIntelligentFolder($folderID)) {
            $sharedFolders = $this->GetSharedFolderList();

            if (count($sharedFolders) > 0) {
                $res = $db->Query(
                    'SELECT COUNT(*) FROM {pre}mails WHERE (userid=? OR folder IN ?) AND ' .
                        $cond,
                    $this->_userID,
                ...
                $res = $db->Query(
                    'SELECT COUNT(*) FROM {pre}mails WHERE userid=? AND ' .
                        $cond,
                    $this->_userID,
                );
            }
            ...
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}mails WHERE userid=? AND ' . $cond,
                $this->_userID,
            );
        }
        $row = $res->FetchArray(MYSQLI_NUM);
    ...
    function GetMailIDList($folderID) {
        ...
        if (
            ...
            $res = $db->Query(
                'SELECT id FROM {pre}mails WHERE folder=? AND ' . $condition,
                $folderID,
            );
        } else {
            $res = $db->Query(
                'SELECT id FROM {pre}mails WHERE userid=? AND ' . $condition,
                $this->_userID,
            );
        }
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    function GetLatestAutoSavedDraft($maxAge) {
        ...
        $res = $db->Query(
            'SELECT `id` FROM {pre}mails WHERE `userid`=? AND `folder`=? AND (`flags`&' .
                FLAG_AUTOSAVEDDRAFT .
                ')!=0 AND (`flags`&' .
                FLAG_NODRAFTNOTIFY .
                ')=0 AND `fetched`>=? ORDER BY `id` DESC LIMIT 1',
            $this->_userID,
            FOLDER_DRAFTS,
            time() - $maxAge,
        );
    ...
    function SetNoDraftNotify() {
        ...
        $db->Query(
            'UPDATE {pre}mails SET `flags`=`flags`|' .
                FLAG_NODRAFTNOTIFY .
                ' WHERE `userid`=? AND `folder`=? AND (`flags`&' .
                FLAG_AUTOSAVEDDRAFT .
                ')!=0 AND (`flags`&' .
                FLAG_NODRAFTNOTIFY .
                ')=0',
    ...
    function GetMailList(
        ...
        ModuleFunction('OnStartMailList', [
            $this->_userID,
        ...
        if (
            ...
            $res = $db->Query(
                'SELECT id,von,an,betreff,datum,flags,priority,size,color FROM {pre}mails WHERE folder=? AND ' .
                    $condition .
                    ' ' .
                    'ORDER BY ' .
                    ($groupMode != '-'
                        ? $this->GetGroupOrderBy($groupMode) . ', '
                        : '') .
                    $sortField .
                    '  ' .
                    $sortBy .
        ...
        } elseif ($this->IsIntelligentFolder((int) $folderID)) {
            $sharedFolders = array_keys($this->GetSharedFolderList());

            if (count($sharedFolders) > 0) {
                $res = $db->Query(
                    'SELECT id,von,an,betreff,datum,flags,priority,size,color FROM {pre}mails WHERE (userid=? OR folder IN ?) AND ' .
                        $condition .
                        ' ' .
                        'ORDER BY ' .
                        ($groupMode != '-'
                ...
                $res = $db->Query(
                    'SELECT id,von,an,betreff,datum,flags,priority,size,color FROM {pre}mails WHERE userid=? AND ' .
                        $condition .
                        ' ' .
                        'ORDER BY ' .
                        ($groupMode != '-'
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            if (count($to) == 0) {
                ...
                $to_name =
                    (isset($to[0]['name']) && trim($to[0]['name']) != ''
                        ? $to[0]['name']
                        : DecodeEMail($to[0]['mail'])) .
                    (count($to) > 1 ? ', ...' : '');
                $to_mail = $to[0]['mail'];
            }
            ...
            ModuleFunction('OnGetMail', [$row['id'], $this->_userID]);
        }
        $res->Free();

        ModuleFunction('OnEndMailList', [
            $this->_userID,
            $folderID == FOLDER_DRAFTS,
        ]);
    ...
    function GetPrevNextMail($folderID, $mailID) {
        ...
        $isShared = BMWorkgroup::AccessAllowed(
            $this->_userID,
            WORKGROUP_TYPE_MAILFOLDER,
            $folderID,
            false,
        ...
        if ($isShared) {
            $res = $db->Query(
                'SELECT id FROM {pre}mails WHERE id<? AND folder=? ORDER BY id DESC LIMIT 1',
                $mailID,
                $folderID,
            );
        } else {
            $res = $db->Query(
                'SELECT id FROM {pre}mails WHERE id<? AND userid=? AND folder=? ORDER BY id DESC LIMIT 1',
                $mailID,
                $this->_userID,
                $folderID,
            );
        ...
        if ($res->RowCount() == 1) {
            [$prevID] = $res->FetchArray(MYSQLI_NUM);
        }
        $res->Free();
        ...
        if ($isShared) {
            $res = $db->Query(
                'SELECT id FROM {pre}mails WHERE id>? AND folder=? ORDER BY id ASC LIMIT 1',
                $mailID,
                $folderID,
            );
        } else {
            $res = $db->Query(
                'SELECT id FROM {pre}mails WHERE id>? AND userid=? AND folder=? ORDER BY id ASC LIMIT 1',
                $mailID,
                $this->_userID,
                $folderID,
            );
        ...
        if ($res->RowCount() == 1) {
            [$nextID] = $res->FetchArray(MYSQLI_NUM);
        }
        $res->Free();

        return [isset($prevID) ? $prevID : -1, isset($nextID) ? $nextID : -1];
    }
    ...
    function GetMailsWithFlags($flags, $set = true, $limit = -1) {
        ...
        $res = $db->Query(
            'SELECT id FROM {pre}mails WHERE userid=? AND ' .
                $cond .
                ($limit != -1 ? ' LIMIT ' . $limit : ''),
            $this->_userID,
        );
    ...
    function IntelligentFolderCondition($folderID) {
        ...
        // get link mode
        $res = $db->Query(
            'SELECT intelligent_link FROM {pre}folders WHERE id=? AND userid=?',
            (int) $folderID,
            $this->_userID,
        );
        assert('$res->RowCount() != 0');
        [$linkMode] = $res->FetchArray();
        $res->Free();

        // get condition
        $res = $db->Query(
            'SELECT field,op,val FROM {pre}folder_conditions WHERE folder=?',
            (int) $folderID,
        );
        while ($row = $res->FetchArray(MYSQLI_NUM)) {
            ...
            if (
                ...
                if ($field == MAILFIELD_SUBJECT) {
                    ...
                    $sqlField = 'cc';
                } elseif ($field == MAILFIELD_READ) {
                    $condition =
                        '(flags&' .
                        FLAG_UNREAD .
                        ')' .
                        ($sqlVal == 'no' ? '!=0' : '=0');
                } elseif ($field == MAILFIELD_ANSWERED) {
                    $condition =
                        '(flags&' .
                        FLAG_ANSWERED .
                        ')' .
                        ($sqlVal == 'no' ? '=0' : '!=0');
                } elseif ($field == MAILFIELD_FORWARDED) {
                    $condition =
                        '(flags&' .
                        FLAG_FORWARDED .
                        ')' .
                    ...
                    $condition =
                        ...
                        ')' .
                        ($sqlVal == 'no' ? '=0' : '!=0');
                } elseif ($field == MAILFIELD_FLAGGED) {
                    $condition =
                        '(flags&' .
                        FLAG_FLAGGED .
                        ')' .
                        ($sqlVal == 'no' ? '=0' : '!=0');
                } elseif ($field == MAILFIELD_DONE) {
                    $condition =
                        '(flags&' .
                        FLAG_DONE .
                        ')' .
                        ($sqlVal == 'no' ? '=0' : '!=0');
                } elseif ($field == MAILFIELD_FOLDER) {
                    $sqlField = 'folder';
                } elseif ($field == MAILFIELD_COLOR) {
                    $sqlField = 'color';
                } else {
                    $sqlField = '0';
                }
    ...
    function FolderCondition($folderID, $moveCondition = false) {
        ...
        if ($folderID > 0) {
            ...
            if (
                !$moveCondition &&
                $this->IsIntelligentFolder((int) $folderID)
            ) {
    ...
    function FolderExists($folderID) {
        ...
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}folders WHERE userid=? AND id=?',
            $this->_userID,
            $folderID,
        );
    ...
    function FlagMail($flag, $value, $mail) {
        ...
        // get current flags
        $res = $db->Query(
            'SELECT folder,flags,userid FROM {pre}mails WHERE id=? LIMIT 1',
            $mail,
        );
        if ($res->RowCount() == 0) {
            return false;
        ...
        if ($value && ($currentFlags & $flag) == 0) {
            $newFlags |= $flag;
        } elseif (!$value && ($currentFlags & $flag) != 0) {
            $newFlags &= ~$flag;
        }
        ...
        if (
            ...
            if (
                ...
                is_object($tpl) &&
                isset($tpl->_tpl_vars) &&
                isset($tpl->_tpl_vars['folderList'])
            ) {
    ...
    function ColorMail($id, $color) {
        ...
        if (!is_array($id)) {
            $db->Query('UPDATE {pre}mails SET color=? WHERE id=?', $color, $id);
        } else {
            $db->Query(
                'UPDATE {pre}mails SET color=? WHERE id IN ?',
                $color,
                $id,
            );
        }
    ...
    function SetMailNotes($id, $notes) {
        ...
        if (trim($notes) == '') {
            $db->Query('DELETE FROM {pre}mailnotes WHERE mailid=?', $id);
        } else {
            $db->Query(
                'REPLACE INTO {pre}mailnotes(mailid,notes) VALUES(?,?)',
                $id,
                $notes,
            );
        }
    ...
    function GetMailNotes($id) {
        ...
        $res = $db->Query(
            'SELECT notes FROM {pre}mailnotes WHERE mailid=?',
            $id,
        );
        if ($res->RowCount() == 0) {
            return '';
    ...
    function SetMessageFlags($mail, $flags) {
        ...
        // update row
        $db->Query('UPDATE {pre}mails SET flags=? WHERE id=?', $flags, $mail);
        ...
        if ($affected) {
            // plugins
            ModuleFunction('AfterChangeMailFlags', [$mail, $flags, &$this]);
    ...
    function MoveMail($mails, $destFolder) {
        ...
        $destIsShared = BMWorkgroup::AccessAllowed(
            $this->_userID,
            WORKGROUP_TYPE_MAILFOLDER,
            $destFolder,
            false,
        ...
        $db->Query('BEGIN');
        $res = $db->Query(
            'SELECT `id`,`folder`,`size` FROM {pre}mails WHERE `id` IN ?',
            $mails,
        );
        while ($row = $res->FetchArray(MYSQLI_NUM)) {
            [$mailID, $srcFolderID, $srcMailSize] = $row;

            // source folder shared?
            $sourceIsShared = BMWorkgroup::AccessAllowed(
                $this->_userID,
                WORKGROUP_TYPE_MAILFOLDER,
                $srcFolderID,
                false,
            ...
            elseif (!$sourceIsShared && $destIsShared) {
                $spaceChange -= $srcMailSize;
            }
        ...
        // plugins
        ModuleFunction('AfterMoveMails', [$mails, $destFolder, &$this]);
    ...
    function GetMail($id) {
        ...
        $res = $db->Query('SELECT * FROM {pre}mails WHERE id=? LIMIT 1', $id);
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $result = _new('BMMail', [
                $this->_userID,
                $row,
    ...
    function GetRecentMailCount($folder = FOLDER_ROOT) {
        ...
        // select new mails
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}mails WHERE ' .
                ($folder == FOLDER_ROOT
                    ? '(folder=' . FOLDER_INBOX . ' OR folder>0)'
                    : '(folder=' . (int) $folder . ')') .
                ' AND (flags&' .
                FLAG_UNREAD .
                ')!=0 AND userid=? AND fetched>=?',
            $userInfo['id'],
    ...
    function UpdateLastNotifyDate() {
        global $db;
        $db->Query(
            'UPDATE {pre}users SET last_notify=? WHERE id=?',
            time(),
            $this->_userID,
        );
    }
    ...
    function UpdateSpace($bytes) {
        ...
        if ($bytes < 0) {
            ...
            $db->Query(
                'UPDATE {pre}users SET mailspace_used=mailspace_used+' .
                    abs($bytes) .
                    ' WHERE id=?',
                $this->_userID,
            );
    ...
    function DeleteMail($id, $hard = false) {
        ...
        // get msg state
        $res = $db->Query(
            'SELECT folder,size,blobstorage,userid FROM {pre}mails WHERE id=?',
            (int) $id,
        );
        if ($res->RowCount() != 1) {
            return false;
        ...
        if ($folder == FOLDER_TRASH || $hard) {
            ...
            $db->Query('DELETE FROM {pre}mails WHERE id=?', (int) $id);
            ...
            // plugins
            ModuleFunction('AfterDeleteMail', [$id, &$this]);
    ...
    function DeleteMailFromSearchIndex($id) {
        ...
        if ($groupRow['ftsearch'] == 'yes') {
            ...
            $idx = _new('BMSearchIndex', [$this->_userID]);
            $idx->deleteItem($id);
            unset($idx);
    ...
    function StoreMail($mail, $folder = FOLDER_INBOX, $trainNoSpam = false) {
        ...
        } elseif ($mailSize > $this->GetMaxMailSize()) {
            fseek($mail->_fp, $oldOffset, SEEK_SET);
        ...
        // insert into DB
        $db->Query(
            'INSERT INTO {pre}mails(userid,betreff,von,an,cc,blobstorage,folder,datum,trashstamp,priority,fetched,msg_id,virnam,trained,refs,flags,size,color) VALUES ' .
                '(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        ...
        if (!$bsProvider->storeBlob(BMBLOB_TYPE_MAIL, $id, $mail->_fp)) {
            ...
            PutLog(
                sprintf(
                    'Failed to store blob for message %d using blob provider %d for user <%s> (%d)',
                    $id,
                    $bsProvider->providerID,
                    $this->_userMail,
                    $this->_userID,
            ...
            // delete message row
            $db->Query(
                'DELETE FROM {pre}mails WHERE id=? AND userid=?',
                $id,
                $this->_userID,
            );
        ...
        if (count($attachments) > 0) {
            ...
            foreach ($attachments as $partID => $attachment) {
                ...
                $db->Query(
                    'INSERT INTO {pre}attachments(`userid`,`mailid`,`partid`,`filename`,`size`,`contenttype`,`flags`) VALUES(?,?,?,?,?,?,?)',
                    $this->_userID,
                    $id,
                    $partID,
                    $attachment['filename'],
        ...
        // plugins
        ModuleFunction('AfterStoreMail', [$id, &$mail, &$this]);
    ...
    function ReceiveMail(
        ...
        // module functions
        $moduleResults = $plugins->callFunction('OnReceiveMail', false, true, [
            &$mail,
            &$this,
            &$this->_userObject,
        ]);
        foreach ($moduleResults as $moduleName => $moduleResult) {
            if ($moduleResult == BM_BLOCK && !$trusted) {
                PutLog(
                    sprintf(
                        'Mail to <%s> (%d) from <%s> blocked by plugin <%s>',
                        $this->_userMail,
                        $this->_userID,
            ...
            } elseif ($moduleResult == BM_DELETE && !$trusted) {
                PutLog(
                    sprintf(
                        'Mail to <%s> (%d) from <%s> deleted by plugin <%s>',
                        $this->_userMail,
                        $this->_userID,
                        ExtractMailAddress($mail->GetHeaderValue('from')),
                        $moduleName,
                ...
                $moduleResult == BM_IS_INFECTED &&
                ($mail->flags & FLAG_INFECTED) == 0 &&
                !$trusted
            ) {
                PutLog(
                    sprintf(
                        'Mail to <%s> (%d) from <%s> marked as infected by plugin <%s>',
                        $this->_userMail,
                        $this->_userID,
                        ExtractMailAddress($mail->GetHeaderValue('from')),
                        $moduleName,
                ...
                $moduleResult == BM_IS_SPAM &&
                ($mail->flags & FLAG_SPAM) == 0 &&
                !$trusted
            ) {
                PutLog(
                    sprintf(
                        'Mail to <%s> (%d) from <%s> marked as spam by plugin <%s>',
                        $this->_userMail,
                        $this->_userID,
                        ExtractMailAddress($mail->GetHeaderValue('from')),
                        $moduleName,
        ...
        if ($bm_prefs['detect_duplicates'] == 'yes') {
            ...
            if (count($messageIDs) > 0) {
                ...
                $res = $db->Query(
                    'SELECT COUNT(*) FROM {pre}mails WHERE msg_id=? AND userid=? AND folder!=?',
                    $messageID,
                    $this->_userID,
                    FOLDER_OUTBOX,
                );
                [$duplicateCount] = $res->FetchArray(MYSQLI_NUM);
                $res->Free();
                ...
                if ($duplicateCount > 0) {
                    PutLog(
                        sprintf(
                            'Message duplicate for <%s> (%d) detected (%s), processing stopped',
                            $this->_userMail,
                            $this->_userID,
                            $messageID,
                        ),
        ...
        // user filter rules
        $filterEval = _new('BMFilterEval', [&$this->_userObject, &$mail]);
        [
            $filterStoreResult,
            $filterFolder,
            $filterActionFlags,
            $mailColor,
        ] = $filterEval->EvalFilters();
        if (
            $folder == FOLDER_INBOX &&
            $filterFolder != -1 &&
            $this->FolderExists($filterFolder)
        ) {
            $folder = $filterFolder;
        }
        if ($filterStoreResult != STORE_RESULT_OK) {
            ...
            if ($storeResult == RECEIVE_RESULT_BLOCKED && !$trusted) {
                PutLog(
                    sprintf(
                        'Mail to <%s> (%d) from <%s> blocked by user filter',
                        $this->_userMail,
                        $this->_userID,
                        ExtractMailAddress($mail->GetHeaderValue('from')),
                    ),
                ...
                return $storeResult;
            } elseif ($storeResult == RECEIVE_RESULT_DELETE && !$trusted) {
                PutLog(
                    sprintf(
                        'Mail to <%s> (%d) from <%s> deleted by user filter',
                        $this->_userMail,
                        $this->_userID,
                        ExtractMailAddress($mail->GetHeaderValue('from')),
                    ),
        ...
        if (
            ...
            $filter = _new('BMMailFilter_Bayes', [&$mail, $this->_userID]);
            $filter->Filter();
        }
        ...
        if ($this->_userObject->_row['addressbook_nospam'] == 'yes') {
            ...
            if (is_array($senders) && count($senders) > 0) {
                $book = _new('BMAddressbook', [$this->_userID]);
        ...
        if (
            ...
            $forwardAddresses = [];
            if (
                $this->_userObject->_row['forward'] == 'yes' &&
                BMUser::AddressValid(
                    ExtractMailAddress($this->_userObject->_row['forward_to']),
                    false,
                )
            ) {
                $forwardAddresses[] = ExtractMailAddress(
                    $this->_userObject->_row['forward_to'],
                );
            }
            ...
            foreach ($forwardAddresses as $forwardTo) {
                if (BMUser::GetID($forwardTo) != $this->_userObject->_id) {
                    if ($fp = fopen('php://temp', 'wb+')) {
                        $isDKIMSigned = !empty(
                            $mail->GetHeaderValue('dkim-signature')
                        );
                        ...
                        if ($sendMail->Send()) {
                            PutLog(
                                sprintf(
                                    'Forwarded mail for <%s> (%d) to <%s> (forwardDelete: %d)',
                                    $this->_userMail,
                                    $this->_userID,
                                    $forwardTo,
                                    $this->_userObject->_row[
                                        'forward_delete'
                                    ] == 'yes',
                                ),
                                PRIO_NOTE,
                            ...
                            if (
                                $this->_userObject->_row['forward_delete'] ==
                                'yes'
                            ) {
                                return RECEIVE_RESULT_DELETE;
                            }
                            ...
                            PutLog(
                                sprintf(
                                    'Failed to forward mail for <%s> (%d) to <%s>',
                                    $this->_userMail,
                                    $this->_userID,
                                    $forwardTo,
                                ),
                    ...
                    PutLog(
                        sprintf(
                            'Will not forward mail for <%s> (storeResultCheck: %d, addressValidityCheck: %d, returnPathCheck: %d, idCheck: %d)',
                            $this->_userMail,
                            $storeResult == STORE_RESULT_OK,
                            BMUser::AddressValid($forwardTo, false),
                            ExtractMailAddress(
                                $mail->GetHeaderValue('return-path'),
                            ) != '' || $bm_prefs['returnpath_check'] == 'no',
                            BMUser::GetID($forwardTo) !=
                                $this->_userObject->_id,
                        ),
                        PRIO_DEBUG,
                        __FILE__,
                        __LINE__,
        ...
        // draft responder?
        if (
            $filterActionFlags & FILTER_ACTIONFLAG_RESPOND &&
            strpos(strtolower($mail->GetHeaderValue('precedence')), 'junk') ===
                false &&
            ...
            foreach ($filterEval->_respondWith as $draftID) {
                if (
                    ($draftMail = $this->GetMail($draftID)) &&
                    is_object($draftMail) &&
                        ...
                        if ($fp = fopen('php://temp', 'wb+')) {
                            ...
                            while (!feof($draftMailFP)) {
                                ...
                                if (
                                    ...
                                    (strtolower(substr($line, 0, 12)) !=
                                        'return-path:' &&
                                        strtolower(substr($line, 0, 3)) !=
                                            'to:' &&
                                        strtolower(substr($line, 0, 3)) !=
                                            'cc:' &&
                                        strtolower(substr($line, 0, 4)) !=
                                            'bcc:' &&
                                        strtolower(substr($line, 0, 13)) !=
                                            'x-forward-to:' &&
                                        strtolower(substr($line, 0, 5)) !=
                                            'date:')
                                ) {
                                    fwrite($fp, $line);
                                }
                            ...
                            $sendMail->SetUserID($this->_userID);
                            $sendMail->SetSender(
                                $draftMail->GetHeaderValue('from'),
                            );
                            $sendMail->SetRecipients($mailFrom);
                            $sendMail->SetSubject(
                                ($subject = $draftMail->GetHeaderValue(
                                    'subject',
                                )) != ''
                                    ? $subject
                                    : '(no subject)',
                            ...
                            if ($sendMail->Send()) {
                                PutLog(
                                    sprintf(
                                        'Sent draft response mail for <%s> (%d) to <%s>',
                                        $this->_userMail,
                                        $this->_userID,
                                        $mailFrom,
                                    ),
                                ...
                                PutLog(
                                    sprintf(
                                        'Failed to send draft response mail from <%s> (%d) to <%s>',
                                        $this->_userMail,
                                        $this->_userID,
                                        $mailFrom,
                                    ),
        ...
        if (
            ...
            if (
                ...
                if ($success) {
                    ...
                    PutLog(
                        sprintf(
                            'Sent autoresponder from <%s> (%d) to <%s>',
                            $this->_userMail,
                            $this->_userID,
                            $mailFrom,
                        ),
                    ...
                    PutLog(
                        sprintf(
                            'Failed to send autoresponder from <%s> (%d) to <%s>',
                            $this->_userMail,
                            $this->_userID,
                            $mailFrom,
                        ),
        ...
        if ($storeResult == STORE_RESULT_OK) {
            ...
            PutLog(
                sprintf(
                    '<%s> (%d%s) received mail from <%s>%s',
                    $this->_userMail,
                    $this->_userID,
                    $aliasReceiver
                        ? sprintf(', to alias/group <%s>', $aliasReceiver)
                        : '',
                    ExtractMailAddress($mail->GetHeaderValue('from')),
        ...
        } elseif ($storeResult == STORE_RESULT_INTERNALERROR) {
            PutLog(
                sprintf(
                    'Failed to store mail for <%s> (%d) from <%s>: Internal error',
                    $this->_userMail,
                    $this->_userID,
                    ExtractMailAddress($mail->GetHeaderValue('from')),
                ),
        ...
        } elseif ($storeResult == STORE_RESULT_MAILTOOBIG) {
            PutLog(
                sprintf(
                    'Failed to store mail for <%s> (%d) from <%s>: Mail too big',
                    $this->_userMail,
                    $this->_userID,
                    ExtractMailAddress($mail->GetHeaderValue('from')),
                ),
        ...
        } elseif ($storeResult == STORE_RESULT_NOTENOUGHSPACE) {
            PutLog(
                sprintf(
                    'Failed to store mail for <%s> (%d) from <%s>: Not enough space left in user mailbox',
                    $this->_userMail,
                    $this->_userID,
                    ExtractMailAddress($mail->GetHeaderValue('from')),
                ),
    ...
    function SendCertMail($mailID, $mailObj) {
        ...
        foreach ($recipients as $to) {
            $db->Query(
                'INSERT INTO {pre}certmails(mail,recipient,user,code,date) VALUES(?,?,?,?,?)',
                $mailID,
                $to,
                $this->_userID,
            ...
            if ($certMailID = $db->InsertId()) {
                $vars = [
                    'user_name' =>
                        $this->_userObject->_row['vorname'] .
                        ' ' .
                        $this->_userObject->_row['nachname'],
                    'user_mail' => DecodeEMail(
                        ExtractMailAddress($mailObj->GetHeaderValue('from')),
                    ),
                    'date' => date(
                        'd.m.Y, H:i',
                        time() + $bm_prefs['einsch_life'],
                    ),
                    'url' => sprintf(
                        '%sindex.php?action=readCertMail&id=%d&key=%s',
                        $bm_prefs['selfurl'],
                        $certMailID,
                        $certMailCode,
                    ),
                ...
                if (
                    ...
                    $db->Query(
                        'DELETE FROM {pre}certmails WHERE id=?',
                        $certMailID,
                    );
                }
            }
    ...
    function GetCertMail($id, $key) {
        ...
        if (strlen($key) == 32) {
            $res = $db->Query(
                'SELECT id,mail,recipient,user,date,code,confirmation_sent FROM {pre}certmails WHERE id=? AND code=?',
                $id,
                $key,
            );
            if ($res->RowCount() != 1) {
                        ...
                        if ($mailObject = $mailbox->GetMail($row['mail'])) {
                            if ($row['confirmation_sent'] == 0) {
                                ...
                                $vars = [
                                    ...
                                    'subject' => $mailObject->GetHeaderValue(
                                        'subject',
                                    ),
                                    'date' => date(
                                        $userObject->_row['datumsformat'],
                                    ),
                                ];
                                SystemMail(
                                ...
                                PutLog(
                                    sprintf(
                                        'Certified mail <%d> read by <%s> (user: %d, mail: %d, key: %s, IP: %s)',
                                        $id,
                                        $row['recipient'],
                                        $row['user'],
                                        $row['mail'],
                                ...
                                if ($fp = fopen('php://temp', 'wb+')) {
                                    ...
                                    $sendMail->SetSender(
                                        $mailObject->GetHeaderValue('from'),
                                    );
                                    ...
                                    if ($sendMail->Send()) {
                                        ...
                                        PutLog(
                                            sprintf(
                                                'Failed to send certified mail <%d> to <%s>',
                                                $row['id'],
                                                $row['recipient'],
                                            ),
                                            PRIO_WARNING,
                                ...
                                //
                                $db->Query(
                                    'UPDATE {pre}certmails SET confirmation_sent=1 WHERE id=?',
                                    $id,
                                );
                            }
    ...
    function GetPageFolderList() {
        ...
        foreach ($folderList as $folderID => $folder) {
            ...
            $pageMenu[] = [
                'link' =>
                    'email.php?folder=' . $folderID . '&sid=' . session_id(),
        ...
        foreach ($pageMenu as $key => $val) {
            ...
            } elseif (isset($idTable[$val['parent']])) {
                $pageMenu[$key]['parent'] = $idTable[$val['parent']];
            } else {
                $pageMenu[$key]['parent'] = -1;
            }
        }
        return [$folderList, $pageMenu];
    }
    ...
    function MailAccessAllowed($mailID, $writeAccess) {
        ...
        if (is_array($mailID)) {
            ...
            $res = $db->Query(
                'SELECT id,folder,userid FROM {pre}mails WHERE `id` IN ?',
                $mailID,
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            $res = $db->Query(
                'SELECT folder,userid FROM {pre}mails WHERE `id`=?',
                $mailID,
            );
            if ($res->RowCount() != 1) {
                return false;
            }
            [$folderID, $userID] = $res->FetchArray(MYSQLI_NUM);
            $res->Free();
            ...
            return BMWorkgroup::AccessAllowed(
                $this->_userID,
                WORKGROUP_TYPE_MAILFOLDER,
                $folderID,
                $writeAccess,
