...
class BMSMS {
    ...
    function GetOutbox($sortColumn = 'id', $sortOrder = 'DESC') {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,`from`,`to`,`text`,price,`date` FROM {pre}smsend WHERE isSMS=1 AND user=? AND deleted=0 ORDER BY `' .
                $sortColumn .
                '` ' .
                $sortOrder .
                ' LIMIT 15',
    ...
    function DeleteOutboxEntry($id) {
        ...
        $db->Query(
            'UPDATE {pre}smsend SET `deleted`=1,`from`=?,`to`=?,`text`=? WHERE id=? AND user=?',
            '',
            '',
            '',
            $id,
    ...
    function GetTypes() {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,titel,typ,std,price,gateway,flags,maxlength FROM {pre}smstypen ORDER BY titel ASC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $result[$row['id']] = [
                'id' => $row['id'],
    ...
    function GetMaxChars($typeID = 0) {
        ...
        if ($typeID > 0) {
            $res = $db->Query(
                'SELECT maxlength FROM {pre}smstypen WHERE id=?',
                $typeID,
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    function GetGateway($id = 0) {
        ...
        $res = $db->Query(
            'SELECT id,titel,getstring,success,`user`,`pass` FROM {pre}smsgateways WHERE id=?',
            $id,
        );
        if ($res->RowCount() == 1) {
    ...
    function Send(
        ...
        if ($type == 0) {
            ...
            if ($type == 0) {
                PutLog(
                    sprintf(
                        'Default SMS type <%d> not found while trying to send SMS from <%s> to <%s> (userID: %d)',
                        $type,
                        $from,
                        $to,
                        $this->_userID,
        ...
        if (isset($types[$type])) {
            ...
            PutLog(
                sprintf(
                    'SMS type <%d> not found while trying to send SMS from <%s> to <%s> (userID: %d)',
                    $type,
                    $from,
                    $to,
                    $this->_userID,
        ...
        if ($charge) {
            ...
            if ($balance < $type['price']) {
                PutLog(
                    sprintf(
                        'Failed to send SMS from <%s> to <%s>: Not enough credits (userID: %d)',
                        $from,
                        $to,
                        $this->_userID,
                    ),
        ...
        if (!$gateway) {
            PutLog(
                sprintf(
                    'Gateway <%d> not found while trying to send SMS with type <%d> from <%s> to <%s> (userID: %d)',
                    $type['gateway'],
                    $type['id'],
                    $from,
                    $to,
        ...
        $getString = str_replace('%%to%%', _urlencode($to), $getString);
        $getString = str_replace('%%toPlus%%', _urlencode($toPlus), $getString);
        $getString = str_replace(
            '%%msg%%',
        ...
        $success =
            (trim($gateway['success']) == '' && trim($result) == '') ||
            strpos(strtolower($result), strtolower($gateway['success'])) !==
                false;

        // log
        PutLog(
            sprintf(
                'SMS success: %d; expected result: <%s>; gateway result: <%s> (userID: %d)',
                $success,
                $gateway['success'],
                $result,
                $this->_userID,
        ...
        if ($success) {
            // status ID?
            if (
                preg_match('/Status\:([0-9]*)/', $result, $reg) &&
                is_array($reg) &&
                isset($reg[1])
            ) {
                $statusID = $reg[1];
            ...
            // charge, if requested
            if ($charge) {
                $outboxID = $this->_userObject->Debit(
                    $type['price'] * -1,
                    $lang_user['tx_sms'],
                );
            }
            ...
            if ($putToOutbox) {
                if ($outboxID == 0) {
                    $db->Query(
                        'INSERT INTO {pre}smsend(user,monat,price,isSMS,`from`,`to`,`text`,statusid,`date`) VALUES(?,?,?,?,?,?,?,?,?)',
                        $this->_userID,
                        (int) date('mY'),
                        0,
                        1,
                    ...
                    $db->Query(
                        'UPDATE {pre}smsend SET isSMS=?,`from`=?,`to`=?,`text`=?,statusid=?,`date`=? WHERE id=? AND user=?',
                        1,
                        $from,
                        $to,
                        $text,
            ...
            PutLog(
                sprintf(
                    'Sent SMS from <%s> to <%s> (type: %d; statusID: %d; charged: %d; userID: %d)',
                    $from,
                    $to,
                    $type['id'],
                    $statusID,
            ...
            PutLog(
                sprintf(
                    'Failed to send SMS from <%s> to <%s> (type: %d; charged: 0; userID: %d)',
                    $from,
                    $to,
                    $type['id'],
                    $this->_userID,
