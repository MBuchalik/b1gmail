...
class BMUnZIP {
            ...
            foreach ($tree as $id => $item) {
                ...
                if ($parentFolderName != '.' && $parentFolderName != '') {
                    ...
                    foreach ($tree as $id2 => $item2) {
                        if (
                            $item2['type'] == 'folder' &&
                            $item2['fileName'] == $parentFolderName
                        ) {
    ...
    function ExtractFile($fileNo, $fp = false, $sizeLimit = -1) {
        ...
        if (
            !in_array($this->_centralDirStruct[$fileNo]['compressionMethod'], [
                8,
                12,
            ])
        ) {
        ...
        fseek(
            $this->_fp,
            $this->_centralDirStruct[$fileNo]['relativeOffset'],
            SEEK_SET,
        );
        ...
        $_fileHeader = @unpack(
            'Vsignature/vversionNeeded/vflags/vcompressionMethod/vmTime/vmDate/Vcrc32/VcompressedSize/VuncompressedSize/vfileNameLength/vextraFieldLength',
            $fileHeader,
        );
        ...
        fseek(
            $this->_fp,
            $_fileHeader['fileNameLength'] + $_fileHeader['extraFieldLength'],
            SEEK_CUR,
        );
        ...
        if ($_fileHeader['compressedSize'] > 0) {
            ...
            if ($fp !== false) {
                fwrite(
                    $fp,
                    $uncompressedData,
                    $sizeLimit != -1 ? $sizeLimit : strlen($uncompressedData),
                );
            } else {
    ...
    function _readCentralDirStruct() {
        ...
        if (substr($endOfCDS, 0, 4) != pack('V', 0x06054b50)) {
            if (DEBUG) {
                trigger_error(
                    'File corrupt or not in ZIP format',
                    E_USER_NOTICE,
                );
            }
            return;
        ...
        $_endOfCDS = @unpack(
            'Vsignature/vdiskNo/vcdsStartDiskNo/vcdsDiskEntryCount/vcdsTotalEntryCount/VcdsSize/VcdsOffset/vcommentLength',
            $endOfCDS,
        );
        if (!$_endOfCDS) {
            if (DEBUG) {
                trigger_error(
                    'File corrupt or not in ZIP format (eoCDS broken)',
                    E_USER_NOTICE,
                );
            }
            return;
        ...
        for (
            $i = 0;
            $i < $_endOfCDS['cdsDiskEntryCount'] && !feof($this->_fp);
            $i++
        ) {
            $fileHeader = fread($this->_fp, 46);

            $_fileHeader = @unpack(
                'Vsignature/vversion/vversionNeeded/vflags/vcompressionMethod/vmTime/vmDate/Vcrc32/VcompressedSize/VuncompressedSize/vfileNameLength/vextraFieldLength/vfileCommentLength/vdiskNumberStart/vinternalAttrs/VexternalAttrs/VrelativeOffset',
                $fileHeader,
            );
            ...
            if ($_fileHeader['fileNameLength'] > 0) {
                $_fileHeader['fileName'] = fread(
                    $this->_fp,
                    $_fileHeader['fileNameLength'],
                );
            }
            if ($_fileHeader['extraFieldLength'] > 0) {
                $_fileHeader['extraField'] = fread(
                    $this->_fp,
                    $_fileHeader['extraFieldLength'],
                );
            }
            if ($_fileHeader['fileCommentLength'] > 0) {
                $_fileHeader['fileComment'] = fread(
                    $this->_fp,
                    $_fileHeader['fileCommentLength'],
                );
            }

            if (
                substr($_fileHeader['fileName'], -1) == '/' &&
                $_fileHeader['uncompressedSize'] == 0
            ) {
                continue;
            }
