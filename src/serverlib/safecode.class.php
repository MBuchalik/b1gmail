...
class BMCaptchaGenerator {
    ...
    function __construct($code) {
        ...
        $this->letterW = floor(
            ($this->w - 2 * $this->borderSpacing) / strlen($this->code),
        );
    ...
    function _placeLetters() {
        for ($i = 0; $i < strlen($this->code); $i++) {
            $letter = $this->code[$i];
            $letterImg = $this->_generateLetter(
                $letter,
                imagecolorsforindex(
                    $this->img,
                    imagecolorat(
                        $this->img,
                        $this->borderSpacing + $i * $this->letterW,
                        $this->borderSpacing,
                    ),
                ),
            );
...
class Safecode {
    ...
    static function RequestCode() {
        ...
        $db->Query(
            'INSERT INTO {pre}safecode(code,generation) VALUES(?,UNIX_TIMESTAMP())',
            $code,
        );
    ...
    function ReleaseCode($id) {
        ...
        $db->Query('DELETE FROM {pre}safecode WHERE id=?', $id);
    }
    ...
    function GetCode($id, $new = false) {
        ...
        if ($new) {
            $code = Safecode::CodeGen(6);
            $db->Query(
                'UPDATE {pre}safecode SET code=? WHERE id=?',
                $code,
                $id,
            );
            return $code;
        } else {
            $res = $db->Query('SELECT code FROM {pre}safecode WHERE id=?', $id);
            $row = $res->FetchArray();
            $res->Free();
            return $row['code'];
        }
    ...
    function DumpCode($id, $perturbation = -1) {
        ...
        // get code from db
        $z = Safecode::GetCode($id, true);

        // generate
        $generator = _new('BMCaptchaGenerator', [$z]);
