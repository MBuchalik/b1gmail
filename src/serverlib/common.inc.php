...
function postProcessCertInfo($certInfo) {
    if (isset($certInfo['subject'])) {
        if (
            isset($certInfo['subject']['OU']) &&
            is_array($certInfo['subject']['OU'])
        ) {
            $certInfo['subject']['OU'] = array_shift(
                $certInfo['subject']['OU'],
            );
        }
        if (
            ...
            $certInfo['subject']['CN'] = array_shift(
                $certInfo['subject']['CN'],
            );
        }
    }
...
function CreateActionToken($userID, $action, $expires) {
    ...
    $db->Query(
        'INSERT INTO {pre}actiontokens(`userid`,`token`,`action`,`created`,`expires`) VALUES(?,?,?,?,?)',
        $userID,
        $token,
        $action,
        time(),
...
function CheckActionToken($userID, $action, $token, $deleteIfValid = true) {
    ...
    $res = $db->Query(
        'SELECT `actiontokenid` FROM {pre}actiontokens WHERE `userid`=? AND `action`=? AND `token`=? AND `expires`>?',
        $userID,
        $action,
        $token,
        time(),
    ...
    [$actionTokenID] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();

    if ($deleteIfValid) {
        $db->Query(
            'DELETE FROM {pre}actiontokens WHERE `actiontokenid`=?',
            $actionTokenID,
        );
    }
...
function GetActionTokenAge($userID, $token) {
    ...
    $res = $db->Query(
        'SELECT `created` FROM {pre}actiontokens WHERE `userid`=? AND `token`=?',
        $userID,
        $token,
    );
    if ($res->RowCount() == 0) {
    ...
    [$created] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();
...
function CheckDateValidity($date, $ruleExpression) {
    ...
    if (
        !preg_match('/([\<\>\=]+)\s*([0-9]+)\s*([DMY])/', $rule, $results) ||
        count($results) != 4
    ) {
        PutLog(
            sprintf(
                'Invalid rule expression for date field: "%s"',
                $ruleExpression,
            ),
            PRIO_WARNING,
            __FILE__,
    ...
    if (!in_array($op, ['=', '>=', '<=', '>', '<'])) {
        PutLog(
            sprintf(
                'Invalid comparison operator "%s" in rule expression for date field: "%s"',
                $op,
                $ruleExpression,
            ),
            PRIO_WARNING,
...
function GetAbuseTypePrefs($type) {
    ...
    $res = $db->Query(
        'SELECT `prefs` FROM {pre}abuse_points_config WHERE `type`=?',
        $type,
    );
...
function AddAbusePoint($userID, $type, $comment) {
    ...
    if (!isset($apConfigCache)) {
        $apConfigCache = [];
        $res = $db->Query(
            'SELECT `type`,`points` FROM {pre}abuse_points_config',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    // add entry
    $db->Query(
        'INSERT INTO {pre}abuse_points(`userid`,`date`,`type`,`points`,`comment`) VALUES(?,?,?,?,?)',
        $userID,
        time(),
        $type,
        $points,
...
function ArchiveLogs(
    ...
    if ($saveInArchive) {
        ...
        if (!$fp) {
            PutLog(
                sprintf(
                    'Failed to create log archive file: %s - log archiving has been aborted',
                    $fileName,
                ),
                PRIO_WARNING,
                __FILE__,
        ...
        $res = $db->Query(
            'SELECT prio,eintrag,zeitstempel FROM {pre}logs WHERE zeitstempel<' .
                $date .
                ' ORDER BY id ASC',
        );
...
function IsMobileUserAgent() {
    $mobileUserAgents = ['iPhone', 'Android', 'webOS', 'BlackBerry', 'iPod'];
...
function ChangelogAdded($itemType, $itemID, $added) {
    ...
    $res = $db->Query(
        'SELECT COUNT(*) FROM {pre}changelog WHERE `itemtype`=? AND `itemid`=?',
        $itemType,
        $itemID,
    );
    ...
    if ($count == 0) {
        $db->Query(
            'INSERT INTO {pre}changelog(`itemtype`,`itemid`,`userid`,`created`) VALUES(?,?,?,?)',
            $itemType,
            $itemID,
            $userRow['id'],
            $added,
        ...
        $db->Query(
            'UPDATE {pre}changelog SET `created`=? WHERE `itemtype`=? AND `itemid`=?',
            $added,
            $itemType,
            $itemID,
        );
...
function ChangelogUpdated($itemType, $itemID, $updated) {
    ...
    if ($count == 0) {
        $db->Query(
            'INSERT INTO {pre}changelog(`itemtype`,`itemid`,`userid`,`updated`) VALUES(?,?,?,?)',
            $itemType,
            $itemID,
            $userRow['id'],
            $updated,
        ...
        $db->Query(
            'UPDATE {pre}changelog SET `updated`=? WHERE `itemtype`=? AND `itemid`=?',
            $updated,
            $itemType,
            $itemID,
        );
...
function ChangelogDeleted($itemType, $itemID, $deleted) {
    ...
    if ($count == 0) {
        $db->Query(
            'INSERT INTO {pre}changelog(`itemtype`,`itemid`,`userid`,`deleted`) VALUES(?,?,?,?)',
            $itemType,
            $itemID,
            $userRow['id'],
            $deleted,
        ...
        $db->Query(
            'UPDATE {pre}changelog SET `deleted`=? WHERE `itemtype`=? AND `itemid`=?',
            $deleted,
            $itemType,
            $itemID,
        );
...
function HTMLFormat(
    ...
    $allowEncodingRepair = true
) {
    global $currentCharset;

    if (PHPNumVersion() >= 523) {
        ...
        $res = @htmlspecialchars($in, ENT_COMPAT, $currentCharset);
        if (!$allowDoubleEncoding) {
            $res = preg_replace('/\&amp;([a-zA-Z0-9\#]*);/', '&$1;', $res);
        }
    }
...
function GetDomainList($purpose = '') {
    ...
    $result = [];
    $res = $db->Query(
        'SELECT `domain` FROM {pre}domains' .
            $where .
            ' ORDER BY `pos` ASC,`domain` ASC',
    );
    while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        $result[] = $row['domain'];
    }
    ...
    if (isset($plugins) && is_object($plugins)) {
        ModuleFunction('OnGetDomainList', [&$result]);
    }
...
function CheckPOP3Login($host, $port, $user, $pass, $ssl = false) {
    ...
    $pop3 = _new('BMPOP3', [($ssl ? 'ssl://' : '') . $host, $port]);
    if ($pop3->Connect()) {
...
function SystemMail($from, $to, $subject, $templateName, $vars, $forUser = -1) {
    ...
    if ($result) {
        ...
        PutLog(
            sprintf(
                'Sent system mail <%s> from <%s> to <%s>',
                $templateName,
        ...
        PutLog(
            sprintf(
                'Failed to send system mail <%s> from <%s> to <%s>',
                $templateName,
                ExtractMailAddress($from),
                ExtractMailAddress($to),
            ),
...
function GetPhraseForUser($userID, $var, $phrase) {
    ...
    // language?
    $res = $db->Query(
        'SELECT language FROM {pre}users WHERE id=?',
        (int) $userID,
    );
    [$language] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();
    if (trim($language) == '') {
...
function GetPhraseForLanguage($language, $var, $phrase) {
    ...
    if (isset($availableLanguages[$language])) {
        if (
            $var == 'lang_custom' &&
            ($lang_custom = $cacheManager->Get('langCustom:' . $language))
        ) {
...
function Add2Stat($type, $count = 1) {
    ...
    $res = $db->Query(
        'SELECT id FROM {pre}stats WHERE d=? AND m=? AND y=? AND typ=? LIMIT 1',
        $d,
        $m,
        $y,
        $type,
    ...
    if ($id != 0) {
        $db->Query(
            'UPDATE {pre}stats SET anzahl=anzahl+' .
                (int) $count .
                ' WHERE id=?',
            $id,
        );
    ...
    // ...or insert new row
        ...
        $db->Query(
            'INSERT INTO {pre}stats(d,m,y,typ,anzahl) VALUES (?,?,?,?,?)',
            $d,
            $m,
            $y,
            $type,
...
function ValidateMailAddress($address) {
    ...
    [$userName, $hostName] = explode('@', $address);
    $errNo = $errStr = '';
    ...
    if (@getmxrr($hostName, $mxHosts)) {
        ...
        $sock = @fsockopen($hostName, 25, $errNo, $errStr, SOCKET_TIMEOUT)
    ) {
        @fclose($sock);
        return true;
    }
...
function _new($class, $args = []) {
    ...
    if (isset($plugins) && is_object($plugins)) {
        foreach ($plugins->_plugins as $key => $value) {
            if (
                ($replacement = $plugins->callFunction(
                    'getClassReplacement',
                    $key,
                    false,
                    [$class],
    ...
    if (!class_exists($class)) {
        DisplayError(
            isset($replacingModule) ? 0x13 : 0x12,
            'Class not found',
            'The requested class cannot be found.' .
                (isset($replacingModule)
                    ? ' An installed plugin seems to be faulty.'
                    : ''),
            isset($replacingModule)
                ? sprintf(
...
function formatComposeText(
    ...
    if ($textMode == 'html') {
        if ($action == 'redirect') {
            ...
            $result =
                sprintf(
                    '<span style="font-family:arial;font-size:12px;">%s<br />' .
                    ...
                    $lang_user['from'],
                    HTMLFormat(
                        HTMLFormat(DecodeEMail($mail->GetHeaderValue('from'))),
                        true,
                    ),
                    $lang_user['date'],
                    date($thisUser->_row['datumsformat'], $mail->date),
                    $lang_user['to'],
                    HTMLFormat(
                        HTMLFormat(DecodeEMail($mail->GetHeaderValue('to'))),
                        true,
                    ),
                    $lang_user['subject'],
                    HTMLFormat(
        ...
        if ($action == 'redirect') {
            ...
            $result =
                sprintf(
                    ...
                    $lang_user['srcmsg'],
                    $lang_user['from'],
                    DecodeEMail($mail->GetHeaderValue('from')),
                    $lang_user['date'],
                    date($thisUser->_row['datumsformat'], $mail->date),
                    $lang_user['to'],
                    DecodeEMail($mail->GetHeaderValue('to')),
                    $lang_user['subject'],
...
function formatEMailHTMLText(
    ...
    $formatter = _new('BMHTMLEMailFormatter', [$in, $currentCharset]);
    ...
    if ($mobile) {
        $formatter->setComposeBaseURL(
            'email.php?action=compose&sid=' . session_id() . '&to=',
        );
        $formatter->setAttachmentBaseURL(
            'email.php?action=attachment&view=true&id=' .
                $mailID .
                '&sid=' .
                session_id() .
                '&attachment=',
        ...
        $formatter->setComposeBaseURL(
            'email.compose.php?sid=' . session_id() . '&to=',
        );
        $formatter->setAttachmentBaseURL(
            'email.read.php?action=downloadAttachment&view=true&id=' .
                $mailID .
                '&sid=' .
                session_id() .
                '&attachment=',
...
function formatEMailText($in, $html = true, $mobile = false) {
    ...
    if ($html) {
        ...
        $links = [];
        $in = preg_replace_callback(
            '/[a-zA-Z0-9\.\_-]*\@[^ ]*\.[a-zA-Z0-9\.\_-]*/' . $pcreSuffix,
            function ($matches) use ($mobile) {
                $match = HTMLEntities($matches[0]);
                return !$mobile
                    ? '<a target="_top" href="email.compose.php?to=' .
                            $match .
                            '&sid=' .
                            session_id() .
                            '">' .
                            $match .
                            '</a>'
                    : '<a target="_top" href="email.php?action=compose&to=' .
                            $match .
                            '&sid=' .
                            session_id() .
                            '">' .
        ...
        foreach ($bmLinks as $i => $link) {
            $in = str_replace(
                sprintf(':::_b1gMailLink:%d_:::', $i),
                sprintf(
                    '<a href="deref.php?%s" target="_blank">%s</a>',
                    $link,
                    $link,
                ),
                $in,
...
function getFileContents($fileName) {
    ...
    if (@filesize($fileName) > 1024 * 500) {
        PutLog(
            sprintf(
                'Getting whole file contents of big file <%s> (%.02f KB > 500 KB)',
                $fileName,
...
function getUploadedFiles($fieldName, $lifeTime = 28800) {
    ...
    if (
        isset($localFile) &&
        is_array($localFile) &&
        ((is_array($localFile['error']) &&
            in_array(UPLOAD_ERR_OK, $localFile['error'])) ||
            (!is_array($localFile['error']) &&
                $localFile['error'] == UPLOAD_ERR_OK))
    ) {
        if (is_array($localFile['tmp_name'])) {
            ...
            return $result;
        } elseif ($localFile['error'] == UPLOAD_ERR_OK) {
            $destinationFileID = RequestTempFile(
                $userRow['id'],
                time() + $lifeTime,
            );
...
function SendFile($file, $speed = -1) {
    if ($fp = @fopen($file, 'rb')) {
        ...
        PutLog(
            sprintf('Cannot open file <%s> for sending', $file),
            PRIO_WARNING,
            __FILE__,
            __LINE__,
        );
...
function ParseMailList($list, $utf8 = false) {
    ...
    $parts = ExplodeOutsideOfQuotation($list, [',', ';']);
 ...
 * @return array
 */
function ExplodeOutsideOfQuotation(
    $string,
    $separator,
    ...
    for ($i = 0; $i < strlen($string); $i++) {
        $c = $string[$i];
        if (
            ((!is_array($separator) && $c == $separator) ||
    ...
    for ($i = 0; $i < strlen((string) $id); $i++) {
        ...
        if (($i + 1) % 2 == 0) {
            ...
            if (
                !$readOnly &&
                !file_exists($dir) &&
                $bm_prefs['structstorage'] == 'yes' &&
                $i < strlen((string) $id) - 1
            ) {
                @mkdir($dir, 0777);
                @chmod($dir, 0777);
            }
...
function ExtractMailAddress($string, $utf8 = false) {
    ...
    if ($utf8) {
        if (
            preg_match_all(
                '/[a-zA-Z0-9&=\'\\.\\-_\\+]+@[^ ]+\\.+[a-zA-Z]{2,12}/u',
                CharsetDecode($string, false, 'utf8'),
                $ret_arr,
            ) > 0
        ) {
...
function RequestPrivileges($privileges, $return = false) {
    ...
    if (($privileges & PRIVILEGES_USER) != 0) {
        if (
            !isset($_SESSION['bm_userLoggedIn']) ||
            !$_SESSION['bm_userLoggedIn'] ||
            !isset($_SESSION['bm_userID']) ||
            !isset($_SESSION['bm_sessionToken']) ||
            $_SESSION['bm_sessionToken'] != SessionToken()
        ) {
            $ok = false;
        } else {
            $thisUser = _new('BMUser', [$_SESSION['bm_userID']]);
    ...
    if (($privileges & PRIVILEGES_ADMIN) != 0) {
        if (
            !isset($_SESSION['bm_adminLoggedIn']) ||
            !$_SESSION['bm_adminLoggedIn'] ||
            !isset($_SESSION['bm_adminID']) ||
            !isset($_SESSION['bm_adminAuth']) ||
            !isset($_SESSION['bm_sessionToken']) ||
            $_SESSION['bm_sessionToken'] != SessionToken()
        ) {
            $ok = false;
            ...
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                if (
                    md5($row['password'] . $_SERVER['HTTP_USER_AGENT']) ===
...
function Array2XML(
    ...
    if ($first) {
        ...
        header('Cache-Control: no-cache');
        $result .=
            '<?xml version="1.0" encoding="' . $currentCharset . '" ?>' . "\n";
    ...
    if (is_array($array)) {
        foreach ($array as $key => $value) {
            ...
            if (
                is_array($value) &&
                isset($value['#nodeParam']) &&
                is_array($value['#nodeParam'])
            ) {
            ...
            } elseif (is_array($value) && isset($value['#nodeData'])) {
...
function ModuleFunction($function, $args = false) {
    ...
    if (isset($plugins) && is_object($plugins)) {
        ...
        $args = [$function, false, false, $args];
        call_user_func_array([&$plugins, 'callFunction'], $args);
    } else {
        if (MAINTENANCE_MODE) {
            return false;
        }
        ...
        DisplayError(
            0x17,
            'Illegal plugin handler call',
            'b1gMail tried to call a plugin handler before the plugin system was intialized.',
            sprintf("Function:\n%s", $function),
            __FILE__,
            __LINE__,
        );
...
function GetTemplatePrefs($template) {
    ...
    $result = [];
    $res = $db->Query(
        'SELECT `key`,`value` FROM {pre}templateprefs WHERE `template`=?',
        $template,
    );
    while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
...
function DetectLanguage() {
    ...
    $acceptLanguages = array_map(
        'strtolower',
        array_filter(
            array_map('trim', explode(',', $_SERVER['HTTP_ACCEPT_LANGUAGE'])),
        ),
    );
...
function DetermineLanguage() {
    ...
    } elseif (isset($_COOKIE['bm_language'])) {
...
function ReadLanguage() {
    ...
    // initialize arrays
    $lang_admin = $lang_user = $lang_client = $lang_custom = [];
    ...
    if (!file_exists(B1GMAIL_DIR . 'languages/' . $language . '.lang.php')) {
        ...
        if (
            ...
            DisplayError(
                ...
                'The requested language file and the default language file do not exist.',
                sprintf(
                    "Language file:\n%s",
                    $bm_prefs['language'] . '.lang.php',
                ),
                __FILE__,
                __LINE__,
            );
    ...
    if (
        ($langData = $cacheManager->Get('langData:' . $language)) &&
        ...
        if (!@include B1GMAIL_DIR . 'languages/' . $language . '.lang.php') {
            ...
            DisplayError(
                0x04,
                'Language file invalid',
                'Parse error while including requested language file (corrupt file?).',
        ...
        $cacheManager->Set('langData:' . $language, [
            'ctime' => $langCTime,
            'admin' => $lang_admin,
            'user' => $lang_user,
            'client' => $lang_client,
...
function PutLog(
    ...
    $db->Query(
        'INSERT INTO {pre}logs(eintrag, zeitstempel, prio) VALUES(?, UNIX_TIMESTAMP(), ?)',
...
function b1gMailShutdown() {
    ...
    if (
        isset($tempFilesToReleaseAtShutdown) &&
        is_array($tempFilesToReleaseAtShutdown)
    ) {
		...
		<table width="100%">
         ...
         $number,
					...
					<div class="addInfo">
						...
						<?php if ($text !== false && $text != '') {
          echo nl2br(HTMLFormat($text));
      } ?>
					</div>
...
function CleanupTempFiles() {
    ...
    $res = $db->Query(
        'SELECT id,user FROM {pre}tempfiles WHERE expires<' . time(),
    );
    while ($row = $res->FetchArray()) {
...
function ValidTempFile($userID, $tempID) {
    ...
    $res = $db->Query(
        'SELECT COUNT(*) FROM {pre}tempfiles WHERE id=? AND user=?',
        $tempID,
        $userID,
    );
    [$count] = $res->FetchArray(MYSQLI_NUM);
...
function RequestTempFile($userID, $expires = -1, $allowMem = false) {
    ...
    // register file
    $db->Query(
        'INSERT INTO {pre}tempfiles(expires,user) VALUES(?,?)',
        $expires,
        $userID,
    );
    $tempID = $db->InsertID();
...
function ReleaseTempFile($userID, $tempID) {
    ...
    if (ValidTempFile($userID, $tempID)) {
        // release
        $db->Query(
            'DELETE FROM {pre}tempfiles WHERE id=? AND user=?',
            $tempID,
            $userID,
        );
