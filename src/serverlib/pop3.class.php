...
class BMPOP3 {
    ...
    function Connect() {
        ...
        if (!is_resource($this->_sock)) {
            PutLog(
                sprintf(
                    'POP3 connection to <%s:%d> failed (%d, %s)',
                    $this->_host,
                    $this->_port,
                    $errNo,
                    $errStr,
            ...
            if (substr($responseLine, 0, 1) != '+') {
                PutLog(
                    sprintf(
                        'POP3 server <%s:%d> did not return +OK',
                        $this->_host,
                        $this->_port,
                    ),
                    PRIO_DEBUG,
    ...
    function Login($user, $pass) {
        if (
            fwrite($this->_sock, 'USER ' . EncodeEMail($user) . "\r\n") &&
            substr(fgets2($this->_sock), 0, 1) == '+'
        ) {
            if (
                fwrite($this->_sock, 'PASS ' . $pass . "\r\n") &&
                substr(fgets2($this->_sock), 0, 1) == '+'
            ) {
                return true;
            } else {
                PutLog(
                    sprintf(
                        'POP3 server <%s:%d> rejected password for user <%s>',
                        $this->_host,
                        $this->_port,
                        EncodeEMail($user),
                    ),
            ...
            PutLog(
                sprintf(
                    'POP3 server <%s:%d> rejected username <%s>',
                    $this->_host,
                    $this->_port,
                    EncodeEMail($user),
                ),
    ...
    function GetMailList() {
        ...
        if (
            fwrite($this->_sock, 'LIST' . "\r\n") &&
            substr(fgets2($this->_sock), 0, 1) == '+'
        ) {
            while (($line = trim(fgets2($this->_sock))) != '.' && $line != '') {
            ...
            // try to get UIDs
            if (
                fwrite($this->_sock, 'UIDL' . "\r\n") &&
                substr(fgets2($this->_sock), 0, 1) == '+'
            ) {
                while (
                    ($line = trim(fgets2($this->_sock))) != '.' &&
                    $line != ''
                ) {
            ...
            PutLog(
                sprintf(
                    'LIST command at POP3 server <%s:%d> failed',
                    $this->_host,
                    $this->_port,
                ),
                PRIO_DEBUG,
    ...
    function RetrieveMail($num, $fp) {
        if (
            fwrite($this->_sock, 'RETR ' . (int) $num . "\r\n") &&
            substr(fgets2($this->_sock), 0, 1) == '+'
        ) {
            $oldPos = ftell($fp);
            $lineNo = 0;
            while (
                ($line = fgets2($this->_sock)) &&
    ...
    function RetrieveMailHeaders($num, $fp, $additionalBodyLines = 0) {
        if (
            fwrite(
                $this->_sock,
                'TOP ' . (int) $num . ' ' . (int) $additionalBodyLines . "\r\n",
            ) &&
            substr(fgets2($this->_sock), 0, 1) == '+'
        ) {
            $oldPos = ftell($fp);
