...
class BMDashboard {
    ...
    function getWidgetArray($widgetOrder) {
        ...
        foreach ($widgets as $widget) {
            if (in_array($widget, $widgetList)) {
                ...
                $tplWidgets[$widget] = [
                    'template' => $plugins->pluginResourcePath(
                        $plugins->getParam('widgetTemplate', $widget),
                        $widget,
                        'template',
                    ),
                    'hasPrefs' => $plugins->getParam('widgetPrefs', $widget),
                    'prefsW' => (int) $plugins->getParam(
                        'widgetPrefsWidth',
                        $widget,
                    ),
                    'prefsH' => (int) $plugins->getParam(
                        'widgetPrefsHeight',
                        $widget,
                    ),
                    'icon' =>
                        $plugins->getParam('widgetIcon', $widget) !== false
                            ? './plugins/templates/images/' .
                                $plugins->getParam('widgetIcon', $widget)
                            : '',
    ...
    function showWidgetPrefs($widgetName) {
        ...
        $allWidgets = array_merge(
            $plugins->getWidgetsSuitableFor(BMWIDGET_ORGANIZER),
            $plugins->getWidgetsSuitableFor(BMWIDGET_START),
        );

        if (
            in_array($widgetName, $allWidgets) &&
            $plugins->getParam('widgetPrefs', $widgetName)
        ) {
            $tpl->assign(
                'widgetPrefsURL',
                sprintf(
                    'start.php?action=showWidgetPrefs&name=%s&sid=%s',
                    $widgetName,
                    session_id(),
                ),
            );
