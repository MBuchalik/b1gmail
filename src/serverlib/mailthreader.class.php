...
class BMMailThreader {
    ...
    function _processReferences($refs, $level, $base) {
        ...
        foreach ($refs as $i => $ref) {
            if (!isset($this->_messages[$ref])) {
                ...
                $res = $db->Query(
                    'SELECT refs,msg_id FROM {pre}mails WHERE refs LIKE ? AND userid=?',
                    '%' . $ref . '%',
                    $this->_userID,
                );
                while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                    $this->_processReferences(
                        array_merge(ExtractMessageIDs($row['refs']), [
                            $row['msg_id'],
                        ]),
                        $thisLevel,
                        $ref,
    ...
    function _fetchMessages() {
        ...
        $res = $db->Query(
            'SELECT id,von,betreff,datum,msg_id FROM {pre}mails WHERE userid=? AND msg_id IN ?',
            $this->_userID,
    ...
    function GetThread($mailID) {
        ...
        // get message info
        $res = $db->Query(
            'SELECT refs,msg_id FROM {pre}mails WHERE id=? AND userid=?',
            (int) $mailID,
            $this->_userID,
        );
        if ($res->RowCount() == 0) {
        ...
        $this->_messages = [];
        $this->_processReferences(
            array_merge(ExtractMessageIDs($mailRow['refs']), [
                $mailRow['msg_id'],
            ]),
            0,
            $mailRow['msg_id'],
