...
class BMCache {
    ...
    function Get($key) {
        ...
        if (
            $bm_prefs['cache_parseonly'] == 'no' ||
            substr($key, 0, 10) == 'parsedMsg:'
        ) {
...
class BMCache_b1gMail extends BMCache {
    ...
    function _Set($key, $obj, $expires = 0) {
        ...
        if ($cacheFP) {
            ...
            $db->Query(
                'REPLACE INTO {pre}file_cache(`key`,expires,size) VALUES(?,?,?)',
                md5($key),
                time() + $expires,
                filesize($cacheFile),
            );
    ...
    function _Delete($key) {
        ...
        if (file_exists($cacheFile)) {
            $db->Query('DELETE FROM {pre}file_cache WHERE `key`=?', md5($key));
            unlink($cacheFile);
            return true;
        }
    ...
    function CleanUp($hard = false) {
        ...
        [$fileCacheSize] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        ...
        if ($hard || (int) $fileCacheSize > $bm_prefs['filecache_size']) {
            ...
            $res = $db->Query(
                'SELECT size,`key` FROM {pre}file_cache WHERE expires>0 ORDER BY expires ASC',
            );
            ...
            if ($hard || $fileCacheSize > $bm_prefs['filecache_size']) {
                $res = $db->Query(
                    'SELECT size,`key` FROM {pre}file_cache WHERE expires=0',
                );
                while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                    @unlink($this->_cacheFilename($row['key'], true));
                    $fileCacheSize -= $row['size'];
                    ...
                    if (
                        !$hard &&
                        $fileCacheSize < $bm_prefs['filecache_size']
                    ) {
                        break;
                    }
                }
            ...
            if (count($deleteIDs) > 0) {
                $db->Query(
                    'DELETE FROM {pre}file_cache WHERE `key` IN(\'' .
                        implode('\',\'', $deleteIDs) .
                        '\')',
                );
            }
...
class BMCache_memcache extends BMCache {
    ...
    function __construct() {
        ...
        if (class_exists('Memcache') || class_exists('Memcached')) {
            if (
                count($memcacheServers) > 0 &&
                trim($memcacheServers[0]) != ''
            ) {
                    ...
                    if (trim($server) != '') {
                        ...
                        if ($this->_memcache) {
                            $this->_memcache->addServer(
                                $serverHost,
                                $serverPort,
                                $bm_prefs['memcache_persistent'] == 'yes',
                                $serverWeight,
                            );
                        } else {
                ...
                PutLog(
                    'Cache configured for use with memcache, but no servers specified - caching will not work',
                    PRIO_WARNING,
                    __FILE__,
                    __LINE__,
                );
            ...
            PutLog(
                'Cache configured for use with memcache, but Memcache(d)-class not defined (missing/outdated memcache(d) extension?) - caching will not work',
                PRIO_WARNING,
                __FILE__,
                __LINE__,
            );
