...
class BMSearchIndex {
    ...
    public function deleteItem($itemID) {
        $stmt = $this->sdb->prepare(
            'DELETE FROM [index] WHERE [itemid]=:itemid',
        );
        $stmt->bindValue(':itemid', $itemID);
        $stmt->execute();

        $stmt = $this->sdb->prepare(
            'DELETE FROM [text] WHERE [itemid]=:itemid',
        );
        $stmt->bindValue(':itemid', $itemID);
        $stmt->execute();
    }
    ...
    public function search($query) {
        ...
        if (count($words) > 0) {
            ...
            $searchWords = [];
            $res = $this->sdb->query(
                'SELECT [wordid] FROM [word] WHERE [word] IN (' .
                    $wordListStr .
                    ')',
            );
            ...
            if (
                count($wordList) > 0 &&
                count($wordList) <= count($searchWords)
            ) {
                $res = $this->sdb->query(
                    'SELECT [index].[itemid],COUNT(*) AS [wordCount],[text].[text],SUM([count]) AS [occurenceCount] FROM [index] ' .
                        'LEFT JOIN [text] ON [text].[itemid]=[index].[itemid] ' .
                        'WHERE [index].[wordid] IN (' .
                        implode(',', $searchWords) .
                        ') ' .
        ...
        if (strlen($text) >= 3) {
            ...
            foreach ($tokens as $token) {
                if (isset($words[$token])) {
                    ...
                    $words[$token] = ['word' => $token, 'count' => 1];
                }
            }
            ...
            if (count($words)) {
                ...
                $stmt = $this->sdb->prepare(
                    'REPLACE INTO [text]([itemid],[text]) VALUES(:itemid,:text)',
                );
                $stmt->bindValue(':itemid', $itemID);
    ...
    public function addToIndex($words, $itemID) {
        ...
        foreach ($words as $item) {
            $stmt = $this->sdb->prepare(
                'INSERT INTO [index]([wordid],[itemid],[count]) VALUES(:wordid,:itemid,:count)',
            );
            $stmt->bindValue(
                ':wordid',
                $wordIDs[$item['word']],
    ...
    public function beginTx($immediate = false) {
        ...
        if ($this->txCounter == 0) {
            $result = $this->sdb->query(
                'BEGIN ' . ($immediate ? 'IMMEDIATE ' : '') . 'TRANSACTION',
            );
        }
        ++$this->txCounter;
        return $result;
    ...
    private function getWordIDs($words) {
        ...
        if (count($words) > 0) {
            ...
            $res = $this->sdb->query(
                'SELECT [wordid],[word] FROM [word] WHERE [word] IN (' .
                    $list .
                    ')',
            );
            while ($row = $res->fetchArray()) {
    ...
    public function optimize() {
        ...
        // remove unused words
        $this->sdb->query(
            'DELETE FROM [word] WHERE [wordid] IN ' .
    ...
    private function initDB() {
        ...
        $this->sdb->query(
            'CREATE INDEX IF NOT EXISTS [index_word] ON [word]([word])',
        );
        $this->sdb->query(
            'CREATE INDEX IF NOT EXISTS [index_wordid] ON [index]([wordid])',
        );
        $this->sdb->query(
            'CREATE INDEX IF NOT EXISTS [index_itemid] ON [index]([itemid])',
        );
    }
