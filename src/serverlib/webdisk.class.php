...
class BMWebdisk {
    ...
    function __construct($userID) {
        ...
        if (
            ...
            $db->Query(
                'UPDATE {pre}users SET traffic_down=0,traffic_up=0,traffic_status=? WHERE id=?',
                (int) date('m'),
                $userID,
            );
        }
    ...
    function GetPageFolderList() {
        ...
        $res = $db->Query(
            'SELECT `id`,`titel`,`parent`,`share` FROM {pre}diskfolders WHERE `user`=? ORDER BY `titel` ASC',
            $this->_userID,
        );
    ...
    function Forbidden($fileName, $mimeType) {
            ...
            foreach ($forbiddenExtensions as $val) {
                if (
                    (substr($val, -1) == '*' &&
                        strpos(
                            strtolower($fileName),
                            strtolower(substr($val, 0, -1)),
                        ) !== false) ||
                    (strlen($val) > 1 &&
                        strtolower(substr($fileName, -strlen($val))) == $val)
                ) {
                    return true;
                }
            }
        ...
        if ($mimeType != '') {
            $forbiddenMIMETypes = explode(
                ':',
                $bm_prefs['forbidden_mimetypes'],
            );
            foreach ($forbiddenMIMETypes as $val) {
                if (
                    strtolower(trim($val)) == strtolower($mimeType) ||
                    (substr($val, -1) == '*' &&
                        strtolower(substr($val, 0, -1)) ==
                            strtolower(substr($mimeType, 0, strlen($val) - 1)))
                ) {
                    return true;
                }
            }
    ...
    function ParsePath($path, $withFiles = false) {
        ...
        foreach ($layers as $layer) {
            $res = $db->Query(
                'SELECT id FROM {pre}diskfolders WHERE parent=? AND titel=? AND user=?',
                $parent,
                $layer,
                $this->_userID,
            );
            if ($res->RowCount() == 0) {
                // not found. file?
                $res = $db->Query(
                    'SELECT id FROM {pre}diskfiles WHERE ordner=? AND user=? AND dateiname=?',
                    $parent,
                    $this->_userID,
                    $layer,
                );
                if ($res->RowCount() == 0 || !$withFiles) {
                    ...
                    [$file] = $res->FetchArray(MYSQLI_NUM);
                    $res->Free();

                    if (substr($path, -(strlen($layer) + 1)) == '/' . $layer) {
                        return [$file, WEBDISK_ITEM_FILE];
                    } else {
                        return false;
                    }
                }
                ...
                // found, continue
                [$parent] = $res->FetchArray(MYSQLI_NUM);
                $res->Free();
                continue;
            }
        }

        return [$parent, WEBDISK_ITEM_FOLDER];
    }
    ...
    function GetUsedSpace() {
        ...
        $res = $db->Query(
            'SELECT diskspace_used FROM {pre}users WHERE id=?',
            $this->_userID,
        );
        assert($res->RowCount() != 0);
        [$usedSpace] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    function SetShareSettings($folderID, $shareFolder, $sharePW) {
        ...
        $db->Query(
            'UPDATE {pre}diskfolders SET share=?, share_pw=?, modified=? WHERE id=? AND user=?',
            $shareFolder ? 'yes' : 'no',
            $sharePW,
            time(),
            $folderID,
    ...
    function GetShares() {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,titel,share_pw,created,accessed,modified FROM {pre}diskfolders WHERE share=? AND user=? ORDER BY titel ASC',
            'yes',
            $this->_userID,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $result[] = [
                ...
                'size' => 0,
                'created' => $row['created'],
                'accessed' => $this->FolderDate(
                    $row['id'],
                    'accessed',
                    $row['accessed'],
                ),
                'modified' => $this->FolderDate(
                    $row['id'],
                    'modified',
                    $row['modified'],
                ),
    ...
    function RenameFile($fileID, $newName) {
        ...
        $db->Query(
            'UPDATE {pre}diskfiles SET dateiname=?, modified=? WHERE id=? AND user=?',
            $newName,
            time(),
            $fileID,
            $this->_userID,
    ...
    function RenameFolder($folderID, $newName) {
        ...
        $db->Query(
            'UPDATE {pre}diskfolders SET titel=?,modified=? WHERE id=? AND user=?',
            $newName,
            time(),
            $folderID,
            $this->_userID,
    ...
    function GetFolderInfo($folderID) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}diskfolders WHERE id=? AND user=?',
            $folderID,
            $this->_userID,
        );
        if ($res->RowCount() == 0) {
    ...
    function GetDBProps($path, $userID) {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT `name`,`value`,`xmlns` FROM {pre}diskprops WHERE user=? AND path=?',
            $userID,
            $path,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    function MoveFile($folderID, $fileID, $newName = false) {
        ...
        $db->Query(
            'UPDATE {pre}diskfiles SET dateiname=?,ordner=?,modified=? WHERE id=? AND user=?',
            $newName ? $newName : $fileInfo['dateiname'],
            $folderID,
            time(),
            $fileID,
    ...
    function MoveFolder($folderID, $moveID, $newName = false) {
        ...
        if (
            $moveID == $folderID ||
            $this->IsFolderChildOf($folderID, $moveID)
        ) {
            return false;
        }
        ...
        $db->Query(
            'UPDATE {pre}diskfolders SET titel=?,parent=?,modified=? WHERE id=? AND user=?',
            $newName ? $newName : $folderInfo['titel'],
            $folderID,
            time(),
            $moveID,
    ...
    function IsShared($folderID) {
        ...
        foreach ($path as $pathItem) {
            if ($pathItem['share'] == 'yes') {
                return [true, trim($pathItem['share_pw'])];
            }
        }
    ...
    function DeleteFolder($folderID) {
        ...
        foreach ($folderContent as $item) {
            ...
            } elseif ($item['type'] == WEBDISK_ITEM_FOLDER) {
        ...
        // delete folder
        $db->Query(
            'DELETE FROM {pre}diskfolders WHERE id=? AND user=?',
            $folderID,
            $this->_userID,
        );
        return true;
    ...
    function IsFolderChildOf($folderID, $srcFolderID) {
        ...
        $res = $db->Query(
            'SELECT id FROM {pre}diskfolders WHERE parent=? AND user=?',
            $srcFolderID,
            $this->_userID,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    function CopyFile($folderID, $fileID, $newName = false) {
        ...
        $sourceFP = BMBlobStorage::createProvider(
            ...
            $this->_userID,
        )->loadBlob(BMBLOB_TYPE_WEBDISK, $fileID);
        if ($sourceFP) {
            if (
                BMBlobStorage::createDefaultWebdiskProvider(
                    $this->_userID,
                )->storeBlob(BMBLOB_TYPE_WEBDISK, $id, $sourceFP)
            ) {
                fclose($sourceFP);
                return $id;
            }
            ...
            PutLog(
                sprintf('Cannot copy webdisk file #%d to #%d', $fileID, $id),
                PRIO_ERROR,
                __FILE__,
                __LINE__,
            );
    ...
    function UpdateSpace($bytes) {
        ...
        if ($bytes < 0) {
            $db->Query(
                'UPDATE {pre}users SET diskspace_used=diskspace_used-LEAST(diskspace_used,' .
                    abs($bytes) .
                    ') WHERE id=?',
                $this->_userID,
            );
            ...
            $db->Query(
                'UPDATE {pre}users SET diskspace_used=diskspace_used+' .
                    abs($bytes) .
                    ' WHERE id=?',
                $this->_userID,
            );
    ...
    function GetFileSize($fileID) {
        ...
        $res = $db->Query(
            'SELECT size FROM {pre}diskfiles WHERE id=? AND user=?',
            $fileID,
            $this->_userID,
        );
    ...
    function CreateFile($folderID, $fileName, $mimeType, $fileSize) {
        ...
        if (
            $this->Forbidden($fileName, $mimeType) ||
        ...
        $db->Query('BEGIN');
        $db->Query(
            'INSERT INTO {pre}diskfiles(user,dateiname,ordner,size,contenttype,created,accessed,modified,blobstorage) VALUES(?,?,?,?,?,?,?,?,?)',
            $this->_userID,
            $fileName,
            $folderID,
            $fileSize,
    ...
    function FileExists($folderID, $fileName) {
        ...
        $res = $db->Query(
            'SELECT id FROM {pre}diskfiles WHERE dateiname=? AND ordner=? AND user=?',
            $fileName,
            $folderID,
            $this->_userID,
        );
    ...
    function GetFilePath($fileID) {
        ...
        $res = $db->Query(
            'SELECT dateiname,ordner FROM {pre}diskfiles WHERE id=? AND user=?',
            $fileID,
            $this->_userID,
        );
        assert($res->RowCount() != 0);
        [$thisFilename, $thisFolder] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();

        $path = $this->GetFolderPath($thisFolder);
        $path[] = ['id' => $fileID, 'title' => $thisFilename];
    ...
    function FolderExists($folderID, $folderName) {
        ...
        $res = $db->Query(
            'SELECT id FROM {pre}diskfolders WHERE titel=? AND parent=? AND user=?',
            $folderName,
            $folderID,
            $this->_userID,
        );
    ...
    function CreateFolder($folderID, $folderName) {
        ...
        $db->Query(
            'INSERT INTO {pre}diskfolders(user,parent,titel,share,share_pw,created,accessed,modified) VALUES(?,?,?,?,?,?,?,?)',
            $this->_userID,
            $folderID,
            $folderName,
            'no',
    ...
    function GetFolderPath($folderID) {
        ...
        while ($parentID != 0) {
            $res = $db->Query(
                'SELECT id,titel,parent,share,share_pw FROM {pre}diskfolders WHERE id=? AND user=?',
                $parentID,
                $this->_userID,
            );
            if ($res->RowCount() == 0) {
    ...
    function GetFolderParent($id) {
        ...
        $res = $db->Query(
            'SELECT parent FROM {pre}diskfolders WHERE user=? AND id=?',
            $this->_userID,
            $id,
        );
        if ($res->RowCount() > 0) {
    ...
    function GetFolderContent($folderID, $sort = 'dateiname', $order = 'ASC') {
        ...
        $result = [];
        if (!in_array($sort, ['dateiname', 'size'])) {
            $sort = 'dateiname';
        }

        // folders
        $res = $db->Query(
            'SELECT id,titel,share,created,accessed,modified FROM {pre}diskfolders WHERE parent=? AND user=? ORDER BY titel ' .
                $order,
            $folderID,
            $this->_userID,
        );
        ...
        // file
        $res = $db->Query(
            'SELECT id,dateiname,size,created,accessed,modified,contenttype FROM {pre}diskfiles WHERE ordner=? AND user=? ORDER BY ' .
                $sort .
                ' ' .
                $order,
            $folderID,
    ...
    function UpdateFolderAccess($folderID) {
        ...
        $db->Query(
            'UPDATE {pre}diskfolders SET accessed=? WHERE user=? AND id=?',
            time(),
            $this->_userID,
            $folderID,
        );
    ...
    function UpdateFileAccess($fileID) {
        ...
        $db->Query(
            'UPDATE {pre}diskfiles SET accessed=? WHERE user=? AND id=?',
            time(),
            $this->_userID,
            $fileID,
        );
    ...
    function FolderDate($id, $type, $alt) {
        ...
        if (!in_array($type, ['modified', 'accessed', 'created'])) {
            return false;
        }
        ...
        $res = $db->Query(
            'SELECT ' .
                $type .
                ' FROM {pre}diskfiles WHERE ordner=? ORDER BY ' .
                $type .
                ' DESC LIMIT 1',
            $id,
        );
        ...
        $res = $db->Query(
            'SELECT ' .
                $type .
                ' FROM {pre}diskfolders WHERE parent=? ORDER BY ' .
                $type .
                ' DESC LIMIT 1',
            $id,
        );
