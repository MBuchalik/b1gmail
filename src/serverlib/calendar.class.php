...
class BMCalendar {
    ...
    function _getOccurencesInTimeframe($row, $start, $end) {
        ...
        $repeatCount = 0;
        $timeTo =
            $repeatFlags & CLNDR_REPEATING_UNTIL_DATE
                ? min($repeatTimes, $end)
                : $end;
        $timeOffset =
            $startDate -
        ...
        if ($repeating) {
            ...
            if ($repeatFlags & CLNDR_REPEATING_DAILY) {
                ...
                for (
                    $time =
                        $repeatStartDate + ($repeatValue - 1) * TIME_ONE_DAY;
                    $time <= $timeTo;
                    $time += $repeatValue * TIME_ONE_DAY
                ) {
                    if (!in_array((int) date('w', $time), $exceptDays, true)) {
                        $repeatCount++;
                        if (
                            $repeatFlags & CLNDR_REPEATING_UNTIL_COUNT &&
            ...
            elseif ($repeatFlags & CLNDR_REPEATING_WEEKLY) {
                for (
                    $time =
                        $repeatStartDate -
                        TIME_ONE_DAY / 2 +
                        $repeatValue * TIME_ONE_WEEK;
                    $time <= $timeTo;
                    $time += $repeatValue * TIME_ONE_WEEK
                ) {
                    $repeatCount++;
                    if (
                        $repeatFlags & CLNDR_REPEATING_UNTIL_COUNT &&
            ...
            // monthly
            elseif (
                $repeatFlags & CLNDR_REPEATING_MONTHLY_MDAY ||
                $repeatFlags & CLNDR_REPEATING_MONTHLY_WDAY
            ) {
                $monthRepeatStartDate = mktime(
                    0,
                    0,
                    0,
                    date('m', $repeatStartDate - TIME_ONE_DAY / 2),
                    1,
                    date('Y', $repeatStartDate - TIME_ONE_DAY / 2),
                );

                for (
                    $time = $monthRepeatStartDate;
                    $time <= $timeTo + TIME_ONE_MONTH;
                    $time += $repeatValue * TIME_ONE_MONTH
                ) {
                    $time += TIME_ONE_WEEK;
                    $time = mktime(
                        0,
                    ...
                    if ($repeatFlags & CLNDR_REPEATING_MONTHLY_MDAY) {
                        ...
                        for (
                            $mDay = 1;
                            $mDay < BMCalendar::GetDaysInMonth($month, $year);
                            $mDay++
                        ) {
                            if (
                                (int) date(
                                    'w',
                                    mktime(0, 0, 0, $month, $mDay, $year),
                                ) == $row['repeat_extra2']
                            ) {
                                $mDays[] = $mDay;
                            }
                    ...
                    $myTime =
                        mktime(0, 0, 0, $month, $day, $year) + $timeOffset;
    ...
    function GetDatesForTimeframe($start, $end, $group = -2) {
        ...
        // get dates
        $res = $db->Query(
            'SELECT id,user,title,location,text,`group`,startdate,enddate,reminder,flags,repeat_flags,repeat_times,repeat_value,repeat_extra1,repeat_extra2 ' .
                'FROM {pre}dates WHERE (((startdate>=? OR enddate<=? OR (startdate<=? AND enddate>=?)) AND repeat_flags=0) OR (repeat_flags>0 AND ((repeat_flags&' .
                CLNDR_REPEATING_UNTIL_DATE .
                ')=0 OR repeat_value<=?))) AND user=?' .
    ...
    function _sendNotification($date) {
        ...
        if ($date['flags'] & CLNDR_REMIND_EMAIL) {
            ...
            if (
                ...
                PutLog(
                    sprintf(
                        'Sent e-mail notification about date <%d> to user <%s> (%d)',
                        $date['id'],
                        $user->_row['email'],
                        $date['user'],
                    ),
        ...
        if ($ok) {
            $db->Query(
                'UPDATE {pre}dates SET last_reminder=? WHERE id=?',
                time(),
                $date['id'],
            );
        }
    ...
    function ProcessNotifications() {
        ...
        $remindDates = [];
        $res = $db->Query(
            'SELECT {pre}dates.id,{pre}dates.last_reminder,{pre}dates.user,{pre}dates.title,{pre}dates.location,{pre}dates.`text`,{pre}dates.`group`,{pre}dates.startdate,{pre}dates.enddate,{pre}dates.reminder,{pre}dates.flags,{pre}dates.repeat_flags,{pre}dates.repeat_times,{pre}dates.repeat_value,{pre}dates.repeat_extra1,{pre}dates.repeat_extra2 ' .
                'FROM {pre}dates ' .
    ...
    function GetGroups($sortColumn = 'title', $sortOrder = 'asc') {
        ...
        $res = $db->Query(
            'SELECT id,user,title,color,dav_uri,dav_uid FROM {pre}dates_groups WHERE user=? ' .
                'ORDER BY ' .
                $sortColumn .
                ' ' .
                $sortOrder,
    ...
    function GetGroup($id) {
        ...
        $res = $db->Query(
            'SELECT id,user,title,color,dav_uri,dav_uid FROM {pre}dates_groups WHERE user=? AND id=?',
            $this->_userID,
            (int) $id,
        );
        if ($res->RowCount() == 1) {
    ...
    function AddGroup($title, $color = 0, $davURI = '', $davUID = '') {
        ...
        $db->Query(
            'INSERT INTO {pre}dates_groups(user,title,color,dav_uri,dav_uid) VALUES(?,?,?,?,?)',
            $this->_userID,
            $title,
            (int) $color,
            $davURI,
    ...
    function UpdateGroup($id, $title, $color = 0) {
        ...
        $db->Query(
            'UPDATE {pre}dates_groups SET title=?,color=? WHERE id=? AND user=?',
            $title,
            (int) $color,
            (int) $id,
            $this->_userID,
    ...
    function DeleteGroup($groupID, $deleteDates = false) {
        ...
        if (!$deleteDates) {
            $res = $db->Query(
                'SELECT `id` FROM {pre}dates WHERE `group`=? AND `user`=?',
                (int) $groupID,
                $this->_userID,
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            $db->Query(
                'UPDATE {pre}dates SET `group`=-1 WHERE `group`=? AND user=?',
                (int) $groupID,
                $this->_userID,
            );
        } else {
            $res = $db->Query(
                'SELECT `id` FROM {pre}dates WHERE `user`=? AND `group`=?',
                $this->_userID,
                (int) $groupID,
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                ...
                $db->Query(
                    'DELETE FROM {pre}dates_attendees WHERE `date`=?',
                    $row['id'],
                );
            }
            $res->Free();
            ...
            $db->Query(
                'DELETE FROM {pre}dates WHERE `user`=? AND `group`=?',
                $this->_userID,
                (int) $groupID,
            );
        }
        ...
        $db->Query(
            'DELETE FROM {pre}dates_groups WHERE user=? AND id=?',
            $this->_userID,
            (int) $groupID,
        );
        return $db->AffectedRows() == 1;
    ...
    function GetDate($id) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}dates WHERE id=? AND user=?',
            (int) $id,
            $this->_userID,
        );
        if ($res->RowCount() == 1) {
    ...
    function GenerateMiniCalendar(
        ...
        [$columns, $days] = $this->GenerateCalendar($month, $year, $userID);
        ...
        $html .= '	<tr>' . "\n";
        $html .= sprintf(
            '		<th class="Caption" colspan="7"><a href="organizer.calendar.php?view=month&date=%d&sid=%s">%s</a></th>' .
                "\n",
        ...
        foreach ($columns as $wDay) {
            $html .=
                '		<th>' . $lang_user['weekdays_long'][$wDay] . '</th>' . "\n";
        }
        $html .= '	</tr>' . "\n";
        ...
        foreach ($days as $arrayKey => $dayItem) {
            ...
            if ($dayItem === false) {
                ...
                $html .= sprintf(
                    '		<td%s><a title="%d %s" href="organizer.calendar.php?date=%d&sid=%s">%d</a></td>' .
                        "\n",
    ...
    function GenerateCalendar(
        ...
        if ($userID == -1) {
            ...
            $dates = $this->GetDatesForTimeframe(
                $datesStart,
                $datesEnd,
                $group,
            );
        ...
        // called statically
            ...
            $datesStart = mktime(0, 0, 0, $month, 1, $year);
            $datesEnd = mktime(23, 59, 59, $month, $daysInMonth, $year);
            $dates = $userCalendar->GetDatesForTimeframe(
                $datesStart,
                $datesEnd,
                $group,
            );
        ...
        for ($i = 1; $i <= $daysInMonth; $i++) {
            ...
            foreach ($dates as $key => $val) {
                if (
                    ($val['startdate'] >= $dateStart &&
                        $val['startdate'] <= $dateEnd) ||
                    ($val['enddate'] >= $dateStart &&
                        $val['enddate'] <= $dateEnd) ||
                    ($val['startdate'] <= $dateStart &&
                        $val['enddate'] >= $dateEnd)
                ) {
    ...
    function AddDate($row, $attendees) {
        ...
        $db->Query(
            'INSERT INTO {pre}dates(user,title,location,text,`group`,startdate,enddate,reminder,flags,repeat_flags,repeat_times,repeat_value,repeat_extra1,repeat_extra2,dav_uri,dav_uid) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            $this->_userID,
            $row['title'],
            $row['location'],
            $row['text'],
        ...
        if ($dateID = $db->InsertId()) {
            ...
            foreach ($attendees as $contactID) {
                $db->Query(
                    'INSERT INTO {pre}dates_attendees(date,address) VALUES(?,?)',
                    $dateID,
                    $contactID,
                );
            }
    ...
    function DeleteDate($id) {
        ...
        $db->Query(
            'DELETE FROM {pre}dates WHERE user=? AND id=?',
            $this->_userID,
            (int) $id,
        );
    ...
    function ChangeDate($id, $row, $attendees) {
        ...
        $db->Query(
            'UPDATE {pre}dates SET title=?, location=?, text=?, `group`=?, startdate=?, enddate=?, reminder=?, flags=?, repeat_flags=?, repeat_times=?, repeat_value=?, repeat_extra1=?, repeat_extra2=?, `dav_uri`=?, `dav_uid`=? WHERE id=? AND user=?',
            $row['title'],
            $row['location'],
            $row['text'],
            $row['group'],
        ...
        if (count($attendees) > 0) {
            $db->Query(
                'DELETE FROM {pre}dates_attendees WHERE date=? AND address NOT IN(' .
                    implode(',', $attendees) .
                    ')',
                (int) $id,
            );
            foreach ($attendees as $contactID) {
                $db->Query(
                    'REPLACE INTO {pre}dates_attendees(date,address) VALUES(?,?)',
                    (int) $id,
                    $contactID,
                );
            }
    ...
    function Form2Row() {
        ...
        if (isset($_REQUEST['repeating'])) {
            ...
            if ($_REQUEST['repeat_interval'] == 'daily') {
                $row['repeat_flags'] |= CLNDR_REPEATING_DAILY;
                $row['repeat_value'] = max(
                    1,
                    $_REQUEST['repeat_interval_daily'],
                );
                $row['repeat_extra1'] =
                    isset($_REQUEST['repeat_daily_exceptions']) &&
                    is_array($_REQUEST['repeat_daily_exceptions']) &&
                    count($_REQUEST['repeat_daily_exceptions']) > 0
                ...
                $row['repeat_value'] = max(
                    1,
                    $_REQUEST['repeat_interval_weekly'],
                );
            } elseif ($_REQUEST['repeat_interval'] == 'monthly_mday') {
                $row['repeat_flags'] |= CLNDR_REPEATING_MONTHLY_MDAY;
                $row['repeat_value'] = max(
                    1,
                    $_REQUEST['repeat_interval_monthly_mday'],
                );
                $row['repeat_extra1'] = max(
                    1,
                    min(31, $_REQUEST['repeat_interval_monthly_mday_extra1']),
                );
            } elseif ($_REQUEST['repeat_interval'] == 'monthly_wday') {
                $row['repeat_flags'] |= CLNDR_REPEATING_MONTHLY_WDAY;
                $row['repeat_value'] = max(
                    1,
                    $_REQUEST['repeat_interval_monthly_wday'],
                );
                $row['repeat_extra1'] = max(
                    0,
                    min(4, $_REQUEST['repeat_interval_monthly_wday_extra1']),
                );
                $row['repeat_extra2'] = max(
                    0,
                    min(6, $_REQUEST['repeat_interval_monthly_wday_extra2']),
                );
            } elseif ($_REQUEST['repeat_interval'] == 'yearly') {
                $row['repeat_flags'] |= CLNDR_REPEATING_YEARLY;
                $row['repeat_value'] = max(
                    1,
                    $_REQUEST['repeat_interval_yearly'],
                );
            }
        }
