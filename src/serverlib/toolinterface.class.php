...
class BMToolInterface {
    ...
    function CreateWebSession($userName, $passwordPlain, $timezoneOffset) {
        ...
        [$res, $param] = BMUser::Login($userName, $passwordPlain, true, true);
    ...
    function CheckForMails($userName, $passwordHash) {
        ...
        if ($userInfo['loginOK'] > 0) {
            ...
            $mailbox = _new('BMMailbox', [$userID, $userName, $this->_user]);
            $result['status'] = 'OK';
            $result['recentMails'] = $mailbox->GetRecentMailCount();
            $result[
                'unreadNotifications'
            ] = $this->_user->GetUnreadNotifications();
        } else {
            $result['status'] = 'Invalid login';
        }
        return $result;
    ...
    function CreateWebdiskFolder(
        ...
        if ($userInfo['loginOK'] > 0) {
            $userID = $userInfo['userID'];
            $webdisk = _new('BMWebdisk', [$userID]);
            $result['folderID'] = $webdisk->CreateFolder(
                $parentFolderID,
                $fileName,
            );
    ...
    function DeleteWebdiskFolder($userName, $passwordHash, $folderID) {
        ...
        if ($userInfo['loginOK'] > 0) {
            $userID = $userInfo['userID'];
            $webdisk = _new('BMWebdisk', [$userID]);
            $result['status'] = $webdisk->DeleteFolder($folderID)
                ? 'OK'
                : 'Failed';
        } else {
            $result['status'] = 'Invalid login';
    ...
    function CreateWebdiskFile(
        ...
        if ($userInfo['loginOK'] > 0) {
            ...
            $spaceLimit =
                $this->_group->_row['webdisk'] +
                $this->_user->_row['diskspace_add'];
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        if ($fpIn) {
                            $uploadOK = BMBlobStorage::createDefaultWebdiskProvider(
                                $userRow['id'],
                            )->storeBlob(
                                BMBLOB_TYPE_WEBDISK,
                                $fileID,
                        ...
                        if (!$uploadOK) {
                            ...
                            // log
                            PutLog(
                                'Failed to create file, deleting webdisk file',
                                PRIO_ERROR,
                                __FILE__,
                                __LINE__,
                            );
                            ...
                            return $this->GetWebdiskFileInfo(
                                $userName,
                                $passwordHash,
                                $fileID,
                            );
    ...
    function CreateDraft($userName, $passwordHash) {
        ...
        if ($userInfo['loginOK'] > 0) {
            ...
            if (!isset($_POST['msg']) || !is_array($_POST['msg'])) {
                ...
                $mailObj = _new('BMMail', [0, false, $mailFP, false]);
                $mailObj->Parse();
                $mailObj->ParseInfo();
                if (
                    $mailbox->StoreMail($mailObj, FOLDER_DRAFTS) ==
                    STORE_RESULT_OK
                ) {
    ...
    function DownloadCurrentVersion($userName, $passwordHash, $os) {
        ...
        if ($userInfo['loginOK'] > 0) {
            $res = $db->Query(
                'SELECT `base_version`,`versionid`,`release_files` FROM {pre}tbx_versions WHERE `status`=? ORDER BY `versionid` DESC LIMIT 1',
                'released',
            );
            if ($res->RowCount() != 1) {
                header('HTTP/1.1 404 Not found');
            ...
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                ...
                header('Pragma: public');
                header(
                    'X-b1gMail-File-Signature: ' . $releaseFiles[$os . '_sig'],
                );
    ...
    function CheckLogin($userName, $passwordHash) {
        ...
        if ($userID != 0) {
            ...
            if ($res->RowCount() == 1) {
                ...
                if (
                    strtolower($row['passwort']) ===
                        strtolower(
                            md5($passwordHash . $row['passwort_salt']),
                        ) &&
                    $row['gesperrt'] == 'no'
                ) {
                    $group = $user->GetGroup();
                    ...
                    $latestVersion = '0.0.0';
                    $verRes = $db->Query(
                        'SELECT `versionid`,`base_version` FROM {pre}tbx_versions WHERE `status`=? ORDER BY `versionid` DESC LIMIT 1',
                        'released',
                    );
                    while ($verRow = $verRes->FetchArray(MYSQLI_ASSOC)) {
                        $latestVersion = sprintf(
                            '%s.%d',
                            $verRow['base_version'],
                            $verRow['versionid'],
                        );
                    ...
                    $result = [
                        ...
                        'notificationInterval' =>
                            $groupRow['notifications'] == 'yes'
                                ? $bm_prefs['notify_interval']
                                : 0,
                        'plugins' => [],
                    ];

                    if ($result['webdiskAccess']) {
                        $result['webdiskSpaceLimit'] =
                            $groupRow['webdisk'] + $user->_row['diskspace_add'];
                        $result['webdiskUsedSpace'] =
                            $user->_row['diskspace_used'];
                        $result['webdiskTrafficLimit'] =
                            $groupRow['traffic'] + $user->_row['traffic_add'];
                        $result['webdiskUsedTraffic'] =
                            $user->_row['traffic_down'] +
                            $user->_row['traffic_up'];
                        $result['webdiskMaxFileSize'] = ParsePHPSize(
                            ini_get('upload_max_filesize'),
                        );
                    }

                    $moduleResult = $plugins->callFunction(
                        'ToolInterfaceCheckLogin',
                        false,
                        true,
                        [&$user],
                    );
    ...
    function HandleNonexistentMethod($method, $params, &$result) {
        ModuleFunction('ToolInterfaceHandler', [
            $method,
            $params,
            &$result,
            &$this,
