...
if (class_exists('DOMDocument')) {
    ...
    class BMHTMLEMailFormatter {
        ...
        protected $hiddenTags = ['html', 'body', 'head'];
        protected $level = 0;
        ...
        public function __construct($code, $encoding) {
            ...
            $this->filteredTags = $this->filteredAttributes = [];
            $this->root = new DOMDocument('1.0', $encoding);
            @$this->root->loadHTML(
                '<?xml encoding="' . $encoding . '">' . $code,
            );
        }
        ...
        protected function formatAttributes($node, $lcTag) {
            ...
            foreach ($node->attributes as $name => $attr) {
                ...
                if (
                    $lcName == 'href' &&
                    preg_match('~^(http|https|ftp)://~i', $val)
                ) {
                    ...
                    if ($mailAddr) {
                        $val =
                            ($this->replyMode ? '' : $this->composeBaseURL) .
                            urlencode(ExtractMailAddress($val));
                        $addTarget = '_top';
                        $allow = true;
                    }
                } elseif (
                    ($lcName == 'src' && $lcTag == 'img') ||
                    $lcName == 'background'
                ) {
                    ...
                    if (
                        !$isExternal &&
                        preg_match('~^cid:~i', $val) &&
                        !$this->replyMode
                    ) {
                        $cid = substr($val, 4);
                        if (isset($this->cidMap[$cid])) {
                            $val =
                                $this->attachmentBaseURL . $this->cidMap[$cid];
                        } else {
                            $allow = false;
                        }
                    }
            ...
            if ($node->hasChildNodes()) {
                for (
                    ...
                    if ($node->nodeType == XML_TEXT_NODE) {
                        ...
                        if ($lcTag == 'style') {
                            if (
                                $this->isExternalCSS($this->formatNode($node))
                            ) {
                        ...
                        if ($allow) {
                            ...
                            if (in_array($lcTag, $this->hiddenTags)) {
                                ...
                                $result .=
                                    '<' .
                                    $node->tagName .
                                    $this->formatAttributes($node, $lcTag);
                                if (
                                    strlen($nodeCode) == 0 &&
                                    ($lcTag == 'br' || $lcTag == 'hr')
                                ) {
    ...
    class BMHTMLEMailFormatter {
        ...
        public function format() {
            ...
            $in = $this->htmlCode;
            $in = preg_replace(
                '/(' . implode('|', $scriptParams) . ")=([\"']*)/i",
                "blocked_\\1=\\2",
                $in,
            );
            $in = preg_replace(
                '/<(object|embed|script)([^>]*)>/i',
                "<span blocked=\"\\1\" style=\"display:none;\">",
                $in,
            );
            $in = preg_replace(
                '/<\/(object|embed|script)([^>]*)>/i',
            ...
            if (!$this->allowExternal) {
                ...
                $in = preg_replace(
                    "/href=\"([a-zA-Z]{3,6}):\/\//i",
                    'target="_blank" href="deref.php?\\1://',
                    $in,
                );
            }

            $in = preg_replace(
                "/href=\"mailto\:([a-zA-Z0-9\.\_-]*\@[a-zA-Z0-9\.\_-]*\.[a-zA-Z0-9\.\_-]*)([\?]*)([^&]*)\"/i",
                'target="_top" href="' . $composeBaseURL . '\\1&\\3"',
                $in,
            );
