...
class BMUser {
    ...
    function CouponValid($code, $where = '') {
        ...
        // get code from DB
        $res = $db->Query(
            'SELECT id,code,von,bis,usedby,anzahl,used,ver,valid_signup,valid_loggedin FROM {pre}codes WHERE code=?',
            $code,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                        ...
                        if (
                            ...
                            if (is_array($benefit)) {
                                ...
                                if ($benefit['sms'] > 0) {
                                    $this->Debit(
                                        $benefit['sms'],
                                        sprintf($lang_user['tx_coupon'], $code),
                                    );
                                }
                                if ($benefit['gruppe'] > 0) {
                                    $db->Query(
                                ...
                                $usedBy[] = $this->_id;
                                $db->Query(
                                    'UPDATE {pre}codes SET used=used+1,usedby=? WHERE id=?',
                                    serialize($usedBy),
                                    $row['id'],
                                );
