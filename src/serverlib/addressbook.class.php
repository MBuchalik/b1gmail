...
class BMAddressbook {
    ...
    function LookupEmail($email) {
        ...
        if (is_array($email)) {
            ...
            $res = $db->Query(
                'SELECT id FROM {pre}adressen WHERE user=? AND (email=? OR work_email=?) LIMIT 1',
                $this->_userID,
                $email,
                $email,
            );
    ...
    function Lookup($text) {
        ...
        $res = $db->Query(
            'SELECT id,vorname,nachname,email,work_email,default_address FROM {pre}adressen WHERE ' .
                'user=? AND (' .
                sprintf(
                    'email LIKE \'%s%%\' OR work_email LIKE \'%s%%\' OR CONCAT(vorname,\' \',nachname) LIKE \'%s%%\' ' .
        ...
        foreach ($groups as $group) {
            ...
            $addr[] = [
                'email' => sprintf('%d@contact.groups', $group['id']),
                'vorname' => '',
                'nachname' => $group['title'],
            ];
        }

        foreach ($addr as $email) {
            if (
                strtolower(substr($email['email'], 0, strlen($text))) ==
                    strtolower($text) ||
                strtolower(
                    substr(
                        $email['vorname'] . ' ' . $email['nachname'],
                        0,
                        strlen($text),
                    ),
                ) == strtolower($text) ||
    ...
    function GetGroupTitle($groupID) {
        ...
        $groupTitle = '-';
        $res = $db->Query(
            'SELECT title FROM {pre}adressen_gruppen WHERE user=? AND id=?',
            $this->_userID,
            $groupID,
        );
    ...
    function FindAddress($vorname, $nachname) {
        ...
        $result = 0;
        $res = $db->Query(
            'SELECT id FROM {pre}adressen WHERE vorname=? AND nachname=? AND user=? LIMIT 1',
            $vorname,
            $nachname,
            $this->_userID,
        );
    ...
    function GetAddressbook(
        ...
        if ($letter == '9') {
            ...
            for ($i = 0; $i <= 9; $i++) {
                $whereClause .= sprintf(
                    '{pre}adressen.nachname LIKE \'%s%%\' OR ',
                    $i,
                );
            }
            ...
            $whereClause = sprintf(
                ' AND {pre}adressen.nachname LIKE \'%s%%\' ',
                $letter,
            );
        } else {
            $whereClause = '';
        ...
        if ($groupID > 0) {
            $res = $db->Query(
                'SELECT {pre}adressen.* FROM {pre}adressen,{pre}adressen_gruppen_member WHERE {pre}adressen.user=?' .
                    $whereClause .
                    ' AND {pre}adressen.id={pre}adressen_gruppen_member.adresse AND {pre}adressen_gruppen_member.gruppe=? ORDER BY ' .
                    '{pre}adressen.' .
                    $sortColumn .
                    ' ' .
                    $sortOrder,
            ...
            $res = $db->Query(
                'SELECT * FROM {pre}adressen WHERE user=?' .
                    $whereClause .
                    ' ORDER BY ' .
                    $sortColumn .
                    ' ' .
    ...
    function GetGroupList($contactID = 0) {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT id,title,`dav_uri`,`dav_uid` FROM {pre}adressen_gruppen WHERE user=? ORDER BY title ASC',
            $this->_userID,
        );
        while ($row = $res->FetchArray()) {
            $result[$row['id']] = $row;
        ...
        if ($contactID > 0) {
            $res = $db->Query(
                'SELECT gruppe FROM {pre}adressen_gruppen_member WHERE adresse=?',
                $contactID,
            );
            while ($row = $res->FetchArray()) {
            ...
            foreach ($result as $id => $val) {
                $res = $db->Query(
                    'SELECT COUNT(*) FROM {pre}adressen_gruppen_member WHERE gruppe=?',
                    $id,
                );
                [$result[$id]['members']] = $res->FetchArray();
                $res->Free();
            }
        }
    ...
    function GroupExists($title) {
        ...
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}adressen_gruppen WHERE title=? AND user=?',
            $title,
            $this->_userID,
        );
        [$count] = $res->FetchArray(MYSQLI_NUM);
    ...
    function GroupAdd($title, $davURI = '', $davUID = '') {
        ...
        $db->Query(
            'INSERT INTO {pre}adressen_gruppen(title,user,`dav_uri`,`dav_uid`) VALUES(?,?,?,?)',
            $title,
            $this->_userID,
            $davURI,
            $davUID,
    ...
    function ChangePicture(
        ...
        $db->Query(
            'UPDATE {pre}adressen SET picture=? WHERE id=? AND user=?',
            $pictureData,
            (int) $contactID,
            $this->_userID,
        );
    ...
    function InvalidateSelfCompleteInvitation($contactID, $key) {
        ...
        $db->Query(
            'UPDATE {pre}adressen SET invitationCode=? WHERE id=? AND invitationCode=? AND LENGTH(invitationCode)=32',
            '',
            $contactID,
            $key,
        );
    ...
    function SendSelfCompleteInvitation($contactID, $addressType) {
        ...
        // fetch mail address
        $res = $db->Query(
            'SELECT email,work_email,invitationCode FROM {pre}adressen WHERE id=? AND user=?',
            $contactID,
            $this->_userID,
        );
        if ($res->RowCount() != 1) {
        ...
        [$eMail, $work_eMail, $invitationCode] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        ...
        // set code
        $db->Query(
            'UPDATE {pre}adressen SET invitationCode=? WHERE id=? AND user=?',
            $invitationCode,
            $contactID,
            $this->_userID,
        );
        ...
        $vars = [
            ...
            'link' => sprintf(
                '%sindex.php?action=completeAddressBookEntry&contact=%d&key=%s',
                $bm_prefs['selfurl'],
                $contactID,
                $invitationCode,
            ),
        ...
        if (
            SystemMail(
                ...
                $eMailTo = ExtractMailAddress(
                    $addressType == ADDRESS_PRIVATE ? $eMail : $work_eMail,
                ),
            ...
            PutLog(
                sprintf(
                    'User <%s> (%d) invited <%s> to complete his/her address book entry (contact id: %d, IP: %s)',
                    $userRow['email'],
                    $this->_userID,
                    $eMailTo,
                    $contactID,
    ...
    function AddContact(
        ...
        // add contact
        $db->Query(
            'INSERT INTO {pre}adressen(user,firma,vorname,nachname,strassenr,plz,ort,land,tel,fax,handy,email,work_strassenr,work_plz,work_ort,work_land,work_tel,work_fax,work_handy,work_email,web,kommentar,default_address,anrede,position,geburtsdatum,dav_uri,dav_uid) VALUES ' .
                '(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            $this->_userID,
            $firma,
            $vorname,
    ...
    function ContactGroup($contactID, $groupID) {
        global $db;
        $db->Query(
            'REPLACE INTO {pre}adressen_gruppen_member(adresse,gruppe) VALUES(?,?)',
            $contactID,
            $groupID,
        );
    ...
    function DeContactGroup2($contactID, $groupID) {
        ...
        $db->Query(
            'DELETE FROM {pre}adressen_gruppen_member WHERE `adresse`=? AND `gruppe`=?',
            $contactID,
            $groupID,
        );
        if ($db->AffectedRows() == 1) {
    ...
    function DeContactGroup($contactID) {
        ...
        $db->Query(
            'DELETE FROM {pre}adressen_gruppen_member WHERE adresse=?',
            $contactID,
        );
        return $db->AffectedRows() == 1;
    }
    ...
    function Delete($contactID) {
        ...
        // delete contact
        $db->Query(
            'DELETE FROM {pre}adressen WHERE id=? AND user=?',
            (int) $contactID,
            $this->_userID,
        );
    ...
    function DeleteGroup($groupID) {
        ...
        $db->Query(
            'DELETE FROM {pre}adressen_gruppen WHERE user=? AND id=?',
            $this->_userID,
            (int) $groupID,
        );
        ...
        if ($db->AffectedRows() == 1) {
            $db->Query(
                'DELETE FROM {pre}adressen_gruppen_member WHERE gruppe=?',
                (int) $groupID,
            );
    ...
    function GetContact($contactID) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}adressen WHERE id=? AND user=?',
            $contactID,
            $this->_userID,
        );
        if ($res->RowCount() == 0) {
    ...
    function GetContactForSelfCompleteInvitation($contactID, $key) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}adressen WHERE id=? AND LENGTH(invitationCode)=32 AND invitationCode=?',
            $contactID,
            $key,
        );
        if ($res->RowCount() == 0) {
    ...
    function GetGroup($groupID) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}adressen_gruppen WHERE id=? AND user=?',
            $groupID,
            $this->_userID,
        );
        if ($res->RowCount() == 0) {
    ...
    function ChangeGroup($groupID, $title) {
        ...
        $db->Query(
            'UPDATE {pre}adressen_gruppen SET title=? WHERE id=? AND user=?',
            $title,
            (int) $groupID,
            $this->_userID,
        );
    ...
    function ExportContacts($contactIDs) {
        ...
        $res = $db->Query(
            'SELECT ' .
                $this->_exportFields .
                ' FROM {pre}adressen WHERE user=? AND id IN ? ORDER BY id ASC',
            $this->_userID,
            $contactIDs,
        );
        $res->ExportCSV();
    ...
    function ExportGroupContacts($groupIDs) {
        ...
        $contactIDs = [];
        $res = $db->Query(
            'SELECT adresse FROM {pre}adressen_gruppen_member WHERE gruppe IN ?',
            $groupIDs,
        );
        while ($row = $res->FetchArray(MYSQLI_NUM)) {
            $contactIDs[] = $row[0];
    ...
    function GetGroupMembers($groupID) {
        ...
        $result = [];
        $res = $db->Query(
            'SELECT {pre}adressen.`id` AS `id`, {pre}adressen.`dav_uri` AS `dav_uri`, {pre}adressen.`dav_uid` AS `dav_uid` ' .
                'FROM {pre}adressen ' .
    ...
    function ExportAddressbook($lineBreakChar, $quoteChar, $sepChar) {
        ...
        $res = $db->Query(
            'SELECT ' .
                $this->_exportFields .
                ' FROM {pre}adressen WHERE user=? ORDER BY id ASC',
            $this->_userID,
        );
