...
$VIEWABLE_TYPES = array('text/html', 'image/jpeg', 'image/gif',
...
else if($bm_prefs['cache_type'] == CACHE_B1GMAIL)
	$cacheManager = new BMCache_b1gMail();
else if($bm_prefs['cache_type'] == CACHE_MEMCACHE)
	$cacheManager = new BMCache_memcache();

/**
 * initialize b1gMail
 */
$tempFilesToReleaseAtShutdown = array();
ReadLanguage();
InitializePlugins();
if(!MAINTENANCE_MODE)
{
...
else if(!file_exists($templateFolder . 'cache/'))
{
	// 0x07
...
else if(!is_writeable($templateFolder . 'cache/'))
{
	// 0x08
