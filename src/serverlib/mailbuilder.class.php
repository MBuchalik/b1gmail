...
class BMMailBuilder {
        ...
        $this->_parts[] = [
            ...
            'headerFields' => [
                'Content-Type' => sprintf(
                    '%s; name="%s"',
                    $contentType,
                    EncodeMailHeaderField($fileName),
                ),
                'Content-Transfer-Encoding' =>
                    $contentType == 'message/rfc822' ? '8bit' : 'base64',
    ...
    function Build($forSending = false, $recipients = false) {
        ...
        } elseif (count($this->_parts) == 1) {
            $multiPart = false;
            $this->_headerFields = array_merge(
                $this->_headerFields,
                $this->_parts[0]['headerFields'],
            );
        } else {
            return false;
        }
        ...
        if ($multiPart) {
            ...
            foreach ($this->_parts as $part) {
                fwrite($this->_fp, "\r\n" . '--' . $boundary . "\r\n");
                $this->_writePartHeader(
                    $part['headerFields'],
                    false,
                    !isset($part['noHeaderEncode']),
                );
    ...
    function Send() {
        ...
        } elseif (isset($this->_headerFields['From'])) {
        ...
        // build, if needed
        if (
            $this->Build(true, $recipients) === false ||
        ...
        $this->_sendMail->SetSubject(
            isset($this->_headerFields['Subject'])
                ? $this->_headerFields['Subject']
                : '(no subject)',
        );
    ...
    function _writePartBody($part) {
        ...
        if ($part['base64']) {
            if (!is_resource($part['data'])) {
                fwrite(
                    $this->_fp,
                    wordwrap(base64_encode($part['data']), 72, "\r\n", true),
                );
            } else {
