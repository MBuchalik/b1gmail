...
class BMMail {
    ...
    function __construct(
        ...
        ModuleFunction('OnGetMail', [$this->id, $this->_userID]);
    }
    ...
    function Parse() {
        ...
        // mail in cache?
        //
        if ($this->_useCache && $this->id > 0) {
            // try to find parsed message in cache
            $this->_parsed = $cacheManager->Get(
                'parsedMsg:' . $this->_userID . ':' . $this->id,
            );
        ...
        $this->_parsed = _new('BMMailParser', [$this->_fp]);
        $this->_parsed->Parse();
    ...
    function IsTrusted() {
        if (($trustToken = $this->GetHeaderValue('x-b1gmail-trusted')) != '') {
            $validToken = GenerateTrustToken(
                $this->GetHeaderValue('message-id'),
    ...
    function IsSigned() {
        ...
        return isset($partList[1]) &&
            isset($partList[1]['content-type']) &&
            in_array(strtolower($partList[1]['content-type']), [
    ...
    function MarkAsTrained() {
        ...
        $db->Query(
            'UPDATE {pre}mails SET trained=1 WHERE id=?',
            $this->_row['id'],
        );
    }
    ...
    function GetSearchText($withSubject = true) {
        ...
        if ($withSubject) {
            $from = DecodeEMail(
                ExtractMailAddress($this->GetHeaderValue('from')),
            );
    ...
    function GetTextParts() {
        ...
        foreach ($parts as $id => $info) {
            ...
            if (
                strtolower($info['content-disposition']) != 'attachment' &&
                (!isset($info['filename']) ||
                    trim($info['filename']) == 'unnamed')
            ) {
                if (
                    strtolower($info['content-type']) == 'text/plain' ||
                    ...
                    $result['text'] .= CharsetDecode(
                        $textPart,
                        $info['charset'] != ''
                            ? $info['charset']
                            : FALLBACK_CHARSET,
                    );
                } elseif (strtolower($info['content-type']) == 'text/html') {
                    $htmlBody = &$info['body'];
                    if ($htmlBody !== false) {
                        $htmlBody->Init();
    ...
    function GetAttachments() {
        ...
        $result = [];
        $parts = $this->GetPartList();
        $attachmentTypes = ['image', 'application'];
        $attachmentTypes2 = [
            'text/calendar' => 'ics',
            'text/vcard' => 'vcf',
            'text/x-vcard' => 'vcf',
    ...
    function GetMessageFP($allowOverride = true) {
        ...
        return BMBlobStorage::createProvider(
            $this->blobStorage,
            $this->_userID,
        )->loadBlob(BMBLOB_TYPE_MAIL, $this->id);
    }
    ...
    function GetMessageSize($allowOverride = true) {
        return BMBlobStorage::createProvider(
            $this->blobStorage,
            $this->_userID,
        )->getBlobSize(BMBLOB_TYPE_MAIL, $this->id);
    }
    ...
    function SendDispositionNotification() {
        ...
        $to = ExtractMailAddress(
            $this->GetHeaderValue('disposition-notification-to'),
        );

        if ($to != '' && ($this->flags & FLAG_DNSENT) == 0) {
            $subject =
                $lang_user['read'] . ': ' . $this->GetHeaderValue('subject');
            $vars = [
                'subject' => $this->GetHeaderValue('subject'),
            ...
            if (
                SystemMail(
                    $this->_userObject->GetDefaultSender(),
                    $to,
                    $subject,
                    'receipt_text',
                    $vars,
    ...
    function GetDeliveryStatus() {
        ...
        $res = $db->Query(
            'SELECT `recipient`,`status`,`delivered_to`,`updated` FROM {pre}maildeliverystatus WHERE `outboxid`=? ORDER BY `recipient` ASC',
            $this->_row['id'],
        );
        if ($res->RowCount() == 0) {
            return false;
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            } elseif ($row['status'] == MDSTATUS_DELIVERY_DEFERRED) {
                ++$result['deferredCount'];
            } elseif ($row['status'] == MDSTATUS_DELIVERY_FAILED) {
                ++$result['failedCount'];
            } elseif (
                $row['status'] == MDSTATUS_SUBMITTED_TO_MTA ||
                $row['status'] == MDSTATUS_RECEIVED_BY_MTA
            ) {
        ...
        $result['allDelivered'] =
            $result['deliveredCount'] == $result['allCount'];

        if ($result['deliveredCount']) {
            $result['statusText'] .=
                ' ' .
                sprintf($lang_user['mds_delivered'], $result['deliveredCount']);
        }
        if ($result['deferredCount']) {
            $result['statusText'] .=
                ' ' .
                sprintf($lang_user['mds_deferred'], $result['deferredCount']);
        }
        if ($result['failedCount']) {
            $result['statusText'] .=
                ' ' . sprintf($lang_user['mds_failed'], $result['failedCount']);
