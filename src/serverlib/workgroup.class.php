...
class BMWorkgroup {
    ...
    function Fetch($id = -1) {
        ...
        $res = $db->Query('SELECT * FROM {pre}workgroups WHERE id=?', $id);
        if ($res->RowCount() == 0) {
            return false;
        }
     ...
     * @return array
     */
    static function GetSimpleWorkgroupList(
        $userID,
        $withMembers = false,
        $excludeDeleted = true
    ) {
        ...
        $groups = [];
        $res = $db->Query(
            'SELECT {pre}workgroups.id AS id,{pre}workgroups.title AS title,{pre}workgroups.email AS email,{pre}workgroups.webdisk AS webdisk FROM {pre}workgroups,{pre}workgroups_member WHERE {pre}workgroups.id={pre}workgroups_member.workgroup AND {pre}workgroups_member.user=? ORDER BY title ASC',
            (int) $userID,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            if ($withMembers) {
                $members = [];
                $res2 = $db->Query(
                    'SELECT {pre}users.vorname AS vorname,{pre}users.nachname AS nachname,{pre}users.email AS email,{pre}users.id AS id FROM {pre}users,{pre}workgroups_member WHERE {pre}users.id={pre}workgroups_member.user AND {pre}workgroups_member.workgroup=? ' .
                        ($excludeDeleted
    ...
    function GetIDbyMail($email) {
        ...
        $res = $db->Query(
            'SELECT id FROM {pre}workgroups WHERE email=? OR email=? LIMIT 1',
            $email,
            '*@' . $domainPart,
        );
        if ($res->RowCount() > 0) {
    ...
    function GetMembers($id = -1, $excludeDeleted = true) {
        ...
        $members = [];
        $res = $db->Query(
            'SELECT {pre}users.id AS id, {pre}users.email AS email FROM {pre}users,{pre}workgroups_member WHERE {pre}users.id={pre}workgroups_member.user AND {pre}workgroups_member.workgroup=?' .
    ...
    function UserInGroup($userID, $groupID) {
        ...
        // get from db
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}workgroups_member WHERE workgroup=? AND user=?',
            (int) $groupID,
            (int) $userID,
        );
    ...
    function GetTitle($id) {
        ...
        $res = $db->Query('SELECT title FROM {pre}workgroups WHERE id=?', $id);
    ...
    static function AccessAllowed($userID, $shareType, $shareID, $writeAccess) {
        ...
        if (!isset($wgAccessCache[$userID])) {
            ...
            $res = $db->Query(
                'SELECT sharetype,shareid,writeaccess FROM {pre}workgroups_shares ' .
            ...
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                ...
                $wgAccessCache[$userID][$row['sharetype']][$row['shareid']] =
                    $row['writeaccess'] == 1 ? 'rw' : 'ro';
            }
            $res->Free();
        }
