...
class BMMailParser_PartBody {
    ...
    function __construct($fp, $offsetStart, $offsetLength, &$header) {
        ...
        $this->contentEncoding = $header->GetValueOnly(
    ...
    function Init() {
        ...
        while (in_array($c = fgetc($this->_fp), [' ', "\n", "\r"])) {
            --$this->offsetLength;
            ++$this->offsetStart;
        }
    ...
    function Read($size) {
        if (
            ftell($this->_fp) + $size >
            $this->offsetStart + $this->offsetLength
        ) {
            $size =
                $this->offsetStart + $this->offsetLength - ftell($this->_fp);
        }
        $str = str_replace(["\n", "\r", ' '], '', fread($this->_fp, $size));
        while (
            strlen($str) < $size &&
            ftell($this->_fp) <= $this->offsetStart + $this->offsetLength &&
...
class BMMailParser_Header {
    ...
    function GetValueParam($key, $param) {
        if (
            ...
            isset($this->items[$key]['params']) &&
            isset($this->items[$key]['params'][$param])
        ) {
    ...
    function ParseLine($line) {
        ...
        if ($spacePos === false) {
            $spacePos = $tabPos;
        } elseif ($tabPos !== false && $tabPos < $spacePos) {
            $spacePos = $tabPos;
        }
        ...
        if ($dotPos !== false && ($spacePos === false || $dotPos < $spacePos)) {
            ...
            if (
                isset($this->items[$fieldKey]) &&
                ($fieldKey == 'received' ||
                    $fieldKey == 'references' ||
                    $fieldKey == 'delivered-to')
            ) {
            ...
            ($dotPos === false ||
                ($spacePos !== false && $dotPos > $spacePos)) &&
            $this->_lastKey != ''
        ) {
    ...
    function DecodeHeaderEntries() {
        foreach ($this->items as $key => $val) {
            $this->items[$key] = DecodeMailHeaderField(
                CharsetDecode($val, FALLBACK_CHARSET),
            );
        }
    }
...
class BMMailParser_Part {
    ...
    function ParseInfo() {
        ...
        if (strpos($contentType, 'multipart/') !== false) {
            if (
                ($this->_boundary = $this->header->GetValueParam(
                    'content-type',
                    'boundary',
                )) != ''
            ) {
    ...
    function ParseBody() {
        ...
        if (
            strtolower($this->header->GetValueOnly('content-disposition')) !=
                'attachment' &&
            strtolower($this->header->GetValueOnly('content-type')) ==
                'message/rfc822'
        ) {
            $newPart = _new('BMMailParser_Part', [$this, $this->_myBoundary]);
            $newPart->Parse();
        ...
        // default processing
            ...
            while (is_resource($this->_fp) && !feof($this->_fp)) {
                ...
                if (
                    $this->_myBoundary !== false &&
                    strpos($line, '--' . $this->_myBoundary) === 0
                ) {
                    if (
                        strpos($line, '--' . $this->_myBoundary . '--') ===
                        false
                    ) {
                ...
                if ($this->_isMultipart) {
                    if (strpos($line, '--' . $this->_boundary) === 0) {
                        $newPart = _new('BMMailParser_Part', [
                            $this,
                            $this->_boundary,
                        ]);
                        $newPart->Parse();
        ...
        $this->bodyLength =
            ftell($this->_fp) -
            $this->bodyOffset -
            (feof($this->_fp) ? 0 : $lineLength);

        // store body object
        $this->body = _new('BMMailParser_PartBody', [
            $this->_fp,
            $this->bodyOffset,
            $this->bodyLength,
            $this->header,
    ...
    function ParseHeader() {
        while (
            !$this->EmptyLine($line = fgets2($this->_fp, PARSER_LINE_MAX)) &&
            is_resource($this->_fp) &&
            !feof($this->_fp)
        ) {
...
class BMMailParser {
    ...
    function __construct($fp) {
        $this->_fp = $fp;
        $this->rootPart = _new('BMMailParser_Part', [$this, false]);
        $this->partList = false;
    }
    ...
    function Parse() {
        if (is_resource($this->_fp)) {
            ...
            if (DEBUG) {
                PutLog(
                    sprintf(
                        'Parsed mail in %.04f seconds (%d bytes; throughput: %.02f KB/s)',
                        $processingTime,
                        ftell($this->_fp),
    ...
    function _generatePartListPart($part, &$a) {
        $filename = $a->header->GetValueParam(
            'content-disposition',
            'filename',
        );
        if ($filename == '') {
        ...
        $this->partList[$part] = [
            ...
            'content-disposition' => $a->header->GetValueOnly(
                'content-disposition',
            ),
            'content-transfer-encoding' => $a->header->GetValueOnly(
                'content-transfer-encoding',
            ),
