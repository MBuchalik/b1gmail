...
function CleanupNotifications() {
    ...
    $db->Query(
        'DELETE FROM {pre}notifications WHERE `date`<? AND `read`=1',
        time() - $lifetime,
    );
}
...
function ProcessBirthdayNotifications() {
    ...
    $res = $db->Query(
        'SELECT {pre}adressen.`id`,{pre}adressen.`user`,{pre}adressen.`geburtsdatum`,{pre}adressen.`vorname`,{pre}adressen.`nachname` FROM {pre}adressen ' .
    ...
    while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        if (
            (int) date('j', $row['geburtsdatum']) == $todayD &&
            (int) date('n', $row['geburtsdatum']) == $todayM
        ) {
            $years = (int) date('Y') - (int) date('Y', $row['geburtsdatum']);

            $user = _new('BMUser', [$row['user']]);
            $user->PostNotification(
                'notify_birthday',
                [HTMLFormat($row['vorname'] . ' ' . $row['nachname']), $years],
            ...
            $db->Query(
                'UPDATE {pre}adressen SET `last_bd_reminder`=? WHERE `id`=?',
                $todayY,
                $row['id'],
            );
        }
...
function AbuseCron() {
    ...
    if ($bm_prefs['ap_expire_last_run'] < time() - TIME_ONE_HOUR) {
        // run every hour
        if ($bm_prefs['ap_expire_mode'] == 'dynamic') {
            $res = $db->Query(
                'SELECT `userid` FROM {pre}abuse_points WHERE `expired`=0 GROUP BY `userid` HAVING MAX(`date`)<?',
            ...
            $res->Free();
        } elseif ($bm_prefs['ap_expire_mode'] == 'static') {
            $db->Query(
                'UPDATE {pre}abuse_points SET `expired`=1 WHERE `expired`=0 AND `date`<?',
                time() - $bm_prefs['ap_expire_time'],
            );
        }

        $db->Query('UPDATE {pre}prefs SET `ap_expire_last_run`=?', time());
    }

    // step 2: find and lock users who exceeded hard point limit
    if ($bm_prefs['ap_autolock'] == 'yes' && $bm_prefs['ap_hard_limit'] > 0) {
        $res = $db->Query(
            'SELECT {pre}users.`id` AS `id`,`email`,SUM(`points`) AS `pointsum` FROM {pre}users ' .
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            PutLog(
                sprintf(
                    'User <%s> (#%d) exceeded abuse points hard limit (%d >= %d) and has been locked',
                    $row['email'],
                    $row['id'],
                    $row['pointsum'],
                    $bm_prefs['ap_hard_limit'],
            ...
            if (
                $bm_prefs['ap_autolock_notify'] == 'yes' &&
                ($notifyTo = ExtractMailAddress(
                    $bm_prefs['ap_autolock_notify_to'],
                )) != ''
            ) {
                if (!isset($types)) {
                    $types = GetAbuseTypes();
                ...
                $points = [];
                $res2 = $db->Query(
                    'SELECT `points`,`type` FROM {pre}abuse_points WHERE `userid`=? AND `expired`=0 ORDER BY `entryid` DESC',
                    $row['id'],
                );
                ...
                $vars = [
                    ...
                    'link' => sprintf(
                        ...
                        $bm_prefs['selfurl'],
                        urlencode(
                            sprintf('abuse.php?do=show&userid=%d&', $row['id']),
                        ),
                    ),
                ];
                SystemMail(
