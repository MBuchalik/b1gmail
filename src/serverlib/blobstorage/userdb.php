...
class BMBlobStorage_UserDB extends BMAbstractBlobStorage {
    ...
    public function storeBlob($type, $id, $data, $limit = -1) {
        ...
        $flags = 0;
        if (
            (($type == BMBLOB_TYPE_WEBDISK &&
                $bm_prefs['blobstorage_webdisk_compress'] == 'yes') ||
                ($type == BMBLOB_TYPE_MAIL &&
                    $bm_prefs['blobstorage_compress'] == 'yes')) &&
        ...
        $stmt = $this->sdb->prepare(
            'REPLACE INTO [blobs]([type],[id],[data],[flags],[size]) VALUES(:type,:id,:data,:flags,:size)',
        );
        ...
        $stmt = $this->sdb->prepare(
            'SELECT COUNT(*) FROM [blobs] WHERE [type]=:type AND [id]=:id',
        );
        $stmt->bindValue(':type', $type, SQLITE3_INTEGER);
        $stmt->bindValue(':id', $id, SQLITE3_INTEGER);
        $res = $stmt->execute();
    ...
    public function loadBlob($type, $id) {
        $stmt = $this->sdb->prepare(
            'SELECT [data],[flags] FROM [blobs] WHERE [type]=:type AND [id]=:id',
        );
        $stmt->bindValue(':type', $type, SQLITE3_INTEGER);
        $stmt->bindValue(':id', $id, SQLITE3_INTEGER);
        $res = $stmt->execute();
    ...
    public function deleteBlob($type, $id) {
        $stmt = $this->sdb->prepare(
            'DELETE FROM [blobs] WHERE [type]=:type AND [id]=:id',
        );
        $stmt->bindValue(':type', $type, SQLITE3_INTEGER);
        $stmt->bindValue(':id', $id, SQLITE3_INTEGER);
        $stmt->execute();
    ...
    public function getBlobSize($type, $id) {
        $stmt = $this->sdb->prepare(
            'SELECT [size] FROM [blobs] WHERE [type]=:type AND [id]=:id',
        );
        $stmt->bindValue(':type', $type, SQLITE3_INTEGER);
        $stmt->bindValue(':id', $id, SQLITE3_INTEGER);
        $res = $stmt->execute();
