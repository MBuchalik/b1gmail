...
class BMUserPOP3Gateway {
    ...
    function Run($maxMails = -1) {
        ...
        $pop3Accounts = $this->_userObject->GetPOP3Accounts(
            'last_fetch',
            'ASC',
            true,
        );
        foreach ($pop3Accounts as $accountID => $account) {
            ...
            if (
                $account['last_fetch'] + $this->_groupRow['ownpop3_interval'] <
                time()
            ) {
                $partResult = true;
                ...
                // update last fetch
                $db->Query(
                    'UPDATE {pre}pop3 SET last_fetch=?,last_success=? WHERE id=?',
                    time(),
                    -1,
                    $this->_account['id'],
                );
    ...
    function ProcessMails($maxMails = -1) {
        ...
        if (!is_array($mailList)) {
            PutLog(
                sprintf(
                    'Failed to retrieve mail list from user defined POP3 server (user: %d)',
                    $this->_userID,
                ),
                PRIO_WARNING,
                __FILE__,
        ...
        if ($this->_account['p_keep']) {
            $res = $db->Query(
                'SELECT uid FROM {pre}uidindex WHERE pop3=?',
                $this->_account['id'],
            );
        ...
        foreach ($mailList as $mailNum => $mailInfo) {
            ...
            if ($mailInfo['size'] > $bm_prefs['mailmax']) {
                ...
                PutLog(
                    sprintf(
                        'Message not downloaded from user defined pop3 box (hard limit; %d > %d bytes; user: %d)',
                        $mailInfo['size'],
                        $bm_prefs['mailmax'],
                        $this->_userID,
                    ),
            ...
            } elseif (!in_array($mailInfo['uid'], $oldUIDs)) {
                // request temp file
                ...
                if ($this->_account['p_target'] != -1) {
                    $mailProcessor->SetTargetFolder(
                        $this->_account['p_target'],
                    );
                }
        ...
        foreach ($processedUIDs as $uid) {
            $db->Query(
                'REPLACE INTO {pre}uidindex(pop3,uid) VALUES(?,?)',
                $this->_account['id'],
                $uid,
            );
        }
    ...
    function ConnectToPOP3Box($account) {
        ...
        // connect
        $this->_pop3 = _new('BMPOP3', [$hostName, $account['p_port']]);
        $this->_pop3->SetTimeout(BMUSERPOP3_TIMEOUT, true);
        if (!$this->_pop3->Connect()) {
            PutLog(
                sprintf(
                    'Connection to user defined POP3 server <%s:%d> failed (user: %d)',
                    $hostName,
                    $account['p_port'],
                    $this->_userID,
                ),
        ...
        if (!$this->_pop3->Login($account['p_user'], $account['p_pass'])) {
            PutLog(
                sprintf(
                    'Login at user defined POP3 server <%s:%d> failed (user: %d)',
                    $hostName,
                    $account['p_port'],
                    $this->_userID,
                ),
