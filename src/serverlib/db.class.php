...
class DB {
    ...
    function Query($query) {
        ...
        if (is_array($bm_modules)) {
            foreach ($bm_modules as $mKey => $module) {
                if ($bm_modules[$mKey]->Features('MySQLHandle')) {
                    $handle = $bm_modules[$mKey]->MySQLHandle(
                        $query,
                        $handle,
                        $szUsername,
                    );
        ...
        if ($ok) {
            return isset($result)
                ? new DB_Result($handle, $result, $query)
                : false;
        } else {
            if (
                strpos(
                    strtolower($query),
                    'insert into ' . strtolower($mysql['prefix']) . 'logs',
                ) !== false
            ) {
                ...
                DisplayError(
                    ...
                    'Failed to write log entry to ' .
                        $mysql['prefix'] .
                        'logs-table. Please check and repair the table.',
                    sprintf(
                        "Process:\n%s\n\nError number:\n%d\n\nError description:\n%s",
                        'Query',
                        mysqli_errno($handle),
...
class DB_Result {
    ...
    function ExportCSV(
        ...
        foreach ($fields as $field) {
            $fieldList .=
                $sepChar . $quoteChar . addslashes($field) . $quoteChar;
        }
