...
class BMMailProcessor {
    ...
    function ProcessInternalMail($uid) {
        ...
        // store mail in DB
        $db->Query(
            'UPDATE {pre}testmails SET data=?, received=?, recv_count=recv_count+1 WHERE uid=?',
            $mailContents,
            time(),
            $uid,
        );
    ...
    function ProcessMail($failProcessing = STORE_RESULT_OK) {
        ...
        // parse mail
        $this->_mail = _new('BMMail', [0, false, $this->_fp, false]);
        $this->_mail->Parse();
        ...
        if (count($this->_recipients) > 0) {
            ...
            if (($storeResult = $this->CommonFilters()) == STORE_RESULT_OK) {
                ...
                foreach ($this->_recipients as $recpMail => $userID) {
                    ...
                    $recipientResult = $mailbox->ReceiveMail(
                        $this->_mail,
                        $this->_targetFolder,
                        $aliasReceiver,
                        $this->isUserPOP3,
                    ...
                    // check result
                    if (
                        $recipientResult != STORE_RESULT_OK &&
            ...
            ModuleFunction('OnMailWithoutValidRecipient', [&$this->_mail]);
        }

        // bounce?
        if (
            ($storeResult != STORE_RESULT_OK || count($recipientErrors) > 0) &&
        ...
        if (DEBUG) {
            PutLog(
                sprintf(
                    'Processed mail in %.04f seconds (mailbox processing time: %.04f seconds)',
                    $processingTime,
                    $mailboxTime,
                ),
                PRIO_DEBUG,
    ...
    function Bounce($storeResult, $recipientErrors) {
        ...
        $errorTable = [
            STORE_RESULT_OK => 'The message was not delivered to one or more recipients.',
            STORE_RESULT_INTERNALERROR => 'A severe internal error occured while trying to process the message.',
            STORE_RESULT_MAILTOOBIG => 'The message is too large. This server accepts messages with a limited size only.',
            STORE_RESULT_NOTENOUGHSPACE => 'The user exceeded his storage limit. Please try to send the message again later.',
            RECEIVE_RESULT_BLOCKED => 'The message was blocked by a content filter.',
            RECEIVE_RESULT_NO_RECIPIENTS => 'No valid recipients for this message could be determined.',
        ];
        ...
        //
        if (
            $bm_prefs['errormail'] == 'yes' ||
            ($bm_prefs['errormail'] == 'soft' &&
                $storeResult != RECEIVE_RESULT_NO_RECIPIENTS)
        ) {
            $returnPath = ExtractMailAddress(
                $this->_mail->GetHeaderValue('return-path'),
            );
            ...
            if (
                $returnPath != '' &&
                strpos(
                    strtolower($this->_mail->GetHeaderValue('content-type')),
                    'delivery-status',
                ) === false
            ) {
                // prepare text
                ...
                if (count($recipientErrors) > 0) {
                    ...
                    foreach (
                        $recipientErrors
                        as $recipientAddress => $recipientResult
                    ) {
                        $text .= sprintf(
                            '<%s>' . "\r\n" . '   (reason: %s)' . "\r\n",
                            $recipientAddress,
                ...
                $mail->AddHeaderField(
                    'From',
                    '"Mail Delivery System" <' . $bounceSender . '>',
                );
                $mail->AddHeaderField(
                    'Subject',
                    'Returned mail: see error report for details',
                );
                ...
                if ($success) {
                    PutLog(
                        sprintf(
                            'Sent delivery status notification about error %d (%d invalid recipients) to <%s>',
                            $storeResult,
                    ...
                    PutLog(
                        sprintf(
                            'Failed to send delivery status notification about error %d (%d invalid recipients) to <%s>',
                            $storeResult,
                            count($recipientErrors),
                            $returnPath,
                        ),
                ...
                PutLog(
                    sprintf(
                        'Sent no error mail to notify about error %d (%d invalid recipients): empty return-path',
                        $storeResult,
                        count($recipientErrors),
                    ),
                    PRIO_DEBUG,
        ...
        //
            ...
            PutLog(
                sprintf(
                    'Sent no error mail to notify about error %d (%d invalid recipients): error reporting disabled for this error',
                    $storeResult,
                    count($recipientErrors),
                ),
                PRIO_NOTE,
    ...
    function CommonFilters() {
        ...
        $recipients = [];
        $res = $db->Query(
            'SELECT id,field,expression,action,value FROM {pre}recvrules WHERE type=? ORDER BY action,id ASC',
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            if (
                ($headerValue = trim(
                    $this->_mail->GetHeaderValue(strtolower($row['field'])),
                )) != ''
            ) {
                ...
                if ($ruleResult > 0) {
                    ...
                    if ($row['action'] == RECVRULE_ACTION_BOUNCE) {
                        PutLog(
                            sprintf(
                                'Mail bounced because of receive rule <%d>',
                                $row['id'],
                            ),
                            PRIO_NOTE,
                            __FILE__,
                    ...
                    } elseif ($row['action'] == RECVRULE_ACTION_DELETE) {
                        PutLog(
                            sprintf(
                                'Mail deleted because of receive rule <%d>',
                                $row['id'],
                            ),
                            PRIO_NOTE,
                            __FILE__,
                        ...
                        $row['action'] == RECVRULE_ACTION_MARKINFECTED &&
                        ($this->_mail->flags & FLAG_INFECTED) == 0
                    ) {
                        $this->_mail->flags |= FLAG_INFECTED;
                    } elseif (
                        $row['action'] == RECVRULE_ACTION_MARKREAD &&
                        ($this->_mail->flags & FLAG_UNREAD) != 0
                    ) {
                        $this->_mail->flags &= ~FLAG_UNREAD;
                    } elseif (
                        $row['action'] == RECVRULE_ACTION_MARKSPAM &&
                        ($this->_mail->flags & FLAG_SPAM) == 0
                    ) {
                        $this->_mail->flags |= FLAG_SPAM;
                    } elseif ($row['action'] == RECVRULE_ACTION_SETINFECTION) {
        ...
        if ($bm_prefs['spamcheck'] == 'yes') {
            $filterModules[] = _new('BMMailFilter_DNSBL', [&$this->_mail]);
        }
        if (
            $bm_prefs['use_bayes'] == 'yes' &&
            $bm_prefs['bayes_mode'] == 'global'
        ) {
            $filterModules[] = _new('BMMailFilter_Bayes', [&$this->_mail]);
        }
        if ($bm_prefs['use_clamd'] == 'yes') {
            $filterModules[] = _new('BMMailFilter_ClamAV', [&$this->_mail]);
        }
    ...
    function ParseRecipients() {
        ...
        if ($bm_prefs['recipient_detection'] == 'static') {
            // x-b1gmail-to?
            $xb1gMailTo = ExtractMailAddress(
                $this->_mail->GetHeaderValue('x-b1gmail-to'),
            );
            if ($xb1gMailTo != '' && BMUser::GetID($xb1gMailTo, false) != 0) {
                $this->_recipients = [$xb1gMailTo];
                return;
            }

            // x-forward-to?
            $xForwardTo = ExtractMailAddress(
                $this->_mail->GetHeaderValue('x-forward-to'),
            );
            if ($xForwardTo != '' && BMUser::GetID($xForwardTo) != 0) {
                $this->_recipients = [$xForwardTo];
                return;
            }

            // x-forwarded-to?
            $xForwardedTo = ExtractMailAddress(
                $this->_mail->GetHeaderValue('x-forwarded-to'),
            );
            if ($xForwardedTo != '' && BMUser::GetID($xForwardedTo) != 0) {
                $this->_recipients = [$xForwardedTo];
                return;
            }

            // try to get recipient from other suitable header fields
            $recipients = ExtractMailAddresses(
                $this->_mail->GetHeaderValue('to'),
            );
            ...
            $recipients = array_merge(
                ...
                ExtractMailAddresses(
                    $this->_mail->GetHeaderValue('delivered-to'),
                ),
            );
            $recipients = array_merge(
                $recipients,
                ExtractMailAddresses(
                    $this->_mail->GetHeaderValue('envelope-to'),
                ),
            );
            $recipients = array_merge(
                $recipients,
                ExtractMailAddresses(
                    $this->_mail->GetHeaderValue('apparently-to'),
                ),
            );
            $recipients = array_merge(
                $recipients,
                ExtractMailAddresses(
                    $this->_mail->GetHeaderValue('x-original-to'),
                ),
            );
            $recipients = array_merge(
                $recipients,
                ExtractMailAddresses(
                    $this->_mail->GetHeaderValue('webde-forward'),
                ),
            );
            $this->_recipients = [];
        ...
        // the new way
        elseif ($bm_prefs['recipient_detection'] == 'dynamic') {
            // process rules
            $recipients = [];
            $res = $db->Query(
                'SELECT field,expression,action,value FROM {pre}recvrules WHERE type=? ORDER BY action,id ASC',
                ...
                if (
                    ...
                    if ($ruleResult > 0) {
                        ...
                        if ($row['action'] == RECVRULE_ACTION_ISRECIPIENT) {
                            ...
                            $row['action'] == RECVRULE_ACTION_SETRECIPIENT
                        ) {
                            if (
                                BMUser::GetID(
                                    ExtractMailAddress($resultValues[0]),
                            ...
                            $row['action'] == RECVRULE_ACTION_ADDRECIPIENT
                        ) {
                            $recipients = array_merge(
                                $recipients,
                                ExtractMailAddresses(
    ...
    function DetermineRecipients() {
        ...
        if (
            ...
            ($postmasterID = BMUser::GetID(
                $postmasterAddress = GetPostmasterMail(),
                true,
            )) != 0
        ) {
            // log
            PutLog(
                sprintf(
                    'Forwarding mail to postmaster (<%s>) because no valid recipients were found',
                    $postmasterAddress,
                ),
                PRIO_NOTE,
                __FILE__,
