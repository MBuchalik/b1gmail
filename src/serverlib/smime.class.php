...
class BMSMIME {
    ...
    function EncryptMail(&$mailBuilder, $recipients) {
        ...
        foreach ($recipients as $recipient) {
            $cert = $this->_userObject->GetCertificateForAddress(
                $recipient,
        ...
        $cert = $this->_userObject->GetCertificateForAddress(
            ExtractMailAddress($mailBuilder->_headerFields['From']),
    ...
    function DecryptMail(&$mail) {
        ...
        if (!$mail->IsEncrypted()) {
            return [SMIME_NOT_ENCRYPTED];
        }

        // get available priv. certs
        $privateCerts = $this->_userObject->GetKeyRing(
            'certificateid',
            'ASC',
            CERTIFICATE_TYPE_PRIVATE,
        );
        ...
        foreach ($privateCerts as $cert) {
            ...
            $privKeyPass = $this->_userObject->GetPrivateKeyPassword(
                $cert['hash'],
            );
        ...
        if ($resultCode == SMIME_DECRYPTED && file_exists($destTempFileName)) {
            ...
            $GLOBALS['tempFilesToReleaseAtShutdown'][] = [
                $this->_userID,
                $destTempID,
                $destFP,
            ];
    ...
    function CheckMailSignature(&$mail) {
        ...
        if (!$mail->IsSigned()) {
            return [SMIME_SIGNATURE_NOT_SIGNED];
        }

        // get root certs
        $rootCerts = array_values(
            array_merge(
                $this->GetRootCertificates(),
                $this->_userObject->GetRootCertificates(),
            ),
        );
        ...
        if (!$result) {
            ...
            $resultCode =
                $result === true
                    ? SMIME_SIGNATURE_OK_NOVERIFY
                    : SMIME_SIGNATURE_BAD;
        } else {
            $resultCode =
                $result === true ? SMIME_SIGNATURE_OK : SMIME_SIGNATURE_BAD;
        }

        // process cert
        if (file_exists($certTempFileName)) {
            $certHash = $this->_userObject->StoreCertificate(
                getFileContents($certTempFileName),
            );
        }
