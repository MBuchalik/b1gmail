...
class BMPOP3Gateway {
    ...
    function ProcessMails($maxMails = -1) {
        ...
        if (!is_array($mailList)) {
            PutLog(
                'Failed to retrieve mail list from catchall POP3 server',
                PRIO_WARNING,
                __FILE__,
                __LINE__,
            );
        ...
        foreach ($mailList as $mailNum => $mailInfo) {
            ...
            if ($mailInfo['size'] > $bm_prefs['mailmax']) {
                ...
                PutLog(
                    sprintf(
                        'Message too big (hard limit; %d > %d bytes) - processing headers only',
                        $mailInfo['size'],
                        $bm_prefs['mailmax'],
                    ),
                    PRIO_NOTE,
    ...
    function ConnectToPOP3Box() {
        ...
        if (!$this->_pop3->Connect()) {
            PutLog(
                sprintf(
                    'Connection to catchall POP3 server <%s:%d> failed',
                    $bm_prefs['pop3_host'],
                    $bm_prefs['pop3_port'],
                ),
                PRIO_WARNING,
        ...
        // login
        if (
            !$this->_pop3->Login($bm_prefs['pop3_user'], $bm_prefs['pop3_pass'])
        ) {
            PutLog(
                'Login at catchall POP3 server failed',
                PRIO_WARNING,
                __FILE__,
                __LINE__,
            );
