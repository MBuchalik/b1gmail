...
if ($_REQUEST['action'] == 'faq') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['add'])) {
            $db->Query(
                'INSERT INTO {pre}faq(frage,typ,lang,required,antwort) VALUES(?,?,?,?,?)',
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($faqIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}faq WHERE id IN(' .
                            implode(',', $faqIDs) .
                            ')',
                    );
                }
        ...
        $faqs = [];
        $res = $db->Query(
            'SELECT id,typ,lang,frage,antwort,required FROM {pre}faq ORDER BY typ,lang,frage ASC',
        );
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}faq SET frage=?,typ=?,lang=?,required=?,antwort=? WHERE id=?',
                HTMLFormat($_REQUEST['frage']),
                $_REQUEST['typ'],
                $_REQUEST['lang'],
                $_REQUEST['required'],
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,typ,lang,frage,antwort,required FROM {pre}faq WHERE id=?',
            (int) $_REQUEST['id'],
        );
