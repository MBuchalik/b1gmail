...
if ($_REQUEST['action'] == 'inactive') {
    ...
    elseif ($_REQUEST['do'] == 'exec') {
        ...
        if (isset($_REQUEST['queryTypeLogin'])) {
            ...
            $condition[] = sprintf(
                '((lastlogin!=0 AND lastlogin<%d AND last_pop3<%d AND last_imap<%d AND last_smtp<%d) OR (lastlogin=0 AND last_pop3=0 AND last_imap=0 AND last_smtp=0 AND reg_date!=0 AND reg_date<%d))',
                $timeDiff,
                $timeDiff,
                $timeDiff,
                $timeDiff,
        ...
        if (
            isset($_REQUEST['queryTypeGroups']) &&
            ...
            $condition[] =
                '(gruppe IN (' .
                implode(',', array_keys($_REQUEST['groups'])) .
                '))';
        }
        ...
        if ($action == 'show') {
            ...
            if (isset($_REQUEST['executeMassAction'])) {
                ...
                if (count($userIDs) > 0) {
                    if ($_REQUEST['massAction'] == 'delete') {
                        ...
                        substr($_REQUEST['massAction'], 0, 7) == 'moveto_'
                    ) {
            ...
            if ($sortBy == 'lastactivity') {
                $qSortBy =
                    'GREATEST(`lastlogin`,`last_pop3`,`last_imap`,`last_smtp`,`reg_date`)';
            } else {
                $qSortBy = $sortBy;
            }
            ...
            if ($action == 'lock') {
                $db->Query(
                    'UPDATE {pre}users SET gesperrt=? ' . $condition,
                    'yes',
                );
                $affectedUsers = $db->AffectedRows();
            } elseif ($action == 'move') {
                $db->Query(
                    'UPDATE {pre}users SET gruppe=? ' . $condition,
                    $_REQUEST['moveGroup'],
                );
                $affectedUsers = $db->AffectedRows();
            ...
            $tpl->assign(
                'msgText',
                sprintf($lang_admin['activity_done'], $affectedUsers),
            );
 ...
 */ elseif ($_REQUEST['action'] == 'trash') {
    ...
    elseif ($_REQUEST['do'] == 'exec') {
        ...
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}users WHERE gruppe IN(' .
                implode(',', array_keys($_REQUEST['groups'])) .
                ')',
        );
        ...
        if ($pos >= $count) {
            ...
            $mails = $mailSizes = 0;
            $res = $db->Query(
                'SELECT id,email FROM {pre}users WHERE gruppe IN(' .
                    implode(',', array_keys($_REQUEST['groups'])) .
            ...
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                ...
                foreach ($trashMails as $mailID => $mail) {
                    if (
                        (!isset($_REQUEST['daysOnly']) ||
                            $mail['timestamp'] <
                                time() -
                                    max(1, $_REQUEST['days']) * TIME_ONE_DAY) &&
 ...
 */ elseif ($_REQUEST['action'] == 'fts' && FTS_SUPPORT) {
    ...
    elseif ($_REQUEST['do'] == 'buildIndex') {
        ...
        $qPart =
            ...
            'WHERE ({pre}mails.`flags`&' .
            (FLAG_INDEXED | FLAG_DECEPTIVE) .
            ')=0 AND {pre}gruppen.`ftsearch`=\'yes\'';
        ...
        $res = $db->Query(
            'SELECT {pre}mails.`id` AS `id`,{pre}mails.`userid` AS `userid`' .
                $qPart .
                ' ORDER BY {pre}mails.`userid` ASC LIMIT ' .
                (int) $perPage,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $success = false;
            ...
            if ($currentUserID != $row['userid']) {
                ...
                $userObject = _new('BMUser', [$currentUserID]);
                ...
                if (is_array($userRow)) {
                    ...
                    $currentIndex = _new('BMSearchIndex', [$currentUserID]);
    ...
    // optimize index
    //
    elseif ($_REQUEST['do'] == 'optimizeIndex') {
        $perPage = max(1, $_REQUEST['perpage']);
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $idx = _new('BMSearchIndex', [$row['userid']]);
            if (is_object($idx)) {
                $idx->optimize();
            }
            ++$pos;
 ...
 */ elseif ($_REQUEST['action'] == 'orphans') {
    ...
    elseif ($_REQUEST['do'] == 'exec') {
        ...
        $res = $db->Query(
            'SELECT `id`,`size`,`blobstorage`,`userid` FROM {pre}mails WHERE `userid`!=-1 AND `userid` NOT IN(SELECT `id` FROM {pre}users)',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
 ...
 */ elseif ($_REQUEST['action'] == 'pop3gateway') {
    ...
    if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'fetch') {
        ...
        [$mailCount, $processedMails] = $pop3Gateway->Run($perPage);
...
$tpl->assign(
    'title',
    $lang_admin['prefs'] . ' &raquo; ' . $lang_admin['maintenance'],
);
