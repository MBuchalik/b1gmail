...
if ($_REQUEST['action'] == 'recvrules') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['add'])) {
            $db->Query(
                'INSERT INTO {pre}recvrules(field,expression,action,value,type) VALUES(?,?,?,?,?)',
                $_REQUEST['field'],
                $_REQUEST['expression'],
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}recvrules WHERE id=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($ruleIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}recvrules WHERE id IN(' .
                            implode(',', $ruleIDs) .
                            ')',
                    );
                    ...
                    $exportArray = [];
                    $res = $db->Query(
                        'SELECT field,expression,action,value,type FROM {pre}recvrules WHERE id IN(' .
                            implode(',', $ruleIDs) .
                            ') ORDER BY id ASC',
                    );
                    ...
                    $exportData = serialize([
                        'type' => 'b1gMailRuleFile',
                        'data' => $exportArray,
                    ]);
                    header('Pragma: public');
                    header('Content-Type: application/octet-stream');
                    header(
                        'Content-Disposition: attachment; filename=rules.bmrecvrules',
                    );
        ...
        $rules = [];
        $res = $db->Query(
            'SELECT id,field,expression,action,value,type FROM {pre}recvrules ORDER BY type,action,id ASC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $rules[$row['id']] = [
                'id' => $row['id'],
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}recvrules SET field=?, expression=?, action=?, value=?, type=? WHERE id=?',
                $_REQUEST['field'],
                $_REQUEST['expression'],
                (int) $_REQUEST['ruleAction'],
                (int) $_REQUEST['value'],
        ...
        // get rule data
        $res = $db->Query(
            'SELECT id,field,expression,action,value,type FROM {pre}recvrules WHERE id=?',
            (int) $_REQUEST['id'],
        );
...
$tpl->assign(
    'title',
    $lang_admin['prefs'] . ' &raquo; ' . $lang_admin['recvrules'],
);
