...
if ($_REQUEST['action'] == 'overview') {
    if (
        ...
        // get data
        $res = $db->Query(
            'SELECT `versionid`,`base_version`,`config` FROM {pre}tbx_versions WHERE `versionid`=?',
            $versionID,
        );
        if ($res->RowCount() != 1) {
        ...
        // show image?
        if (
            isset($_REQUEST['showImage']) &&
            isset($_REQUEST['group']) &&
            isset($_REQUEST['key']) &&
            isset(
                $versionConfig[$_REQUEST['group']]['options'][$_REQUEST['key']],
            )
        ) {
            $imgPath =
                $versionConfig[$_REQUEST['group']]['options'][$_REQUEST['key']][
        ...
        $tpl->assign(
            'versionNo',
            sprintf('%s.%d', $tbxLatestVersion, $versionID),
        );
        $tpl->assign('page', 'toolbox.versions.edit.tpl');
    } elseif (
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'saveVersionConfig' &&
        isset($_REQUEST['versionid']) &&
        isset($_POST['prefs']) &&
        is_array($_POST['prefs'])
    ) {
        $versionID = (int) $_REQUEST['versionid'];
        ...
        foreach ($versionConfig as $groupName => $group) {
            ...
            foreach ($group['options'] as $itemKey => $item) {
                ...
                if ($item['type'] == FIELD_IMAGE && is_array($itemValue)) {
                    ...
                    if (
                        $itemValue['mode'] == 'upload' &&
                        isset(
                            $_FILES['prefs']['error'][$groupName][$itemKey][
                                'file'
                            ],
                        )
                    ) {
                        if (
                            ...
                            if (
                                @move_uploaded_file(
                                    $_FILES['prefs']['tmp_name'][$groupName][
                                        $itemKey
                                    ]['file'],
                                    $destFileName,
                                )
                                ...
                                if ($imgOK) {
                                    ...
                                    $fileErrors[] = [
                                        $versionConfig[$groupName]['title'],
                                        $versionConfig[$groupName]['options'][
                                            $itemKey
                                        ]['title'],
                                    ];
                                }
                ...
                } elseif ($item['type'] == FIELD_CHECKBOX) {
        ...
        // store config
        $db->Query(
            'UPDATE {pre}tbx_versions SET `config`=? WHERE `versionid`=?',
            serialize($dbConfig),
            $versionID,
        );
        ...
        if (count($fileErrors) > 0) {
            ...
            header(
                'Location: toolbox.php?do=release&versionid=' .
                    $versionID .
                    '&sid=' .
                    session_id(),
            );
        ...
        // get data
        $res = $db->Query(
            'SELECT `versionid`,`base_version`,`release_files` FROM {pre}tbx_versions WHERE `versionid`=? AND `status`=?',
            $versionID,
            'created',
        );
        if ($res->RowCount() != 1) {
        ...
        if (
            is_array($releaseFiles) &&
            isset($releaseFiles['win']) &&
            isset($releaseFiles['mac'])
        ) {
            $db->Query(
                'UPDATE {pre}tbx_versions SET `status`=? WHERE `versionid`=?',
                'released',
                $versionID,
            );
        ...
        // get data
        $res = $db->Query(
            'SELECT `versionid`,`base_version`,`config` FROM {pre}tbx_versions WHERE `versionid`=? AND `status`=?',
            $versionID,
            'created',
        );
        if ($res->RowCount() != 1) {
        ...
        $tpl->assign(
            'versionNo',
            sprintf('%s.%d', $row['base_version'], $versionID),
        );
        $tpl->assign('page', 'toolbox.versions.release.tpl');
    } elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'addVersion') {
        ...
        $res = $db->Query(
            'SELECT `config` FROM {pre}tbx_versions ORDER BY `versionid` DESC LIMIT 1',
        );
        ...
        // save
        $db->Query(
            'INSERT INTO {pre}tbx_versions(`base_version`,`status`,`create_date`,`config`) VALUES(?,?,?,?)',
            $tbxLatestVersion,
            'created',
            time(),
        ...
        header(
            'Location: toolbox.php?do=editVersionConfig&versionid=' .
                $versionID .
                '&sid=' .
                session_id(),
        );
        ...
        // get data
        $res = $db->Query(
            'SELECT `versionid`,`base_version`,`release_files` FROM {pre}tbx_versions WHERE `versionid`=?',
            $versionID,
        );
        if ($res->RowCount() != 1) {
            die('Version not found.');
        ...
        if (
            !is_array($releaseFiles) ||
            !isset($releaseFiles[$_REQUEST['os']])
        ) {
        ...
        header(
            sprintf(
                'Content-Disposition: attachment; filename="Toolbox-%s.%d-%s.%s"',
                $row['base_version'],
                $versionID,
                $_REQUEST['os'],
        ...
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'generateVersion' &&
        isset($_REQUEST['versionid']) &&
        isset($_REQUEST['os']) &&
        in_array($_REQUEST['os'], ['win', 'mac'])
    ) {
        $versionID = (int) $_REQUEST['versionid'];

        // get data
        $res = $db->Query(
            'SELECT `versionid`,`base_version`,`config`,`release_files` FROM {pre}tbx_versions WHERE `versionid`=? AND `status`=?',
            $versionID,
            'created',
        );
        if ($res->RowCount() != 1) {
        ...
        // make request
        $headers = null;
        $http = _new('BMHTTP', [TOOLBOX_SERVER]);
        $http->DownloadToFP_POST(
            $tempFileFP,
            $postData,
        ...
        if ($headers['x-b1gmail-status'] != 'ok') {
            ...
            if (
                $headers['x-b1gmail-signature'] ==
                md5(md5_file($tempFileName) . B1GMAIL_SIGNKEY)
            ) {
                $relDestFileName =
                    'toolboxRelease_' . $versionID . '_' . $_REQUEST['os'];
                $destFileName = B1GMAIL_DATA_DIR . $relDestFileName;
                ...
                if (copy($tempFileName, $destFileName)) {
                    ...
                    $releaseFiles[$_REQUEST['os'] . '_sig'] =
                        $headers['x-b1gmail-file-signature'];
                    $db->Query(
                        'UPDATE {pre}tbx_versions SET `release_files`=? WHERE `versionid`=?',
        ...
        $tbxVersions = [];
        $res = $db->Query(
            'SELECT `versionid`,`base_version`,`status` FROM {pre}tbx_versions ORDER BY `versionid` DESC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
...
$tpl->assign(
    'title',
    $lang_admin['welcome'] . ' &raquo; ' . $lang_admin['toolbox'],
);
