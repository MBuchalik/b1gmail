...
if ($_REQUEST['action'] == 'fields') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}profilfelder WHERE id=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($fieldIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}profilfelder WHERE id IN(' .
                            implode(',', $fieldIDs) .
                            ')',
                    );
                }
        ...
        $fields = [];
        $res = $db->Query(
            'SELECT id,feld,typ,pflicht,rule,extra FROM {pre}profilfelder ORDER BY id ASC',
        );
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}profilfelder SET feld=?,typ=?,pflicht=?,rule=?,extra=?,show_signup=?,show_li=? WHERE id=?',
                $_REQUEST['feld'],
                (int) $_REQUEST['typ'],
                isset($_REQUEST['pflicht']) ? 'yes' : 'no',
                $_REQUEST['rule'],
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,feld,typ,pflicht,rule,extra,show_signup,show_li FROM {pre}profilfelder WHERE id=?',
            (int) $_REQUEST['id'],
        );
...
$tpl->assign(
    'title',
    $lang_admin['prefs'] . ' &raquo; ' . $lang_admin['profilefields'],
);
