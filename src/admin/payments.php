...
if ($_REQUEST['action'] == 'payments') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (
            ...
            in_array($_REQUEST['singleAction'], [
                'download',
                'activate',
                'delete',
            ])
            ...
            $_POST['payment'] = [(int) $_REQUEST['singleID'] => true];
        }

        // mass action
        if (isset($_REQUEST['executeMassAction'])) {
            // get payment IDs
            $paymentIDs = isset($_POST['payment']) ? $_POST['payment'] : [];
            ...
            if (count($paymentIDs) > 0) {
                ...
                if ($_REQUEST['massAction'] == 'delete') {
                    $db->Query(
                        'DELETE FROM {pre}orders WHERE `orderid` IN ?',
                        $paymentIDs,
                    );
                    $db->Query(
                        'DELETE FROM {pre}invoices WHERE `orderid` IN ?',
                        $paymentIDs,
                    );
                }
                ...
                elseif ($_REQUEST['massAction'] == 'activate') {
                    $res = $db->Query(
                        'SELECT `orderid`,`amount` FROM {pre}orders WHERE `orderid` IN ? AND `status`=?',
                        $paymentIDs,
                        ORDER_STATUS_CREATED,
                    );
                    while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                        BMPayment::ActivateOrder(
                            $row['orderid'],
                            $row['amount'],
                        );
                    }
                ...
                // download invoices
                elseif ($_REQUEST['massAction'] == 'download') {
                    ...
                    // fetch invoices
                    $res = $db->Query(
                        'SELECT `orderid`,`invoice` FROM {pre}invoices WHERE `orderid` IN ?',
                        $paymentIDs,
                    );
                    while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        ...
        // sort options
        $sortBy = isset($_REQUEST['sortBy']) ? $_REQUEST['sortBy'] : 'created';
        ...
        $statusIDs = [0 => true, 1 => true];
        $paymentMethods = [0 => true, 1 => true, 2 => true, 3 => true];
        ...
        if (isset($_REQUEST['filter'])) {
            ...
            $queryStatus =
                count(array_keys($statusIDs)) > 0
                    ? implode(',', array_keys($statusIDs))
                    : '-1';
            ...
            $queryPaymentMethods =
                count(array_keys($paymentMethods)) > 0
                    ? implode(',', array_keys($paymentMethods))
                    : '-1';
        ...
        [$paymentCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            if (!isset($users[$row['userid']])) {
                $res2 = $db->Query(
                    'SELECT `id`,`email`,`vorname`,`nachname` FROM {pre}users WHERE `id`=?',
                    $row['userid'],
                );
    ...
    // details
    //
    elseif ($_REQUEST['do'] == 'details' && isset($_REQUEST['orderid'])) {
        $payMethods = BMPayment::GetCustomPaymentMethods();
        ...
        [$row['hasInvoice']] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    if (!isset($_REQUEST['do'])) {
        ...
        $tpl->assign(
            ...
            mktime(
                ...
                0,
                0,
                date('m', time() - TIME_ONE_MONTH),
                1,
                date('Y', time() - TIME_ONE_MONTH),
            ),
        );
        $tpl->assign(
            'end',
            mktime(
                ...
                59,
                date('m', time() - TIME_ONE_MONTH),
                date('t', time() - TIME_ONE_MONTH),
                date('Y', time() - TIME_ONE_MONTH),
            ),
        );
        $tpl->assign('page', 'payments.export.tpl');
    } elseif (
        $_REQUEST['do'] == 'exportAccEntries' &&
        ...
        $res = $db->Query(
            'SELECT `orderid`,`amount`,`tax`,`orderid`,`paymethod`,`activated`,`cart` FROM {pre}orders WHERE `status`=1 AND `activated`>=? AND `activated`<=? ORDER BY `activated` ASC',
            $from,
            $to,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        ...
        if (isset($_REQUEST['paidOnly'])) {
            $res = $db->Query(
                'SELECT {pre}orders.`orderid`,{pre}invoices.`invoice` FROM {pre}orders,{pre}invoices WHERE {pre}orders.`activated`>=? AND {pre}orders.`activated`<=? AND {pre}invoices.`orderid`={pre}orders.`orderid`',
                $from,
                $to,
            );
        } else {
            $res = $db->Query(
                'SELECT {pre}orders.`orderid`,{pre}invoices.`invoice` FROM {pre}orders,{pre}invoices WHERE (({pre}orders.`status`=1 AND {pre}orders.`activated`>=? AND {pre}orders.`activated`<=?) OR ({pre}orders.`status`=0 AND {pre}orders.`created`>=? AND {pre}orders.`created`<=?)) AND {pre}invoices.`orderid`={pre}orders.`orderid`',
                $from,
                $to,
                $from,
                $to,
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            $zip->AddFile(
                $invTempFileName,
                sprintf('%s.html', BMPayment::InvoiceNo($row['orderid'])),
            );
        }
        $res->Free();
 ...
 */ elseif (
    ...
    [$invoice] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();
 ...
 */ elseif (
    ...
    if (
        !empty($_REQUEST['vkCode']) &&
        !empty($_REQUEST['amount']) &&
        BMPayment::ActivateOrderWithVKCode(
            $_REQUEST['vkCode'],
            round((float) str_replace(',', '.', $_REQUEST['amount']) * 100, 0),
        )
    ) {
...
$tpl->assign(
    'title',
    $lang_admin['usersgroups'] . ' &raquo; ' . $lang_admin['payments'],
);
