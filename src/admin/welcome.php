...
if ($_REQUEST['action'] == 'welcome') {
    ...
    if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'saveNotes') {
        $db->Query(
            'UPDATE {pre}admins SET notes=? WHERE adminid=?',
            $_REQUEST['notes'],
            $adminRow['adminid'],
        );
    ...
    $res = $db->Query(
        'SELECT COUNT(*) FROM {pre}users WHERE gesperrt=?',
        'locked',
    );
    [$notActivatedUserCount] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();
    $res = $db->Query(
        'SELECT COUNT(*) FROM {pre}users WHERE gesperrt=?',
        'yes',
    );
    [$lockedUserCount] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();

    $emailCount = $tables[$mysql['prefix'] . 'mails']['rows'];
    if ($emailCount < 20000) {
        $res = $db->Query(
            'SELECT COUNT(*),SUM(size) FROM {pre}mails WHERE `userid`!=-1',
        );
        [$emailCount, $emailSize] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    } else {
    ...
    if ($diskFileCount < 20000) {
        ...
        [$diskFileCount, $diskSize] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    } else {
        $diskSize = false;
    }
    ...
    if (!SERVER_WINDOWS) {
        if (@file_exists('/usr/bin/uptime')) {
            $uptime = @exec('/usr/bin/uptime');
        } elseif (@file_exists('/bin/uptime')) {
            $uptime = @exec('/bin/uptime');
        } elseif (@file_exists('/usr/sbin/uptime')) {
            $uptime = @exec('/usr/sbin/uptime');
        } elseif (@file_exists('/sbin/uptime')) {
            $uptime = @exec('/sbin/uptime');
        } else {
            $uptime = @exec('uptime');
        }
    ...
    } elseif (!is_writeable(B1GMAIL_DATA_DIR)) {
        $notices[] = [
            'type' => 'error',
    ...
    if ($bm_prefs['recipient_detection'] == 'dynamic') {
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}recvrules WHERE type=?',
            RECVRULE_TYPE_RECEIVERULE,
        );
        [$recvRuleCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    if ($emailCount < 20000) {
        $res = $db->Query(
            'SELECT COUNT(*),SUM(`size`) FROM {pre}mails WHERE `userid`!=-1 AND `userid` NOT IN(SELECT `id` FROM {pre}users)',
        );
        [$orphanCount, $orphanSize] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        if ($orphanCount > 0) {
            $notices[] = [
                'type' => 'warning',
    ...
    if (!function_exists('imagecreate')) {
        $notices[] = ['type' => 'info', 'text' => $lang_admin['gdlib']];
    }

    // idn?
    if (!IDN_SUPPORT) {
        $notices[] = ['type' => 'info', 'text' => $lang_admin['idnlib']];
    }

    // iconv/mbstring?
    if (!function_exists('mb_convert_encoding') && !function_exists('iconv')) {
        $notices[] = ['type' => 'info', 'text' => $lang_admin['mbstring']];
    }

    // debug mode?
    if (DEBUG) {
        $notices[] = ['type' => 'info', 'text' => $lang_admin['debugmode']];
    }
    ...
    if ($notActivatedUserCount > 0) {
        $notices[] = [
            ...
            'text' => sprintf(
                ...
                $notActivatedUserCount,
            ),
            'link' =>
                'users.php?filter=true&statusNotActivated=true&allGroups=true&',
        ];
    }
    ...
    [$deleteCount] = $res->FetchArray(MYSQLI_NUM);
    $res->Free();
    if ($deleteCount > 0) {
        $notices[] = [
            'type' => 'info',
            ...
            'link' =>
                'users.php?filter=true&statusDeleted=true&allGroups=true&',
        ];
    }
    ...
    //
    $res = $db->Query(
        'SELECT `id`,`titel`,`maxsize` FROM {pre}gruppen WHERE `maxsize`>? ORDER BY `titel` ASC',
        $bm_prefs['mailmax'],
    );
    while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        $notices[] = [
...
} elseif ($_REQUEST['action'] == 'phpinfo' && $adminRow['type'] == 0) {
