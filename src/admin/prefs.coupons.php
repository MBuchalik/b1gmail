...
if ($_REQUEST['action'] == 'coupons') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['add'])) {
            ...
            // benefits
            $benefits = [];
            $benefits['sms'] = isset($_REQUEST['ver_credits'])
                ? (int) $_REQUEST['ver_credits_count']
                : 0;
            $benefits['gruppe'] = isset($_REQUEST['ver_gruppe'])
                ? (int) $_REQUEST['ver_gruppe_id']
                : 0;
            ...
            foreach ($codes as $code) {
                if (($code = trim($code)) != '') {
                    $db->Query(
                        'INSERT INTO {pre}codes(code,von,bis,anzahl,ver,valid_signup,valid_loggedin) VALUES(?,?,?,?,?,?,?)',
                        $code,
                        $from,
                        $to,
                        $count,
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}codes WHERE id=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($couponIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}codes WHERE id IN(' .
                            implode(',', $couponIDs) .
                            ')',
                    );
                }
        ...
        $coupons = [];
        $res = $db->Query(
            'SELECT id,code,von,bis,anzahl,ver,used,valid_loggedin,valid_signup FROM {pre}codes ORDER BY id ASC',
        );
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            ...
            // save
            $db->Query(
                'UPDATE {pre}codes SET code=?,von=?,bis=?,anzahl=?,ver=?,valid_loggedin=?,valid_signup=? WHERE id=?',
                $_REQUEST['code'],
                $from,
                $to,
                $count,
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,code,von,bis,anzahl,ver,usedby,valid_signup,valid_loggedin FROM {pre}codes WHERE id=?',
            (int) $_REQUEST['id'],
        );
        ...
        if (count($usedByIDs) > 0) {
            $res = $db->Query(
                'SELECT id,email,vorname,nachname,strasse,hnr,plz,ort FROM {pre}users WHERE id IN(' .
                    implode(',', $usedByIDs) .
                    ') ORDER BY id ASC',
            );
...
$tpl->assign(
    'title',
    $lang_admin['prefs'] . ' &raquo; ' . $lang_admin['coupons'],
);
