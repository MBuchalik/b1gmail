...
if ($_REQUEST['action'] == 'newsletter') {
    ...
    if (!isset($_REQUEST['do'])) {
        ...
        $templates = [];
        $res = $db->Query(
            'SELECT `templateid`,`title` FROM {pre}newsletter_templates ORDER BY `title` ASC',
        );
        ...
        $countries = [
            $bm_prefs['std_land'] => $allCountries[$bm_prefs['std_land']],
        ];
    ...
    //
    elseif (
        $_REQUEST['do'] == 'getTemplateData' &&
        isset($_REQUEST['templateID'])
    ) {
        $res = $db->Query(
            'SELECT * FROM {pre}newsletter_templates WHERE `templateid`=?',
        ...
        header(
            'Content-Type: application/json; charset="' . $currentCharset . '"',
        );
        echo '{ ' . "\n";
    ...
    //
    elseif ($_REQUEST['do'] == 'determineRecipients') {
        ...
        // determine count
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}users,{pre}gruppen WHERE {pre}gruppen.id={pre}users.gruppe AND {pre}users.land IN ? AND {pre}users.gesperrt IN(' .
                implode(',', $lockedValues) .
                ') AND {pre}users.gruppe IN(' .
                $groups .
                ') AND ({pre}gruppen.allow_newsletter_optout=\'no\' OR {pre}users.newsletter_optin=\'yes\')' .
                ($sendto == 'altmails'
                    ? ' AND LENGTH({pre}users.altmail)>3'
                    : ''),
            $countries,
        );
        [$recpCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
    ...
    elseif ($_REQUEST['do'] == 'send') {
        ...
        // build condition
        $condition =
            '{pre}gruppen.id={pre}users.gruppe AND {pre}users.land IN (' .
            implode(',', $countries) .
            ') AND {pre}users.gesperrt IN(' .
            implode(',', $lockedValues) .
            ') AND {pre}users.gruppe IN(' .
            implode(',', $groups) .
            ')' .
        ...
        // determine count
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}users,{pre}gruppen WHERE ' . $condition,
        );
        [$recpCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        if ($recpCount == 0) {
        ...
        if (isset($_REQUEST['exportRecipients'])) {
            ...
            header(
                'Content-Disposition: attachment; filename="newsletter-recipients.csv"',
            );
            header('Content-Type: text/csv');

            // send recipients
            $res = $db->Query(
                'SELECT {pre}users.id AS UserID,{pre}users.email AS Email,{pre}users.altmail AS AltEmail,{pre}users.vorname AS Firstname,{pre}users.nachname AS Lastname,{pre}users.anrede AS Salutation FROM {pre}users,{pre}gruppen WHERE ' .
                    $condition .
                    ' ORDER BY {pre}users.id ASC',
            );
            $count = $res->RowCount();
            if ($count > 0) {
                PutLog(
                    sprintf(
                        'Admin <%s> exports %d newsletter recipients as CSV file',
                        $adminRow['username'],
                        $count,
                    ),
                    PRIO_NOTE,
        ...
        PutLog(
            sprintf(
                'Admin <%s> sends newsletter to %d recipients',
                $adminRow['username'],
                $recpCount,
            ),
            PRIO_NOTE,
    ...
    elseif (
        ...
        // get count
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}users,{pre}gruppen WHERE ' .
        ...
        if ($pos >= $count) {
            ...
            // select recipients
            $res = $db->Query(
                'SELECT {pre}users.id,{pre}users.email,{pre}users.altmail,{pre}users.vorname,{pre}users.nachname,{pre}users.anrede FROM {pre}users,{pre}gruppen WHERE ' .
                    $newsletter['condition'] .
                    ' ORDER BY {pre}users.id ASC LIMIT ' .
                    $pos .
                    ',' .
                    max(1, (int) $_REQUEST['perpage']),
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $to =
                    $newsletter['sendto'] == 'altmails'
                        ? $row['altmail']
                        : $row['email'];
                ...
                if ($row['anrede'] == 'herr') {
                    ...
                    $greeting = sprintf(
                        $lang_admin['greeting_mr'],
                        $row['nachname'],
                    );
                } elseif ($row['anrede'] == 'frau') {
                    $salutation = $lang_admin['mrs'];
                    $greeting = sprintf(
                        $lang_admin['greeting_mrs'],
                        $row['nachname'],
                    );
                } else {
                    $salutation = '';
                ...
                if ($newsletter['priority'] != 0) {
                    $mail->AddHeaderField(
                        'X-Priority',
                        $newsletter['priority'] == ITEMPRIO_HIGH
                            ? 1
...
} elseif ($_REQUEST['action'] == 'templates') {
    $tpl->assign(
        'usertpldir',
        B1GMAIL_REL . 'templates/' . $bm_prefs['template'] . '/',
    );
    ...
    if (!isset($_REQUEST['do'])) {
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}newsletter_templates WHERE `templateid`=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['add'])) {
            $db->Query(
                'INSERT INTO {pre}newsletter_templates(`title`,`subject`,`from`,`mode`,`priority`,`body`) VALUES(?,?,?,?,?,?)',
                $_REQUEST['title'],
                $_REQUEST['subject'],
                $_REQUEST['from'],
                $_REQUEST['mode'],
        ...
        $templates = [];
        $res = $db->Query(
            'SELECT `templateid`,`title`,`subject` FROM {pre}newsletter_templates ORDER BY `title` ASC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    // edit
    //
    elseif ($_REQUEST['do'] == 'edit' && isset($_REQUEST['templateID'])) {
        if (isset($_REQUEST['save']) && isset($_POST['priority'])) {
            $db->Query(
                'UPDATE {pre}newsletter_templates SET `title`=?,`mode`=?,`from`=?,`subject`=?,`priority`=?,`body`=? WHERE `templateid`=?',
                $_REQUEST['title'],
                $_REQUEST['mode'],
                $_REQUEST['from'],
                $_REQUEST['subject'],
            ...
            header(
                'Location: newsletter.php?action=templates&sid=' . session_id(),
            );
            exit();
        }
...
$tpl->assign(
    'title',
    $lang_admin['usersgroups'] . ' &raquo; ' . $lang_admin['newsletter'],
);
