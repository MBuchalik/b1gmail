 ...
 */ elseif ($_REQUEST['action'] == 'extensions') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['add'])) {
            if (
                ...
                if (
                    ...
                    $db->Query(
                        'INSERT INTO {pre}extensions(ext,ctype,bild) VALUES(?,?,?)',
                        str_replace([' ', '.'], '', $_REQUEST['ext']),
                        $_FILES['icon']['type'],
                        $iconData,
                    );
                }
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}extensions WHERE id=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($extIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}extensions WHERE id IN(' .
                            implode(',', $extIDs) .
                            ')',
                    );
                }
        ...
        $extensions = [];
        $res = $db->Query(
            'SELECT id,ext,ctype FROM {pre}extensions ORDER BY ext ASC',
        );
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            ...
            if (isset($_REQUEST['ext'])) {
                $db->Query(
                    'UPDATE {pre}extensions SET ext=? WHERE id=?',
                    str_replace([' ', '.'], '', $_REQUEST['ext']),
                    $_REQUEST['id'],
                );
            }
            ...
            if (
                ...
                if (
                    ...
                    $db->Query(
                        'UPDATE {pre}extensions SET ctype=?,bild=? WHERE id=?',
                        $_FILES['icon']['type'],
                        $iconData,
                        $_REQUEST['id'],
                    );
            ...
            header(
                'Location: prefs.webdisk.php?action=extensions&sid=' .
                    session_id(),
            );
            exit();
        }
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,ext FROM {pre}extensions WHERE id=?',
            (int) $_REQUEST['id'],
        );
 ...
 */ elseif ($_REQUEST['action'] == 'displayExt' && isset($_REQUEST['id'])) {
    $res = $db->Query(
        'SELECT bild,ctype FROM {pre}extensions WHERE id=?',
        (int) $_REQUEST['id'],
    );
