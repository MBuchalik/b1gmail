...
if ($_REQUEST['action'] == 'overview') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['singleAction'])) {
            if ($_REQUEST['singleAction'] == 'lock') {
                ...
                if ($_REQUEST['singleAction'] == 'unlock') {
                    $db->Query(
                        'UPDATE {pre}abuse_points SET `expired`=1 WHERE `userid`=?',
                        $_REQUEST['singleID'],
                    );
                }
            ...
            if (count($userIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    ...
                    if ($_REQUEST['massAction'] == 'unlock') {
                        $db->Query(
                            'UPDATE {pre}abuse_points SET `expired`=1 WHERE `userid` IN ?',
                            $userIDs,
                        );
                    }
        ...
        // sort options
        $sortBy = isset($_REQUEST['sortBy']) ? $_REQUEST['sortBy'] : 'pointsum';
        ...
        // page calculation
        $res = $db->Query(
            'SELECT COUNT(DISTINCT(`userid`)) FROM {pre}abuse_points',
        );
        [$entryCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        ...
        $users = [];
        $res = $db->Query(
            'SELECT id,gruppe,email,gesperrt,SUM(points) AS pointsum FROM {pre}users INNER JOIN {pre}abuse_points ON {pre}abuse_points.`userid`={pre}users.`id` WHERE {pre}abuse_points.`expired`=0 GROUP BY `id` ORDER BY ' .
                $sortBy .
                ' ' .
                $sortOrder .
                ' ' .
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            if ($row['pointsum'] >= $bm_prefs['ap_hard_limit']) {
                $row['indicator'] = 'red';
            } elseif ($row['pointsum'] >= $bm_prefs['ap_medium_limit']) {
    ...
    //
    elseif (
        $_REQUEST['do'] == 'statsDetails' &&
        isset($_REQUEST['userid']) &&
        isset($_REQUEST['day']) &&
        isset($_REQUEST['type'])
    ) {
        $type = in_array($_REQUEST['type'], ['send', 'recv'])
            ? $_REQUEST['type']
            : 'send';
        if ($type == 'send') {
            $field = 'recipients';
        ...
        $res = $db->Query(
            'SELECT `' .
                ...
                '`,`time` FROM ' .
                $table .
                ' WHERE `time`>=? AND `time`<=? AND `userid`=? ORDER BY `time` ASC',
            $dayBegin,
            $dayEnd,
            $_REQUEST['userid'],
        );
        ...
        foreach ($stats as $hour => $item) {
            if ($type == 'recv') {
                $item['size'] = TemplateSize(['bytes' => $item['size']], $tpl);
            }
            printf(
                '{ "mails": %d, "%s": "%s", "timeStamp": %d, "hour": "%d:00 - %d:59" },',
                $item['mails'],
                $field,
                $item[$field],
                $item['timeStamp'],
    ...
    // show points
    //
    elseif ($_REQUEST['do'] == 'show' && isset($_REQUEST['userid'])) {
        ...
        if (isset($_REQUEST['singleAction'])) {
            if ($_REQUEST['singleAction'] == 'delete') {
                $db->Query(
                    'DELETE FROM {pre}abuse_points WHERE `userid`=? AND `entryid`=?',
                    $userID,
                    $_REQUEST['singleID'],
                );
            } elseif ($_REQUEST['singleAction'] == 'lockUser') {
                $db->Query(
                    'UPDATE {pre}users SET gesperrt=? WHERE id=?',
                    'yes',
                    $_REQUEST['singleID'],
                ...
                $_REQUEST['singleAction'] == 'unlockUser' ||
            ...
            } elseif ($_REQUEST['singleAction'] == 'deleteUser') {
                $res = $db->Query(
                    'SELECT gesperrt FROM {pre}users WHERE id=?',
                    $_REQUEST['singleID'],
                );
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            if (
                ...
                count($_POST['entries']) > 0
            ) {
                $_POST['entries'] = array_map('intval', $_POST['entries']);

                if ($_REQUEST['massAction'] == 'delete') {
                    $db->Query(
                        'DELETE FROM {pre}abuse_points WHERE `userid`=? AND `entryid` IN ?',
                        $userID,
                        $_POST['entries'],
                    );
                }
        ...
        if (isset($_POST['save']) && isset($_POST['notes'])) {
            $db->Query(
                'UPDATE {pre}users SET `notes`=? WHERE `id`=?',
                $_POST['notes'],
                $userID,
            );
        }
        ...
        $points = [];
        $res = $db->Query(
            'SELECT `entryid`,`date`,`type`,`points`,`comment`,`expired` FROM {pre}abuse_points ' .
                'WHERE `userid`=? ORDER BY `expired` DESC,`entryid` ASC',
            $userID,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            if ($row['expired']) {
                ...
                if ($sum >= $bm_prefs['ap_hard_limit']) {
                    $row['indicator'] = 'red';
                } elseif ($sum >= $bm_prefs['ap_medium_limit']) {
                    $row['indicator'] = 'yellow';
                } else {
                    $row['indicator'] = 'green';
                }
        ...
        $sendStats = [];
        $res = $db->Query(
            'SELECT `recipients`,`time` FROM {pre}sendstats WHERE `time`>=? AND `userid`=? ORDER BY `time` DESC',
            time() - TIME_ONE_WEEK,
            $userID,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        ...
        $recvStats = [];
        $res = $db->Query(
            'SELECT `size`,`time` FROM {pre}recvstats WHERE `time`>=? AND `userid`=? ORDER BY `time` DESC',
            time() - TIME_ONE_WEEK,
            $userID,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
...
$tpl->assign(
    'title',
    $lang_admin['usersgroups'] . ' &raquo; ' . $lang_admin['abuseprotect'],
);
