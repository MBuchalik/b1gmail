...
if ($_REQUEST['action'] == 'countries') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['add'])) {
            $db->Query(
                'INSERT INTO {pre}staaten(land) VALUES(?)',
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}staaten WHERE id=?',
                (int) $_REQUEST['delete'],
            );
            $cacheManager->Delete('countryList');
            $cacheManager->Delete('countryListWithDetails');
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($countryIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}staaten WHERE id IN(' .
                            implode(',', $countryIDs) .
                            ')',
                    );
                }
        ...
        $countries = [];
        $res = $db->Query(
            'SELECT id,land,is_eu,vat FROM {pre}staaten ORDER BY id ASC',
        );
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}staaten SET land=?,is_eu=?,vat=? WHERE id=?',
                HTMLFormat($_REQUEST['land']),
                isset($_REQUEST['is_eu']) ? 'yes' : 'no',
                min(
                    100,
                    max(0, (float) str_replace(',', '.', $_REQUEST['vat'])),
                ),
                (int) $_REQUEST['id'],
            );
            $cacheManager->Delete('countryList');
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,land,is_eu,vat FROM {pre}staaten WHERE id=?',
            (int) $_REQUEST['id'],
        );
...
$tpl->assign(
    'title',
    $lang_admin['prefs'] . ' &raquo; ' . $lang_admin['countries'],
);
