    ...
    // extension is available
        ...
        if (!isset($_REQUEST['do'])) {
                ...
                if (
                    ...
                    if (
                        ...
                        if ($cp) {
                            ...
                            // exists?
                            $res = $db->Query(
                                'SELECT COUNT(*) FROM {pre}certificates WHERE `hash`=? AND `type`=? AND `userid`=?',
                                $certInfo['hash'],
                            ...
                            if ($certCount != 0) {
                                ...
                                if ($encryptCA || $signCA) {
                                    $db->Query(
                                        'INSERT INTO {pre}certificates(`type`,`userid`,`hash`,`cn`,`validfrom`,`validto`,`pemdata`) ' .
                                            'VALUES(?,?,?,?,?,?,?)',
                                        CERTIFICATE_TYPE_ROOT,
                                        0,
                                        $certInfo['hash'],
            ...
            if (isset($_REQUEST['delete'])) {
                $db->Query(
                    'DELETE FROM {pre}certificates WHERE `certificateid`=? AND `type`=? AND `userid`=?',
                    (int) $_REQUEST['delete'],
                    CERTIFICATE_TYPE_ROOT,
                    0,
                );
            ...
            if (isset($_REQUEST['export'])) {
                $res = $db->Query(
                    'SELECT `hash`,`pemdata` FROM {pre}certificates WHERE `certificateid`=? AND `type`=? AND `userid`=?',
                    (int) $_REQUEST['export'],
                    CERTIFICATE_TYPE_ROOT,
                    0,
                );
            ...
            if (
                ...
                if ($_REQUEST['massAction'] == 'delete') {
                    $db->Query(
                        'DELETE FROM {pre}certificates WHERE (`certificateid` IN ?) AND `type`=? AND `userid`=?',
                        $certs,
                        CERTIFICATE_TYPE_ROOT,
                        0,
                    );
                } elseif ($_REQUEST['massAction'] == 'export') {
                    $tempFileID = RequestTempFile(0);
                    $tempFileName = TempFileName($tempFileID);
                    ...
                    $res = $db->Query(
                        'SELECT `hash`,`pemdata` FROM {pre}certificates WHERE (`certificateid` IN ?) AND `type`=? AND `userid`=?',
                        $certs,
                        CERTIFICATE_TYPE_ROOT,
                        0,
                    );
            ...
            if (!$stopIt) {
                $certs = [];
                $res = $db->Query(
                    'SELECT `certificateid`,`cn`,`validfrom`,`validto` FROM {pre}certificates WHERE `type`=? AND `userid`=? ORDER BY `cn` ASC',
                    CERTIFICATE_TYPE_ROOT,
                    0,
                );
                while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                ...
                $tpl->assign(
                    'caAvailable',
                    trim($bm_prefs['ca_cert']) != '' &&
                        trim($bm_prefs['ca_cert_pk']) != '',
                );
                $tpl->assign('now', time());
                ...
                if (
                    ...
                    if ($cert) {
                        if (
                            ...
                            if ($signCA && $encryptCA) {
                                $db->Query(
                                    'UPDATE {pre}prefs SET ca_cert=?,ca_cert_pk=?,ca_cert_pk_pass=?',
                                    $certData,
                                    $keyData,
                                    $_REQUEST['cert_ca_pass'] != ''
                                        ? base64_encode(
                                            CryptPKPassPhrase(
                                                $_REQUEST['cert_ca_pass'],
                                            ),
