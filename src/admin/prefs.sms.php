...
if ($_REQUEST['action'] == 'common') {
    ...
    if (isset($_REQUEST['save'])) {
        $db->Query(
            'UPDATE {pre}prefs SET sms_gateway=?, clndr_sms_abs=?, mail2sms_abs=?, smsreply_abs=?, charge_min_amount=?, clndr_sms_type=?, mail2sms_type=?, smsvalidation_type=?, sms_enable_charge=?',
            $_REQUEST['sms_gateway'],
            $_REQUEST['clndr_sms_abs'],
            $_REQUEST['mail2sms_abs'],
            $_REQUEST['smsreply_abs'],
            (float) str_replace(',', '.', $_REQUEST['charge_min_amount']) * 100,
    ...
    $gateways = [];
    $res = $db->Query(
        'SELECT id,titel FROM {pre}smsgateways ORDER BY titel ASC',
    );
    ...
    // prepare values
    $bm_prefs['charge_min_amount'] = sprintf(
        '%.02f',
    ...
    $tpl->assign(
        'prjPass',
        substr(GenerateRandomKey('sofortueberweisungProjectPassword'), 0, 16),
    );
 ...
 */ elseif ($_REQUEST['action'] == 'gateways') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}smsgateways WHERE id=?',
                $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($gatewayIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}smsgateways WHERE id IN(' .
                            implode(',', $gatewayIDs) .
                            ')',
                    );
                }
        ...
        if (isset($_REQUEST['add'])) {
            $db->Query(
                'INSERT INTO {pre}smsgateways(titel,getstring,success,`user`,`pass`) VALUES(?,?,?,?,?)',
                $_REQUEST['titel'],
                $_REQUEST['getstring'],
                $_REQUEST['success'],
                $_REQUEST['user'],
    ...
    elseif ($_REQUEST['do'] == 'edit' && isset($_REQUEST['id'])) {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}smsgateways SET titel=?, getstring=?, success=?, `user`=?, `pass`=? WHERE id=?',
                $_REQUEST['titel'],
                $_REQUEST['getstring'],
                $_REQUEST['success'],
                $_REQUEST['user'],
            ...
            header(
                'Location: prefs.sms.php?action=gateways&sid=' . session_id(),
            );
            exit();
        }
 ...
 */ elseif ($_REQUEST['action'] == 'types') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}smstypen WHERE id=?',
                $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['setDefault'])) {
            $db->Query(
                'UPDATE {pre}smstypen SET std=(id=?)',
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($typeIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}smstypen WHERE id IN(' .
                            implode(',', $typeIDs) .
                            ')',
                    );
                }
        ...
        if (isset($_REQUEST['add'])) {
            ...
            [$stdCount] = $res->FetchArray();
            $res->Free();

            $db->Query(
                'INSERT INTO {pre}smstypen(titel,typ,price,gateway,std,flags,maxlength) VALUES(?,?,?,?,?,?,?)',
                $_REQUEST['titel'],
                $_REQUEST['typ'],
                $_REQUEST['price'],
                $_REQUEST['gateway'],
                $stdCount == 0 ? 1 : 0,
                isset($_REQUEST['flags'])
                    ? array_sum(array_keys($_REQUEST['flags']))
                    : 0,
        ...
        $types = [];
        $res = $db->Query(
            'SELECT id,titel,typ,std,price,maxlength FROM {pre}smstypen ORDER BY titel ASC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $types[$row['id']] = $row;
        }
    ...
    elseif ($_REQUEST['do'] == 'edit' && isset($_REQUEST['id'])) {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}smstypen SET titel=?, typ=?, price=?, gateway=?, flags=?, maxlength=? WHERE id=?',
                $_REQUEST['titel'],
                $_REQUEST['typ'],
                $_REQUEST['price'],
                $_REQUEST['gateway'],
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,titel,typ,std,price,gateway,flags,maxlength FROM {pre}smstypen WHERE id=?',
            $_REQUEST['id'],
        );
