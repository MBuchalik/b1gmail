...
if ($_REQUEST['action'] == 'plugins' || $_REQUEST['action'] == 'widgets') {
    if (
        ...
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'deletePackage' &&
        isset($_REQUEST['package']) &&
        strlen($_REQUEST['package']) == 32
    ) {
 ...
 */ elseif ($_REQUEST['action'] == 'install') {
    ...
    elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'uploadPlugin') {
        ...
        if (
            isset($_FILES['package']) &&
            $_FILES['package']['error'] == 0 &&
            ...
            if ($package->ParseFile()) {
                ...
                $tpl->assign(
                    'versionsMatch',
                    $package->metaInfo['for_b1gmail'] == B1GMAIL_VERSION,
                );
    ...
    elseif (
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'checkSignature' &&
        isset($_REQUEST['signature']) &&
        strlen($_REQUEST['signature']) == 32
    ) {
    ...
    elseif (
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'installPlugin' &&
        isset($_REQUEST['id']) &&
        ValidTempFile(0, (int) $_REQUEST['id'])
    ) {
        ...
        if ($package->ParseFile()) {
            if ($step == 1) {
                if ($package->InstallStep1()) {
                    $url = sprintf(
                        'plugins.php?action=install&do=installPlugin&id=%d&step=2&sid=%s',
                        $id,
                        session_id(),
                    );
                    header('Location: ' . $url);
