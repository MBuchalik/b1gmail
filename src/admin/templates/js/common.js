...
function toggleFieldset(elem) {
  while (elem) {
    if (
      ...
      elem.tagName.toUpperCase() == 'FIELDSET'
    )
      break;
...
function userMassActionFormSubmit(form) {
  ...
  _refreshURL =
    form.action +
    '&page=' +
    encodeURIComponent(EBID('page').value) +
    '&sortBy=' +
    encodeURIComponent(EBID('sortBy').value) +
    '&sortOrder=' +
    encodeURIComponent(EBID('sortOrder').value) +
    '&massAction=-';
  if (EBID('query'))
  ...
  for (var i = 0; i < inputs.length; i++) {
    ...
    if (
      (name.length > 6 && name.substr(0, 6) == 'status') ||
...
function addEvent(elem, event, handler) {
  if (document.addEventListener) elem.addEventListener(event, handler, false);
  else if (document.attachEvent) elem.attachEvent(event, handler);
}
...
function openWindow(url, name, w, h, clean) {
  ...
  if (clean)
    features =
      'scrollbars=no,scrolling=no,toolbar=no,statusbar=no,menubar=no,resizable=no,width=' +
      w +
      ',height=' +
      h +
      ',top=' +
    ...
    features =
      'scrollbars=yes,scrolling=yes,toolbar=no,statusbar=no,menubar=no,resizable=yes,width=' +
      w +
      ',height=' +
      h +
      ',top=' +
...
function documentLoader() {
  for (var i = 0; i < loadActions.length; i++)
    if (typeof loadActions[i] == 'string') eval(loadActions[i]);
    else loadActions[i]();
}
  ...
  if (e.readyState == 4) {
    ...
    if (text == 'DONE') {
      document.location.href =
        'newsletter.php?sid=' + currentSID + '&do=done&id=' + escape(rc_id);
    } else {
      text = text.split('/');
      if (text.length == 1) {
...
function tbxRelease2(versionID) {
  document.location.href =
    'toolbox.php?do=doRelease&versionid=' + versionID + '&sid=' + currentSID;
}
    ...
    function (e) {
      if (e.readyState == 4) {
        if (e.responseText == 'OK') {
          ...
          if (winDone && macDone) tbxRelease2(versionID);
        } else {
      ...
      if (e.readyState == 4) {
        if (e.responseText == 'OK') {
          ...
          parent.location.href =
            'toolbox.php?do=downloadVersion&versionid=' +
            versionID +
            '&os=' +
            os +
            '&sid=' +
...
function _rebuildCaches(e) {
  if (e.readyState == 4) {
    ...
    if (text == 'DONE') {
      document.location.href =
        'optimize.php?action=cache&sid=' + currentSID + '&do=done';
    } else {
      text = text.split('/');
      if (text.length == 1) {
        rc_statusdiv.innerHTML = text[0] + ' ...';
...
function updateSort(field) {
  if (EBID('sortBy').value != field) EBID('sortBy').value = field;
  else
...
function invertSelection2(form, prefix, suffix) {
  for (var i = 0; i < form.elements.length; i++) {
    if (
      form.elements[i].type == 'checkbox' &&
      form.elements[i].name.substring(0, prefix.length) == prefix &&
      form.elements[i].name.substring(
        form.elements[i].name.length - suffix.length,
      ) == suffix
    ) {
...
function executeAction(f) {
  ...
  if (url && url.length > 1)
    if (url.substring(0, 6) == 'popup;') window.open(url.substring(6));
...
function _fetchPOP3(e) {
  if (e.readyState == 4) {
    ...
    if (text == 'DONE') {
      document.location.href =
        'maintenance.php?action=pop3gateway&sid=' + currentSID + '&do=done';
    } else {
      text = text.split('/');
      if (text.length == 1) {
        rc_statusdiv.innerHTML = text[0] + ' ...';
      } else if (text.length == 2) {
        if (rc_all == -1) rc_all = text[1];
...
function rebuildBlobStor() {
  ...
  var _rebuildBlobStor = function (e) {
    if (e.readyState == 4) {
      ...
      if (text == 'DONE') {
        document.location.href =
          'optimize.php?action=filesystem&sid=' + currentSID;
      } else {
        text = text.split('/');
        if (text.length == 1) {
...
function vacuumBlobStor() {
  var all = -1,
    fetched = 0,
    form = EBID('vacuumForm'),
    perpage = EBID('vacuumPerPage').value;
...
function optimizeIndex() {
  var all = -1,
    pos = 0,
    form = EBID('optimizeForm'),
    perpage = EBID('optimizePerPage').value;
...
function trashExec() {
  _trashExecParams =
    '&days=' +
    encodeURIComponent(EBID('days').value) +
    '&size=' +
    encodeURIComponent(EBID('size').value) +
  ...
  for (var i = 0; i < inputs.length; i++) {
    ...
    var name = inputs[i].getAttribute('name');
    if (name.substr(0, 7) != 'groups[') continue;

    if (inputs[i].checked) _trashExecParams += '&' + name + '=true';
  }
...
function generateCodes(f) {
  ...
  if (EBID('generator_az').checked) randChars += 'abcdefghijklmnopqrstuvwxyz';
  if (EBID('generator_az2').checked) randChars += 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  if (EBID('generator_09').checked) randChars += '0123456789';
  if (EBID('generator_special').checked) randChars += '.,_-&$';
...
function cachePrefs() {
  ...
  if (cacheDisable.checked) prefs0.style.display = '';
  else if (cacheb1gMail.checked) prefs1.style.display = '';
  else if (cachememcache.checked) prefs2.style.display = '';

  if (!cacheDisable.checked) prefs3.style.display = '';
}
...
function determineNewsletterRecipients() {
  ...
  for (var i = 0; i < form.elements.length; i++)
    if (
      form.elements[i].type == 'checkbox' &&
      form.elements[i].name.substring(0, 6) == 'group_' &&
      form.elements[i].checked
    )
      groups += ',' + form.elements[i].name.substring(6);
  if (groups.length > 0) groups = groups.substring(1);

  // status params
  if (EBID('statusActive').checked) status += '&statusActive=true';
  if (EBID('statusLocked').checked) status += '&statusLocked=true';
  if (EBID('statusNotActivated').checked) status += '&statusNotActivated=true';

  // countries
  var countries = '',
    countryInputs = EBID('countrySelectBox').getElementsByTagName('input');
...
function loadNewsletterTemplate(box) {
  ...
  if (!confirm(lang['nwslttrtplwarn'])) return;
...
function insertNoticeRow(image, text, link, target) {
  ...
  if (link) {
    ...
    if (target) noticeLink.setAttribute('target', target);
...
function _checkPluginSignature(obj) {
  if (obj.readyState == 4) EBID('sigLayer').innerHTML = obj.responseText;
}
...
function _checkForPluginUpdates(obj) {
  if (obj.readyState == 4) {
    ...
    if (text.length == 4) {
      ...
      if (resultCode == 0) {
        EBID(layerName).innerHTML =
          '<font color="#666666">(' + lang['unknown'] + ')</font>';
      } else if (resultCode == 1) {
        EBID(layerName).innerHTML =
          '<img src="templates/images/ok.png" border="0" alt="" width="16" height="16" align="absmiddle" /> ' +
          lang['version'] +
          ': ' +
          text[2];
      } else if (resultCode == 2) {
        EBID(layerName).innerHTML =
          '<img src="templates/images/warning.png" border="0" alt="" width="16" height="16" align="absmiddle" /> ' +
          '<a href="' +
          text[3] +
          '" target="_blank">' +
          lang['version'] +
...
function activatePayment() {
  if (EBID('activateButton').disabled) return;

  var vkCode = EBID('vkCode').value,
    amount = EBID('amount').value;

  EBID('activationResult').innerHTML = '';
  EBID('activateButton').disabled = true;

  MakeXMLRequest(
    'payments.php?action=activatePayment&sid=' +
      currentSID +
      '&vkCode=' +
      escape(vkCode) +
      '&amount=' +
