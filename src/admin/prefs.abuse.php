...
if ($_REQUEST['action'] == 'prefs') {
    if (
        ...
        $db->Query(
            'UPDATE {pre}prefs SET `ap_medium_limit`=?,`ap_hard_limit`=?,`ap_expire_time`=?,`ap_expire_mode`=?,`ap_autolock`=?,`ap_autolock_notify`=?,`ap_autolock_notify_to`=?',
        ...
        foreach ($_POST['types'] as $type => $details) {
            ...
            $db->Query(
                'REPLACE INTO {pre}abuse_points_config(`type`,`points`,`prefs`) VALUES(?,?,?)',
                $type,
                $details['points'],
                $prefs,
            );
    ...
    $res = $db->Query(
        'SELECT `type`,`points`,`prefs` FROM {pre}abuse_points_config',
    );
...
$tpl->assign(
    'title',
    $lang_admin['prefs'] . ' &raquo; ' . $lang_admin['abuseprotect'],
);
