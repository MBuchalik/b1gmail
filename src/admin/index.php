...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'login') {
    ...
    if (trim($username) != '' && trim($pw) != '') {
        $res = $db->Query(
            'SELECT `adminid`,`username`,`password`,`password_salt`,`last_try` FROM {pre}admins WHERE `username`=?',
            $username,
        );
        if ($res->RowCount() == 1) {
            ...
            if ($adminUserRow['last_try'] > time() - 60 * 5) {
                $errorMsg = sprintf(
                    ...
                    FormatDate($adminUserRow['last_try'] + 60 * 5),
                );
            } else {
                if ($adminUserRow['last_try'] > 10) {
                    $adminUserRow['last_try'] = 0;
                    $db->Query(
                        'UPDATE {pre}admins SET `last_try`=0 WHERE `adminid`=?',
                        $adminUserRow['adminid'],
                    );
                }

                if (
                    md5($pw . $adminUserRow['password_salt']) ===
                    $adminUserRow['password']
                ) {
                    $db->Query(
                        'UPDATE {pre}admins SET `last_try`=0 AND `adminid`=?',
                        $adminUserRow['adminid'],
                    );
                    ...
                    $_SESSION['bm_adminAuth'] = md5(
                        $adminUserRow['password'] . $_SERVER['HTTP_USER_AGENT'],
                    );
                    ...
                    PutLog(
                        sprintf(
                            'Admin login from <%s> as <%s> failed (invalid password)',
                            $_SERVER['REMOTE_ADDR'],
                            $adminUserRow['username'],
                        ),
                        PRIO_NOTE,
                    ...
                    if ($adminUserRow['last_try'] + 1 > 4) {
                        ...
                        PutLog(
                            sprintf(
                                'Admin login for <%s> locked until %s',
                    ...
                    $db->Query(
                        'UPDATE {pre}admins SET `last_try`=? WHERE `adminid`=?',
                        $last_try,
                        $adminUserRow['adminid'],
                    );
            ...
            PutLog(
                sprintf(
                    'Admin login from <%s> as <%s> failed (invalid username)',
                    $_SERVER['REMOTE_ADDR'],
                    $username,
                ),
                PRIO_NOTE,
