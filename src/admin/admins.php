...
if ($_REQUEST['action'] == 'account') {
    ...
    if (isset($_REQUEST['changePassword']) && isset($_POST['newpw1'])) {
        if (
            ...
            $db->Query(
                'UPDATE {pre}admins SET `password`=?,`password_salt`=? WHERE `adminid`=?',
                $newPW,
                $newSalt,
                $adminRow['adminid'],
            );
            $_SESSION['bm_adminAuth'] = md5(
                $newPW . $_SERVER['HTTP_USER_AGENT'],
            );
        }
    }
...
} elseif ($_REQUEST['action'] == 'admins' && $adminRow['type'] == 0) {
    if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save']) && isset($_POST['username'])) {
            $res = $db->Query(
                'SELECT `adminid` FROM {pre}admins WHERE `username`=?',
                $_POST['username'],
            );
            $existingCount = $res->RowCount();
            if ($existingCount > 0) {
                [$existingID] = $res->FetchArray(MYSQLI_NUM);
            }
            $res->Free();

            if (
                ($existingCount > 0 && $existingID != $admin['adminid']) ||
                trim($_POST['username']) == ''
            ) {
                $tpl->assign('msgTitle', $lang_admin['error']);
                ...
                $db->Query(
                    'UPDATE {pre}admins SET `username`=?,`password`=?,`password_salt`=?,`firstname`=?,`lastname`=?,`type`=?,`privileges`=? WHERE `adminid`=?',
                    $_POST['username'],
                    $pw,
                    $salt,
                    $_POST['firstname'],
                ...
                header(
                    'Location: admins.php?action=admins&sid=' . session_id(),
                );
                exit();
            }
        }
        ...
        if ($displayPage) {
            ...
            foreach ($plugins->_plugins as $className => $pluginInfo) {
                if ($plugins->getParam('admin_pages', $className)) {
                    $pluginList[$className] = $plugins->getParam(
                        'admin_page_title',
                        $className,
                    );
                }
        ...
        if (isset($_REQUEST['add']) && isset($_POST['username'])) {
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}admins WHERE `username`=?',
                $_POST['username'],
            );
            [$existingCount] = $res->FetchArray(MYSQLI_NUM);
            $res->Free();
            ...
            if ($existingCount > 0 || trim($_POST['username']) == '') {
                ...
                $db->Query(
                    'INSERT INTO {pre}admins(`username`,`firstname`,`lastname`,`password`,`password_salt`,`type`) VALUES(?,?,?,?,?,?)',
                    $_POST['username'],
                    $_POST['firstname'],
                    $_POST['lastname'],
                    $pw,
                ...
                header(
                    'Location: admins.php?action=admins&do=edit&id=' .
                        $adminID .
                        '&sid=' .
                        session_id(),
                );
        ...
        } elseif (isset($_REQUEST['delete']) && (int) $_REQUEST['delete'] > 1) {
            $db->Query(
                'DELETE FROM {pre}admins WHERE `adminid`=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        elseif (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($massAdmins) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete admin
                    $db->Query(
                        'DELETE FROM {pre}admins WHERE `adminid` IN ?',
                        $massAdmins,
                    );
                }
            }
        ...
        if ($displayPage) {
            $admins = [];
            $res = $db->Query(
                'SELECT `adminid`,`username`,`firstname`,`lastname`,`type` FROM {pre}admins ORDER BY `username` ASC',
            );
...
$tpl->assign(
    'title',
    $lang_admin['welcome'] . ' &raquo; ' . $lang_admin['admins'],
);
