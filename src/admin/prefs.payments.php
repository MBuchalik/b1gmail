...
if ($_REQUEST['action'] == 'common') {
    ...
    if (
        ...
        echo '<!--' . "\n";
        printf(
            ' try { window.opener.parent.frames[\'content\'].EBID(\'su_kdnr\').value=\'%s\';' .
                "\n",
            addslashes($_REQUEST['user_id']),
        );
        printf(
            ' window.opener.parent.frames[\'content\'].EBID(\'su_prjnr\').value=\'%s\';' .
                "\n",
            addslashes($_REQUEST['project_id']),
        );
        printf(
            ' window.opener.parent.frames[\'content\'].EBID(\'su_prjpass\').value=\'%s\';' .
                "\n",
            addslashes($_REQUEST['prjPass']),
        );
        printf(
            ' window.opener.parent.frames[\'content\'].EBID(\'su_notifypass\').value=\'%s\';' .
                "\n",
            addslashes($_REQUEST['notifyPass']),
        );
        printf(
            ' window.opener.parent.frames[\'content\'].EBID(\'su_enable\').checked = true;' .
                "\n",
        );
        printf(
            ' window.opener.parent.frames[\'content\'].EBID(\'su_inputcheck\').checked = true;' .
                "\n",
        );
        printf(
            ' window.opener.parent.frames[\'content\'].EBID(\'prefsForm\').submit();' .
                "\n",
        );
    ...
    if (isset($_REQUEST['save'])) {
        $db->Query(
            'UPDATE {pre}prefs SET `currency`=?,`send_pay_notification`=?,`pay_notification_to`=?,`pay_emailfrom`=?,`pay_emailfromemail`=?,`mwst`=?,`enable_paypal`=?,`paypal_mail`=?,`enable_su`=?,`su_kdnr`=?,`su_prjnr`=?,`su_prjpass`=?,`su_notifypass`=?,`su_inputcheck`=?,`enable_vk`=?,`vk_kto_inh`=?,`vk_kto_nr`=?,`vk_kto_blz`=?,`vk_kto_inst`=?,`vk_kto_iban`=?,`vk_kto_bic`=?,`default_paymethod`=?,`enable_skrill`=?,`skrill_mail`=?,`skrill_secret`=?',
            $_REQUEST['currency'],
    ...
    $tpl->assign(
        'prjPass',
        substr(GenerateRandomKey('sofortueberweisungProjectPassword'), 0, 20),
    );
    $tpl->assign(
        'notifyPass',
        substr(GenerateRandomKey('sofortueberweisungNotifyPassword'), 0, 20),
    );
 ...
 */ elseif ($_REQUEST['action'] == 'paymethods') {
    ...
    if (!isset($_REQUEST['do'])) {
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}paymethods WHERE `methodid`=?',
                (int) $_REQUEST['delete'],
            );
        }
        ...
        if (
            ...
            if (count($ids) > 0) {
                $db->Query(
                    'DELETE FROM {pre}paymethods WHERE `methodid` IN ?',
                    $ids,
                );
            }
        }
        ...
        if (isset($_REQUEST['enable'])) {
            $db->Query(
                'UPDATE {pre}paymethods SET `enabled`=1 WHERE `methodid`=?',
                $_REQUEST['enable'],
            );
        }
        ...
        if (isset($_REQUEST['disable'])) {
            $db->Query(
                'UPDATE {pre}paymethods SET `enabled`=0 WHERE `methodid`=?',
                $_REQUEST['disable'],
            );
        }
        ...
        if (isset($_REQUEST['add']) && isset($_POST['title'])) {
            $db->Query(
                'INSERT INTO {pre}paymethods(`title`,`fields`) VALUES(?,?)',
                $_POST['title'],
                serialize([]),
            );
            $id = $db->InsertId();
            header(
                'Location: prefs.payments.php?action=paymethods&do=edit&methodid=' .
                    $id .
                    '&sid=' .
                    session_id(),
            );
        ...
        $methods = [];
        $res = $db->Query(
            'SELECT `methodid`,`enabled`,`title` FROM {pre}paymethods ORDER BY `methodid` ASC',
        );
    ...
    // edit
    //
    elseif ($_REQUEST['do'] == 'edit' && isset($_REQUEST['methodid'])) {
        $id = (int) $_REQUEST['methodid'];

        $res = $db->Query(
            'SELECT * FROM {pre}paymethods WHERE `methodid`=?',
            $id,
        );
        if ($res->RowCount() != 1) {
        ...
        if (
            ...
            $db->Query(
                'UPDATE {pre}paymethods SET `title`=?,`fields`=?,`enabled`=?,`invoice`=? WHERE `methodid`=?',
                $row['title'],
                serialize($fields),
                $row['enabled'],
                $row['invoice'],
 ...
 */ elseif ($_REQUEST['action'] == 'invoices') {
    if (isset($_REQUEST['save'])) {
        $db->Query(
            'UPDATE {pre}prefs SET `sendrg`=?,`rgnrfmt`=?,`kdnrfmt`=?,`rgtemplate`=?',
    ...
    $tpl->assign(
        'title',
        $lang_admin['payments'] . ' &raquo; ' . $lang_admin['invoices'],
    );
}
