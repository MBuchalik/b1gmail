...
if ($_REQUEST['action'] == 'banners') {
    ...
    if ($_REQUEST['do'] == 'list') {
        ...
        if (isset($_REQUEST['add'])) {
            $db->Query(
                'INSERT INTO {pre}ads(code,weight,paused,category,`comments`) VALUES(?,?,?,?,?)',
                $_REQUEST['code'],
        ...
        if (isset($_REQUEST['reset'])) {
            $db->Query(
                'UPDATE {pre}ads SET `views`=0 WHERE id=?',
                (int) $_REQUEST['reset'],
            );
        }
        ...
        if (isset($_REQUEST['activate'])) {
            $db->Query(
                'UPDATE {pre}ads SET paused=? WHERE id=?',
                'no',
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($adIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    // delete row
                    $db->Query(
                        'DELETE FROM {pre}ads WHERE id IN(' .
                            implode(',', $adIDs) .
                            ')',
                    );
                } elseif ($_REQUEST['massAction'] == 'pause') {
                    $db->Query(
                        'UPDATE {pre}ads SET paused=? WHERE id IN(' .
                            implode(',', $adIDs) .
                            ')',
                        'yes',
                    );
        ...
        $ads = [];
        $res = $db->Query(
            'SELECT id,code,views,paused,weight,category FROM {pre}ads ORDER BY paused DESC, views DESC',
        );
    ...
    elseif ($_REQUEST['do'] == 'edit') {
        ...
        if (isset($_REQUEST['save'])) {
            $db->Query(
                'UPDATE {pre}ads SET code=?, weight=?, paused=?, category=?, `comments`=? WHERE id=?',
                $_REQUEST['code'],
                max(1, min($_REQUEST['weight'], 100)),
                isset($_REQUEST['paused']) ? 'yes' : 'no',
                $_REQUEST['category'],
        ...
        // fetch
        $res = $db->Query(
            'SELECT id,code,paused,weight,category,comments FROM {pre}ads WHERE id=?',
            (int) $_REQUEST['id'],
        );
