 ...
 * read mail
 */
if ($_REQUEST['action'] == 'read' && isset($_REQUEST['id'])) {
    ...
    if ($mail !== false) {
        $enableExternal =
            isset($_REQUEST['enableExternal']) ||
            ($mail->flags & FLAG_SHOWEXTERNAL) != 0;
        ...
        $confirmationTo = 0;
        if (
            ($mail->flags & FLAG_DNSENT) == 0 &&
            $mail->_row['folder'] != FOLDER_OUTBOX
        ) {
            $dispositionNotificationTo = $mail->GetHeaderValue(
                'disposition-notification-to',
            );
            if ($dispositionNotificationTo) {
                $confirmationTo = ExtractMailAddress(
        ...
        if (
            isset($textParts['html']) &&
            ($userRow['soforthtml'] == 'yes' ||
                isset($_REQUEST['htmlView']) ||
                $mail->IsTrusted())
        ) {
            $textMode = 'html';
            $text = formatEMailHTMLText(
                $textParts['html'],
        ...
        } elseif (isset($textParts['html']) && $userRow['soforthtml'] == 'no') {
            $textMode = 'text';
        ...
        if ($textMode == 'html') {
            $text =
                '<base target="_blank" /><div id="__bmMailText"><font face="arial" size="2">' .
                $text .
                '</font></div>';
        } else {
            $text =
                '<base target="_blank" /><div id="__bmMailText"><font face="' .
                ($userRow['plaintext_courier'] == 'yes' ? 'courier' : 'arial') .
                '" size="2">' .
                $text .
                '</font></div>';
        }
        ...
        if (
            isset($_REQUEST['do']) &&
            $_REQUEST['do'] == 'downloadAttachments' &&
            isset($_REQUEST['att']) &&
            is_array($_REQUEST['att'])
        ) {
            ...
            header(
                sprintf(
                    'Content-Disposition: attachment; filename="attachments-%d.zip"',
                    (int) $_REQUEST['id'],
                ),
            );
        ...
        foreach ($attachments as $key => $info) {
            if (
                in_array(strtolower($info['mimetype']), [
                    'text/directory',
                    'text/x-vcard',
                    'application/vcard',
                    'text/anytext',
        ...
        // prev & next mail
        [$prevID, $nextID] = $mailbox->GetPrevNextMail(
            $mail->_row['folder'],
            (int) $_REQUEST['id'],
        );
        if ($prevID != -1) {
        ...
        $tpl->assign(
            'trained',
            $mail->trained ||
                $bm_prefs['use_bayes'] == 'no' ||
                $userRow['spamfilter'] == 'no' ||
                $mail->_row['folder'] == FOLDER_OUTBOX ||
                $mail->IsTrusted(),
        );
        ...
        $tpl->assign(
            ...
            !$enableExternal &&
                $textMode == 'html' &&
                !$mail->IsTrusted() &&
                formatEMailHTMLText($textParts['html'], true) !=
                    formatEMailHTMLText($textParts['html'], false),
        );
        $tpl->assign(
            'htmlAvailable',
            isset($textParts['html']) && $textMode != 'html',
        );
        ...
        if (isset($_REQUEST['preview'])) {
            ...
            $tpl->assign(
                'plaintextCourier',
                $textMode != 'html' && $userRow['plaintext_courier'] == 'yes',
            );
    ...
    if ($mail !== false) {
        ...
        if (isset($parts[$_REQUEST['attachment']])) {
            ...
            if (
                ...
                header(
                    sprintf('Content-Length: %d', $file['uncompressedSize']),
                );
 ...
 */ elseif (
    ...
    if ($mail !== false) {
        ...
        if (isset($parts[$_REQUEST['attachment']])) {
            ...
            $tpl->assign(
                'fromAddresses',
                ParseMailList($attMail->GetHeaderValue('from')),
            );
            $tpl->assign(
                'toAddresses',
                ParseMailList($attMail->GetHeaderValue('to')),
            );
            $tpl->assign(
                'ccAddresses',
                ParseMailList($attMail->GetHeaderValue('cc')),
            );
            $tpl->assign(
                'replyToAddresses',
                ParseMailList($attMail->GetHeaderValue('reply-to')),
            );
 ...
 */ elseif (
    ...
    isset($_REQUEST['mode']) &&
    in_array($_REQUEST['mode'], ['html', 'text'])
) {
    $mail = $mailbox->GetMail((int) $_REQUEST['id']);
 ...
 */ elseif ($_REQUEST['action'] == 'download' && isset($_REQUEST['id'])) {
    ...
    if ($mail !== false) {
        ...
        if ($messageFP) {
            ...
            header(
                sprintf(
                    'Content-Disposition: attachment; filename="%d.eml"',
                    (int) $_REQUEST['id'],
                ),
            );
            header('Content-Type: message/rfc822');
            header(sprintf('Content-Length: %d', $messageSize));
 ...
 */ elseif (
    ...
    if ($mail !== false) {
        ...
        if ($mail->AttachmentToFP($_REQUEST['attachment'], $cardFP)) {
            header(
                'Location: organizer.addressbook.php?sid=' .
                    session_id() .
                    '&action=addContact&importFile=' .
                    $tempID,
            );
            fclose($cardFP);
        } else {
 ...
 */ elseif ($_REQUEST['action'] == 'showSource' && isset($_REQUEST['id'])) {
    ...
    if ($mail !== false) {
        ...
        if ($messageFP) {
            ...
            // source
            $messageSource =
                nl2br($messageHeader) . '<br /><br />' . nl2br($messageBody);
