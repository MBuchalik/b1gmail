...
class BMCardDAVBackend extends Sabre\CardDAV\Backend\AbstractBackend {
    ...
    public function getCards($addressbookId) {
        ...
        if (count($book) > 0) {
            foreach (
                $os->getLastModified(array_keys($book), BMCL_TYPE_CONTACT)
                as $itemID => $lastModified
            ) {
    ...
    public function getCard($addressBookId, $cardUri) {
        ...
        } elseif ($contactID[0] == BMCL_TYPE_CONTACTGROUP) {
    ...
    public function createCard($addressBookId, $cardUri, $cardData) {
        ...
        if ($addressBookId > 0) {
            $groups = [$addressBookId];
        } else {
            $groups = [];
        }

        $parsedCard = Sabre\VObject\Reader::read(
            $cardData,
            Sabre\VObject\Reader::OPTION_FORGIVING,
        );

        if (
            isset($parsedCard->{'X-ADDRESSBOOKSERVER-KIND'}) &&
            strtoupper($parsedCard->{'X-ADDRESSBOOKSERVER-KIND'}->getValue()) ==
                'GROUP'
        ) {
    ...
    public function updateCard($addressBookId, $cardUri, $cardData) {
        ...
        $cardData = Sabre\VObject\Reader::read(
            $cardData,
            Sabre\VObject\Reader::OPTION_FORGIVING,
        );
        ...
        if ($contactID[0] == BMCL_TYPE_CONTACT) {
            ...
            $os->addressbook->Change(
                ...
                $contact['kommentar'],
                $contact['geburtsdatum'],
                $oldContact['default_address'],
                false,
            );

            if (isset($contact['pictureFile'])) {
                $os->addressbook->ChangePicture(
                    $contactID[1],
                    $contact['pictureFile'],
                    $contact['pictureMIME'],
                    false,
    ...
    private function updateGroupMembers($groupID, $cardData) {
        ...
        foreach ($oldMembers as $member) {
            $uid = $os->genUID(
                $member['dav_uid'],
                'contact-' . $member['id'] . '@' . $os->userRow['id'],
            );
    ...
    private function contactIdByUID($uid) {
        ...
        foreach ($contacts as $contact) {
            if (
                (!empty($contact['dav_uid']) && $contact['dav_uid'] == $uid) ||
                (empty($contact['dav_uid']) &&
                    $os->genUID(
                        '',
                        'contact-' . $contact['id'] . '@' . $os->userRow['id'],
                    ) == $uid)
            ) {
                return $contact['id'];
            }
    ...
    private function cardURItoID($cardUri) {
        ...
        if (substr($cardUri, 0, 6) == 'group-') {
            ...
            return [BMCL_TYPE_CONTACTGROUP, $groupID];
        } elseif (substr($cardUri, 0, 5) == 'item-') {
            $contactID = (int) substr($cardUri, 5);
            return [BMCL_TYPE_CONTACT, $contactID];
        } elseif (!empty($cardUri)) {
            $result = false;

            $res = $db->Query(
                'SELECT `id` FROM {pre}adressen WHERE `user`=? AND `dav_uri`=?',
                $os->userRow['id'],
                $cardUri,
            );
            ...
            if ($result) {
                return [BMCL_TYPE_CONTACT, $result];
            }

            $res = $db->Query(
                'SELECT `id` FROM {pre}adressen_gruppen WHERE `user`=? AND `dav_uri`=?',
                $os->userRow['id'],
                $cardUri,
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            if ($result) {
                return [BMCL_TYPE_CONTACTGROUP, $result];
            }
        }
    ...
    private function vObjectToRow($obj, $baseRow = null) {
        ...
        if ($obj->N) {
            [$row['nachname'], $row['vorname']] = $obj->N->getParts();
        }

        if ($obj->ORG) {
            [$row['firma']] = $obj->ORG->getParts();
        }
        ...
        if ($obj->PHOTO) {
            ...
            } elseif (substr($photoData, 1, 3) == 'PNG') {
                $row['pictureMIME'] = 'image/png';
            } elseif (strpos(substr($photoData, 0, 32), 'JFIF') !== false) {
    ...
    private function groupToVObject($row) {
        ...
        $obj = new Sabre\VObject\Component\VCard([
            'PRODID' => $os->getProdID(),
        ]);
    ...
    private function rowToVObject($row) {
        ...
        $obj->add(
            ...
            $os->genUID(
                $row['dav_uid'],
                'contact-' . $row['id'] . '@' . $os->userRow['id'],
            ),
        );
        $obj->add('N', [$row['nachname'], $row['vorname'], '', '', '']);
        ...
        if (!empty($row['tel'])) {
            $obj->add('TEL', $row['tel'], ['type' => 'HOME']);
        }
        if (!empty($row['work_tel'])) {
            $obj->add('TEL', $row['work_tel'], ['type' => 'WORK']);
        }

        if (!empty($row['fax'])) {
            $obj->add('TEL', $row['fax'], ['type' => 'FAX']);
        }
        if (!empty($row['work_fax'])) {
            $obj->add('TEL', $row['work_fax'], ['type' => ['FAX', 'WORK']]);
        }

        if (!empty($row['handy'])) {
            $obj->add('TEL', $row['handy'], ['type' => 'CELL']);
        }
        if (!empty($row['work_handy'])) {
            $obj->add('TEL', $row['work_handy'], ['type' => ['CELL', 'WORK']]);
        }

        if (!empty($row['email'])) {
            $obj->add('EMAIL', $row['email'], ['type' => 'HOME']);
        }
        if (!empty($row['work_email'])) {
            $obj->add('EMAIL', $row['work_email'], ['type' => 'WORK']);
        }
        ...
        if (!empty($row['picture'])) {
            ...
            if (is_array($picture)) {
                $obj->add('PHOTO', $picture['data'], ['BASE64' => null]);
            }
        }
...
class BMCardDAVAuthBackend extends BMAuthBackend {
    ...
    function setupState() {
        ...
        $os->addressbook = _new('BMAddressbook', [$os->userRow['id']]);
    }
}
...
$authPlugin = new DAV\Auth\Plugin(
    $authBackend,
    $bm_prefs['titel'] . ' ' . $lang_user['addressbook'],
);
