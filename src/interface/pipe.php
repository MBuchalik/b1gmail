...
function ProcessPipeMail(
    ...
    if ($inputSize < 3) {
        ...
        PutLog(
            'Message not processed (input size < 3 bytes)',
            PRIO_DEBUG,
            __FILE__,
            __LINE__,
        );
    ...
    if ($inputSize > $bm_prefs['mailmax']) {
        ...
        PutLog(
            sprintf(
                'Message not processed (hard limit; %d > %d bytes)',
                $inputSize,
                $bm_prefs['mailmax'],
            ),
            PRIO_NOTE,
        ...
        $error = sprintf(
            'Message processing aborted (hard limit; %d > %d bytes)',
            $inputSize,
            $bm_prefs['mailmax'],
        );
        $errorCode = 552;
...
if (isset($_SERVER['argv'])) {
    foreach ($_SERVER['argv'] as $param) {
        if ($param == '--') {
            $passed_h = true;
        } elseif ($param == '--keep-alive') {
            $keepAlive = true;
        } elseif (substr($param, 0, 10) == '--timeout=') {
            $timeout = (int) substr($param, 10);
        } elseif (substr($param, 0, 8) == '--flags=') {
...
// keep alive mode
    ...
    while (!feof($handle)) {
        ...
        if (!$dataMode) {
            ...
            } elseif (strtolower(substr($buff, 0, 5)) == 'flags') {
                $flags = (int) substr($buff, 5);
                echo '250 OK' . "\r\n";
                flush();
            } elseif (strtolower($buff) == 'data') {
                $dataMode = true;
                printf(
                    '354 Send data (max %d bytes), end with \r\n.\r\n' . "\r\n",
                    $bm_prefs['mailmax'],
                );
                flush();
            } elseif (strtolower($buff) == 'quit') {
