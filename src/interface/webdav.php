...
class BMWebdiskFile extends BMWebdiskNode implements Sabre\DAV\IFile {
    ...
    private function getFileID() {
        ...
        if ($this->fileID == -1) {
            [$fileID, $itemType] = $wds->webdisk->ParsePath($this->path);

            if ($itemType == WEBDISK_ITEM_FILE) {
                $this->fileID = $fileID;
            } else {
                throw new Sabre\DAV\Exception\NotFound(
                    'File ' . $this->path . ' not found',
                );
            }
        }
    ...
    public function setName($name) {
        ...
        [$parentPath] = URLUtil::splitPath($this->path);
        [, $newName] = URLUtil::splitPath($name);
    ...
    public function put($data) {
        ...
        $spaceLimit =
            $wds->groupRow['webdisk'] + $wds->userRow['diskspace_add'];
        $usedSpace = $wds->webdisk->GetUsedSpace();

        if (
            $sizeDiff >= 0 &&
            $spaceLimit != -1 &&
            $usedSpace + $sizeDiff > $spaceLimit
        ) {
            throw new Sabre\DAV\Exception\InsufficientStorage();
        }

        if (
            $wds->groupRow['traffic'] > 0 &&
            $wds->userRow['traffic_down'] +
                $wds->userRow['traffic_up'] +
                $fileSize >
                $wds->groupRow['traffic'] + $wds->userRow['traffic_add']
        ) {
        ...
        $db->Query(
            'UPDATE {pre}diskfiles SET `size`=? WHERE `id`=? AND `user`=?',
            $fileSize,
            $this->getFileID(),
            $wds->userRow['id'],
        );
    ...
    public function get() {
        ...
        $provider = BMBlobStorage::createProvider(
            $file['blobstorage'],
            $wds->userRow['id'],
        );
    ...
    public function getETag() {
        ...
        $fp = BMBlobStorage::createProvider(
            $file['blobstorage'],
            $wds->userRow['id'],
        )->loadBlob(BMBLOB_TYPE_WEBDISK, $file['id']);
        if ($fp) {
            while (!feof($fp)) {
...
class BMWebdiskDirectory extends BMWebdiskNode implements
    ...
    private function getFolderID() {
        ...
        if ($this->folderID == -1) {
            [$folderID, $itemType] = $wds->webdisk->ParsePath(
                $this->path == '' ? '/' : $this->path,
            );
    ...
    public function createFile($name, $data = null) {
        ...
        if (
            $oldFileID = $wds->webdisk->FileExists($this->getFolderID(), $name)
        ) {
        ...
        BMBlobStorage::createDefaultWebdiskProvider(
            $wds->userRow['id'],
        )->storeBlob(BMBLOB_TYPE_WEBDISK, $fileID, $data);
    }
    ...
    public function getChild($name) {
        ...
        } elseif ($item[1] == WEBDISK_ITEM_FILE) {
    ...
    public function getLastModified() {
        ...
        return $wds->webdisk->FolderDate(
            $this->getFolderID(),
            'modified',
            WEBDAV_ALT_TIMESTAMP,
        );
    }
}
...
class BMWebdiskAuthBackend extends BMAuthBackend {
    ...
    function setupState() {
        ...
        $wds->webdisk = _new('BMWebdisk', [$wds->userRow['id']]);
    }
}
...
class BMWebdiskLockBackend extends Sabre\DAV\Locks\Backend\AbstractBackend {
    public function getLocks($uri, $returnChildLocks) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}disklocks WHERE `user`=?',
            $wds->userRow['id'],
        );
    ...
    public function lock($uri, Sabre\DAV\Locks\LockInfo $lockInfo) {
        ...
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}disklocks WHERE `user`=? AND `path`=? AND `token`=?',
            $wds->userRow['id'],
            $lockInfo->uri,
            $lockInfo->token,
        );
        [$lockCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();

        if ($lockCount > 0) {
            $db->Query(
                'UPDATE {pre}disklocks SET `expires`=?,`modified`=?,`scope`=?,`owner`=?,`type`=? WHERE `user`=? AND `path`=? AND `token`=?',
            ...
            $db->Query(
                'REPLACE INTO {pre}disklocks(`user`,`path`,`token`,`type`,`created`,`modified`,`expires`,`scope`,`owner`) ' .
                    'VALUES(?,?,?,?,?,?,?,?,?)',
                $wds->userRow['id'],
                $lockInfo->uri,
                $lockInfo->token,
    ...
    public function unlock($uri, Sabre\DAV\Locks\LockInfo $lockInfo) {
        ...
        $db->Query(
            'DELETE FROM {pre}disklocks WHERE `user`=? AND `token`=?',
            $wds->userRow['id'],
            $lockInfo->token,
        );
...
$authPlugin = new DAV\Auth\Plugin(
    $authBackend,
    $bm_prefs['titel'] . ' ' . $lang_user['webdisk'],
);
