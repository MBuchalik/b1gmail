...
if ($_REQUEST['action'] == 'folders') {
    ...
    $theFolderList = $mailbox->GetUserFolderList(
        $sortColumn,
        $sortOrder,
        true,
        true,
 ...
 */ elseif ($_REQUEST['action'] == 'editFolder' && isset($_REQUEST['id'])) {
    ...
    if ($folderID <= 0 && isset($folderList[$folderID])) {
        ...
        if ($folder !== false) {
            $tpl->assign(
                'realFolderList',
                $mailbox->GetUserFolderList('titel', 'ASC', false),
            );
            ...
            foreach ($_POST as $key => $val) {
                if (substr($key, 0, 6) == 'field_') {
                    ...
                    if (isset($conditions[$id])) {
                        ...
                        if (in_array($field, [6, 7, 8, 10, 11, 15])) {
