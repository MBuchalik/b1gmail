...
if ($_REQUEST['action'] == 'search') {
    if (
        ...
        if (
            isset($_REQUEST['massAction_' . $category]) &&
            isset($_REQUEST['items'][$category]) &&
            is_array($_REQUEST['items'][$category])
        ) {
            $action = $_REQUEST['massAction_' . $category];

            $plugins->callFunction('HandleSearchMassAction', false, false, [
                $category,
                $action,
                $_REQUEST['items'][$category],
            ]);
        }
    }

    // categories
    $categoryPieces = $plugins->callFunction(
        'GetSearchCategories',
        false,
        true,
        [],
    ...
    foreach ($categoryPieces as $categoryPiece) {
        foreach ($categoryPiece as $catName => $catInfo) {
            $searchIn[$catName] =
                ...
                in_array($catName, $_REQUEST['searchIn']);
        }
        $categories = array_merge($categories, $categoryPiece);
    }

    // date
    $dateFrom = isset($_REQUEST['dateFromDay'])
        ? SmartyDateTime('dateFrom')
        : 0;
    ...
    // sort info
    $sortColumns = ['score', 'date', 'size', 'title'];
    $sortColumn =
    ...
    if (strlen($q) >= 3) {
        // search
        $resultPieces = $plugins->callFunction('OnSearch', false, true, [
            $q,
            $dateFrom,
            $dateTo,
        ]);
        ...
        foreach ($resultPieces as $resultPiece) {
            foreach ($resultPiece as $subPieceKey => $subPiece) {
                if (
                    isset($subPiece['name']) &&
                    (!isset($searchIn[$subPiece['name']]) ||
                        !$searchIn[$subPiece['name']])
                ) {
 ...
 */ elseif (
    ...
    // search
    $resultPieces = $plugins->callFunction('OnSearch', false, true, [$q]);
    ...
    if ($resultCount > MAX_SEARCH_RESULTS) {
        ...
        foreach ($results as $key => $val) {
            $results[$key]['results'] = array_slice(
                $val['results'],
                0,
                ceil($factor * count($val['results'])),
            );
        }
    }
 ...
 */ elseif (
    ...
    $mails = $mailbox->GetMailsWithFlags(
        FLAG_INDEXED,
        false,
        FTS_BGINDEX_COUNT + 1,
    );
    ...
    $idx = _new('BMSearchIndex', [$userRow['id']]);
    $processedIDs = [];
    $haveMore = false;
