...
if ($_REQUEST['action'] == 'compose') {
    ...
    if (
        isset($composeDefaults['signature']) &&
        $composeDefaults['signature'] > 0
    ) {
    ...
    if (
        ...
        if ($action == 'redirect') {
            ...
            $mail['replyto'] = DecodeEMail(
                $sourceMail->GetHeaderValue('reply-to'),
            );
            $mail['priority'] =
                $sourceMail->GetHeaderValue('x-priority') == 5
                    ? ITEMPRIO_LOW
                    : ($sourceMail->GetHeaderValue('x-priority') == 1
                        ? ITEMPRIO_HIGH
                        : ITEMPRIO_NORMAL);
            $mail['isAutoSavedDraft'] =
                ($sourceMail->flags & FLAG_AUTOSAVEDDRAFT) != 0;
            if ($mail['isAutoSavedDraft']) {
                $mail['baseDraftID'] = $sourceMail->id;
            }
        } elseif ($action == 'reply') {
            if (
                ($replyTo = $sourceMail->GetHeaderValue('reply-to')) &&
                $replyTo != ''
            ) {
            ...
            if (
                ($origTo = ExtractMailAddress(
                    $sourceMail->GetHeaderValue('to'),
                )) != '' &&
            ...
            if (isset($_REQUEST['all'])) {
                $recpList = array_merge(
                    ExtractMailAddresses($sourceMail->GetHeaderValue('to')),
                    ExtractMailAddresses($sourceMail->GetHeaderValue('cc')),
                );
                foreach ($recpList as $key => $val) {
                    if (
                        strtolower($val) ==
                        strtolower(ExtractMailAddress($mail['to']))
                    ) {
                        unset($recpList[$key]);
                    } else {
                        foreach ($possibleSenders as $possibleSender) {
                            if (
                                strtolower(
                                    ExtractMailAddress($possibleSender),
                                ) == strtolower($val)
                            ) {
                                unset($recpList[$key]);
                                break;
        ...
        $defaultSenderWithName =
            strchr($thisUser->GetDefaultSender(), '"') !== false;
        foreach ($recpList as $val) {
            foreach ($possibleSenders as $possibleSenderID => $possibleSender) {
                if (
                    strtolower(ExtractMailAddress($possibleSender)) ==
                    strtolower($val)
                ) {
                    if (
                        (strchr($possibleSender, '"') !== false &&
                            $defaultSenderWithName) ||
                        (strchr($possibleSender, '"') === false &&
                            !$defaultSenderWithName)
                    ) {
        ...
        // attachments?
        if ($action == 'forward' || $action == 'redirect') {
    ...
    // multi forward
    elseif (isset($_REQUEST['forward']) && is_array($_REQUEST['forward'])) {
        $attachmentList = '';
    ...
    // new email
        ...
        if ($latestDraftID !== false) {
            ...
            if ($latestDraftMail) {
                $latestDraftTo = ExtractMailAddresses(
                    $latestDraftMail->GetHeaderValue('to'),
                );
                if (count($latestDraftTo) > 1) {
                    $latestDraftTo = array_shift($latestDraftTo) . ', ...';
                } elseif (is_array($latestDraftTo)) {
                    $latestDraftTo = array_shift($latestDraftTo);
                } else {
                    $latestDraftTo = '';
                }
    ...
    if (!isset($mail['text'])) {
        ...
        } elseif ($textMode == 'text' && $sigText !== false) {
            $mail['text'] .= "\n\n" . $sigText;
        }
    }
 ...
 */ elseif ($_REQUEST['action'] == 'addAttachment') {
    ...
    // assign
    $tpl->assign('bar', ['value' => $attSize, 'max' => $groupRow['anlagen']]);
    $tpl->assign('title', $lang_user['addattach']);
    $tpl->assign('text', $lang_user['addattachtext']);
    $tpl->assign('multiple', true);
    $tpl->assign(
        'formAction',
        'email.compose.php?action=uploadAttachment&spaceLeft=' .
            $attLeft .
            '&sid=' .
            session_id(),
    );
 ...
 */ elseif (
    ...
    if ($tempFP) {
        ...
        header(sprintf('Content-Type: %s', $_REQUEST['type']));
        header(sprintf('Content-Length: %d', filesize($tempFileName)));
 ...
 */ elseif ($_REQUEST['action'] == 'deleteDraft' && isset($_REQUEST['id'])) {
    ...
    if (
        $draftMail === false ||
        ($draftMail->flags & FLAG_AUTOSAVEDDRAFT) == 0
    ) {
        echo '0';
        exit();
    }
 ...
 */ elseif (
    ...
    foreach ($files as $file) {
        if ($file['size'] <= $spaceLeft) {
            ...
            echo 'parent.document.getElementById(\'attachments\').value += \'' .
                addslashes($attachmentString) .
                '\';' .
                "\n";
        } else {
