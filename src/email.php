...
if ($_REQUEST['action'] == 'folder') {
    $sortColumns = ['von', 'an', 'betreff', 'recdate', 'size'];

    // get folder id (default = inbox)
    $folderID =
        isset($_REQUEST['folder']) &&
        (isset($folderList[(int) $_REQUEST['folder']]) ||
            $mailbox->FolderExists((int) $_REQUEST['folder']))
            ? (int) $_REQUEST['folder']
            : FOLDER_INBOX;
    ...
    if (
        ...
        foreach ($mails as $mailID) {
            if (
                $mailbox->FlagMail(
                    FLAG_UNREAD,
                    isset($_REQUEST['unread']) ? true : false,
                    $mailID,
                )
            );
        }
        ...
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'setViewOptions' &&
        IsPOSTRequest()
    ) {
        if (
            in_array($_REQUEST['group_mode'], [
                '-',
                'fetched',
                'von',
                'beantwortet',
    ...
    } elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'viewOptions') {
        ...
        exit();
    } elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'action') {
        //
        // collect IDs
        //
        $mailIDs = [];
        ...
        if (
            isset($_POST['selectedMailIDs']) &&
            trim($_POST['selectedMailIDs']) != ''
        ) {
        ...
        if (count($mailIDs) > 0) {
            ...
            // forward
            elseif ($_REQUEST['massAction'] == 'forward') {
            ...
            // markread
            elseif ($_REQUEST['massAction'] == 'markread') {
                foreach ($mailIDs as $mailID) {
                    $mailbox->FlagMail(FLAG_UNREAD, false, $mailID);
                }
            }

            // markunread
            elseif ($_REQUEST['massAction'] == 'markunread') {
                foreach ($mailIDs as $mailID) {
                    $mailbox->FlagMail(FLAG_UNREAD, true, $mailID);
                }
            }

            // mark
            elseif ($_REQUEST['massAction'] == 'mark') {
                foreach ($mailIDs as $mailID) {
                    $mailbox->FlagMail(FLAG_FLAGGED, true, $mailID);
                }
            }

            // unmark
            elseif ($_REQUEST['massAction'] == 'unmark') {
                foreach ($mailIDs as $mailID) {
                    $mailbox->FlagMail(FLAG_FLAGGED, false, $mailID);
                }
            }

            // mark done
            elseif ($_REQUEST['massAction'] == 'done') {
                foreach ($mailIDs as $mailID) {
                    $mailbox->FlagMail(FLAG_DONE, true, $mailID);
                }
            }

            // unmark done
            elseif ($_REQUEST['massAction'] == 'undone') {
                foreach ($mailIDs as $mailID) {
                    $mailbox->FlagMail(FLAG_DONE, false, $mailID);
                }
            }

            // mark as spam
            elseif ($_REQUEST['massAction'] == 'markspam') {
                foreach ($mailIDs as $mailID) {
            ...
            // mark as nonspam
            elseif ($_REQUEST['massAction'] == 'marknonspam') {
                foreach ($mailIDs as $mailID) {
            ...
            // move to group
            elseif (substr($_REQUEST['massAction'], 0, 6) == 'color_') {
        ...
        isset($_REQUEST['do']) &&
        $_REQUEST['do'] == 'setPreviewPosition' &&
        isset($_REQUEST['pos'])
    ) {
    ...
    $tpl->assign(
        'folderString',
        sprintf(
            'folder=%d&sort=%s&order=%s&page=%d',
            $folderID,
            $sortColumn,
            $sortOrder,
            $pageNo,
    ...
    if (isset($_REQUEST['inline'])) {
        ...
        echo "\t\"windowTitle\": \"" .
            addslashes(
                DecodeHTMLEntities($folderTitle) . ' - ' . $bm_prefs['titel'],
            ) .
            "\",\n";
 ...
 */ elseif (
    ...
    $mailbox->MoveMail(
        explode(',', $_REQUEST['mails']),
        (int) $_REQUEST['destFolderID'],
    );
