...
if ($bm_prefs['last_userpop3_cron'] < time() - $bm_prefs['cron_interval']) {
    ...
    $startTime = time();
    $res = $db->Query(
        'SELECT DISTINCT({pre}pop3.user) FROM {pre}pop3,{pre}users,{pre}gruppen WHERE {pre}gruppen.id={pre}users.gruppe AND {pre}users.id={pre}pop3.user AND {pre}users.gesperrt=\'no\' AND {pre}pop3.last_fetch+{pre}gruppen.ownpop3_interval<? AND {pre}pop3.paused=\'no\'',
        time(),
    );
    while ($row = $res->FetchArray(MYSQLI_NUM)) {
        $user = _new('BMUser', [$row[0]]);
        $userPOP3Gateway = _new('BMUserPOP3Gateway', [$row[0], &$user]);
        $userPOP3Gateway->Run();
    ...
    $db->Query('UPDATE {pre}prefs SET last_userpop3_cron=?', time());
}
