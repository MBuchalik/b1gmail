...
if ($_REQUEST['action'] == 'login') {
    if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'login') {
        ...
        // login
        [$result, $param] = BMUser::Login($email, $password);

        // login ok?
        if ($result == USER_OK) {
            // allowed to access mobile interface?
            $user = _new('BMUser', [BMUser::GetID($email)]);
            $group = $user->GetGroup();
            ...
            switch ($result) {
                case USER_BAD_PASSWORD:
                    $tpl->assign(
                        'msg',
                        sprintf($lang_user['badlogin'], $param),
                    );
                    break;
                ...
                case USER_LOGIN_BLOCK:
                    $tpl->assign(
                        'msg',
                        sprintf($lang_user['loginblocked'], FormatDate($param)),
                    );
                    break;
            }
            $tpl->assign('backLink', './');
