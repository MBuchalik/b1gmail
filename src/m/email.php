 ...
 */ elseif ($_REQUEST['action'] == 'read' && isset($_REQUEST['id'])) {
    ...
    if ($mail !== false) {
        ...
        if (($mail->flags & FLAG_UNREAD) != 0 && !isset($_REQUEST['unread'])) {
            ...
            ($mail->flags & FLAG_UNREAD) == 0 &&
            isset($_REQUEST['unread'])
        ) {
        ...
        $tpl->assign(
            'pageTitle',
            HTMLFormat(
                strlen($mail->GetHeaderValue('subject')) > 25
        ...
        $tpl->assign(
            'replySubject',
            urlencode($userRow['re'] . ' ' . $mail->GetHeaderValue('subject')),
        );
 ...
 */ elseif ($_REQUEST['action'] == 'compose') {
    ...
    if (isset($_REQUEST['reply']) || isset($_REQUEST['forward'])) {
        $sourceMail = $mailbox->GetMail(
            isset($_REQUEST['reply'])
                ? (int) $_REQUEST['reply']
                : (int) $_REQUEST['forward'],
        );
        ...
        if ($sourceMail !== false) {
            ...
            foreach ($recpList as $val) {
                foreach (
                    $possibleSenders
                    as $possibleSenderID => $possibleSender
                ) {
                    if (
            ...
            // subject
            $mail['subject'] =
                trim($userRow[isset($_REQUEST['reply']) ? 're' : 'fwd']) .
                ' ' .
 ...
 */ elseif ($_REQUEST['action'] == 'moveMail' && isset($_REQUEST['id'])) {
    ...
    if ($mail !== false) {
        if (isset($_REQUEST['to']) && isset($folderList[$_REQUEST['to']])) {
            ...
            header(
                'Location: email.php?action=read&id=' .
                    ((int) $_REQUEST['id']) .
                    '&sid=' .
                    session_id(),
            );
    ...
    if (count($recipients) > 0) {
        ...
        elseif (!$thisUser->MaySendMail(count($recipients))) {
            ...
            if ($outboxFP && is_resource($outboxFP)) {
                ...
                PutLog(
                    sprintf(
                        '<%s> (%d, IP %s) sends mail from <%s> to <%s> using mobile compose form',
                        $userRow['email'],
                        $userRow['id'],
                        $_SERVER['REMOTE_ADDR'],
