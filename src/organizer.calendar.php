...
if (
    isset($_REQUEST['view']) &&
    in_array($_REQUEST['view'], ['day', 'month', 'week'])
) {
 ...
 */
$viewMode =
    ($viewMode = $thisUser->GetPref('calendarViewMode')) === false
        ? $bm_prefs['calendar_defaultviewmode']
        : $viewMode;
$date = time();
if (isset($_REQUEST['date'])) {
    $date = (int) $_REQUEST['date'];
} elseif (isset($_REQUEST['date_Month']) && !isset($_REQUEST['jumpToday'])) {
    if ($viewMode == 'day') {
        $date = mktime(
            0,
            0,
    ...
    isset($_REQUEST['date_Week']) &&
    isset($_REQUEST['date_Year']) &&
    !isset($_REQUEST['jumpToday']) &&
    $viewMode == 'week'
) {
...
if ($viewMode == 'day') {
    ...
    $dateEnd = mktime(
        ...
        59,
        date('m', $date),
        $calendar->GetDaysInMonth(date('m', $date), date('Y', $date)),
        date('Y', $date),
    );
} elseif ($viewMode == 'week') {
    $dateStart = BMCalendar::GetWeekStartDay($date);
    $dateEnd = BMCalendar::GetWeekEndDay($date) + 86400 - 1;
}
$groups = $calendar->GetGroups();
$group =
    ($group = $thisUser->GetPref('calendarGroup')) !== false &&
    isset($groups[$group])
        ? $group
        : -2;
...
$tpl->assign(
    'prevMonth',
    mktime(0, 0, 0, date('m', $date), 15, date('Y', $date)) - TIME_ONE_MONTH,
);
$tpl->assign(
    'nextMonth',
    mktime(0, 0, 0, date('m', $date), 15, date('Y', $date)) + TIME_ONE_MONTH,
...
$tpl->assign(
    'weekYear',
    date('o') == 'o' ? date('Y', $date) : date('o', $date),
);
$tpl->assign(
    'thisMonthText',
...
if ($_REQUEST['action'] == 'start') {
    if ($viewMode == 'day') {
        ...
        [$columns, $days] = $calendar->GenerateCalendar(
            (int) date('m', $dateStart),
 ...
 */ elseif ($_REQUEST['action'] == 'groups') {
    ...
    if (
        ...
        if ($group !== false) {
            ...
            $tpl->assign(
                'pageContent',
                'li/organizer.calendar.groups.edit.tpl',
            );
            $tpl->display('li/index.tpl');
        }
    }
    ...
    // add
    //
    elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'addForm') {
        // page output
 ...
 */ elseif ($_REQUEST['action'] == 'addDate') {
    ...
    $startTime = isset($_REQUEST['time']) ? (int) $_REQUEST['time'] : time();
 ...
 */ elseif ($_REQUEST['action'] == 'createDate' && IsPOSTRequest()) {
    ...
    if (trim($_REQUEST['attendees']) != '') {
        ...
        foreach ($attendeeList as $attendeeItem) {
            [$attendeeID] = explode(',', $attendeeItem);
    ...
    header(
        'Location: organizer.calendar.php?sid=' .
            session_id() .
            '&date=' .
            $row['startdate'],
    );
 ...
 */ elseif ($_REQUEST['action'] == 'editDate' && isset($_REQUEST['id'])) {
    ...
    if ($date) {
        $date['repeating'] =
            $date['repeat_flags'] & CLNDR_REPEATING_DAILY ||
        ...
        if (($date['flags'] & CLNDR_WHOLE_DAY) == 0) {
            ...
            $durationMinutes = ceil(
                ($duration - $durationHours * TIME_ONE_HOUR) / TIME_ONE_MINUTE,
            );
        ...
        foreach ($attendees as $attendee) {
            $attendeeStr[] = sprintf(
                '%d,%s,%s',
                $attendee['id'],
                str_replace([',', ';'], '', $attendee['nachname']),
                str_replace([',', ';'], '', $attendee['vorname']),
            );
        }
 ...
 */ elseif (
    ...
    $jumpbackDate = isset($_REQUEST['jumpbackDate'])
        ? (int) $_REQUEST['jumpbackDate']
        : $row['startdate'];
