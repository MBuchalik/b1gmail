...
if ($_REQUEST['action'] == 'start') {
    $sortColumns = ['vorname', 'nachname', 'firma', 'email', 'gruppe'];

    // group id
    $currentGroup = -1;
    if (
        isset($_REQUEST['group']) &&
        (substr($_REQUEST['group'], 0, 3) == 'wg_' ||
            (int) $_REQUEST['group'] > 0)
    ) {
    ...
    // get sort info
    $sortColumn =
        isset($_REQUEST['sort']) && in_array($_REQUEST['sort'], $sortColumns)
            ? $_REQUEST['sort']
            : 'nachname';
    $sortOrder =
    ...
    $addressList = $book->GetAddressBook(
        isset($_REQUEST['letter']) ? $_REQUEST['letter'] : '*',
        $currentGroup,
        $sortColumn,
        $sortOrder,
        true,
 ...
 */ elseif ($_REQUEST['action'] == 'quickAdd' && IsPOSTRequest()) {
    if (is_array($_POST['addr'])) {
        foreach ($_POST['addr'] as $addr) {
            ...
            $groups =
                isset($addr['groups']) && is_array($addr['groups'])
                    ? $addr['groups']
                    : [];
            $book->AddContact(
                $addr['company'],
                ...
                '',
                '',
                empty($addr['company']) ? ADDRESS_PRIVATE : ADDRESS_WORK,
                $groups,
            );
        }
    }
 ...
 */ elseif (
    ...
    $pictureMime = false;
    if (
        isset($_REQUEST['pictureFile']) &&
        isset($_REQUEST['pictureMime']) &&
        is_numeric($_REQUEST['pictureFile']) &&
        $_REQUEST['pictureFile'] > 0 &&
        ValidTempFile($userRow['id'], (int) $_REQUEST['pictureFile'])
    ) {
    ...
    // submit action?
    if (
        isset($_REQUEST['submitAction']) &&
        $_REQUEST['submitAction'] == 'selfComplete'
    ) {
        // self complete page
        header(
            'Location: organizer.addressbook.php?action=selfComplete&id=' .
                (int) $contactID .
                '&sid=' .
                session_id(),
        );
 ...
 */ elseif (
    ...
    elseif (
        isset($_REQUEST['submitAction']) &&
        $_REQUEST['submitAction'] == 'sendMail'
    ) {
        // compose
        $email = urlencode(
            $_REQUEST['default'] == 'work'
                ? $_REQUEST['work_email']
                : $_REQUEST['email'],
        );
        header(
            'Location: email.compose.php?to=' . $email . '&sid=' . session_id(),
        );
    }
    ...
    elseif (
        isset($_REQUEST['submitAction']) &&
        $_REQUEST['submitAction'] == 'exportVCF'
    ) {
        // send VCF
        ...
        header(
            sprintf(
                'Content-Disposition: attachment; filename="%s %s.vcf"',
                $_REQUEST['vorname'],
                $_REQUEST['nachname'],
            ),
        );
        header('Content-Type: text/x-vcard; charset=' . $currentCharset);
        header(sprintf('Content-Length: %d', strlen($vcfData)));
    ...
    elseif (
        isset($_REQUEST['submitAction']) &&
        $_REQUEST['submitAction'] == 'intelliFolder'
    ) {
        ...
        $folderID = $mailbox->AddFolder($folderTitle, -1, true, -1, true, true);
        ...
        if (
            ($value = ExtractMailAddress($_REQUEST['work_email'], true)) != ''
        ) {
            $conditionID = $mailbox->AddCondition($folderID);
            $mailbox->UpdateCondition(
                $conditionID,
        ...
        header(
            'Location: email.folders.php?action=editFolder&id=' .
                $folderID .
                '&sid=' .
                session_id(),
        );
 ...
 */ elseif (
    ...
    $result = $book->SendSelfCompleteInvitation(
        (int) $_REQUEST['id'],
        $_REQUEST['destMail'] == 'private' ? ADDRESS_PRIVATE : ADDRESS_WORK,
    );
    if (!$result) {
    ...
    $tpl->assign(
        'backLink',
        'organizer.addressbook.php?action=editContact&id=' .
            (int) $_REQUEST['id'] .
            '&sid=' .
            session_id(),
    );
 ...
 */ elseif ($_REQUEST['action'] == 'selfComplete' && isset($_REQUEST['id'])) {
    ...
    if ($contact !== false) {
        ...
        if (
            strpos($workMail, '@') === false &&
            strpos($privateMail, '@') === false
        ) {
            ...
            $tpl->assign('otherButton', [
                'caption' => $lang_user['send_anyhow'],
                'href' =>
                    'organizer.addressbook.php?action=selfComplete&id=' .
                    (int) $_REQUEST['id'] .
                    '&sid=' .
                    session_id() .
                    '&anyhow=true',
            ...
            $tpl->assign(
                'workMail',
                strpos($workMail, '@') !== false ? $workMail : false,
            );
            $tpl->assign(
                'privateMail',
                strpos($privateMail, '@') !== false ? $privateMail : false,
            );
            $tpl->assign('id', (int) $_REQUEST['id']);
            $tpl->assign(
                'pageContent',
                'li/organizer.addressbook.selfcomplete.tpl',
            );
        }
 ...
 */ elseif ($_REQUEST['action'] == 'vcfImportDialog') {
    ...
    $tpl->assign(
        'formAction',
        'organizer.addressbook.php?action=vcfImportDialogSubmit&sid=' .
            session_id(),
    );
 ...
 */ elseif ($_REQUEST['action'] == 'vcfImportDialogSubmit' && IsPOSTRequest()) {
    ...
    if ($vcfFile) {
        ...
        foreach ($cardData as $key => $val) {
            if ($key != 'geburtsdatum' && $key != 'anrede') {
                printf(
                    'parent.document.getElementById(\'%s\').value = \'%s\';' .
                        "\n",
                    $key,
                    addslashes(str_replace(["\r", "\n"], '', $val)),
                );
 ...
 */ elseif ($_REQUEST['action'] == 'action') {
    ...
    if ($_REQUEST['do'] == 'delete') {
        ...
        foreach ($addrIDs as $id) {
            ...
            if (trim($email) != '') {
                if (empty($contact['vorname']) && empty($contact['nachname'])) {
                    array_push(
                        $to,
                        sprintf('"%s" <%s>', $contact['firma'], $email),
                    );
                } else {
                    array_push(
                        $to,
        ...
        strlen($_REQUEST['do']) > 11 &&
        substr($_REQUEST['do'], 0, 11) == 'addtogroup_'
    ) {
 ...
 */ elseif ($_REQUEST['action'] == 'groups') {
    ...
    } elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'export') {
        // export
        header('Pragma: public');
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="export.csv"');
        $book->ExportGroupContacts([(int) $_REQUEST['id']]);
        exit();
    }
 ...
 */ elseif ($_REQUEST['action'] == 'groupAction') {
    ...
    // mail
    elseif (isset($_REQUEST['do']) && $_REQUEST['do'] == 'sendmail') {
        // collect IDs
        $groupIDs = [];
        foreach ($_POST as $key => $val) {
            if (substr($key, 0, 6) == 'group_') {
 ...
 */ elseif (
    ...
    if (
        ...
        $book->GroupExists($_REQUEST['title'])
    ) {
        // already exists
        $tpl->assign('msg', $lang_user['groupexists']);
        $tpl->assign('pageContent', 'li/error.tpl');
 ...
 */ elseif (
    ...
    $tpl->assign(
        'formAction',
        'organizer.addressbook.php?action=userPictureDialogSubmit&id=' .
            (int) $_REQUEST['id'] .
            '&sid=' .
            session_id(),
    );
 ...
 */ elseif (
    ...
    if (
        $uploadFile &&
        in_array(strtolower($uploadFile['type']), [
            'image/png',
            'image/x-png',
            'image/jpg',
            'image/pjpeg',
        ...
        if ($_REQUEST['id'] == -1) {
            $pictureURL =
                'organizer.addressbook.php?action=addressbookPicture&id=-1&contentType=' .
                urlencode($uploadFile['type']) .
                '&tempID=' .
                $tempID .
                '&sid=' .
                session_id();
            echo 'parent.document.getElementById(\'pictureFile\').value = ' .
                $tempID .
                ';' .
                "\n";
            echo 'parent.document.getElementById(\'pictureMime\').value = \'' .
                addslashes($uploadFile['type']) .
                '\';' .
                "\n";
        } else {
            if (
                ...
                $pictureURL =
                    'organizer.addressbook.php?action=addressbookPicture&id=' .
                    (int) $_REQUEST['id'] .
                    '&t=' .
                    time() .
                    '&sid=' .
        ...
        if (isset($pictureURL)) {
            echo 'parent.document.getElementById(\'pictureDiv\').style.backgroundImage = \'url(' .
                $pictureURL .
                ')\';' .
                "\n";
        }
        ...
        echo 'alert(\'' .
            addslashes(
                sprintf(
                    ...
                    round($bm_prefs['max_userpicture_size'] / 1024, 2),
                ),
            ) .
            '\');' .
            "\n";
 ...
 */ elseif (
    ...
    if ($_REQUEST['lineBreakChar'] == 'lf') {
        $lineBreakChar = "\n";
    } elseif ($_REQUEST['lineBreakChar'] == 'cr') {
        $lineBreakChar = "\r";
    } else {
        $lineBreakChar = "\r\n";
    }
 ...
 */ elseif (
    ...
    $tpl->assign(
        'formAction',
        'organizer.addressbook.php?action=importDialogSubmit&type=' .
            urlencode($_REQUEST['type']) .
            '&encoding=' .
            urlencode($_REQUEST['encoding']) .
            '&sid=' .
            session_id(),
    );
 ...
 */ elseif (
    ...
    if ($uploadFile) {
        ...
        if (
            strlen($firstLine) > 3 &&
            substr($firstLine, 0, 3) == "\xEF\xBB\xBF"
        ) {
            $encoding = 'UTF-8';
        } elseif (
            strlen($firstLine) > 4 &&
            substr($firstLine, 0, 4) == "\x00\x00\xFE\xFF"
        ) {
            $encoding = 'UTF-32BE';
        } elseif (
            strlen($firstLine) > 4 &&
            substr($firstLine, 0, 4) == "\xFF\xFE\x00\x00"
        ) {
            $encoding = 'UTF-32LE';
        } elseif (
            strlen($firstLine) > 2 &&
            substr($firstLine, 0, 2) == "\xFE\xFF"
        ) {
            $encoding = 'UTF-16BE';
        } elseif (
            strlen($firstLine) > 2 &&
            substr($firstLine, 0, 2) == "\xFF\xFE"
        ) {
            $encoding = 'UTF-16LE';
        }
 ...
 */ elseif (
    ...
    if ($_REQUEST['type'] == 'csv') {
        ...
        if ($fp) {
            ...
            $csvReader = _new('CSVReader', [$fp, $encoding]);
            ...
            $tpl->assign(
                'pageContent',
                'li/organizer.addressbook.importcsv.tpl',
            );
            $tpl->display('li/index.tpl');
 ...
 */ elseif ($_REQUEST['action'] == 'addressPopup') {
    ...
    foreach ($addressBook as $id => $entry) {
        $addresses[] = [
            'name' => $entry['nachname'] . ', ' . $entry['vorname'],
            'email1' =>
                $entry['default_address'] == ADDRESS_WORK
                    ? $entry['work_email']
                    : $entry['email'],
            'email2' =>
                $entry['default_address'] == ADDRESS_WORK
    ...
    if (trim($_REQUEST['attendeeList']) != '') {
        ...
        foreach ($attendeeList as $attendeeItem) {
            if (trim($attendeeItem) != '') {
                [$attendeeID, $attendeeLastName, $attendeeFirstName] = explode(
                    ',',
                    $attendeeItem,
                );
                $addresses[] = [
