 ...
 */ elseif ($_REQUEST['action'] == 'imprint') {
    ...
    $tpl->assign(
        'contactform_subject',
        $bm_prefs['contactform_subject'] == 'yes',
    );
    $tpl->assign(
        'contactform_subjects',
        array_filter(
 ...
 */ elseif ($_REQUEST['action'] == 'signup') {
    ...
    // sign up ip lock?
    if (
        $bm_prefs['regenabled'] == 'yes' &&
        ($bm_prefs['user_count_limit'] == 0 ||
            BMUser::GetUserCount() < $bm_prefs['user_count_limit'])
    ) {
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}users WHERE reg_ip=? AND reg_date>?',
            $_SERVER['REMOTE_ADDR'],
        ...
        if (
            $row[0] == 0 &&
            $bm_prefs['signup_dnsbl_enable'] == 'yes' &&
        ...
        if ($row[0] != 0) {
        ...
        } elseif ($isInDNSBL && $bm_prefs['signup_dnsbl_action'] == 'block') {
            // block sign up
            ...
            if (isset($_POST['do']) && $_POST['do'] == 'createAccount') {
                ...
                if (
                    !BMUser::AddressValid($suEMail) ||
                    !BMUser::AddressAvailable($suEMail) ||
                ...
                if ($bm_prefs['f_anrede'] != 'n') {
                    ...
                    if (
                        (!in_array($suSalutation, ['herr', 'frau']) &&
                            $bm_prefs['f_anrede'] == 'p') ||
                        ($bm_prefs['f_anrede'] == 'v' &&
                            !in_array($suSalutation, ['herr', 'frau', '']))
                    ) {
                ...
                if ($bm_prefs['f_strasse'] != 'n') {
                    ...
                    if (
                        strlen($suStreet) < 3 &&
                        (strlen($suStreet) > 0 || $bm_prefs['f_strasse'] == 'p')
                    ) {
                    ...
                    if (
                        strlen($suNo) < 1 &&
                        (strlen($suNo) > 0 || $bm_prefs['f_strasse'] == 'p')
                    ) {
                        $invalidFields[] = 'no';
                    }
                    ...
                    if (
                        strlen($suZIP) < 3 &&
                        (strlen($suZIP) > 0 || $bm_prefs['f_strasse'] == 'p')
                    ) {
                        $invalidFields[] = 'zip';
                    }
                    ...
                    if (
                        strlen($suCity) < 3 &&
                        (strlen($suCity) > 0 || $bm_prefs['f_strasse'] == 'p')
                    ) {
                        $invalidFields[] = 'city';
                    }
                    ...
                    if (
                        ...
                        !in_array($suCountry, array_keys(CountryList()))
                    ) {
                ...
                if ($bm_prefs['f_telefon'] != 'n') {
                    ...
                    if (
                        strlen($suPhone) < 5 &&
                        (strlen($suPhone) > 0 || $bm_prefs['f_telefon'] == 'p')
                    ) {
                        $invalidFields[] = 'phone';
                    }
                } elseif ($bm_prefs['f_telefon'] == 'n') {
                    $suPhone = '';
                }
                ...
                if ($bm_prefs['f_fax'] != 'n') {
                    ...
                    if (
                        strlen($suFax) < 5 &&
                        (strlen($suFax) > 0 || $bm_prefs['f_fax'] == 'p')
                    ) {
                        $invalidFields[] = 'fax';
                    }
                } elseif ($bm_prefs['f_fax'] == 'n') {
                    $suFax = '';
                }

                // 'altmail'-field
                if (
                    $bm_prefs['f_alternativ'] != 'n' ||
                    ...
                    if (
                        (strlen($suAltMail) > 0 ||
                            ...
                            $bm_prefs['reg_validation'] == 'email') &&
                        (strtolower(trim($suAltMail)) ==
                            strtolower(trim($suEMail)) ||
                            !BMUser::AddressValid($suAltMail, false) ||
                            AltMailLocked($suAltMail) ||
                            ($bm_prefs['alt_check'] == 'yes' &&
                                !ValidateMailAddress($suAltMail)))
                    ) {
                        ...
                        $res = $db->Query(
                            'SELECT COUNT(*) FROM {pre}users WHERE `altmail`=?',
                            $suAltMail,
                        );
                ...
                } elseif ($bm_prefs['f_alternativ'] == 'n') {
                    $suAltMail = '';
                }

                // 'mail2sms_nummer'-field
                if (
                    $bm_prefs['f_mail2sms_nummer'] != 'n' ||
                    ...
                    $suMobileNr = trim(
                        preg_replace(
                            '/[^0-9]/',
                            '',
                            str_replace('+', '00', $_POST['mail2sms_nummer']),
                        ),
                    );
                    if (
                        strlen($suMobileNr) < 6 &&
                        ...
                        $res = $db->Query(
                            'SELECT COUNT(*) FROM {pre}users WHERE `mail2sms_nummer`=?',
                            $suMobileNr,
                        );
                ...
                } elseif ($bm_prefs['f_mail2sms_nummer'] == 'n') {
                    $suMobileNr = '';
                }
                ...
                if (
                    $suCoupon != '' &&
                    !BMUser::CouponValid($suCoupon, 'signup')
                ) {
                    $invalidFields[] = 'code';
                ...
                $suProfile = [];
                $res = $db->Query(
                    "SELECT id,rule,pflicht,typ FROM {pre}profilfelder WHERE show_signup='yes'",
                );
                ...
                if (count($invalidFields) > 0) {
                    ...
                    $tpl->assign(
                        'errorInfo',
                        $lang_user['checkfields'] . $errorInfo,
                    );
                    $tpl->assign('invalidFields', $invalidFields);
                } else {
                    // create account
                    ...
                    if ($userId !== false && $userId > 0) {
                        ...
                        if ($isInDNSBL) {
                            PutLog(
                                sprintf(
                                    'New user <%s> (%d) locked because IP <%s> was found in DNSBL',
                                    $suEMail,
                                    $userId,
                                    $_SERVER['REMOTE_ADDR'],
                                ),
                        ...
                        // module handler
                        ModuleFunction('AfterSuccessfulSignup', [
                            $userId,
                            $suEMail,
                        ]);
                    } else {
            ...
            if ($showForm) {
                // codes?
                $res = $db->Query(
                    'SELECT COUNT(*) FROM {pre}codes WHERE `valid_signup`=\'yes\'',
                );
                ...
                $profilfelder = [];
                $res = $db->Query(
                    'SELECT feld,pflicht,id,extra,typ FROM {pre}profilfelder WHERE show_signup=\'yes\'',
                );
                while ($row = $res->FetchArray()) {
                    array_push($profilfelder, [
                        'feld' => $row['feld'],
                ...
                $tpl->assign(
                    ...
                    $bm_prefs['reg_validation'] == 'email'
                        ? 'p'
                        : $bm_prefs['f_alternativ'],
                );
                $tpl->assign(
                    'f_mail2sms_nummer',
                    $bm_prefs['reg_validation'] == 'sms'
                        ? 'p'
                        : $bm_prefs['f_mail2sms_nummer'],
                );
                $tpl->assign(
                    'f_safecode',
                    $captcha !== false && $captcha->isAvailable()
                        ? $bm_prefs['f_safecode']
                        : 'n',
                );
                ...
                $tpl->assign(
                    'tos_html',
                    nl2br(HTMLFormat($lang_custom['tos'])),
                );
 ...
 */ elseif (
    ...
    foreach ($suggestions as $local) {
        ...
        if (
            !BMUser::AddressValid($addr) ||
            BMUser::AddressLocked($local) ||
 ...
 */ else {
    if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'login') {
        ...
        $ValidationCode =
            $requiresValidation && isset($_REQUEST['sms_validation_code'])
                ? $_REQUEST['sms_validation_code']
                : '';

        // login
        [$result, $param] = BMUser::Login(
            $email,
            $password,
            true,
            true,
        ...
        if ($result == USER_OK) {
            ...
            if ($result == USER_LOCKED && $requiresValidation) {
                ...
                if ($bm_prefs['reg_validation_max_resend_times'] > 0) {
                    $res = $db->Query(
                        'SELECT id,sms_validation_last_send,sms_validation_send_times,altmail,mail2sms_nummer,sms_validation_code FROM {pre}users WHERE email=?',
                        $email,
                    );
                    if ($res->RowCount() != 1) {
                        die('User not found');
                    ...
                    $allowResend =
                        $bm_prefs['reg_validation_max_resend_times'] >
                        $userRow['sms_validation_send_times'];
                    $resendTimeLimit = max(
                        0,
                        $bm_prefs['reg_validation_min_resend_interval'] -
                            (time() - $userRow['sms_validation_last_send']),
                    );
                    ...
                    if ($allowResend && $resendTimeLimit == 0) {
                        if (isset($_POST['resendCode'])) {
                            if (
                                ...
                                PutLog(
                                    sprintf(
                                        'Resending signup validation code by SMS to user <%s> (#%d)',
                                        $email,
                                        $userRow['id'],
                                    ),
                                    PRIO_NOTE,
                                ...
                                PutLog(
                                    sprintf(
                                        'Resending signup validation code by email to user <%s> (#%d)',
                                        $email,
                                        $userRow['id'],
                                    ),
                                    PRIO_NOTE,
                            ...
                            $db->Query(
                                'UPDATE {pre}users SET sms_validation_last_send=?,sms_validation_send_times=sms_validation_send_times+1 WHERE id=?',
                                time(),
                                $userRow['id'],
                            );
                            ...
                            $tpl->assign(
                                ...
                                sprintf(
                                    ...
                                    $bm_prefs[
                                        'reg_validation_max_resend_times'
                                    ] - $userRow['sms_validation_send_times'],
                                ),
                            );
                        ...
                        $tpl->assign(
                            ...
                            sprintf(
                                ...
                                floor($resendTimeLimit / 60),
                                $resendTimeLimit -
                                    60 * floor($resendTimeLimit / 60),
                            ),
                        );
                        $tpl->assign('allowResend', false);
                    }
                ...
                $tpl->assign(
                    'password',
                    strlen($password) == 32 ? $password : md5($password),
                );
