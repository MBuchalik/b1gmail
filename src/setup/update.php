...
// target version
$target_version = '7.4.0';

// known versions
$knownVersions = array('7.0.0-Beta1', '7.0.0-Beta2', '7.0.0-Beta3', '7.0.0-RC1', '7.0.0',
						'7.1.0',
						'7.2.0-Beta1', '7.2.0-Beta2', '7.2.0-Beta3', '7.2.0',
						'7.3.0-Beta1', '7.3.0-Beta2', '7.3.0-Beta3', '7.3.0-Beta4',
						'7.3.0-Beta5', '7.3.0-Beta6', '7.3.0',
						'7.4.0-Beta1', '7.4.0-Beta2', '7.4.0-Beta3', '7.4.0-Beta4');
	...
	|| !in_array($b1gmail_version, $knownVersions))
{
	...
	else if(!in_array($b1gmail_version, $knownVersions))
	{
		?>
		<h1><?php echo($lang_setup['error']); ?></h1>
...
else if($step == STEP_SYSTEMCHECK)
{
	...
	<table class="list">
		...
		if($numVersion <= 7309)
		{
			$res = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_mails WHERE LENGTH(`body`)!=4');
		...
		<tr>
			...
			<td><img src="../admin/templates/images/<?php if($dbMails == 0) echo 'ok'; else { echo 'error'; $nextStep = STEP_SYSTEMCHECK; } ?>.png" border="0" alt="" width="16" height="16" /></td>
		</tr>
			<?php
...
else if($step == STEP_UPDATE_STEP)
{
	...
	else if($do == 'config')
	{
		if($numVersion <= 7001)
		{
			...
			{
				mysqli_query($connection, sprintf('UPDATE bm60_smsgateways SET `user`=\'%s\',`pass`=\'%s\'',
		...
		if($numVersion <= 7100)
		{
			$res = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_certificates');
		...
		{
			mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `reg_validation`=\'%s\'',
					    $bm_prefs['reg_smsvalidation'] == 'yes' ? 'sms' : 'off'));
		}

		if($numVersion < 7202)
		{
			mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `selffolder`=\'%s\',`enable_paypal`=\'%s\',`paypal_mail`=\'%s\',`sms_enable_charge`=\'%s\',`rgtemplate`=\'%s\'',
								...
								$defaultInvoice));
			mysqli_query($connection, sprintf('UPDATE bm60_gruppen SET `max_recps`=%d',
								$bm_prefs['max_bcc']));

			if(isset($bm_prefs['sms_enable_su']))
			{
				mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `enable_su`=\'%s\',`su_kdnr`=\'%s\',`su_prjnr`=\'%s\',`su_prjpass`=\'%s\'',
			...
			{
				if($row[0] == 'bm60_mod_premium_prefs')
					$havePAcc = true;
				else if($row[0] == 'bm60_mod_taborder')
					$haveTabOrder = true;
			}
			mysqli_free_result($res);
			...
			if($haveTabOrder)
			{
				...
				$res = mysqli_query($connection, 'SELECT * FROM bm60_mod_taborder');
				while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
					$tabOrder[$row['key']] = $row['order'];
				mysqli_free_result($res);

				mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `taborder`=\'%s\'',
			...
			if($havePAcc)
			{
				$res = mysqli_query($connection, 'SELECT * FROM bm60_mod_premium_prefs');
				$pacc_prefs = mysqli_fetch_array($res, MYSQLI_ASSOC);
				mysqli_free_result($res);

				mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `send_pay_notification`=\'%s\',`pay_notification_to`=\'%s\',`pay_emailfrom`=\'%s\',`pay_emailfromemail`=\'%s\',`mwst`=\'%s\',`enable_vk`=\'%s\',`vk_kto_inh`=\'%s\',`vk_kto_nr`=\'%s\',`vk_kto_blz`=\'%s\',`vk_kto_inst`=\'%s\',`vk_kto_iban`=\'%s\',`vk_kto_bic`=\'%s\',`sendrg`=\'%s\',`rgnrfmt`=\'%s\',`kdnrfmt`=\'%s\'',
			...
			else
			{
				...
				mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `pay_emailfrom`=\'%s\',`pay_emailfromemail`=\'%s\'',
			...
			if($havePAcc)
			{
				...
				$res = mysqli_query($connection, 'SELECT * FROM bm60_mod_premium_payments');
				while($row = mysqli_fetch_array($res))
				{
					...
					{
						$res2 = mysqli_query($connection, sprintf('SELECT `txn_id` FROM bm60_payments WHERE `id`=%d',
													$row['payment_id']));
						list($txnID) = mysqli_fetch_array($res2, MYSQLI_NUM);
						mysqli_free_result($res2);
					}

					$res2 = mysqli_query($connection, sprintf('SELECT `titel` FROM bm60_mod_premium_packages WHERE `id`=%d',
												$row['paket']));
					...
					mysqli_query($connection, sprintf('INSERT INTO bm60_orders(`orderid`,`userid`,`vkcode`,`txnid`,`cart`,`paymethod`,`amount`,`tax`,`inv_firstname`,`inv_lastname`,`inv_street`,`inv_no`,`inv_zip`,`inv_city`,`inv_country`,`created`,`activated`,`status`) '
					...
					if($row['rechnung'] != '')
					{
						mysqli_query($connection, sprintf('INSERT INTO bm60_invoices(`orderid`,`invoice`) VALUES(%d,\'%s\')',
											$row['id'],
			...
			// payments table
			$res = mysqli_query($connection, 'SELECT * FROM bm60_payments');
			while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
			{
				if($row['typ'] == 'credits')
				{
					...
					mysqli_query($connection, sprintf('INSERT INTO bm60_orders(`userid`,`txnid`,`cart`,`paymethod`,`amount`,`created`,`activated`,`status`) '
			...
			mysqli_query($connection, 'DELETE FROM bm60_certificates WHERE `hash` IN(\'e268a4c5\',\'4166ec0c\',\'c80493cb\',\'f73e89fd\',\'f64d9715\')');

			// disable obsolete / outdated plugins
			mysqli_query($connection, 'UPDATE bm60_mods SET `installed`=0 WHERE `modname` IN (\'TabOrderPlugin\',\'WidgetOrderPlugin\',\'PremiumAccountPlugin\')');
		}

		if($numVersion <= 7203)
		{
			mysqli_query($connection, 'UPDATE bm60_users SET `last_smtp`=-1 WHERE `last_smtp`=0');

			$searchIn = @unserialize($bm_prefs['search_in']);
			if(is_array($searchIn))
			{
				$searchIn['attachments'] = true;
				mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `search_in`=\'%s\'',
		...
		{
			// salt PWs
			$res = mysqli_query($connection, 'SELECT `id`,`passwort` FROM bm60_users WHERE `passwort_salt`=\'\'');
			while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
			{
				$newSalt = GeneratePW();
				$newPassword = md5($row['passwort'] . $newSalt);

				mysqli_query($connection, sprintf('UPDATE bm60_users SET `passwort`=\'%s\',`passwort_salt`=\'%s\' WHERE `id`=%d',
					$newPassword,
			...
			foreach($domains as $domain)
			{
				...
				mysqli_query($connection, sprintf('REPLACE INTO bm60_domains(`domain`,`in_login`,`in_signup`,`in_aliases`,`pos`) VALUES(\'%s\',1,1,1,%d)',
			...
			mysqli_query($connection, sprintf('REPLACE INTO bm60_admins(`adminid`,`username`,`firstname`,`lastname`,`password`,`password_salt`,`type`,`notes`) VALUES '
				. '(1,\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',0,\'%s\')',
				'admin',
				'Super',
				'Administrator',
			...
			// set new prefs
			mysqli_query($connection, 'UPDATE bm60_prefs SET `template`=\'modern\',`adminpw`=\'\'');

			// enable preview
			mysqli_query($connection, 'UPDATE bm60_users SET `preview`=\'yes\'');
		}

		if($numVersion < 7302)
		{
			mysqli_query($connection, sprintf('UPDATE bm60_gruppen SET `allow_newsletter_optout`=\'%s\'',
				$bm_prefs['allow_newsletter_optout']));
			mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `mobile_url`=\'%s\'',
				SQLEscape($bm_prefs['selfurl'].'m/', $connection)));
		}

		if($numVersion <= 7306)
		{
			mysqli_query($connection, 'UPDATE bm60_prefs SET `signup_dnsbl`=\'dnsbl.tornevall.org\' WHERE `signup_dnsbl`=\'\'');
		}
		...
		if($numVersion <= 7309)
		{
			...
			$apQueries = array();
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(1,5,\'\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(2,25,\'\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(3,15,\'\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(4,10,\'\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(5,10,\'\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(6,20,\'interval=60\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(7,20,\'interval=5\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(21,5,\'amount=50;interval=5\')';
			$apQueries[] = 'REPLACE INTO bm60_abuse_points_config(`type`,`points`,`prefs`) VALUES(22,5,\'amount=100;interval=5\')';

			foreach($apQueries as $apQuery)
				mysqli_query($connection, $apQuery);

			// default values for new prefs
			mysqli_query($connection, sprintf('UPDATE bm60_prefs SET `contactform_to`=\'%s\',`ap_autolock_notify_to`=\'%s\',`fts_bg_indexing`=\'no\'',
				SQLEscape($bm_prefs['passmail_abs'], $connection),
				SQLEscape($bm_prefs['passmail_abs'], $connection)));

			// default values for new group settings
			$res = mysqli_query($connection, 'SELECT `id`,`send_limit` FROM bm60_gruppen');
			while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
			{
				$ftsSupport = class_exists('SQLite3') || (class_exists('PDO') && in_array('sqlite', PDO::getAvailableDrivers()));
				mysqli_query($connection, sprintf('UPDATE bm60_gruppen SET `send_limit_count`=1,`send_limit_time`=%d,`mail_send_code`=\'%s\',`sms_send_code`=\'%s\',`ftsearch`=\'%s\' WHERE `id`=%d',
					$row['send_limit'],
			...
			mysqli_query($connection, 'UPDATE bm60_dates SET `flags`=`flags`|8');
			...
			{
				mysqli_query($connection, sprintf('UPDATE bm60_staaten SET `is_eu`=\'yes\',`vat`=%d WHERE `id`=%d',
					$vatRate,
					$countryID));
			}
			...
			$res = mysqli_query($connection, 'SELECT `id`,`sms_kontigent` FROM bm60_users WHERE `sms_kontigent`!=0');
			while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
			{
				mysqli_query($connection, sprintf('INSERT INTO bm60_transactions(`userid`,`description`,`amount`,`date`,`status`) VALUES(%d,\'lang:startingbalance\',%d,%d,%d)',
					$row['id'],
					$row['sms_kontigent'],
					time(),
					1)) || die('Failed to insert transaction');
				mysqli_query($connection, sprintf('UPDATE bm60_users SET `sms_kontigent`=0 WHERE `id`=%d',
					$row['id'])) || die('Failed to reset sms_kontigent');
			}
			mysqli_free_result($res);
			mysqli_query($connection, 'COMMIT')  || die('Failed to commit transaction');
			mysqli_query($connection, 'ALTER TABLE bm60_users DROP sms_kontigent');
		}
		...
		{
			$res = mysqli_query($connection, sprintf('SELECT COUNT(*) FROM bm60_certificates WHERE `type`=0 AND `userid`=0 AND `hash`=\'%s\'',
		...
		{
			$langFiles = array('../languages/deutsch.lang.php', '../languages/english.lang.php');
	...
	else if($do == 'struct3')
	{
		...
		if($numVersion <= 7001)
		{
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_user';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_pass';
		}

		if($numVersion <= 7201
			&& isset($bm_prefs['reg_smsvalidation']))
		{
			$queries[] = 'ALTER TABLE bm60_prefs DROP reg_smsvalidation';
		}

		if($numVersion < 7202)
		{
			// keep compatibility with older b1gMailServer releases -- $queries[] = 'ALTER TABLE bm60_prefs DROP max_bcc';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_enable_paypal';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_paypal_mail';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_enable_su';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_su_kdnr';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_su_prjnr';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_su_prjpass';
			$queries[] = 'DROP TABLE bm60_payments';
			$queries[] = 'DROP TABLE IF EXISTS bm60_mod_taborder';
		}

		if($numVersion < 7302)
		{
			$queries[] = 'ALTER TABLE bm60_prefs DROP allow_newsletter_optout';
		}

		if($numVersion < 7401)
		{
			$queries[] = 'ALTER TABLE bm60_gruppen DROP httpmail';
			$queries[] = 'ALTER TABLE bm60_gruppen DROP send_limit';
			$queries[] = 'ALTER TABLE bm60_prefs DROP steuersatz';
			$queries[] = 'ALTER TABLE bm60_prefs DROP mail_send_code';
			$queries[] = 'ALTER TABLE bm60_prefs DROP sms_send_code';
			$queries[] = 'ALTER TABLE bm60_mails DROP body';
		}
	...
	else if($do == 'resetcache')
	{
		...
		$res = mysqli_query($connection, 'SELECT size,`key` FROM bm60_file_cache');
		while($row = mysqli_fetch_array($res, MYSQLI_ASSOC))
		{
		...
		if(count($deleteIDs) > 0)
			mysqli_query($connection, 'DELETE FROM bm60_file_cache WHERE `key` IN(\'' . implode('\',\'', $deleteIDs) . '\')');
	...
	{
		mysqli_query($connection, 'UPDATE bm60_prefs SET wartung=\'no\',patchlevel=0');
