 ...
 */ elseif ($step == STEP_SYSTEMCHECK) {
    ...
    $mysqlVersionOK = in_array(CompareVersions($mysqlVersion, '4.1.1'), [
        VERSION_IS_NEWER,
        VERSION_IS_EQUAL,
    ]);
    ?>
	...
	<table class="list">
       ...
       (int) str_replace('.', '', phpversion()) >= 422
   ) {
       echo 'ok';
   } else {
       echo 'error';
       $nextStep = STEP_SYSTEMCHECK;
		...
		<tr>
			...
			<td><?php echo function_exists('mb_convert_encoding')
       ? 'mb_string'
  ...
  ?>
	...
	<br />
	<?php echo $nextStep == STEP_CONVERT
	...
	<table class="list">
       ...
       'convert_prepare'
		...
		<tr>
			...
			<th id="step_analyzedb_text" style="font-weight:normal;">2. <?php echo $lang_setup[
       'convert_analyzedb'
   ]; ?></th>
		...
		<tr>
			...
			<th id="step_preptables_text" style="font-weight:normal;">3. <?php echo $lang_setup[
       'convert_prepare_tables'
   ]; ?></th>
			<td id="step_preptables_progress">&nbsp;</td>
		</tr>
		<tr>
			<td id="step_convert_status">&nbsp;</td>
			<th id="step_convert_text" style="font-weight:normal;">4. <?php echo $lang_setup[
       'convert_convertdata'
   ]; ?></th>
		...
		<tr>
			...
			<th id="step_collations_text" style="font-weight:normal;">5. <?php echo $lang_setup[
       'convert_collations'
   ]; ?></th>
		...
		<tr>
			...
			<th id="step_langfiles_text" style="font-weight:normal;">6. <?php echo $lang_setup[
       'convert_langfiles'
   ]; ?></th>
		...
		<tr>
			...
			<th id="step_resetcache_text" style="font-weight:normal;">7. <?php echo $lang_setup[
       'convert_resetcache'
   ]; ?></th>
		...
		<tr>
			...
			<th id="step_complete_text" style="font-weight:normal;">8. <?php echo $lang_setup[
       'convert_complete'
   ]; ?></th>
 ...
 */ elseif ($step == STEP_CONVERT_STEP) {
    ...
    if ($do == 'prepare') {
        ...
        mysqli_query($connection, 'DROP TABLE IF EXISTS bm60_utf8convert');
        mysqli_query(
            $connection,
            'CREATE TABLE bm60_utf8convert(' .
        ...
        while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
            $table = $row[0];
            if (
                substr($table, 0, strlen($mysql['prefix'])) ==
                    $mysql['prefix'] &&
                $table != 'bm60_utf8convert' &&
                $table != 'bm60_spamindex'
            ) {
                $tables[] = $table;
            }
        }
        ...
        foreach ($tables as $table) {
            mysqli_query(
                $connection,
                'INSERT INTO bm60_utf8convert(`table`) VALUES(\'' .
                    SQLEscape($table, $connection) .
                    '\')',
            );
        }
    ...
    elseif ($do == 'analyzedb') {
        ...
        $result = mysqli_query(
            $connection,
            'SELECT COUNT(*) FROM bm60_utf8convert',
        );
        [$allCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);

        // analyze table
        $result = mysqli_query(
            $connection,
            'SELECT `table` FROM bm60_utf8convert WHERE `analyzed`=0 LIMIT 1',
        );
        while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
            // get row count
            ...
            [$rowCount] = mysqli_fetch_array($result2, MYSQLI_NUM);
            ...
            while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC)) {
                if (
                    (strpos(strtolower($row2['Type']), 'char') !== false ||
                        strpos(strtolower($row2['Type']), 'text') !== false) &&
                    !(
                        $row[0] == $mysql['prefix'] . 'mails' &&
                        $row2['Field'] == 'body'
                    )
                ) {
            ...
            // convert data?
            $convertData =
                count($fieldsToConvert) > 0 &&
                (count($primaryKeys) > 0 || $rowCount == 1) &&
                $rowCount > 0;
            ...
            mysqli_query(
                $connection,
                sprintf(
                    'UPDATE bm60_utf8convert SET `rows`=%d,`convert_data`=%d,`fields_to_convert`=\'%s\',`primary_keys`=\'%s\',`analyzed`=1 WHERE `table`=\'%s\'',
                    $rowCount,
                    $convertData ? 1 : 0,
        ...
        $result = mysqli_query(
            $connection,
            'SELECT COUNT(*) FROM bm60_utf8convert WHERE `analyzed`=1',
        );
        [$doneCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
    ...
    elseif ($do == 'preptables') {
        ...
        $result = mysqli_query(
            $connection,
            'SELECT COUNT(*) FROM bm60_utf8convert WHERE `convert_data`=1',
        );
        [$allCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
        ...
        $result = mysqli_query(
            $connection,
            'SELECT `table` FROM bm60_utf8convert WHERE `column_added`=0 AND `convert_data`=1 LIMIT 1',
        );
        while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
            // add status column
            mysqli_query(
                $connection,
                sprintf(
                    'ALTER TABLE %s ADD `__converted` tinyint(4) NOT NULL DEFAULT 0',
                    $row[0],
                ),
            );
            ...
            mysqli_query(
                $connection,
                sprintf(
                    'UPDATE bm60_utf8convert SET `column_added`=1 WHERE `table`=\'%s\'',
                    SQLEscape($row[0], $connection),
                ),
            );
        }
        ...
        $result = mysqli_query(
            $connection,
            'SELECT COUNT(*) FROM bm60_utf8convert WHERE `convert_data`=1 AND `column_added`=1',
        );
        [$doneCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
    ...
    elseif ($do == 'convert') {
        ...
        $result = mysqli_query(
            $connection,
            'SELECT SUM(`rows`) FROM bm60_utf8convert WHERE `convert_data`=1',
        );
        [$allCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
        ...
        $result = mysqli_query(
            $connection,
            'SELECT `table`,`rows`,`done_rows`,`fields_to_convert`,`primary_keys` FROM bm60_utf8convert WHERE `data_converted`=0 AND `convert_data`=1 AND `table`!=\'tcbms_plugin_search_word\' ORDER BY `table` ASC LIMIT 1',
        );
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            $result2 = mysqli_query(
                $connection,
                sprintf(
                    'SELECT %s FROM %s WHERE `__converted`=0 LIMIT 500',
            ...
            while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC)) {
                ...
                foreach ($row2 as $fieldName => $fieldValue) {
                    if (in_array($fieldName, $fieldsToConvert)) {
                        $convertedValue = ConvertEncoding(
                            $fieldValue,
                            'ISO-8859-15',
                            'UTF-8',
                        );
                        ...
                        if ($convertedValue != $fieldValue) {
                            $query .= sprintf(
                                '`%s`=\'%s\', ',
                                $fieldName,
                                SQLEscape($convertedValue, $connection),
                            );
                        }
                    }
                }
                ...
                if (count($primaryKeys) > 0) {
                    ...
                    foreach ($primaryKeys as $fieldName) {
                        $query .= sprintf(
                            '`%s`=\'%s\' AND ',
                            $fieldName,
                            SQLEscape($row2[$fieldName], $connection),
                        );
                    }
                }
            ...
            if ($addCount >= $row['rows']) {
                mysqli_query(
                    $connection,
                    sprintf(
                        'UPDATE bm60_utf8convert SET `data_converted`=1,`done_rows`=%d WHERE `table`=\'%s\'',
                        $addCount,
                ...
                mysqli_query(
                    $connection,
                    sprintf(
                        'UPDATE bm60_utf8convert SET `done_rows`=%d WHERE `table`=\'%s\'',
                        $addCount,
                        SQLEscape($row['table'], $connection),
                    ),
                );
        ...
        $result = mysqli_query(
            $connection,
            'SELECT SUM(`rows`) FROM bm60_utf8convert WHERE `data_converted`=1 AND `convert_data`=1',
        );
        [$doneCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
    ...
    elseif ($do == 'collations') {
        ...
        if ($pos == 0) {
            ...
            $result = mysqli_query(
                $connection,
                'SELECT `table`,`fields_to_convert` FROM bm60_utf8convert WHERE `collation_converted`=0 LIMIT 1',
            );
            while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
                if (trim($row[1]) != '') {
                    ...
                    while ($row2 = mysqli_fetch_array($result2, MYSQLI_NUM)) {
                        ...
                        if (isset($binConvertFields[strtolower($fieldType)])) {
                            $newFieldType = str_replace(
                                $fieldType,
                                $binConvertFields[strtolower($fieldType)],
                                $row2[1],
                            );
                        ...
                        $query = sprintf(
                            'ALTER TABLE %s %s `%s` %s CHARACTER SET utf8 COLLATE utf8_general_ci%s%s%s',
                            $row[0],
                            'MODIFY',
                            $row2[0],
                            $row2[1],
                ...
                mysqli_query(
                    $connection,
                    sprintf(
                        'UPDATE bm60_utf8convert SET `collation_converted`=1 WHERE `table`=\'%s\'',
                        SQLEscape($row[0], $connection),
                    ),
                );
            }
            ...
            $result = mysqli_query(
                $connection,
                'SELECT COUNT(*) FROM bm60_utf8convert WHERE `collation_converted`=1',
            );
            [$doneCount] = mysqli_fetch_array($result, MYSQLI_NUM);
            mysqli_free_result($result);
    ...
    elseif ($do == 'langfiles') {
        ...
        foreach ($langFiles as $file) {
            ...
            $contents = str_replace(
                ...
                $newLangDef .
                    "\n" .
                    '// Converted to UTF-8 by setup/utf8convert.php at ' .
                    date('r'),
                $contents,
            );
    ...
    elseif ($do == 'complete') {
        ...
        $result = mysqli_query(
            $connection,
            'SELECT COUNT(*) FROM bm60_utf8convert WHERE `column_added`=1',
        );
        [$allCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
        ...
        $result = mysqli_query(
            $connection,
            'SELECT `table` FROM bm60_utf8convert WHERE `column_added`=1 AND `column_removed`=0 LIMIT 1',
        );
        while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
            // remove status column
            mysqli_query(
                $connection,
                sprintf('ALTER TABLE %s DROP `__converted`', $row[0]),
            );
            ...
            mysqli_query(
                $connection,
                sprintf(
                    'UPDATE bm60_utf8convert SET `column_removed`=1 WHERE `table`=\'%s\'',
                    SQLEscape($row[0], $connection),
                ),
            );
        }
        ...
        $result = mysqli_query(
            $connection,
            'SELECT COUNT(*) FROM bm60_utf8convert WHERE `column_added`=1 AND `column_removed`=1',
        );
        [$doneCount] = mysqli_fetch_array($result, MYSQLI_NUM);
        mysqli_free_result($result);
        ...
        if ($allCount == $doneCount) {
            mysqli_query(
                $connection,
                'ALTER TABLE bm60_prefs ADD `db_is_utf8` tinyint(4) NOT NULL DEFAULT 0',
            );
            mysqli_query(
                $connection,
                'UPDATE bm60_prefs SET `wartung`=\'no\',`db_is_utf8`=1',
            );
            mysqli_query($connection, 'DROP TABLE IF EXISTS bm60_utf8convert');
