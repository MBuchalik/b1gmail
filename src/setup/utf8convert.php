...
else if($step == STEP_SYSTEMCHECK)
{
	...
	<table class="list">
		...
		<tr>
			...
			<td><img src="../admin/templates/images/<?php if((int)str_replace('.', '', phpversion()) >= 422) echo 'ok'; else { echo 'error'; $nextStep = STEP_SYSTEMCHECK; } ?>.png" border="0" alt="" width="16" height="16" /></td>
		</tr>
		<tr>
...
else if($step == STEP_CONVERT_STEP)
{
	...
	if($do == 'prepare')
	{
		...
		mysqli_query($connection, 'CREATE TABLE bm60_utf8convert('
		...
		foreach($tables as $table)
			mysqli_query($connection, 'INSERT INTO bm60_utf8convert(`table`) VALUES(\'' . SQLEscape($table, $connection) . '\')');
	...
	{
		// get table count
		$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert');
		list($allCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);

		// analyze table
		$result = mysqli_query($connection, 'SELECT `table` FROM bm60_utf8convert WHERE `analyzed`=0 LIMIT 1');
		while($row = mysqli_fetch_array($result, MYSQLI_NUM))
		{
			// get row count
			...
			// save
			mysqli_query($connection, sprintf('UPDATE bm60_utf8convert SET `rows`=%d,`convert_data`=%d,`fields_to_convert`=\'%s\',`primary_keys`=\'%s\',`analyzed`=1 WHERE `table`=\'%s\'',
							$rowCount,
							$convertData ? 1 : 0,
		...
		// get status info
		$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert WHERE `analyzed`=1');
	...
	{
		// get table count
		$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert WHERE `convert_data`=1');
		list($allCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);

		// get table to alter
		$result = mysqli_query($connection, 'SELECT `table` FROM bm60_utf8convert WHERE `column_added`=0 AND `convert_data`=1 LIMIT 1');
		while($row = mysqli_fetch_array($result, MYSQLI_NUM))
		{
			// add status column
			...
			// update status
			mysqli_query($connection, sprintf('UPDATE bm60_utf8convert SET `column_added`=1 WHERE `table`=\'%s\'',
							SQLEscape($row[0], $connection)));
		}
		mysqli_free_result($result);

		// get status info
		$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert WHERE `convert_data`=1 AND `column_added`=1');
		list($doneCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);
	...
	else if($do == 'convert')
	{
		...
		$result = mysqli_query($connection, 'SELECT SUM(`rows`) FROM bm60_utf8convert WHERE `convert_data`=1');
		list($allCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);

		// get table to convert
		$addCount = 0;
		$result = mysqli_query($connection, 'SELECT `table`,`rows`,`done_rows`,`fields_to_convert`,`primary_keys` FROM bm60_utf8convert WHERE `data_converted`=0 AND `convert_data`=1 AND `table`!=\'tcbms_plugin_search_word\' ORDER BY `table` ASC LIMIT 1');
		while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
		{
			...
			{
				mysqli_query($connection, sprintf('UPDATE bm60_utf8convert SET `data_converted`=1,`done_rows`=%d WHERE `table`=\'%s\'',
								$addCount,
			...
			{
				mysqli_query($connection, sprintf('UPDATE bm60_utf8convert SET `done_rows`=%d WHERE `table`=\'%s\'',
								$addCount,
								SQLEscape($row['table'], $connection)));
			}
		}
		...
		// get status info
		$result = mysqli_query($connection, 'SELECT SUM(`rows`) FROM bm60_utf8convert WHERE `data_converted`=1 AND `convert_data`=1');
		list($doneCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);
	...
	else if($do == 'collations')
	{
		...
		else
		{
			...
			// get table to alter
			$result = mysqli_query($connection, 'SELECT `table`,`fields_to_convert` FROM bm60_utf8convert WHERE `collation_converted`=0 LIMIT 1');
			while($row = mysqli_fetch_array($result, MYSQLI_NUM))
			{
				if(trim($row[1]) != '')
				{
				...
				// update status
				mysqli_query($connection, sprintf('UPDATE bm60_utf8convert SET `collation_converted`=1 WHERE `table`=\'%s\'',
								SQLEscape($row[0], $connection)));
			}
			mysqli_free_result($result);

			// get status info
			$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert WHERE `collation_converted`=1');
			list($doneCount) = mysqli_fetch_array($result, MYSQLI_NUM);
			mysqli_free_result($result);
	...
	else if($do == 'resetcache')
	{
		...
		$res = mysqli_query($connection, 'SELECT size,`key` FROM bm60_file_cache', $connection);
	...
	{
		// get table count
		$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert WHERE `column_added`=1');
		list($allCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);

		// get table to alter
		$result = mysqli_query($connection, 'SELECT `table` FROM bm60_utf8convert WHERE `column_added`=1 AND `column_removed`=0 LIMIT 1');
		while($row = mysqli_fetch_array($result, MYSQLI_NUM))
		{
			...
			// update status
			mysqli_query($connection, sprintf('UPDATE bm60_utf8convert SET `column_removed`=1 WHERE `table`=\'%s\'',
							SQLEscape($row[0], $connection)));
		}
		mysqli_free_result($result);

		// get status info
		$result = mysqli_query($connection, 'SELECT COUNT(*) FROM bm60_utf8convert WHERE `column_added`=1 AND `column_removed`=1');
		list($doneCount) = mysqli_fetch_array($result, MYSQLI_NUM);
		mysqli_free_result($result);

		if($allCount == $doneCount)
		{
			mysqli_query($connection, 'ALTER TABLE bm60_prefs ADD `db_is_utf8` tinyint(4) NOT NULL DEFAULT 0');
			mysqli_query($connection, 'UPDATE bm60_prefs SET `wartung`=\'no\',`db_is_utf8`=1');
			mysqli_query($connection, 'DROP TABLE IF EXISTS bm60_utf8convert');
