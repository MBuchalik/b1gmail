...
if ($step == STEP_SELECT_LANGUAGE) {
	...
	<blockquote>
		<input type="radio" id="lang_deutsch" name="lng" value="deutsch"<?php if (
      $lang == 'deutsch'
  ) {
      echo ' checked="checked"';
		...
		<br />
		<input type="radio" id="lang_english" name="lng" value="english"<?php if (
      $lang == 'english'
  ) {
      echo ' checked="checked"';
  } ?> />
 ...
 */ elseif ($step == STEP_SYSTEMCHECK) {
  ...
  ?>
	...
	<?php if ($nextStep != STEP_MYSQL) { ?>
	<br />
	<div style="text-align:center;display:;" id="chmodCommandsNote">
		<a href="#" onclick="document.getElementById('chmodCommandsNote').style.display='none';document.getElementById('chmodCommands').style.display='';"><?php echo $lang_setup[
      'showchmod'
  ]; ?></a>
	</div>
	<div style="display:none;" id="chmodCommands">
		<textarea readonly="readonly" class="installLog" style="height:120px;"><?php foreach (
      $chmodCommands
      as $mode => $files
  ) {
      echo 'chmod ' . $mode;
    ...
    if (
        ...
        'mode_private'
			...
			<fieldset>
				...
				<label for="receive_method_pop3"><?php echo $lang_setup[
        'pop3gateway'
    ]; ?></label>
				<blockquote>
			...
			<fieldset>
				...
				<label for="send_method_phpmail"><?php echo $lang_setup[
        'phpmail'
    ]; ?></label><br />
				...
				<blockquote>
					<label for="sendmail_path"><?php echo $lang_setup[
         'sendmail_path'
     ]; ?></label><br />
			...
			<input type="hidden" name="mysql_host" value="<?php echo htmlentities(
       $_REQUEST['mysql_host'],
   ); ?>" />
			<input type="hidden" name="mysql_user" value="<?php echo htmlentities(
       $_REQUEST['mysql_user'],
   ); ?>" />
			<input type="hidden" name="mysql_pass" value="<?php echo htmlentities(
       $_REQUEST['mysql_pass'],
   ); ?>" />
			<input type="hidden" name="mysql_db" value="<?php echo htmlentities(
       $_REQUEST['mysql_db'],
   ); ?>" />
			<?php
        }
 ...
 */ elseif ($step == STEP_CHECK_EMAIL) {
    if (
        ...
        if (
            $_REQUEST['send_method'] != 'sendmail' ||
            (file_exists($_REQUEST['sendmail_path']) &&
                is_executable($_REQUEST['sendmail_path']))
        ) {
...
example.org</textarea>
				...
				<label for="url"><?php echo $lang_setup['url']; ?></label><br />
					<input id="url" name="url" type="text" value="http://<?php echo $_SERVER[
         'HTTP_HOST'
     ] .
         preg_replace(
             '/\/setup\/index\.php(.*)/',
       ...
       $_REQUEST['receive_method'],
			...
			<input type="hidden" name="send_method" value="<?php echo htmlentities(
       $_REQUEST['send_method'],
   ); ?>" />
			<input type="hidden" name="pop3_host" value="<?php echo htmlentities(
       $_REQUEST['pop3_host'],
   ); ?>" />
			<input type="hidden" name="pop3_user" value="<?php echo htmlentities(
       $_REQUEST['pop3_user'],
   ); ?>" />
			<input type="hidden" name="pop3_pass" value="<?php echo htmlentities(
       $_REQUEST['pop3_pass'],
   ); ?>" />
			<input type="hidden" name="smtp_host" value="<?php echo htmlentities(
       $_REQUEST['smtp_host'],
   ); ?>" />
			<input type="hidden" name="sendmail_path" value="<?php echo htmlentities(
 ...
 */ elseif ($step == STEP_INSTALL) {
        ...
        if (mysqli_select_db($connection, $_REQUEST['mysql_db'])) {
            ...
            foreach ($result as $query => $queryResult) {
                if ($queryResult !== true) {
                    ...
                    echo 'Failed to execute structure query: ' .
                        $queryResult .
                        "\n";
                }
            }
            ...
            $dataFolder = preg_replace(
                '/\/setup\/index\.php(.*)/',
                '/data/',
                str_replace('\\', '/', $_SERVER['SCRIPT_FILENAME']),
            );
            $selfFolder = preg_replace(
                '/\/setup\/index\.php(.*)/',
                '/',
                str_replace('\\', '/', $_SERVER['SCRIPT_FILENAME']),
            );
            ...
            $groupQuery = sprintf(
                'INSERT INTO bm60_gruppen(id,titel,ftsearch) VALUES(1,\'%s\',\'%s\')',
            ...
            if (mysqli_query($connection, $groupQuery)) {
                ...
                echo 'Failed to create default group: ' .
                    mysqli_error($connection) .
                    "\n";
            }
            ...
            foreach ($domains as $domain) {
                $domainQuery = sprintf(
                    'INSERT INTO bm60_domains(`domain`,`pos`) VALUES(\'%s\',%d)',
            ...
            // create postmaster
            $salt = GeneratePW();
            $postmasterQuery = sprintf(
                'INSERT INTO bm60_users(id,email,vorname,nachname,passwort,passwort_salt,gruppe,preview,plaintext_courier,soforthtml) ' .
            ...
            if (mysqli_query($connection, $postmasterQuery)) {
                ...
                echo 'Failed to create postmaster: ' .
                    mysqli_error($connection) .
                    "\n";
            }
            ...
            foreach ($domains as $domain) {
                if ($domain != $firstDomain) {
                    $aliasQuery = sprintf(
                        'INSERT INTO bm60_aliase(email,user) VALUES(\'%s\',1)',
                        SQLEscape(
                            'postmaster@' . EncodeDomain($domain),
                            $connection,
                        ),
                    );
            ...
            foreach ($exampleData as $query) {
                if (!mysqli_query($connection, $query)) {
                    echo 'Failed to execute example data query: ' .
                        mysqli_error($connection) .
                        "\n";
 ...
 if ($installLog != '') {
     echo $lang_setup['log_text']; ?>
		<textarea readonly="readonly" class="installLog"><?php echo htmlentities(
      $installLog,
  ); ?></textarea>
		<br /><br />
		<?php
 ...
 ?>
	<blockquote>
		...
		<a target="_blank" href="<?php echo $_REQUEST['url']; ?>"><?php echo $_REQUEST[
    'url'
]; ?></a><br /><br />

		<b><?php echo $lang_setup['adminlogin']; ?></b><br />
		<a target="_blank" href="<?php echo $_REQUEST[
      'url'
  ]; ?>admin/"><?php echo $_REQUEST['url']; ?>admin/</a><br /><br />
