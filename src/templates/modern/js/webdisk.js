...
function initWDSel() {
  ...
  sel.cbItemDragStart = function (element, event) {
    ...
    if (this.sel.length > 1) {
      ...
      if (itemID[0] == 2) dragImg.src = tplDir + 'images/li/drag_wdfile.png';
      else dragImg.src = tplDir + 'images/li/drag_wdfolder.png';
    }
  ...
  sel.cbItemDoubleClick = function (element) {
    ...
    if (itemID[0] == 1) switchWebdiskFolder(itemID[1]);
    else
      document.location.href =
        'webdisk.php?action=downloadFile&id=' +
        itemID[1] +
        '&sid=' +
        currentSID;
  };
    ...
    if (accelKey) {
      switch (e.keyCode) {
        case 65: // a
          ...
          else _wdSel.selectAll();
          return false;

        case 67: // c
          if (currentID > 0) webdiskClipboardAction('copy');
          return false;
        ...
        case 85: // u
          if (EBID('fileCount').value <= 1) EBID('fileCount').value = 5;
        ...
        case 88: // x
          if (currentID > 0) webdiskClipboardAction('cut');
          return false;
      }
    } else {
      switch (e.keyCode) {
...
function showWebdiskItemDetails(type, id) {
  ...
  MakeXMLRequest(
    'webdisk.php?action=itemInfo&type=' +
      escape(type) +
      '&id=' +
      escape(id) +
      '&sid=' +
    ...
    function (e) {
      if (e.readyState == 4) {
        if (_requestTimeWDFolder != currentWebdiskFolderID) return;
        if (e.responseXML) {
          var x = e.responseXML;
          var title = x
          ...
          var shortTitle = x
            .getElementsByTagName('shortTitle')
            .item(0)
            .childNodes.item(0).nodeValue;
          var size = x
          ...
          var created = x
            ...
            .childNodes.item(0).nodeValue;
          var ext = x
            .getElementsByTagName('ext')
            .item(0)
            .childNodes.item(0).nodeValue;
          var share =
            x.getElementsByTagName('share').item(0).childNodes.item(0)
              .nodeValue == '1';
          var viewable =
            x.getElementsByTagName('viewable').item(0).childNodes.item(0)
              .nodeValue == '1';
...
function selectedWebdiskCountChanged(no) {
  ...
  EBID('wdExt-multiple').style.display = '';
  EBID('wdExt-file').style.display = 'none';
  EBID('wdExt-folder').style.display = 'none';
...
function transferSelectedWebdiskItems() {
  var f = EBID('selectedWebdiskItems'),
    i;
...
function webdiskShowUploadForm() {
  MakeXMLRequest(
    'webdisk.php?inline=true&do=uploadFilesForm&fileCount=' +
      escape(EBID('fileCount').value) +
      '&folder=' +
...
function switchWebdiskFolder(folderID) {
  ...
  MakeXMLRequest(
    'webdisk.php?inline=true&folder=' + folderID + '&sid=' + currentSID,
    function (e) {
      if (e.readyState == 4) {
        webdiskClearInfo();
...
function updateWebdiskViewMode(c, folder, sid) {
  var fld = '';
  if (currentWebdiskFolderID > -1) fld = '&folder=' + currentWebdiskFolderID;
  else if (folder != '') fld = '&folder=' + folder;
  document.location.href =
    'webdisk.php?sid=' + sid + fld + '&do=changeViewMode&viewmode=' + c.value;
}
...
function webdiskShowInfo(
  ...
  EBID('wdExt-multiple').style.display = 'none';
  if (type == 1) {
    EBID('wdExt-folder').style.display = '';
    EBID('wdExt-file').style.display = 'none';
  } else {
    EBID('wdExt-folder').style.display = 'none';
    EBID('wdExt-file').style.display = '';
  }
  ...
  EBID('webdiskDetailFileActionsView').style.display =
    type == 2 && viewable ? '' : 'none';

  // zip
  EBID('webdiskDetailZIPActions').style.display =
    type == 2 && ext == 'zip' ? '' : 'none';
...
function webdiskClipboardAction(action) {
  if (_wdSel.sel.length < 1) return;

  transferSelectedWebdiskItems();
  var itemStr = EBID('selectedWebdiskItems').value;

  MakeXMLRequest(
    'webdisk.php?action=clipboardAction&do=' +
      action +
      '&items=' +
      escape(itemStr) +
      '&sid=' +
...
function webdiskDoRename(newName, id, type) {
  ...
  currentType = type;
  MakeXMLRequest(
    'webdisk.php?action=renameItem&folder=' +
      currentFolder +
      '&type=' +
      type +
      '&id=' +
...
function webdiskRename(folder, id, type, title) {
  ...
  span.innerHTML =
    '<input type="text" style="text-align:center;width:100%;" name="newName" id="tNewName" value="' +
    title +
    '" size="16" onkeypress="if(event.keyCode == 13) ' +
    call +
    '" onblur="' +
    call +
    '" />';
...
function webdiskCreateFolder() {
  ...
  MakeXMLRequest(
    'webdisk.php?action=createFolder&rpc=true&folder=' +
      currentWebdiskFolderID +
      '&folderName=' +
...
function reloadWebdiskFolderList() {
  ...
  MakeXMLRequest(
    'webdisk.php?action=getFolderList&sid=' + currentSID,
    function (http) {
      if (
        http.readyState == 4 &&
        http.responseText.length > 10 &&
        http.responseText.indexOf('var') >= 0
      ) {
...
function moveWebdiskItems(items, destFolder) {
  ...
  MakeXMLRequest(
    'webdisk.php?action=moveItems&items=' +
      escape(items) +
      '&destFolderID=' +
      destFolder +
      '&sid=' +
    ...
    function (http) {
      if (http.readyState == 4) {
        switchWebdiskFolder(currentWebdiskFolderID);
        if (http.responseText.indexOf(',ReloadFolderList') != -1)
          reloadWebdiskFolderList();
      }
    },
  );
...
function enableWebdiskDragTargets() {
  var dragEnter = function (event) {
    if (
      (event.dataTransfer.types.contains &&
        !event.dataTransfer.types.contains('wditems')) ||
      (event.dataTransfer.types.indexOf &&
        event.dataTransfer.types.indexOf('wditems') < 0)
    )
      return;
  };
  ...
  var dragDrop = function (event) {
    ...
    if (!event.dataTransfer.getData('wditems')) return;
