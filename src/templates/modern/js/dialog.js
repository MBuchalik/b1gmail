...
function _createFolder(e) {
  if (e.readyState == 4) {
    ...
    if (responseCode != 0) changePath(responseCode);
    else alert(lang['foldererror']);
  }
}
...
function changeFile(id, path, title) {
  ...
  if (lastFile != -1) EBID('file_' + lastFile).className = 'contentItem';
  lastFile = id;
  lastPath = path;
}

function closeOpenDialog(field, param) {
  parent.overlayDocument().document.getElementById(field).value =
    EBID('filename').value;
  parent.overlayDocument().document.getElementById(field + '_id').value =
    lastFile;
  parent.hideOverlay();
}
...
function selectAddressItem(item) {
  if (lastAddress) lastAddress.className = 'addressItem';
...
function fillAddressField(f, data, o) {
  ...
  if (field.hasChildNodes()) {
    var node;
    while ((node = field.firstChild)) field.removeChild(node);
  }
...
function exchangeAddress(a) {
  ...
  if (current == Addr[id][1]) current = Addr[id][2];
  else current = Addr[id][1];
  a.innerHTML = current;
}
...
function addAddressField(data, field, i, o) {
  ...
  else image.setAttribute('src', tplDir + 'images/li/addr_priv.png');
...
function deleteAddress(img) {
  ...
  if (field == 'to') toAddr[id] = null;
  else if (field == 'cc') ccAddr[id] = null;
  else if (field == 'bcc') bccAddr[id] = null;
  else if (field == 'attendees') attAddr[id] = null;
}
...
function generateAttendeeString(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var addr = addresses[i];
    if (addr != null) ret += ';' + addr[0] + ',' + addr[2] + ',' + addr[1];
  }
...
function submitAddressDialog(mode) {
  if (mode != 'handy') {
    ...
    parent.document.getElementById('bcc').value =
      generateAddressString(bccAddr);

    if (
      parent.document.getElementById('bcc').value.length > 0 &&
...
function submitNumberDialog() {
  if (lastAddress) {
    ...
    else if (Addr[elem][1] != ' - ') number = Addr[elem][1];
    else number = Addr[elem][2];
    ...
    if (toPre && toNo) {
      for (var i = 0; i < toPre.options.length; i++) {
        if (
          number.length > toPre.options[i].text.length &&
          number.substr(0, toPre.options[i].text.length) ==
            toPre.options[i].text
        ) {
...
function submitAttendeeDialog() {
  parent.document.getElementById('attendees').value =
    generateAttendeeString(attAddr);
