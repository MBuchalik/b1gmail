...
$(document).ready(function () {
  ...
  $(document.body).on('click', '.domainMenu li', function (e) {
    ...
    if (
      typeof checkEMailAvailability != 'undefined' &&
  ...
  function loginFunc(event) {
    var $form = $(event.currentTarget).closest('form'),
      formData = $form.serialize() + '&ajax=true',
      $alert = $form.find('.alert');

    $alert.css('display', 'none');

    if ($form.data('realSubmit') || $alert.length == 0) return true;
  ...
  $(document)
    ...
    .each(function (index, obj) {
      if (obj.name == 'timezone') obj.value = clientTZ;
    });
...
function updateFormSSL(elem) {
  var ssl = elem.checked,
    $form = $(elem).closest('form');

  if (ssl) $form.attr('action', sslURL + 'index.php?action=login');
  else $form.attr('action', 'index.php?action=login');
}
...
function markFieldAsInvalid(name) {
  ...
  var $group = $field.closest('[class="form-group"]');
