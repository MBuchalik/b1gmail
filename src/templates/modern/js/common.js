...
function setCookie(name, value) {
  ...
  document.cookie =
    name + '=' + escape(value) + ';expires=' + expires.toUTCString();
}
...
function addEvent(elem, event, handler) {
  ...
  else if (document.attachEvent) elem.attachEvent('on' + event, handler);
}

function removeEvent(elem, event, handler) {
  if (document.removeEventListener) elem.removeEventListener(event, handler);
  else if (document.detachEvent) elem.detachEvent('on' + event, handler);
}
...
function documentLoader() {
  if (
    EBID('mainBanner') &&
    EBID('mainContent') &&
    trim(EBID('mainBanner').innerHTML).length > 1
  ) {
  ...
  addEvent(window, 'dragover', function (event) {
    event.preventDefault();
  });
  addEvent(window, 'drop', function (event) {
    event.preventDefault();
  });
...
function ftsBGIndex() {
  if (
    typeof ftsBGIndexing != 'undefined' &&
    typeof currentSID != 'undefined' &&
    ftsBGIndexing
  ) {
    MakeXMLRequest(
      'search.php?action=ftsBGIndexing&sid=' + currentSID,
      function (e) {
        if (e.readyState == 4) {
          if (e.responseText == '0') window.setTimeout(ftsBGIndex, 1000);
          else if (e.responseText == '2') window.setTimeout(ftsBGIndex, 15000);
        }
      },
    );
  }
...
function getDocumentMetrics(m) {
  ...
  if (self.pageYOffset) rScrollY = self.pageYOffset;
  else rScrollY = docElement.scrollTop;
...
function Shorten(s, l) {
  if (s.length > l - 3) s = s.substr(0, l - 3) + '...';
  return s;
}
...
function DateFormat(format, d) {
  ...
  for (var i = 0; i < format.length; i++) {
    ...
    if (c == 'd') {
      ...
      if (d.getMonth() + 1 < 10) res += '0' + (d.getMonth() + 1);
...
function prepareMailTextHTML(s, secure) {
  ...
  s = s.replace(
    /mailto\:([^\n\r\\\"\'\> ]+)/gi,
    "javascript:parent.composeMail('$1');",
  );
  return s;
}
...
function MakeXMLRequest(url, callback, param, cClose) {
  ...
  if (!xmlHTTP) {
    ...
    if (cClose) xmlHTTP.setRequestHeader('Connection', 'close');
...
function isNumeric(str) {
  ...
  for (var i = 0; i < str.length; i++)
    if (charset.indexOf(str.substr(i, 1), 0) == -1) return false;
  return true;
}
...
function passwordSecurity(pw, div) {
  ...
  for (var i = 0; i < pw.length; i++) {
    ...
    if (isNumeric(c)) numbers++;
    else if (isAlphabetic(c)) alpha++;
    else others++;
    ...
    for (var j = i; j < pw.length; j++) {
      var d = pw.substr(j, 1);
      if (d == c && j != i) unique = false;
    }

    if (unique) differentChars++;
  }

  pwLength = differentChars;
  value = (pwLength / 8) * 100;
  if (numbers == pwLength) value *= 0.5;
  value += others * 18;

  if (pwLength < 4) value = 0;
...
function markFieldAsInvalid(f) {
  ...
  if (EBID(f)) {
    ...
    if (inputs.length == 1) field = inputs[0];
  }

  if (field) field.className = 'invalidField';
}

function advancedOptions(field, dir1, dir2, tpldir) {
  var body = EBID('advanced_' + field + '_body'),
    body2 = EBID('advanced_' + field + '_body2');
...
function _checkAddressAvailability(xmlHTTP) {
  if (xmlHTTP.readyState == 4) {
    ...
    if (tag.length > 0 && tag.item(0).childNodes.length > 0) {
...
function checkAddressAvailability() {
  ...
  MakeXMLRequest(
    'index.php?action=checkAddressAvailability&address=' + encodeURI(address),
    _checkAddressAvailability,
  );
}

function getTZOffset() {
  if (!serverTZ) return clientTZ;
  else return clientTZ - serverTZ;
}
