...
function hideDraftNote(setNoNotify, draftID) {
  ...
  if (setNoNotify) {
    if (EBID('deleteDraft') && EBID('deleteDraft').checked)
      MakeXMLRequest(
        'email.compose.php?action=deleteDraft&id=' +
          draftID +
          '&sid=' +
          currentSID,
        false,
      ...
      MakeXMLRequest(
        'email.compose.php?action=setNoDraftNotify&sid=' + currentSID,
        false,
      );
  }
}
...
function beginDraftAutoSave() {
  ...
  editor.submit();
  if (EBID('subject').length == 0 && EBID('emailText').length == 0) return;
    ...
    xh.onreadystatechange = function () {
      if (xh.readyState == 4) {
        ...
        if (draftID > 0) {
          ...
          EBID('autoSaveNote').innerHTML =
            '<small><i class="fa fa-floppy-o"></i> ' +
            lang['lastsavedat']
              .replace('%1', ('0' + saveDate.getHours()).slice(-2))
...
function readMailHideBottomLayers() {
  ...
  EBID('mailReadScrollContainer').setAttribute(
    'class',
    'scrollContainer withBottomBar',
  );
}
...
function readMailShowBottomLayer(name) {
  ...
  if (!justClose) {
    EBID('mailReadScrollContainer').setAttribute(
      'class',
      'scrollContainer withBottomBarAndLayer',
    );
...
function switchFolder(folderID, retainPreviewMail) {
  if (EBID('folderMailArea')) {
    if (!retainPreviewMail) {
      ...
      if (EBID('previewArea')) EBID('previewArea').innerHTML = '';
    }
    if (EBID('folderLoading')) EBID('folderLoading').style.display = '';
    MakeXMLRequest(
      'email.php?inline=true&folder=' +
        folderID +
        (typeof folderNarrowView != 'undefined' && folderNarrowView
          ? '&narrow=true'
          : '') +
        '&sid=' +
        currentSID,
    ...
    document.location.href =
      'email.php?folder=' + folderID + '&sid=' + currentSID;
  }
}
...
function initMailSel() {
  ...
  sel.cbRowFilter = function (element) {
    return (
      element.id.length > 9 &&
      element.id.substr(0, 5) == 'mail_' &&
      element.id.substr(element.id.length - 4) == '_ntr'
    );
  };
  ...
  sel.cbItemDoubleClick = function (element) {
    if (currentFolderID == -3)
      ...
      document.location.href =
        'email.read.php?id=' + this.getItemID(element) + '&sid=' + currentSID;
  };
...
function composeSizer(justFire) {
  var or = function () {
    ...
    if (editor) editor.setHeight(height);

    if (typeof IE7 != 'undefined') IE7.recalc();
  };

  if (!justFire) addEvent(window, 'resize', or);
...
function readSizer() {
  ...
  height -=
    getElementMetrics(EBID('textP'), 'h') - getElementMetrics(textArea, 'h');
  height -= 50;
...
function folderViewOptions(folderID) {
  openOverlay(
    'email.php?sid=' + currentSID + '&do=viewOptions&folder=' + folderID,
    lang['viewoptions'],
    650,
    140,
    true,
...
function registerFolderHotkeyHandler() {
  window.onkeydown = function (e) {
    ...
    if (accelKey) {
      switch (e.keyCode) {
        case 65: // a
          if (EBID('allChecker')) EBID('allChecker').click();
          else _mailSel.selectAll();
          return false;
...
function reloadFolderList(data) {
  ...
  MakeXMLRequest(
    'email.php?action=getFolderList&sid=' + currentSID,
    function (http) {
...
function showMailSource(id) {
  openOverlay(
    'email.read.php?action=showSource&id=' + id + '&sid=' + currentSID,
    lang['source'],
    500,
    460,
    true,
...
function showAttachedZIP(mailID, attID, title) {
  openOverlay(
    'email.read.php?action=attachedZIP&id=' +
      mailID +
      '&attachment=' +
      attID +
      '&sid=' +
...
function moveMail(id) {
  openOverlay(
    'email.read.php?action=move&id=' + id + '&sid=' + currentSID,
    lang['movemail'],
    450,
    380,
    true,
...
function updateGroupMode(c, fs, sid) {
  document.location.href =
    ...
    '&' +
    fs +
    '&do=changeGroupMode&groupmode=' +
    c.value;
}

function initEMailTextArea(code) {
  const iframe = EBID('textArea');
  let lastSize = 0;
  ...
  function resizeCallback() {
    ...
    window.setTimeout(resizeCallback, 500);
  }
  ...
  function initializeIFrame() {
    ...
    if (iframeDoc.location.href == 'about:blank') {
      removeEvent(iframe, 'load', initializeIFrame);

      const html = iframeDoc.getElementsByTagName('html');
    ...
    addEvent(iframeDoc, 'mousemove', handleMouseMove);
    addEvent(iframeDoc, 'click', handleMouseClick);
    resizeCallback();
  }
}
...
function togglePreviewPane(mailID, tpldir, sid) {
  if (mailID != -1 && _lastSelectedMailID != mailID) {
    ...
    MakeXMLRequest(
      'email.read.php?preview=true&id=' +
        mailID +
        (typeof folderNarrowView != 'undefined' && folderNarrowView
          ? '&narrow=true'
          : '') +
    ...
    if (
      (EBID('mail_' + mailID + '_nspan1') &&
        EBID('mail_' + mailID + '_nspan1').className.indexOf('unread') >= 0) ||
      (EBID('mail_' + mailID + '_span1') &&
        EBID('mail_' + mailID + '_span1').className.indexOf('unread') >= 0)
    )
...
function enableFolderDragTargets() {
  var dragEnter = function (event) {
    if (
      (event.dataTransfer.types.contains &&
        !event.dataTransfer.types.contains('emails')) ||
      (event.dataTransfer.types.indexOf &&
        event.dataTransfer.types.indexOf('emails') < 0)
    )
      return;
  };
  ...
  var dragDrop = function (event) {
    ...
    if (!event.dataTransfer.getData('emails')) return;
  ...
  for (var i = 0; i < treeDIVs.length; i++) {
    ...
    if (imgs.length == 1 && imgs[0].src.indexOf('intellifolder') != -1)
      continue;
...
function hideMailMenu(e) {
  ...
  var mailMenu = EBID('mailMenu'),
    multiMailMenu = EBID('multiMailMenu');
...
function showMailMenu(e, posElem, multi) {
  ...
  if (typeof posElem == 'undefined' || posElem == null) {
    var oX = getElementMetrics(mailMenu, 'w'),
      oY = 0;
    ...
    mailMenu.style.top =
      min(
        getElementMetrics(mailMenu.parentNode, 'h') -
          getElementMetrics(mailMenu, 'h') -
          20,
        e.clientY + getPageYOffset() - offsetY - oY,
      ) + 'px';
  } else {
    mailMenu.style.left =
      getElementMetrics(posElem, 'x') +
      getElementMetrics(posElem, 'w') +
      getPageXOffset() -
      offsetX -
      getElementMetrics(mailMenu, 'w') +
      'px';
    mailMenu.style.top =
      getElementMetrics(posElem, 'y') +
      getElementMetrics(posElem, 'h') +
      getPageYOffset() -
      offsetY +
      'px';
  }
  ...
  if (!multi) {
    ...
    for (var i = 1; i <= 4; i++) {
      if (
        EBID('mail_' + currentID + '_col' + i) &&
        EBID('mail_' + currentID + '_col' + i).className.substring(0, 10) ==
          'mailColor_'
      ) {
        color = EBID('mail_' + currentID + '_col' + i).className.substring(
          10,
          11,
        );
        break;
    ...
    for (var i = 0; i <= 6; i++)
      EBID('mailColorButton_' + i).className =
        'mailColorButton_' + i + (i == color ? '_a' : '');
  }
}
...
function folderFlagMail(id, flag, value) {
  ...
  if (flag == 1) add = '&getFolderList=true';
  ...
  var idsParam = '';
  for (var i = 0; i < id.length; ++i) idsParam += '&ids[' + i + ']=' + id[i];

  MakeXMLRequest(
    'email.php?action=flagMessage&flag=' +
      flag +
      add +
      '&value=' +
      (value ? 1 : 0) +
      ...
      if (http.readyState == 4) {
        ...
        for (var i = 0; i < id.length; ++i) {
          ...
          if (flag == 1) {
            ...
            if (span1 && span2) {
              span1.className = span2.className = unread
                ? 'unreadMail'
                : 'readMail';
            } else {
              ...
              if (span1 && span2) {
                ...
                if (typeof icon.src != 'undefined') {
                  ...
                  else icon.src = icon.src.replace('markunread', 'markread');
                } else {
                  icon.className = unread
                    ? 'fa fa-envelope'
                    : 'fa fa-envelope-o';
                }
              }
            ...
            if (typeof img.src != 'undefined') {
              img.src = img.src.replace(
                /mailico(.*)/gi,
                results[i] == '1' ? 'mailico_flagged.png' : 'mailico_empty.gif',
              );
              if (narrowMode)
            ...
            if (typeof img.src != 'undefined')
              img.style.display = results[i] == '1' ? '' : 'none';
            else img.className = results[i] == '1' ? 'fa fa-check' : '';
          }
        }
...
function deleteMail(id) {
  ...
  MakeXMLRequest(
    'email.php?do=deleteMail&rpc=true&getFolderList=true&' +
      idsParam +
      '&sid=' +
      currentSID,
    function (http) {
...
function moveMails(mails, destFolder) {
  ...
  MakeXMLRequest(
    'email.php?action=moveMails&getFolderList=true&mails=' +
      escape(mails) +
      '&destFolderID=' +
      destFolder +
      '&sid=' +
...
function _folderColorMail(http) {
  if (http.readyState == 4) {
    ...
    if (EBID('mail_' + id + '_col2'))
      EBID('mail_' + id + '_col2').className =
        currentSortColumn == 'von' && color == 0
          ? 'listTableTDActive'
          : color == 0
          ? ''
          : 'mailColor_' + color;
    ...
    if (EBID('mail_' + id + '_col3'))
      EBID('mail_' + id + '_col3').className =
        currentSortColumn == 'betreff' && color == 0
          ? 'listTableTDActive'
          : color == 0
          ? ''
          : 'mailColor_' + color;
    ...
    if (EBID('mail_' + id + '_col4'))
      EBID('mail_' + id + '_col4').className =
        currentSortColumn == 'fetched' && color == 0
          ? 'listTableTDActive'
          : color == 0
          ? ''
          : 'mailColor_' + color;
    ...
    if (EBID('mail_' + id + '_ncol1'))
      EBID('mail_' + id + '_ncol1').className =
        'narrowRow ' + (color == 0 ? '' : 'mailColor_' + color);
  }
}
function folderColorMail(id, color) {
  MakeXMLRequest(
    'email.php?action=colorMessage&color=' +
      color +
      '&id=' +
      id +
      '&sid=' +
...
function mailReply(mailID, all) {
  ...
  if (typeof sText == 'object') sText = sText.toString();
...
function sendMailConfirmation(id) {
  MakeXMLRequest(
    'email.read.php?action=sendConfirmation&id=' + id + '&sid=' + currentSID,
    false,
  );
...
function setMailSpamStatus(id, spam, interactive) {
  ...
  MakeXMLRequest(
    'email.read.php?action=setSpamStatus&spam=' +
      (spam ? 'true' : 'false') +
      idsParam +
      '&sid=' +
      currentSID,
    function (http) {
      if (
        interactive &&
        typeof currentPageNo != 'undefined' &&
        http.readyState == 4
      )
        switchPage(currentPageNo);
    },
...
function checkFolderRefresh() {
  ...
  MakeXMLRequest(
    'email.php?action=getRecentMailCount&folder=' +
      refreshFolder +
      '&sid=' +
      currentSID,
    _checkFolderRefresh,
...
function printMail(id, sid) {
  ...
  if (
    parent.frames &&
    parent.frames.mailFrame &&
    parent.frames.mailFrame.location
  )
    enableExternal =
      parent.frames.mailFrame.location.href.indexOf('enableExternal') != -1;
  else if (
    parent.frames &&
    parent.frames.textArea &&
    parent.frames.textArea.location
  )
    enableExternal =
      parent.frames.textArea.location.href.indexOf('enableExternal') != -1;

  openWindow(
    'email.read.php?id=' +
      id +
      '&print=true' +
      (enableExternal ? '&enableExternal=true' : '') +
      '&sid=' +
      sid,
    'print' + id,
    560,
...
function updateFolderSubscription(id, obj, sid) {
  MakeXMLRequest(
    'email.folders.php?action=setFolderSubscription&id=' +
      id +
      '&subscribe=' +
      (obj.checked ? '1' : '0') +
      '&sid=' +
      sid,
    function (e) {
      if (e.readyState == 4) reloadFolderList();
    },
  );
}
...
function placeSignature(id) {
  MakeXMLRequest(
    'email.compose.php?action=getSignature&id=' +
      escape(id) +
      '&mode=' +
      escape(editor.mode) +
      '&sid=' +
...
function addAttachment(sid) {
  openOverlay(
    'email.compose.php?sid=' +
      sid +
      '&action=addAttachment&attList=' +
      escape(EBID('attachments').value),
    lang['addattach'],
    520,
    150,
    true,
...
function generateAttachmentList() {
  ...
  // clear
  var node;
  while ((node = attachments.firstChild)) attachments.removeChild(node);
  ...
  for (var i = 0; i < files.length; i++) {
    if (files[i] != '') {
      ...
      link.target = '_blank';
      link.href =
        'email.compose.php?action=getAttachment&id=' +
        attID +
        '&type=' +
        escape(attType) +
        '&name=' +
  ...
  if (
    fileCount > 0 &&
    getElementMetrics(firstElem, 'y') != getElementMetrics(lastElem, 'y')
  )
    attachments.style.height = '54px';
  else attachments.style.height = '';
...
function deleteAttachment(id) {
  ...
  for (var i = 0; i < files.length; i++) {
    if (files[i] != '') {
      var params = files[i].split(',');
      if (params[0] != id) newFiles += ';' + files[i];
    }
  }

  // delete on server
  MakeXMLRequest(
    'email.compose.php?action=deleteAttachment&sid=' + currentSID + '&id=' + id,
    null,
  );
...
function checkComposeForm(form, attCheck, attKeywords) {
  ...
  if (form.elements['subject'].value.length < 1) {
    if (!confirm(lang['sendwosubject'])) return false;
  }
  ...
  if (attCheck && EBID('attachments').value.length < 3) {
    ...
    var showWarning = false,
      msgText = editor.getPlainText().toLowerCase();
...
function submitComposeForm() {
  if (EBID('composeLoading')) EBID('composeLoading').style.display = '';
  document.forms.f1.submit();
}
