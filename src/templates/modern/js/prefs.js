...
function showSignature(id) {
  ...
  for (var i = 0; i < elems.length; ++i) {
    if (elems[i].dataset.sigId == id) elems[i].className = 'sel';
    else elems[i].className = '';
  }
  ...
  MakeXMLRequest(
    'prefs.php?action=signatures&do=edit&id=' + id + '&sid=' + currentSID,
    function (http) {
...
function addSignature() {
  EBID('sigItem').innerHTML = '';
  MakeXMLRequest(
    'prefs.php?action=signatures&do=add&sid=' + currentSID,
    function (http) {
      if (http.readyState == 4 && http.responseText) {
        EBID('sigItem').innerHTML = http.responseText;
        EBID('removeButton').disabled = true;
