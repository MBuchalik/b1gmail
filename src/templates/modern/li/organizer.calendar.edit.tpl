<div id="contentHeader">
	<div class="left">
		...
		{if isset($eDate)}{lng p="editdate"}{else}{lng p="adddate"}{/if}
	</div>
</div>

<div class="scrollContainer"><div class="pad">

<form name="f2" method="post" action="organizer.calendar.php?action={if isset($eDate)}saveDate&id={$eDate.id}{if isset($smarty.get.jumpbackDate)}&jumpbackDate={text value=$smarty.get.jumpbackDate allowEmpty=true}{/if}{else}createDate{/if}&sid={$sid}" onsubmit="return(checkCalendarDateForm(this));">
	<table class="listTable">
		<tr>
			<th class="listTableHead" colspan="2"> {if isset($eDate)}{lng p="editdate"}{else}{lng p="adddate"}{/if}</th>
		</tr>
		...
		<tr>
			...
			<td class="listTableRight">
				<input type="text" name="title" id="title" value="{text value=$eDate.title|default:'' allowEmpty=true}" size="34" style="width:100%;" />
			</td>
		</tr>
		<tr>
			<td class="listTableLeft"><label for="location">{lng p="location"}:</label></td>
			<td class="listTableRight">
				<input type="text" name="location" id="location" value="{text value=$eDate.location|default:'' allowEmpty=true}" size="34" style="width:60%;" />
			</td>
		</tr>
		<tr>
			<td class="listTableLeft"><label for="text">{lng p="text"}:</label></td>
			<td class="listTableRight">
				<textarea style="width:100%;height:100px;" name="text" id="text">{text value=$eDate.text|default:'' allowEmpty=true}</textarea>
			</td>
		</tr>
		...
		<tr>
			...
			<td class="listTableRight">
				<table>
					<tr>
						<td><input type="radio" id="wholeDay_0" name="wholeDay" value="0"{if !isset($eDate) || !($eDate.flags&1)} checked="checked"{/if} /></td>
						<td>
							<input type="text" onfocus="EBID('wholeDay_0').checked=true;" name="durationHours" id="durationHours" value="{$durationHours|default:''}" size="3" />
							{lng p="hours"},
							<input type="text" onfocus="EBID('wholeDay_0').checked=true;" name="durationMinutes" id="durationMinutes" value="{$durationMinutes|default:''}" size="3" />
							{lng p="minutes"}
						</td>
					</tr>
					<tr>
						<td><input type="radio" id="wholeDay_1" name="wholeDay" value="1"{if isset($eDate) && $eDate.flags&1} checked="checked"{/if} /></td>
		...
		<tr>
			...
			<td class="listTableRight">
				<input type="checkbox" name="repeating" id="repeating"{if isset($eDate) && $eDate.repeating} checked="checked"{/if} onclick="toggleRepeatingDiv(this)" />
				<label for="repeating">{lng p="repeating"}</label>
			</td>
		</tr>
		<tbody id="repeatingDiv" style="display:{if !isset($eDate) || !$eDate.repeating}none{/if};">
		<tr>
			<td class="listTableLeft"><label for="repeatCount">{lng p="repeatcount"}:</label></td>
			<td class="listTableRight">
				<table>
					<tr>
						<td><input type="radio" name="repeat_until" id="repeat_until_endless" value="endless"{if !isset($eDate) || $eDate.repeat_flags&1} checked="checked"{/if} /></td>
						<td><label for="repeat_until_endless">{lng p="endless"}</label></td>
					</tr>
					<tr>
						<td><input type="radio" name="repeat_until" id="repeat_until_count" value="count"{if isset($eDate) && $eDate.repeat_flags&2} checked="checked"{/if} /></td>
					...
					<tr>
						<td><input type="radio" name="repeat_until" id="repeat_until_date" value="date"{if isset($eDate) && $eDate.repeat_flags&4} checked="checked"{/if} /></td>
		...
		<tr>
			...
			<td class="listTableRight">
				<table>
					<tr>
						<td valign="top"><input type="radio" name="repeat_interval" id="repeat_interval_daily" value="daily"{if !isset($eDate) || $eDate.repeat_flags&8} checked="checked"{/if} /></td>
					...
					<tr>
						<td valign="top"><input type="radio" name="repeat_interval" id="repeat_interval_weekly" value="weekly"{if isset($eDate) && $eDate.repeat_flags&16} checked="checked"{/if} /></td>
					...
					<tr>
						<td valign="top"><input type="radio" name="repeat_interval" id="repeat_interval_monthly_mday" value="monthly_mday"{if isset($eDate) && $eDate.repeat_flags&32} checked="checked"{/if} /></td>
					...
					<tr>
						<td valign="top"><input type="radio" name="repeat_interval" id="repeat_interval_monthly_wday" value="monthly_wday"{if isset($eDate) && $eDate.repeat_flags&64} checked="checked"{/if} /></td>
					...
					<tr>
						<td valign="top"><input type="radio" name="repeat_interval" id="repeat_interval_yearly" value="yearly"{if isset($eDate) && $eDate.repeat_flags&128} checked="checked"{/if} /></td>
		...
		<tr>
			...
			<td class="listTableRight">
				...
				{foreach from=$groups item=group key=groupID}
					<option value="{$groupID}"{if (!isset($eDate)&&$groupID==-1) || (isset($eDate)&&$eDate.group==$groupID)} selected="selected"{/if}>{text value=$group.title}</option>
				{/foreach}
				</select>
			</td>
		</tr>
		<tr>
			...
			<td class="listTableRight">
				<table>
					<tr>
						<td>
							<input type="checkbox" name="reminder_notify" id="reminderNotify"{if !isset($eDate)||($eDate.flags&8)} checked="checked"{/if} /> 
							...
							<fieldset>
								...
								<select name="reminder">
									<optgroup label="{lng p="minutes"}">
										<option value="5"{if !isset($eDate)||$eDate.reminder/60==5} selected="selected"{/if}>5 {lng p="minutes"}</option>
										<option value="15"{if isset($eDate) && $eDate.reminder/60==15} selected="selected"{/if}>15 {lng p="minutes"}</option>
										<option value="30"{if isset($eDate) && $eDate.reminder/60==30} selected="selected"{/if}>30 {lng p="minutes"}</option>
										<option value="45"{if isset($eDate) && $eDate.reminder/60==45} selected="selected"{/if}>45 {lng p="minutes"}</option>
									</optgroup>
									
									<optgroup label="{lng p="hours"}">
										<option value="60"{if isset($eDate) && $eDate.reminder/60==60} selected="selected"{/if}>1 {lng p="hours"}</option>
										<option value="120"{if isset($eDate) && $eDate.reminder/60==120} selected="selected"{/if}>2 {lng p="hours"}</option>
										<option value="240"{if isset($eDate) && $eDate.reminder/60==240} selected="selected"{/if}>4 {lng p="hours"}</option>
										<option value="480"{if isset($eDate) && $eDate.reminder/60==480} selected="selected"{/if}>8 {lng p="hours"}</option>
										<option value="720"{if isset($eDate) && $eDate.reminder/60==720} selected="selected"{/if}>12 {lng p="hours"}</option>
									</optgroup>
									
									<optgroup label="{lng p="days"}">
										<option value="1440"{if isset($eDate) && $eDate.reminder/60==1440} selected="selected"{/if}>1 {lng p="days"}</option>
										<option value="2880"{if isset($eDate) && $eDate.reminder/60==2880} selected="selected"{/if}>2 {lng p="days"}</option>
										<option value="5760"{if isset($eDate) && $eDate.reminder/60==5760} selected="selected"{/if}>4 {lng p="days"}</option>
										<option value="8640"{if isset($eDate) && $eDate.reminder/60==8640} selected="selected"{/if}>6 {lng p="days"}</option>
									</optgroup>
									
									<optgroup label="{lng p="weeks"}">
										<option value="10080"{if isset($eDate) && $eDate.reminder/60==10080} selected="selected"{/if}>1 {lng p="weeks"}</option>
										<option value="20160"{if isset($eDate) && $eDate.reminder/60==20160} selected="selected"{/if}>2 {lng p="weeks"}</option>
										<option value="30240"{if isset($eDate) && $eDate.reminder/60==30240} selected="selected"{/if}>3 {lng p="weeks"}</option>
										<option value="40320"{if isset($eDate) && $eDate.reminder/60==40320} selected="selected"{/if}>4 {lng p="weeks"}</option>
									</optgroup>
								</select>
		...
		<tr>
			...
			<td class="listTableRight">
				<input type="hidden" name="attendees" value="{text value=$attendees|default:'' allowEmpty=true}" id="attendees" />
