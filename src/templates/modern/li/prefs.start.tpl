				...
				<tr>
					<td class="listTableIconSide">
						{if $prefsIcons[$item]}
							<img src="{$prefsIcons[$item]}" width="48" height="48" style="margin-right: 3px;" border="0" alt="" align="absmiddle" />
						{else}
							<i class="fa {if $item=='autoresponder'}fa-reply{elseif $item=='aliases'}fa-user{elseif $item=='common'}fa-cogs{elseif $item=='antispam'}fa-ban{elseif $item=='antivirus'}fa-bug{elseif $item=='antivirus'}fa-bug{elseif $item=='orders'}fa-shopping-cart{elseif $item=='faq'}fa-question-circle-o{elseif $item=='filters'}fa-filter{elseif $item=='membership'}fa-id-card-o{elseif $item=='extpop3'}fa-compress{elseif $item=='keyring'}fa-key{elseif $item=='signatures'}fa-quote-right{elseif $item=='contact'}fa-user-o{else}{$item}{/if} fa-4x" aria-hidden="true"></i>
						{/if}
					</td>
