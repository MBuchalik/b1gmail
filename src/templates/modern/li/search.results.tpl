...
{foreach from=$resultCat.results item=result}
	<tr>
		...
		<td class="resultRight" onclick="{if $result.extLink}window.open('{$result.extLink}');{else}document.location.href='{$result.link}sid={$sid}';{/if}parent.hideSearchPopup(true);" title="{if isset($result.excerpt)}{text value=$result.excerpt allowEmpty=true stripTags=true}{/if}">
