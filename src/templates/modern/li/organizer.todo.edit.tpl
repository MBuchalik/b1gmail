...
<form name="f1" method="post" action="organizer.todo.php?action={if isset($task)}saveTask&id={$task.id}{else}createTask{/if}&sid={$sid}" onsubmit="return(checkTodoForm(this));">
	<table class="listTable">
		...
		<tr>
			...
			<td class="listTableRight">
				<input type="text" name="titel" id="titel" value="{text value=$task.titel|default:'' allowEmpty=true}" style="width:100%;" />
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="listTableRight">
				<textarea class="textInput" name="comments" id="comments">{text value=$task.comments|default:'' allowEmpty=true}</textarea>
			</td>
		</tr>
		<tr>
