...
class BMPlugin_Widget_Mailspace extends BMPlugin {
    ...
    function renderWidget() {
        ...
        $tpl->assign(
            'bmwidget_mailspace_spaceUsed',
            $userRow['mailspace_used'],
        );
        $tpl->assign(
            'bmwidget_mailspace_spaceLimit',
            $groupRow['storage'] + $userRow['mailspace_add'],
        );
        return true;
    }
}
...
class BMPlugin_Widget_EMail extends BMPlugin {
    ...
    function renderWidget() {
        ...
        $hideSystemFolders = (bool) $thisUser->getPref(
            'widget_email.hideSystemFolders',
        );
        $hideCustomFolders = (bool) $thisUser->getPref(
            'widget_email.hideCustomFolders',
        );
        $hideIntelliFolders = (bool) $thisUser->getPref(
            'widget_email.hideIntelliFolders',
        );
        ...
        foreach ($folderList as $folderID => $folder) {
            ...
            $emailItems[$folderID] = [
                ...
                'allMails' => $folder['all'],
                'flaggedMails' => $mailbox->GetMailCount(
                    $folderID,
                    false,
                    true,
                ),
    ...
    function renderWidgetPrefs() {
        ...
        if (!isset($_REQUEST['save'])) {
            $tpl->assign(
                'hideSystemFolders',
                $thisUser->getPref('widget_email.hideSystemFolders') == 1,
            );
            $tpl->assign(
                'hideCustomFolders',
                $thisUser->getPref('widget_email.hideCustomFolders') == 1,
            );
            $tpl->assign(
                'hideIntelliFolders',
                $thisUser->getPref('widget_email.hideIntelliFolders') == 1,
            );
            $tpl->display($this->_templatePath('widget.email.prefs.tpl'));
        } else {
            $thisUser->setPref(
                'widget_email.hideSystemFolders',
                isset($_REQUEST['hideSystemFolders']) ? 1 : 0,
            );
            $thisUser->setPref(
                'widget_email.hideCustomFolders',
                isset($_REQUEST['hideCustomFolders']) ? 1 : 0,
            );
            $thisUser->setPref(
                'widget_email.hideIntelliFolders',
                isset($_REQUEST['hideIntelliFolders']) ? 1 : 0,
            );
