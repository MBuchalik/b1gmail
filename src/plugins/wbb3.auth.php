...
class WBB3AuthPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // create prefs table
        $db->Query(
            'CREATE TABLE {pre}wbb3_plugin_prefs(enableAuth tinyint(4) NOT NULL DEFAULT 0, mysqlHost varchar(128) NOT NULL, mysqlUser varchar(128) NOT NULL, mysqlPass varchar(128) NOT NULL, mysqlDB varchar(128) NOT NULL, mysqlPrefix varchar(128) NOT NULL, userDomain varchar(128) NOT NULL, userGroups varchar(255) NOT NULL DEFAULT \'\')',
        );

        // insert initial row
        [$domain] = $this->_getDomains();
        $db->Query(
            'REPLACE INTO {pre}wbb3_plugin_prefs(enableAuth, mysqlHost, mysqlUser, mysqlPass, mysqlDB, mysqlPrefix, userDomain) VALUES' .
                '(?,?,?,?,?,?,?)',
            0,
            'localhost',
            'wbb3-user',
    ...
    function _normalizeUsername($userName) {
        ...
        $userName = str_replace([chr(0xfc), chr(0xdc)], 'ue', $userName);
        $userName = str_replace([chr(0xf6), chr(0xd6)], 'oe', $userName);
        $userName = str_replace([chr(0xe4), chr(0xc4)], 'ae', $userName);
    ...
    function OnAuthenticate(
        ...
        $mysql = @mysqli_connect(
            $wbb3_prefs['mysqlHost'],
            $wbb3_prefs['mysqlUser'],
            $wbb3_prefs['mysqlPass'],
            true,
        );
        if ($mysql) {
            if (mysqli_select_db($wbb3_prefs['mysqlDB'], $mysql)) {
                ...
                // search user
                $res = $wbb3DB->Query(
                    'SELECT `userID`,`password`,`salt`,`email` FROM ' .
                        $wbb3_prefs['mysqlPrefix'] .
                        'user WHERE LOWER(`username`)=LOWER(?) AND `activationCode`=0 AND `banned`=0 AND `reactivationCode`=0',
                    $userName,
                );
                if ($res->RowCount() == 0) {
                    return false;
                ...
                if (trim($wbb3_prefs['userGroups']) != '') {
                    ...
                    $res = $wbb3DB->Query(
                        'SELECT COUNT(*) FROM ' .
                            $wbb3_prefs['mysqlPrefix'] .
                            'user_to_groups WHERE `userID`=? AND `groupID` IN ?',
                        $row['userID'],
                        $allowedGroups,
                    );
                    ...
                    if ($rowCount < 1) {
                        PutLog(
                            sprintf(
                                'Rejected login attempt of WBB3 user <%s> (%d) (not in an allowed group)',
                                $userName,
                                $row['userID'],
                            ),
                            PRIO_PLUGIN,
                ...
                if (
                    ...
                    sha1(
                        $row['salt'] .
                            sha1($row['salt'] . sha1($passwordPlain)),
                    )
                ) {
                    $uid = sprintf($this->_uidFormat, $row['userID']);
                    $myUserName = sprintf(
                        '%s@%s',
                        $this->_normalizeUsername($userName),
                        $userDomain,
                    );
                    ...
                    if (BMUser::GetID($myUserName) == 0) {
                        PutLog(
                            sprintf(
                                'Creating b1gMail user <%s> for wbb3 user <%s> (%d)',
                                $myUserName,
                                $userName,
                                $row['userID'],
                            ),
                        ...
                        $res = $db->Query(
                            'SELECT `uid` FROM {pre}users WHERE `email`=?',
                            $myUserName,
                        );
                        $row = $res->FetchArray(MYSQLI_ASSOC);
                        $res->Free();
                        ...
                        if ($row['uid'] != $uid) {
                            PutLog(
                                sprintf(
                                    'Username conflict in wbb3 auth plugin: UID of user <%s> is not <%s>',
                                    $myUserName,
                                    $uid,
                                ),
                                PRIO_WARNING,
            ...
            PutLog(
                'MySQL connection to wbb3 db failed',
                PRIO_PLUGIN,
                __FILE__,
                __LINE__,
            );
    ...
    function _prefsPage() {
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'save') {
            $db->Query(
                'UPDATE {pre}wbb3_plugin_prefs SET enableAuth=?,mysqlHost=?,mysqlUser=?,mysqlPass=?,mysqlDB=?,mysqlPrefix=?,userDomain=?,userGroups=?',
            ...
            if (mysqli_select_db($wbb3_prefs['mysqlDB'], $mysql)) {
                ...
                $res = $wbb3DB->Query(
                    'SELECT `groupID`,`groupName` FROM ' .
                        $wbb3_prefs['mysqlPrefix'] .
                        'group WHERE `groupType`>=3 ORDER BY `groupName` ASC',
                );
