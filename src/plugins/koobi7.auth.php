...
class Koobi7AuthPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // create prefs table
        $db->Query(
            'CREATE TABLE {pre}koobi7_plugin_prefs(enableAuth tinyint(4) NOT NULL DEFAULT 0, mysqlHost varchar(128) NOT NULL, mysqlUser varchar(128) NOT NULL, mysqlPass varchar(128) NOT NULL, mysqlDB varchar(128) NOT NULL, mysqlPrefix varchar(128) NOT NULL, userDomain varchar(128) NOT NULL)',
        );

        // insert initial row
        [$domain] = explode(':', $bm_prefs['domains']);
        $db->Query(
            'REPLACE INTO {pre}koobi7_plugin_prefs(enableAuth, mysqlHost, mysqlUser, mysqlPass, mysqlDB, mysqlPrefix, userDomain) VALUES' .
                '(?,?,?,?,?,?,?)',
            0,
            'localhost',
            'koobi7-user',
    ...
    function OnAuthenticate(
        ...
        // our domain?
        if (
            strtolower($userDomain) != strtolower($koobi7_prefs['userDomain'])
        ) {
            return false;
        }
        ...
        $mysql = @mysqli_connect(
            $koobi7_prefs['mysqlHost'],
            $koobi7_prefs['mysqlUser'],
            $koobi7_prefs['mysqlPass'],
            true,
        );
        if ($mysql) {
            if (mysqli_select_db($koobi7_prefs['mysqlDB'], $mysql)) {
                ...
                // search user
                $res = $koobi7DB->Query(
                    'SELECT `Id`,`Kennwort`,`Email`,`Vorname`,`Nachname`,`Strasse_Nr`,`Postleitzahl`,`Ort`,`Telefon`,`Telefax`,`Land` FROM ' .
                        $koobi7_prefs['mysqlPrefix'] .
                        'benutzer WHERE `Benutzername`=? AND `Aktiv`=1',
                    $userName,
                );
                if ($res->RowCount() == 0) {
                    return false;
                ...
                if (md5($passwordMD5) === $row['Kennwort']) {
                    ...
                    if (BMUser::GetID($myUserName) == 0) {
                        PutLog(
                            sprintf(
                                'Creating b1gMail user for Koobi7 user <%s> (%d)',
                                $userName,
                                $row['Id'],
                            ),
                            PRIO_PLUGIN,
            ...
            PutLog(
                'MySQL connection to koobi7 db failed',
                PRIO_PLUGIN,
                __FILE__,
                __LINE__,
            );
    ...
    function _prefsPage() {
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'save') {
            $db->Query(
                'UPDATE {pre}koobi7_plugin_prefs SET enableAuth=?,mysqlHost=?,mysqlUser=?,mysqlPass=?,mysqlDB=?,mysqlPrefix=?,userDomain=?',
        ...
        $tpl->assign(
            'page',
            $this->_templatePath('koobi7auth.plugin.prefs.tpl'),
        );
    }
}
