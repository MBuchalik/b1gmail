...
class jfChatAuthPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // create prefs table
        $db->Query(
            'CREATE TABLE {pre}jfchat_plugin_prefs(enableAuth tinyint(4) NOT NULL DEFAULT 0, mysqlHost varchar(128) NOT NULL, mysqlUser varchar(128) NOT NULL, mysqlPass varchar(128) NOT NULL, mysqlDB varchar(128) NOT NULL, mysqlPrefix varchar(128) NOT NULL, userDomain varchar(128) NOT NULL)',
        );

        // insert initial row
        [$domain] = explode(':', $bm_prefs['domains']);
        $db->Query(
            'REPLACE INTO {pre}jfchat_plugin_prefs(enableAuth, mysqlHost, mysqlUser, mysqlPass, mysqlDB, mysqlPrefix, userDomain) VALUES' .
                '(?,?,?,?,?,?,?)',
            0,
            'localhost',
            'jfchat-user',
    ...
    function OnAuthenticate(
        ...
        // our domain?
        if (
            strtolower($userDomain) != strtolower($jfchat_prefs['userDomain'])
        ) {
            return false;
        }
        ...
        $mysql = @mysqli_connect(
            $jfchat_prefs['mysqlHost'],
            $jfchat_prefs['mysqlUser'],
            $jfchat_prefs['mysqlPass'],
            true,
        );
        if ($mysql) {
            if (mysqli_select_db($jfchat_prefs['mysqlDB'], $mysql)) {
                ...
                // search user
                $res = $jfchatDB->Query(
                    'SELECT `username`,`password`,`emailadresse`,`wohnort` FROM ' .
                        $jfchat_prefs['mysqlPrefix'] .
                        'registry WHERE `username`=?',
                    $userName,
                );
                if ($res->RowCount() == 0) {
                    return false;
                ...
                if ($row['password'] === $passwordMD5) {
                    ...
                    if (BMUser::GetID($myUserName) == 0) {
                        PutLog(
                            sprintf(
                                'Creating b1gMail user for jfChat user <%s>',
                                $userName,
                            ),
                            PRIO_PLUGIN,
                            __FILE__,
            ...
            PutLog(
                'MySQL connection to jfChat db failed',
                PRIO_PLUGIN,
                __FILE__,
                __LINE__,
            );
    ...
    function _prefsPage() {
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'save') {
            $db->Query(
                'UPDATE {pre}jfchat_plugin_prefs SET enableAuth=?,mysqlHost=?,mysqlUser=?,mysqlPass=?,mysqlDB=?,mysqlPrefix=?,userDomain=?',
        ...
        $tpl->assign(
            'page',
            $this->_templatePath('jfchatauth.plugin.prefs.tpl'),
        );
    }
}
