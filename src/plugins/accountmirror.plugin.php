...
class AccountMirror extends BMPlugin {
    ...
    function OnReadLang(
        ...
        if ($lang == 'deutsch') {
            $lang_admin['am_notice1'] =
                'Die Verwendung dieses Plugins ist nur unter strenger Einhaltung der entsprechenden Gesetze, insbesondere der Datenschutzbestimmungen, gestattet.';
            $lang_admin['am_notice2'] =
                'Es wird keinerlei Funktionsgarantie oder Garantie f&uuml;r die Eignung zu einem bestimmten Zweck &uuml;bernommen. Spiegelungen k&ouml;nnen unter Umst&auml;nden z.B. unvollst&auml;ndig oder fehlerhaft sein.';
            $lang_admin['am_notice3'] =
                'Die Spiegelung erfolgt nur f&uuml;r das Ereignis &quot;E-Mail im Account gespeichert&quot;, z.B. bei Empfang einer E-Mail. Andere Ereignisse, z.B. Markierung, Verschiebung, L&ouml;schung von E-Mails werden nicht gespiegelt. Es erfolgt keine Spiegelung/Ber&uuml;cksichtigung der Ordnerstruktur (mit Ausnahme von System-Ordnern). Andere Daten, z.B. Webdisk oder Adressbuch/Kalender, werden nicht gespiegelt. Die Spiegelung erfolgt nur f&uuml;r E-Mails, die im Zeitraum der Spiegelma&szlig;nahme gespeichert werden.';
            $lang_admin['am_notice4'] =
                'Der Spiegel-Ziel-Account sollte <em>mindestens</em> doppelt so viel Speicher zur Verf&uuml;gung haben wie der Quell-Account.';
            $lang_admin['am_notice5'] =
                'Im Spiegel-Ziel-Account sollten keine Filterregeln, Autoresponder oder andere Benachrichtigungsfunktionen oder Funktionen, die auf den Empfang einer E-Mail reagieren, aktiviert sein.';
            ...
            $lang_admin['am_error_0'] =
                'Der Quelle-/Ziel-Account wurde nicht gefunden. Bitte geben Sie die prim&auml;re E-Mail-Adresse des jeweiligen Accounts korrekt an.';
            $lang_admin['am_error_1'] =
                'Ein Account kann nicht in sich selbst gespiegelt werden.';
            $lang_admin['am_error_2'] =
                'Der Ziel-Account darf nicht die Quelle einer anderen Spiegelma&szlig;nahme sein.';
            $lang_admin['am_error_3'] =
                'Der Endzeitpunkt darf nicht vor dem Anfangszeitpunkt liegen.';
        } else {
            $lang_admin['am_notice1'] =
                'This plugin may only be used in compliance with privacy laws.';
            $lang_admin['am_notice2'] =
                'This plugin comes without any warranties. Mirrorings may contain errors or be incomplete.';
            $lang_admin['am_notice3'] =
                'Only events of type &quot;Email stored&quot; will be mirrored. The folder structure will not be preserved (with exception of system folders). Only mails saved in the specified timeframe will be mirrored.';
            $lang_admin['am_notice4'] =
                'The target account should have <em>at least</em> twice the free storage of the source account.';
            $lang_admin['am_notice5'] =
                'There should be no filter rules or autoresponders activated in the target account.';
            ...
            $lang_admin['am_to'] = 'to';
            $lang_admin['am_error_0'] =
                'The source/target account could not be found. Please ensure you entered the correct primary email addresses of the accounts.';
            $lang_admin['am_error_1'] =
                'You cannot mirror an account to itself.';
            $lang_admin['am_error_2'] =
                'The target account may not be the source account of another mirroring.';
            $lang_admin['am_error_3'] =
                'The end time may not be before the start time.';
        }
    }
    ...
    function Install() {
        ...
        // db struct
        $databaseStructure =
            // checksum: da0bba6e13cd8b6e5adcd3e795c8d844
    ...
    function AdminHandler() {
        ...
        if (isset($_REQUEST['add'])) {
            ...
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}mod_accountmirror WHERE `userid`=?',
                $mirrorTo,
            );
        ...
        if (isset($_REQUEST['delete'])) {
            $db->Query(
                'DELETE FROM {pre}mod_accountmirror WHERE `mirrorid`=?',
                $_REQUEST['delete'],
            );
        }
        ...
        if (isset($_REQUEST['executeMassAction'])) {
            ...
            if (count($mirrorIDs) > 0) {
                if ($_REQUEST['massAction'] == 'delete') {
                    $db->Query(
                        'DELETE FROM {pre}mod_accountmirror WHERE `mirrorid` IN(' .
                            implode(',', $mirrorIDs) .
                            ')',
                    );
                }
        ...
        $mirrorings = [];
        $res = $db->Query(
            'SELECT * FROM {pre}mod_accountmirror ORDER BY `mirrorid` ASC',
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            $source = _new('BMUser', [$row['userid']]);
            $dest = _new('BMUser', [$row['mirror_to']]);
    ...
    function AfterStoreMail($mailID, &$mail, &$mailbox) {
        ...
        $res = $db->Query(
            'SELECT * FROM {pre}mod_accountmirror WHERE `userid`=?',
            $mailbox->_userID,
        );
        if ($res->RowCount() < 1) {
            return;
        ...
        if ($mail2 === false || !is_object($mail2)) {
            ...
            PutLog(
                sprintf(
                    'Account mirror plugin: Could not retrieve mail #%d from mailbox',
                    $mailID,
                ),
                PRIO_WARNING,
                __FILE__,
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            // store the mail
            $recipientResult = $targetMailbox->StoreMail(
                $mail,
            ...
            if ($recipientResult != STORE_RESULT_OK) {
                PutLog(
                    sprintf(
                        'Account mirror plugin: Mirroring of mail to user #%d to target mailbox #%d failed (%d)',
                        $mailbox->_userID,
                        $row['mirror_to'],
                        $recipientResult,
                    ),
                ...
                $db->Query(
                    'UPDATE {pre}mod_accountmirror SET `error_count`=`error_count`+1 WHERE `mirrorid`=?',
                    $row['mirrorid'],
                );
            } else {
                PutLog(
                    sprintf(
                        'Account mirror plugin: Mirrored mail to user #%d to target mailbox #%d',
                        $mailbox->_userID,
                        $row['mirror_to'],
                    ),
                    PRIO_NOTE,
                ...
                $db->Query(
                    'UPDATE {pre}mod_accountmirror SET `mail_count`=`mail_count`+1 WHERE `mirrorid`=?',
                    $row['mirrorid'],
                );
            }
        }
