...
class VBulletinAuthPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // create prefs table
        $db->Query(
            'CREATE TABLE {pre}vb_plugin_prefs(enableAuth tinyint(4) NOT NULL DEFAULT 0, mysqlHost varchar(128) NOT NULL, mysqlUser varchar(128) NOT NULL, mysqlPass varchar(128) NOT NULL, mysqlDB varchar(128) NOT NULL, mysqlPrefix varchar(128) NOT NULL, userDomain varchar(128) NOT NULL)',
        );

        // insert initial row
        [$domain] = explode(':', $bm_prefs['domains']);
        $db->Query(
            'REPLACE INTO {pre}vb_plugin_prefs(enableAuth, mysqlHost, mysqlUser, mysqlPass, mysqlDB, mysqlPrefix, userDomain) VALUES' .
                '(?,?,?,?,?,?,?)',
            0,
            'localhost',
            'vbulletin-user',
    ...
    function OnAuthenticate(
        ...
        if ($mysql) {
            if (mysqli_select_db($vb_prefs['mysqlDB'], $mysql)) {
                ...
                // search user
                $res = $vbDB->Query(
                    'SELECT userid,salt,password,email FROM ' .
                        $vb_prefs['mysqlPrefix'] .
                        'user WHERE username=?',
                    $userName,
                );
                if ($res->RowCount() == 0) {
                ...
                if ($row['password'] === md5($passwordMD5 . $row['salt'])) {
                    ...
                    if (BMUser::GetID($myUserName) == 0) {
                        PutLog(
                            sprintf(
                                'Creating b1gMail user for vBulletin user <%s> (%d)',
                                $userName,
                                $row['userid'],
                            ),
                            PRIO_PLUGIN,
            ...
            PutLog(
                'MySQL connection to vBulletin db failed',
                PRIO_PLUGIN,
                __FILE__,
                __LINE__,
            );
    ...
    function FileHandler($file, $action) {
        ...
        if (
            strpos($userRow['uid'], 'vBulletin:') === false ||
            $userRow['vorname'] != '' ||
            $userRow['nachname'] != ''
        ) {
            return false;
    ...
    function _prefsPage() {
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'save') {
            $db->Query(
                'UPDATE {pre}vb_plugin_prefs SET enableAuth=?,mysqlHost=?,mysqlUser=?,mysqlPass=?,mysqlDB=?,mysqlPrefix=?,userDomain=?',
