...
class ProfileCheckPlugin extends BMPlugin {
    ...
    function OnReadLang(
        ...
        if ($lang == 'deutsch') {
            $lang_admin['profilecheck_msg'] =
                'Ihr Kontakt-Profil ist derzeit unvollst&auml;ndig.<br />Bitte pr&uuml;fen Sie die mit einem Sternchen (*) markierten Pflichtfelder und speichern Sie das Profil, um Ihren Account wieder nutzen zu k&ouml;nnen.';
        } else {
            $lang_admin['profilecheck_msg'] =
                'Your contact profile is incomplete.<br />Please check the obligatory fields marked with an asterisk (*) and save your profile to use your account again.';
        }
    }
    ...
    function FileHandler($file, $action) {
        ...
        if (
            isset($_SESSION['ProfileCheckPlugin_profileChecked']) &&
        ...
        if (
            ...
            exit();
        } elseif ($file == 'start.php' && $action == 'usersearch') {
            exit();
        } elseif (
            $file == 'prefs.php' &&
            $action == 'contact' &&
    ...
    function _checkProfile() {
        ...
        // fax
        if ($bm_prefs['f_fax'] == 'p' && strlen(trim($userRow['fax'])) < 5) {
            $invalidFields[] = 'fax';
        }
        ...
        $res = $db->Query(
            'SELECT id,rule,typ FROM {pre}profilfelder WHERE pflicht=? AND typ IN ?',
            'yes',
            [FIELD_RADIO, FIELD_TEXT, FIELD_DATE],
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            ...
            } elseif ($row['typ'] == FIELD_TEXT) {
                if (
                    !isset($profileFields[$row['id']]) ||
                    !preg_match(
                        '/' . $row['rule'] . '/',
                        $profileFields[$row['id']],
                    )
                ) {
                    $invalidFields[] = 'field_' . $row['id'];
                }
            } elseif ($row['typ'] == FIELD_DATE) {
                if (
                    !isset($profileFields[$row['id']]) ||
