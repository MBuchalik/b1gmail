...
class PluginUpdatesPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // db struct
        $databaseStructure =
            // checksum: 3fcac3f06a07e9ebdda20613c81ebec2
    ...
    function OnReadLang(
        ...
        if ($lang == 'deutsch') {
            $lang_admin['pluginupd_notification'] =
                'F&uuml;r die folgenden Plugins stehen Updates zur Verf&uuml;gung:';
        } else {
            $lang_admin['pluginupd_notification'] =
                'Updates for the following plugins are available:';
        }
    }
    ...
    function OnCron() {
        ...
        $table = [];
        $res = $db->Query(
            'SELECT `modName`,`lastCheck`,`latestVersion` FROM {pre}mod_pluginupdates',
        );
        ...
        foreach ($plugins->_plugins as $className => $pluginInfo) {
            if (
                !isset($table[$className]) ||
                $table[$className]['lastCheck'] + 3 * TIME_ONE_HOUR <= time()
            ) {
                $latestVersion = '';
                $resultCode = $plugins->callFunction(
                    'CheckForUpdates',
                    $className,
                    false,
                    [&$latestVersion],
                ...
                $db->Query(
                    'REPLACE INTO {pre}mod_pluginupdates(`modName`,`lastCheck`,`latestVersion`) VALUES(?,?,?)',
                    $className,
                    time(),
                    $latestVersion,
                );
    ...
    function getNotices() {
        ...
        $outdatedPlugins = [];
        $res = $db->Query(
            'SELECT `modName`,`latestVersion` FROM {pre}mod_pluginupdates WHERE `latestVersion`!=? ORDER BY `modName` ASC',
            '',
        );
            ...
            if (isset($plugins->_plugins[$row['modName']])) {
                ...
                if (
                    $plugins->callFunction(
                        ...
                        $row['modName'],
                        false,
                        [$row['latestVersion'], $pluginVersion],
                    )
                ) {
                    $outdatedPlugins[] = [
                        'name' => $plugins->getParam('name', $row['modName']),
                        'website' => $plugins->getParam(
                            'website',
                            $row['modName'],
                        ),
                        'installed' => $plugins->getParam(
                            'version',
                            $row['modName'],
                        ),
        ...
        if (count($outdatedPlugins) > 0) {
            ...
            foreach ($outdatedPlugins as $plugin) {
                $text .= sprintf(
                    '<b><a href="%s" target="_blank">%s</a></b> (%s), ',
                    $plugin['website'],
