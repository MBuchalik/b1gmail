...
class JoomlaAuthPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // create prefs table
        $db->Query(
            'CREATE TABLE {pre}joomla_plugin_prefs(enableAuth tinyint(4) NOT NULL DEFAULT 0, mysqlHost varchar(128) NOT NULL, mysqlUser varchar(128) NOT NULL, mysqlPass varchar(128) NOT NULL, mysqlDB varchar(128) NOT NULL, mysqlPrefix varchar(128) NOT NULL, userDomain varchar(128) NOT NULL)',
        );

        // insert initial row
        [$domain] = $this->_getDomains();
        $db->Query(
            'REPLACE INTO {pre}joomla_plugin_prefs(enableAuth, mysqlHost, mysqlUser, mysqlPass, mysqlDB, mysqlPrefix, userDomain) VALUES' .
                '(?,?,?,?,?,?,?)',
            0,
            'localhost',
            'joomla-user',
    ...
    function OnAuthenticate(
        ...
        // our domain?
        if (
            strtolower($userDomain) != strtolower($joomla_prefs['userDomain'])
        ) {
            return false;
        }
        ...
        $mysql = @mysqli_connect(
            $joomla_prefs['mysqlHost'],
            $joomla_prefs['mysqlUser'],
            $joomla_prefs['mysqlPass'],
            true,
        );
        if ($mysql) {
            if (mysqli_select_db($joomla_prefs['mysqlDB'], $mysql)) {
                ...
                // search user
                $res = $joomlaDB->Query(
                    'SELECT `id`,`password`,`email`,`name` FROM ' .
                        $joomla_prefs['mysqlPrefix'] .
                ...
                if (strpos($row['password'], ':') !== false) {
                    [$joomlaHash, $joomlaSalt] = explode(':', $row['password']);
                } else {
                ...
                if ($joomlaHash === md5($passwordPlain . $joomlaSalt)) {
                    ...
                    if (count($userNameParts) > 1) {
                        ...
                        $userLastName = implode(
                            ' ',
                            array_slice($userNameParts, 1),
                        );
                    } else {
                    ...
                    if (BMUser::GetID($myUserName) == 0) {
                        PutLog(
                            sprintf(
                                'Creating b1gMail user for Joomla! user <%s> (%d)',
                                $userName,
                                $row['id'],
                            ),
                            PRIO_PLUGIN,
            ...
            PutLog(
                'MySQL connection to Joomla! db failed',
                PRIO_PLUGIN,
                __FILE__,
                __LINE__,
            );
    ...
    function _prefsPage() {
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'save') {
            $db->Query(
                'UPDATE {pre}joomla_plugin_prefs SET enableAuth=?,mysqlHost=?,mysqlUser=?,mysqlPass=?,mysqlDB=?,mysqlPrefix=?,userDomain=?',
        ...
        $tpl->assign(
            'page',
            $this->_templatePath('joomlaauth.plugin.prefs.tpl'),
        );
    }
}
