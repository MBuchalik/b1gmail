...
class SignaturePlugin extends BMPlugin {
    ...
    function OnSendMail(&$text, $html) {
        ...
        $res = $db->Query(
            'SELECT `signatureid`,`text` FROM {pre}mod_signatures ' .
        ...
        [$signatureID, $signatureText] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        ...
        $db->Query(
            'UPDATE {pre}mod_signatures SET `counter`=`counter`+1 WHERE `signatureid`=?',
            $signatureID,
        );
    }
    ...
    function AdminHandler() {
        ...
        if (!isset($_REQUEST['action'])) {
            if (isset($_REQUEST['activate'])) {
                $db->Query(
                    'UPDATE {pre}mod_signatures SET `paused`=0 WHERE `signatureid`=?',
                    $_REQUEST['activate'],
                );
            }
            if (isset($_REQUEST['deactivate'])) {
                $db->Query(
                    'UPDATE {pre}mod_signatures SET `paused`=1 WHERE `signatureid`=?',
                    $_REQUEST['deactivate'],
                );
            }
            if (isset($_REQUEST['delete'])) {
                $db->Query(
                    'DELETE FROM {pre}mod_signatures WHERE `signatureid`=?',
                    $_REQUEST['delete'],
                );
            }
            if (isset($_REQUEST['add'])) {
                $db->Query(
                    'INSERT INTO {pre}mod_signatures(`text`,`html`,`weight`,`groups`,`paused`) VALUES(?,?,?,?,?)',
                    $_REQUEST['text'],
            ...
            if (
                ...
                if ($_REQUEST['massAction'] == 'pause') {
                    $db->Query(
                        'UPDATE {pre}mod_signatures SET `paused`=1 WHERE `signatureid` IN ?',
                        $sigs,
                    );
                } elseif ($_REQUEST['massAction'] == 'continue') {
                    $db->Query(
                        'UPDATE {pre}mod_signatures SET `paused`=0 WHERE `signatureid` IN ?',
                        $sigs,
                    );
                } elseif ($_REQUEST['massAction'] == 'delete') {
                    $db->Query(
                        'DELETE FROM {pre}mod_signatures WHERE `signatureid` IN ?',
                        $sigs,
                    );
                }
            }
            ...
            $signatures = [];
            $res = $db->Query(
                'SELECT * FROM {pre}mod_signatures ORDER BY `html` ASC, `text` ASC',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $row['displayText'] =
                    $row['html'] == 1
                        ? $row['text']
                        : '<span style="font-family:courier;size:11px;">' .
                            nl2br(HTMLFormat($row['text'])) .
                            '</span>';
        ...
        elseif ($_REQUEST['action'] == 'edit' && isset($_REQUEST['id'])) {
            if (isset($_REQUEST['save'])) {
                $db->Query(
                    'UPDATE {pre}mod_signatures SET `text`=?,`html`=?,`weight`=?,`groups`=?,`paused`=? WHERE `signatureid`=?',
                    $_REQUEST['text'],
                    isset($_REQUEST['html']) ? 1 : 0,
                    $_REQUEST['weight'],
                    is_array($_REQUEST['groups'])
                ...
                header(
                    'Location: ' . $this->_adminLink() . '&sid=' . session_id(),
                );
                exit();
            }

            $res = $db->Query(
                'SELECT * FROM {pre}mod_signatures WHERE `signatureid`=?',
                $_REQUEST['id'],
            );
            if ($res->RowCount() != 1) {
                die();
