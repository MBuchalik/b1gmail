...
class B1GMailServerAdmin extends BMPlugin {
    ...
    function __construct() {
        ...
        $this->RegisterGroupOption(
            ...
            [
                'no' => 'Nein',
                'mailfrom' => 'Nur MAIL FROM',
                'full' => 'MAIL FROM und From-Header',
            ],
            'mailfrom',
        );
        $this->RegisterGroupOption(
        ...
        [$vMajor, $vMinor] = explode('.', B1GMAIL_VERSION);
    ...
    function Install() {
        ...
        if ($haveGreylistTable) {
            ...
            if ($greylistTableOldFormat) {
                $db->Query(
                    'ALTER TABLE bm60_bms_greylist DROP PRIMARY KEY, ADD COLUMN `id` int(11) NOT NULL auto_increment FIRST, ADD PRIMARY KEY (`id`)',
                );
            }
        }
        ...
        // db struct
        $databaseStructure =
            // checksum: 16f0f236904779f7a54e4291197d7fc6
        ...
        if ($rowCount == 0) {
            ...
            if (SERVER_WINDOWS) {
                ...
                if (
                    ($path = @exec('php-config --php-binary')) &&
                    trim($path) != '' &&
                    @file_exists($path) &&
                    is_executable($path)
                ) {
            ...
            $db->Query(
                'INSERT INTO {pre}bms_prefs(id,licstate,core_version,outbound_target,php_path) VALUES(1,0,?,2,?)',
                '',
                $phpPath,
            );
            ...
            // 192.168.0.0
            $db->Query(
                'INSERT INTO {pre}bms_subnets(ip,mask,classification) VALUES(?,?,?)',
                '192.168.0.0',
                '16',
                2,
            );
            ...
            if (
                ...
                $db->Query('UPDATE {pre}prefs SET `b1gmta_host`=?', $hostName);
            }
        }

        // update subnet table data
        $db->Query(
            'UPDATE {pre}bms_subnets SET `ip`=inet_ntoa(`ip`),`mask`=inet_ntoa(`mask`) WHERE NOT(`ip` LIKE \'%.%\' OR `ip` LIKE \'%:%\')',
        );

        //  ipv6 ::1
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}bms_subnets WHERE `ip`=?',
            '::1',
        );
        [$ipv6Count] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        if ($ipv6Count == 0) {
            $db->Query(
                'INSERT INTO {pre}bms_subnets(ip,mask,classification) VALUES(?,?,?)',
        ...
        // set server names?
        $db->Query(
            'UPDATE {pre}bms_prefs SET `user_pop3server`=? WHERE `user_pop3server`=\'\'',
            $bm_prefs['b1gmta_host'],
        );
        $db->Query(
            'UPDATE {pre}bms_prefs SET `user_imapserver`=? WHERE `user_imapserver`=\'\'',
            $bm_prefs['b1gmta_host'],
        );
        $db->Query(
            'UPDATE {pre}bms_prefs SET `user_smtpserver`=? WHERE `user_smtpserver`=\'\'',
            $bm_prefs['b1gmta_host'],
        );
        ...
        $db->Query(
            'CREATE TRIGGER `bms_imapseen_update` BEFORE UPDATE ON `{pre}mails` ' .
                'FOR EACH ROW ' .
                'BEGIN ' .
        ...
        $db->Query(
            'CREATE TRIGGER `bms_imapuid_update` AFTER UPDATE ON `{pre}mails` ' .
                'FOR EACH ROW ' .
                'BEGIN ' .
                '    IF NEW.`folder` <> OLD.`folder` THEN ' .
        ...
        $db->Query(
            'CREATE TRIGGER `bms_imapuid_insert` AFTER INSERT ON `bm60_mails` ' .
                'FOR EACH ROW ' .
                '    INSERT INTO `bm60_bms_imapuid`(`mailid`) VALUES(NEW.`id`)',
        );
        $db->Query(
            'CREATE TRIGGER `bms_imapuid_delete` AFTER DELETE ON `bm60_mails` ' .
                'FOR EACH ROW ' .
                '    DELETE FROM `bm60_bms_imapuid` WHERE `bm60_bms_imapuid`.`mailid`=OLD.`id`',
        );
    ...
    function OnReadLang(
        ...
        if ($lang == 'deutsch') {
            ...
            $_lang_admin['bms_reject_norevdns'] =
                'Anonyme Clients mit fehlendem Reverse-DNS ablehnen';
            $_lang_admin['bms_control_addr_help'] =
                'Die hier angegebene IP-Adresse vom Warteschleifen-Server muss vom Webserver aus erreichbar sein.\n\nEs wird dringend empfohlen, keine �ffentlich erreichbare IP-Adresse zu verwenden.\n\nSollten b1gMailServer und Webserver auf dem gleichen Server betrieben werden, sollte diese Einstellung auf 127.0.0.1 belassen werden.\n\nNach �nderung muss der Warteschleifen-Dienst manuell per SSH/RDP neu gestartet werden.';
            $_lang_admin['bms_control_addr'] = 'Kontroll-Kanal-Interface';
            $_lang_admin['bms_apnsqueuerestartnote'] =
                '&Auml;nderungen der Push-Konfiguration werden erst nach Neustart des Warteschleifen-Dienstes wirksam!';
            $_lang_admin['bms_certerr_format'] =
                'Zertifikat oder Private Key haben ein ung&uuml;ltiges Format.';
            $_lang_admin['bms_certerr_purpose'] =
                'Das Zertifikat ist nicht als SSL-Client-Zertifikat geeignet.';
            $_lang_admin['bms_certerr_pkcheck'] =
                'Der Private Key passt nicht zum Zertifikat.';
            ...
            $_lang_admin['bms_apnsnote'] =
                'zur Aktivierung muss zuerst ein Zertifikat hinterlegt werden';
            ...
            $_lang_admin['bms_closeduringidle'] =
                'Zwischen IDLE-Polls freigeben';
            $_lang_admin['bms_closewhenidle'] =
                'Bei Inaktivit&auml;t freigeben';
            ...
            $_lang_admin['bms_untrusted_limits'] =
                'Einschr&auml;nkungen f&uuml;r nicht-vertrauensw&uuml;rdige Verbindungen';
            ...
            $_lang_admin['bms_peer_classification'] =
            ...
            $_lang_admin['bms_redirecttosendmail'] =
                'An Sendmail &uuml;bergeben';
            $_lang_admin['bms_redirecttosmtprelay'] =
                'An SMTP-Relay-Server &uuml;bergeben';
            $_lang_admin['bms_queuerestartnote'] =
                '&Auml;nderungen werden erst nach Neustart des Warteschleifen-Dienstes wirksam!';
            $_lang_admin['bms_subnet'] = 'Subnetz';
            $_lang_admin['bms_classification'] = 'Klassifizierung';
            $_lang_admin['bms_origin_default'] =
                'Nicht vertrauensw&uuml;rdig (Standard)';
            ...
            $_lang_admin['bms_licerror'] =
                'Ihr Lizenzschl&uuml;ssel ist ung&uuml;ltig oder abgelaufen.';
            $_lang_admin['bms_adminplugin'] = 'Admin-Plugin';
            $_lang_admin['bms_core'] = 'Core';
            $_lang_admin['bms_licstatus'] = 'Lizenz-Status';
            $_lang_admin['bms_debugmode'] =
                'Das Logging von Debug-Meldungen ist derzeit aktiviert. Es wird dringend empfohlen, Debug-Logging in Produktiv-Umgebungen zu deaktivieren!';
            ...
            $_lang_admin['bms_real_reset'] =
                'Sollen die Sitzungs-Statistiken wirklich zur�ckgesetzt werden?';
            ...
            $_lang_admin['bms_add_signature'] =
                'Gruppen-Signatur anh&auml;ngen';
            ...
            $_lang_admin['bms_updatesdesc'] =
                'Klicken Sie auf den folgenden Button, um nach verf&uuml;gbaren Updates (z.B. wichtige Sicherheits-Aktualisierungen) f&uuml;r Ihre b1gMailServer-Plugins zu suchen.';
            $_lang_admin['bms_origin_nogrey'] = 'Grey-Listing deaktivieren';
            $_lang_admin['bms_origin_nogreyandban'] =
            ...
            $_lang_admin['bms_reallyrestartqueue'] =
                'Soll der Warteschleifen-Dienst wirklich neu gestartet werden?';
            $_lang_admin['bms_queueitemgone'] =
                'Der Warteschleifen-Eintrag ist nicht mehr vorhanden. M&ouml;glicherweise wurde er mittlerweile abgearbeitet oder hat das Ende seine maximale Lebensdauer erreicht.';
            ...
            $_lang_admin['bms_enableolder'] =
                'Aktivieren f&uuml;r Eintr&auml;ge &auml;lter als';
            ...
            $_lang_admin['bms_checkhelo'] =
                'Mails ablehnen, wenn HELO-Hostname abweicht';
            $_lang_admin['bms_headers'] = 'Kopfzeilen';
            $_lang_admin['bms_download'] = 'Herunterladen';
            $_lang_admin['bms_sizelimitissue'] =
                'Das SMTP-Mailgr&ouml;&szlig;enlimit ist kleiner als das b1gMail-Mailgr&ouml;&szlig;enlimit (Einstellungen &raquo; E-Mail &raquo; Empfang).';
            $_lang_admin['bms_ownheaders'] = 'Eigene Header';
            $_lang_admin['bms_headersnote'] =
                'Ung&uuml;ltige Eingaben in diesen Feldern k&ouml;nnen besch&auml;digte E-Mails verursachen.';
            $_lang_admin['bms_clearqueue'] = 'Alle Eintr&auml;ge l&ouml;schen';
            $_lang_admin['bms_clearquestion'] =
                'M&ouml;chten Sie wirklich ALLE Warteschleifen-Eintr&auml;ge l&ouml;schen?';
            $_lang_admin['bms_delbyattr'] =
                'Alle Eintr&auml;ge mit gew&auml;hltem Attribut l&ouml;schen';
            $_lang_admin['bms_reallydelbyattr'] =
                'Sollen wirklich alle Eintr&auml;ge, die das gew&auml;hlte Attribut besitzen, inklusive diesem Eintrag, gel&ouml;scht werden?';
            $_lang_admin['bms_deletedmsg'] =
                'Es wurden <strong>%d</strong> Warteschleifen-Eintr&auml;ge gel&ouml;scht.';
            $_lang_admin['bms_spf'] = 'Sender Policy Framework (SPF)';
            $_lang_admin['bms_spf_onpass'] = 'Bei erfolgreicher Pr&uuml;fung';
            $_lang_admin['bms_spf_onfail'] =
                'Bei fehlgeschlagener Pr&uuml;fung';
            ...
            $_lang_admin['bms_user_chosepop3folders'] =
                'Benutzer kann Ordner w&auml;hlen';
            $_lang_admin['bms_userfolders'] = 'Vom Benutzer erstellte Ordner';
            $_lang_admin['bms_usetls'] = 'Nach M&ouml;glichkeit TLS verwenden';
            $_lang_admin['bms_usednssecdane'] =
                'DNSSEC und DANE-Verifizierung aktivieren';
            ...
            $_lang_admin['bms_imaplimit'] =
                'Ordner-Gr&ouml;&szlig;en-Limitierung';
            $_lang_admin['bms_user_choseimaplimit'] =
                'Durch Benutzer &auml;nderbar';
            ...
            $_lang_admin['bms_auth_no_received'] =
                '\'Received\'-Header bei auth. Clients unterdr&uuml;cken';

            $_lang_user['bms_userarea'] = 'POP3/IMAP/SMTP';
            $_lang_user['prefs_d_bms_userarea'] =
                'Verwalten Sie Ihre E-Mail-Client-Zugriffsm&ouml;glichkeiten.';
            $_lang_user['bms_userlogin'] = 'Login-Daten';
            $_lang_user['bms_userloginnote'] =
            ...
            $_lang_user['bms_folderstofetchnote'] =
                'Hier k&ouml;nnen Sie ausw&auml;hlen, welche Ordner beim POP3-Abruf ber&uuml;cksichtigt werden sollen. Um selbst angelegte Ordner individuell ein- oder auszuschlie&szlig;en, deaktivieren Sie einfach das H&auml;kchen bei &quot;Alle selbst erstellten Ordner&quot;.';
            $_lang_user['bms_userfolders'] = 'Alle selbst erstellen Ordner';
            $_lang_user['bms_folderssaved'] =
                'Die abzurufenden Ordner wurden erfolgreich gespeichert. Die Einstellungen werden bei der n&auml;chsten POP3-Sitzung ber&uuml;cksichtigt.';
            $_lang_user['bms_sslport'] = 'Verbindungssicherheit: SSL';
            $_lang_user['bms_imaplimit'] =
                'Beschr&auml;nkung der IMAP-Ordner-Gr&ouml;&szlig;e';
            $_lang_user['bms_imaplimitnote'] =
                'Sie k&ouml;nnen die Anzahl der E-Mails, die pro IMAP-Ordner maximal angezeigt werden, limitieren. Dies kann z.B. n&uuml;tzlich sein, wenn Sie sehr viele E-Mails haben und Ihr Postfach per mobilem Endger&auml;t abrufen.';
            $_lang_user['bms_limit'] = 'Beschr&auml;nkung';
            $_lang_user['bms_emails'] = 'E-Mails';
            $_lang_user['bms_nolimit'] = 'Unbeschr&auml;nkt';
            $_lang_user['bms_imaplimitsaved'] =
                'Die IMAP-Ordner-Beschr&auml;nkung wurde erfolgreich gespeichert. Die Einstellungen werden bei der n&auml;chsten IMAP-Sitzung ber&uuml;cksichtigt.';
        } else {
            // english
            ...
            $_lang_admin['bms_apns'] = 'Apple Push Service';
            $_lang_admin['bms_apnsnote'] =
                'to enable, please set a certificate first';
            ...
            $_lang_admin['bms_untrusted_limits'] =
                'Restrictions for untrustworthy peers';
            ...
            $_lang_admin['bms_redirecttosmtprelay'] =
                'Pass to SMTP relay server';
            $_lang_admin['bms_queuerestartnote'] =
                'You need to restart the queue service after saving your changes!';
            ...
            $_lang_admin['bms_debugmode'] =
                'Debug logging is activated. We strongly recommend to disable debug logging in production environments!';
            ...
            $_lang_admin['bms_real_reset'] =
                'Do you really want to reset the session statistics?';
            $_lang_admin['bms_feature_tls'] = 'TLS support';
            ...
            $_lang_admin['bms_updatesdesc'] =
                'Please click the following button to check for updates (e.g. important security updates) for your b1gMailServer plugins.';
            $_lang_admin['bms_origin_nogrey'] = 'Disable greylisting';
            $_lang_admin['bms_origin_nogreyandban'] =
                'Disable greylisting and IP ban';
            ...
            $_lang_admin['bms_reallyrestartqueue'] =
                'Do you really want to restart the queue service?';
            $_lang_admin['bms_queueitemgone'] =
                'The queue item is not available anymore. Maybe it has been processed in the meantime or exceeded it\'s lifetime.';
            ...
            $_lang_admin['bms_smtp_user'] = 'SMTP user';
            $_lang_admin['bms_checkhelo'] =
                'Reject mails when actual hostname differs from HELO host';
            $_lang_admin['bms_headers'] = 'Headers';
            $_lang_admin['bms_download'] = 'Download';
            $_lang_admin['bms_sizelimitissue'] =
                'The SMTP mail size limit is smaller than the b1gMail mail size limit (Preferences &raquo; Email &raquo; Receiving).';
            $_lang_admin['bms_ownheaders'] = 'Own headers';
            $_lang_admin['bms_headersnote'] =
                'Invalid input in these fields can cause broken emails.';
            $_lang_admin['bms_clearqueue'] = 'Clear queue';
            $_lang_admin['bms_clearquestion'] =
                'Do you really want to delete ALL queue entries?';
            $_lang_admin['bms_delbyattr'] =
                'Delete all entries with the same selected attribute';
            $_lang_admin['bms_reallydelbyattr'] =
                'Do you really want to delete all entries which share the same selected attribute, including this entry?';
            $_lang_admin['bms_deletedmsg'] =
                '<strong>%d</strong> queue entries have been deleted.';
            $_lang_admin['bms_spf'] = 'Sender Policy Framework (SPF)';
            ...
            $_lang_admin['bms_auth_no_received'] =
                'Suppress \'Received\' header for authenticated clients';

            $_lang_user['bms_userarea'] = 'POP3/IMAP/SMTP';
            $_lang_user['prefs_d_bms_userarea'] =
                'Manage your email client access methods.';
            $_lang_user['bms_userlogin'] = 'Login information';
            $_lang_user['bms_userloginnote'] =
                'Use the following login information to set up your email account in an email client like Outlook, Thunderbird or Apple Mail.';
            $_lang_user['bms_pop3server'] = 'POP3 server';
            $_lang_user['bms_imapserver'] = 'IMAP server';
            $_lang_user['bms_smtpserver'] = 'SMTP server';
            ...
            $_lang_user['bms_folderstofetchnote'] =
                'Here you can define which folders should be fetched using POP3. To individually select/deselect custom folders, just uncheck the item &quot;All user-created folders&quot;.';
            $_lang_user['bms_userfolders'] = 'All user-created folders';
            $_lang_user['bms_folderssaved'] =
                'The folders have been saved successfully. The new settings will become active in your next POP3 session.';
            $_lang_user['bms_sslport'] = 'connection security: SSL';
            $_lang_user['bms_imaplimit'] = 'IMAP folder size limit';
            $_lang_user['bms_imaplimitnote'] =
                'You can limit the count of emails which will be displayed in each IMAP folder. This can be useful in case you have a large amount of emails and use your IMAP account on a mobile device.';
            $_lang_user['bms_limit'] = 'Limit';
            $_lang_user['bms_emails'] = 'Emails';
            $_lang_user['bms_nolimit'] = 'Unlimited';
            $_lang_user['bms_imaplimitsaved'] =
                'The IMAP folder size limit has been saved successfully. The new setting will become active in you next IMAP session.';
        }
    ...
    function AfterStoreMail($mailID, &$mail, &$mailbox) {
        ...
        $res = $db->Query(
            'SELECT {pre}bms_apns_subscription.`subscriptionid` FROM {pre}bms_apns_subscription ' .
        ...
        if ($res->RowCount() > 0) {
            if ($fp = $this->_openControlChannel()) {
                while ($row = $res->FetchArray(MYSQLI_NUM)) {
                    ...
                    $response = $this->_queueControlCommand(
                        $fp,
                        sprintf('APNS_NOTIFY %d', $subscriptionID),
                    );

                    if (!$response || $response[0] != '+') {
                        PutLog(
                            sprintf(
                                'APNS_NOTIFY command failed for subscription #%d, mail #%d: %s',
                                $subscriptionID,
                                $mailID,
                                $response,
                            ),
                    ...
                    PutLog(
                        sprintf(
                            'Sent APNS_NOTIFY for subscription #%d because of mail #%d',
                            $subscriptionID,
                            $mailID,
                        ),
                        PRIO_DEBUG,
                ...
                PutLog(
                    sprintf(
                        'Failed to open control channel to send APNS_NOTIFY command for mail #%d - is the bms-queue service running?',
                        $mailID,
                    ),
                    PRIO_WARNING,
                    __FILE__,
    ...
    function OnCron() {
        ...
        $db->Query(
            'DELETE FROM {pre}bms_greylist WHERE (`time`<?) OR (`confirmed`=0 AND `time`<?)',
            time() - $this->prefs['grey_good_time'],
            time() - $this->prefs['grey_wait_time'],
        );

        $db->Query(
            'DELETE FROM {pre}bms_failban WHERE `last_update`<?',
            time() - $this->prefs['failban_time'],
        );

        $db->Query(
            'DELETE FROM {pre}bms_smtpstats WHERE `time`<?',
            time() - TIME_ONE_DAY,
        );
        ...
        // auto archive logs?
        if (
            $this->prefs['logs_autodelete'] == 1 &&
            $this->prefs['logs_autodelete_days'] >= 1 &&
            $this->prefs['logs_autodelete_last'] < time() - 86400
        ) {
            $db->Query(
                'UPDATE {pre}bms_prefs SET `logs_autodelete_last`=?',
                time(),
            );
            $date =
                time() - TIME_ONE_DAY * $this->prefs['logs_autodelete_days'];
            if (
                $this->_archiveLogs(
                    $date,
                    $this->prefs['logs_autodelete_archive'] == 1,
                    false,
                    $count,
                )
            ) {
                PutLog(
                    sprintf(
                        'Auto-archived %d b1gMailServer log entries',
                        $count,
                    ),
                    PRIO_NOTE,
                    __FILE__,
    ...
    function ProcessEventQueue() {
        ...
        // process events
        $res = $db->Query(
            'SELECT `eventid`,`userid`,`type`,`param1`,`param2` FROM {pre}bms_eventqueue ORDER BY `userid` ASC,`eventid` ASC LIMIT ' .
                BMS_EVENTQUEUE_MAX,
        );
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            if ($row['userid'] != $currentUserID) {
                $currentUserID = $row['userid'];
                $currentUserObject = _new('BMUser', [$currentUserID]);

                if (
                    !is_object($currentUserObject) ||
                    !is_array($currentUserObject->_row)
                ) {
                    $currentUserID = 0;
                    continue;
                }

                $currentMailbox = _new('BMMailbox', [
                    $currentUserID,
                    $currentUserObject->_row['email'],
                    $currentUserObject,
                ]);
            }
            ...
            switch ($row['type']) {
                ...
                case BMS_EVENT_DELETEMAIL:
                    if (
                        method_exists(
                            $currentMailbox,
                            'DeleteMailFromSearchIndex',
                        )
                    ) {
                        $currentMailbox->DeleteMailFromSearchIndex(
                            (int) $row['param1'],
                        );
                    }
                    ModuleFunction('AfterDeleteMail', [
                        (int) $row['param1'],
                        &$currentMailbox,
                    ]);
                    break;
                ...
                case BMS_EVENT_CHANGEMAILFLAGS:
                    ModuleFunction('AfterChangeMailFlags', [
                        (int) $row['param1'],
                        (int) $row['param2'],
                        &$currentMailbox,
                    ]);
        ...
        if (count($processedEvents) > 0) {
            $db->Query(
                'DELETE FROM {pre}bms_eventqueue WHERE `eventid` IN ?',
                $processedEvents,
            );
        }
    }
    ...
    function getNotices() {
        ...
        // license
        if (
            $this->prefs['licstate'] != BMS_LIC_VALID &&
            $this->prefs['licstate'] != BMS_LIC_CHECKING
        ) {
            $notices[] = [
                'type' => 'warning',
        ...
        [$logCount] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        if ($logCount > 250000) {
            $notices[] = [
                'type' => 'info',
    ...
    function AdminHandler() {
        ...
        if ($_REQUEST['action'] == 'msgqueue') {
            $this->_msgqueuePage();
        } elseif ($_REQUEST['action'] == 'plugins') {
            $this->_pluginsPage();
        } elseif ($_REQUEST['action'] == 'stats') {
            $this->_statsPage();
        } elseif ($_REQUEST['action'] == 'logs') {
            $this->_logsPage();
        } elseif ($_REQUEST['action'] == 'lookupIP' && isset($_REQUEST['ip'])) {
    ...
    function _commonPage() {
        ...
        if (!isset($_REQUEST['do'])) {
            ...
            if (isset($_REQUEST['save']) && IsPOSTRequest()) {
                ...
                if (
                    isset($_REQUEST['loglevel']) &&
                    is_array($_REQUEST['loglevel'])
                ) {
                ...
                if (
                    isset($_REQUEST['failban_types']) &&
                    is_array($_REQUEST['failban_types'])
                ) {
                ...
                if (trim($_REQUEST['license']) != $this->prefs['license']) {
                    $db->Query(
                        'UPDATE {pre}bms_prefs SET license=?, licstate=?',
            ...
            // ban list
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}bms_failban WHERE `banned_until`>=?',
                time(),
            );
            [$banCount] = $res->FetchArray(MYSQLI_NUM);
            $res->Free();
        ...
        } elseif ($_REQUEST['do'] == 'banlist') {
            // delete?
            if (isset($_REQUEST['delete']) && is_array($_REQUEST['delete'])) {
                $db->Query(
                    'DELETE FROM {pre}bms_failban WHERE `id` IN ?',
                    $_REQUEST['delete'],
                );
            }
            ...
            $banlist = [];
            $res = $db->Query(
                'SELECT `id`,`ip`,`ip6`,`entry_date`,`banned_until`,`last_update`,`attempts`,`type` FROM {pre}bms_failban WHERE `banned_until`>=? ORDER BY `banned_until` ASC LIMIT ' .
                    $startPos .
                    ',' .
                    $perPage,
                time(),
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.common.banlist.tpl'),
            );
        } elseif ($_REQUEST['do'] == 'tlsaRecord') {
            if ($fp = $this->_openControlChannel()) {
                $tlsaResponse = $this->_queueControlCommand(
                    $fp,
                    'GET_TLSARECORD',
                );
                $this->_closeControlChannel($fp);
                ...
                if (
                    strlen($tlsaResponse) > 4 &&
                    substr($tlsaResponse, 0, 3) == '+OK'
                ) {
    ...
    function _overviewPage() {
        ...
        // pop3
        $res = $db->Query(
            'SELECT `connections`,`in`,`out` FROM {pre}bms_stats WHERE `component`=? AND `date`=CURDATE()',
            BMS_CMP_POP3,
        );
        if ($res->RowCount() == 0) {
            $pop3Today = $pop3In = $pop3Out = 0;
        } else {
            [$pop3Today, $pop3In, $pop3Out] = $res->FetchArray(MYSQLI_NUM);
        }
        $res->Free();
        ...
        if ($res->RowCount() == 0) {
            ...
            [$imapToday, $imapIn, $imapOut] = $res->FetchArray(MYSQLI_NUM);
        }
        $res->Free();
        ...
        if ($res->RowCount() == 0) {
            ...
            [$smtpToday, $smtpIn, $smtpOut] = $res->FetchArray(MYSQLI_NUM);
        }
        $res->Free();

        // queue entries
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}bms_queue WHERE `deleted`=0',
        );
        [$queueEntries] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();

        // inbound queue entries
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}bms_queue WHERE `type`=0 AND `deleted`=0',
        );
        [$queueInbound] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();

        // outbound queue entries
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}bms_queue WHERE `type`=1 AND `deleted`=0',
        );
        [$queueOutbound] = $res->FetchArray(MYSQLI_NUM);
        $res->Free();
        ...
        if ($fp = $this->_openControlChannel()) {
            $queueRunning = true;
            $threadResponse = $this->_queueControlCommand(
                $fp,
                'GET_THREADCOUNT',
            );
            $this->_closeControlChannel($fp);
            ...
            if (
                strlen($threadResponse) > 4 &&
                substr($threadResponse, 0, 3) == '+OK'
            ) {
        ...
        $tpl->assign(
            'coreVersion',
            explode(' ', $this->prefs['core_version'])[0],
        );
        $tpl->assign('pageURL', $this->_adminLink());
    ...
    function _pop3Page() {
        ...
        if (isset($_REQUEST['save']) && IsPOSTRequest()) {
            ...
            $db->Query(
                'UPDATE {pre}bms_prefs SET pop3greeting=?,pop3_timeout=?,altpop3=?,user_chosepop3folders=?,pop3_folders=?',
                $_REQUEST['pop3greeting'],
                (int) $_REQUEST['pop3_timeout'],
                isset($_REQUEST['altpop3_enable'])
                    ? (int) $_REQUEST['altpop3_port']
                    : 0,
    ...
    function _imapPage() {
        ...
        if ($_REQUEST['do'] == 'prefs') {
            ...
            if (isset($_REQUEST['save']) && IsPOSTRequest()) {
                $db->Query(
                    'UPDATE {pre}bms_prefs SET imapgreeting=?,imap_timeout=?,imap_idle_poll=?,imap_idle_mysqlclose=?,imap_mysqlclose=?,imap_intelligentfolders=?,imap_folder_sent=?,imap_folder_spam=?,imap_folder_drafts=?,imap_folder_trash=?,imap_autoexpunge=?,user_choseimaplimit=?,imap_limit=?,apns_enable=?,apns_host=?,apns_port=?',
                    ...
                    if ($cert) {
                        if (@openssl_x509_check_private_key($cert, $keyData)) {
                            ...
                            foreach ($certInfo['purposes'] as $purpose) {
                                if ($purpose[2] == 'sslclient' && $purpose[0]) {
                                    $sslClient = true;
                                }
                            }
                            ...
                            if ($sslClient) {
                                $db->Query(
                                    'UPDATE {pre}bms_prefs SET `apns_certificate`=?,`apns_privatekey`=?',
                                    $certData,
                                    $keyData,
                                );
                                $this->prefs = $this->_getPrefs();
            ...
            if (!$stopIt) {
                ...
                $tpl->assign(
                    'page',
                    $this->_templatePath('bms.admin.imap.apns.tpl'),
                );
            }
        }
    }
    ...
    function _getStatData($type, $time) {
        ...
        for (
            $i = 1;
            $i <=
            BMCalendar::GetDaysInMonth(date('m', $time), date('Y', $time));
            $i++
        ) {
            $result[(int) $i] = [$type => 0];
        }
        ...
        if (substr($type, -7) == 'traffic') {
            for (
                ...
                $res = $db->Query(
                    'SELECT `in`,`out` FROM {pre}bms_stats WHERE `component`=? AND `date`=?',
                    $component,
                    sprintf(
                        '%04d-%02d-%02d',
                        date('Y', $time),
                ...
                if ($res->RowCount() == 0) {
                    ...
                    [$inTraffic, $outTraffic] = $res->FetchArray(MYSQLI_NUM);
                }
                $res->Free();

                $result[(int) $i] = [
                    $type => round(($inTraffic + $outTraffic) / 1024 / 1024, 2),
                ];
            }
        }

        // sessions
        elseif (substr($type, -8) == 'sessions') {
            for (
                $i = 1;
                $i <=
                BMCalendar::GetDaysInMonth(date('m', $time), date('Y', $time));
                ...
                $res = $db->Query(
                    'SELECT `connections` FROM {pre}bms_stats WHERE `component`=? AND `date`=?',
                    $component,
                    sprintf(
                        '%04d-%02d-%02d',
                        date('Y', $time),
    ...
    function _pluginsPage() {
        ...
        if (
            !isset($_REQUEST['do']) ||
            in_array($_REQUEST['do'], ['activatePlugin', 'deactivatePlugin'])
        ) {
            // activate?
            if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'activatePlugin') {
                $db->Query(
                    'UPDATE bm60_bms_mods SET `active`=1 WHERE `filename`=?',
                    $_REQUEST['filename'],
                );
            }
            if (
                ...
                $db->Query(
                    'UPDATE bm60_bms_mods SET `active`=0 WHERE `filename`=?',
                    $_REQUEST['filename'],
                );
            }
            ...
            $plugins = [];
            $res = $db->Query(
                'SELECT `filename`,`name`,`title`,`version`,`author`,`author_website`,`update_url`,`active` FROM bm60_bms_mods ORDER BY `active` DESC, `title` ASC',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
        ...
        } elseif ($_REQUEST['do'] == 'updateCheck') {
            $res = $db->Query(
                'SELECT `version`,`update_url`,`author_website`,`name` FROM bm60_bms_mods WHERE `filename`=?',
                $_REQUEST['filename'],
            );
    ...
    function _statsPage() {
        ...
        if (
            ...
            $chart = new BMChart(
                sprintf(
                    '%s (%d/%d)',
                    $lang_admin['bms_stat_' . $statTypeItem],
                    date('m', $time),
                    date('Y', $time),
                ),
                520,
        ...
        // special types
        $statsSpecial = [
            'pop3' => ['pop3sessions', 'pop3traffic'],
            'imap' => ['imapsessions', 'imaptraffic'],
            'smtp' => ['smtpsessions', 'smtptraffic'],
        ];
     ...
     * @return boolean
     */
    function _archiveLogs(
        $date,
        $archive,
        $dieOnError = true,
        &$archivedLogEntryCount = null
    ) {
        global $db;
        ...
        if ($archive) {
            $fileName =
                B1GMAIL_REL . 'logs/b1gMailServerLog-' . time() . '.log';
            ...
            if (!$fp) {
                if ($dieOnError) {
                    DisplayError(
                        0x15,
                        'Cannot create log archive file',
                    ...
                    PutLog(
                        'Failed to archive b1gMailServer logs (cannot create log archive file) - log cleanup process aborted',
                        PRIO_WARNING,
                        __FILE__,
                        __LINE__,
                    );
            ...
            fwrite(
                $fp,
                '# b1gMailServer ' . $this->prefs['core_version'] . "\n",
            );
            fwrite($fp, '# Log file' . "\n");
            fwrite($fp, '#' . "\n");
            ...
            $res = $db->Query(
                'SELECT iComponent,iSeverity,iDate,szEntry FROM {pre}bms_logs WHERE iDate<' .
                    $date .
                    ' ORDER BY id ASC',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
    ...
    function _logsPage() {
        ...
        $start = isset($_REQUEST['startDay'])
            ? SmartyDateTime('start')
            : (isset($_REQUEST['start'])
                ? (int) $_REQUEST['start']
                : mktime(0, 0, 0, date('m'), date('d'), date('Y')));
        $end = isset($_REQUEST['endDay'])
            ? SmartyDateTime('end') + 59
            : (isset($_REQUEST['end'])
                ? (int) $_REQUEST['end']
                : time());
        $component =
            isset($_REQUEST['component']) && is_array($_REQUEST['component'])
                ? $_REQUEST['component']
                : [
                    1 => true,
                    2 => true,
        ...
        $prio =
            isset($_REQUEST['prio']) && is_array($_REQUEST['prio'])
                ? $_REQUEST['prio']
                : [1 => true, 2 => true, 4 => true];
        $addQ =
            isset($_REQUEST['q']) && trim($_REQUEST['q']) != ''
                ? ' AND szEntry LIKE \'%' . $db->Escape($_REQUEST['q']) . '%\''
                : '';

        // get log count
        $res = $db->Query(
            'SELECT COUNT(*) FROM {pre}bms_logs WHERE iDate>=' .
                $start .
                ' AND iDate<=' .
                $end .
                ' AND iSeverity IN ? AND iComponent IN ?' .
                $addQ,
            array_keys($prio),
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'export') {
            ...
            header(
                sprintf(
                    'Content-Disposition: attachment; filename=b1gMailServerLog-%d-%d.log',
                    $start,
                    $end,
                ),
            );
        ...
        while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
            if ($exportMode) {
                ...
                if (isset($_REQUEST['q']) && trim($_REQUEST['q']) != '') {
                    $row['szEntry'] = preg_replace(
                        '/' .
                            str_replace('/', '\/', preg_quote($_REQUEST['q'])) .
                            '/i',
                ...
                $row['szEntry'] = preg_replace(
                    '/^\#([0-9]*)/',
                    sprintf(
                        '<a href="%s&action=logs&q=%%23$1%%20-&start=%d&end=%d&sid=%s" style="color:blue;">$0</a>',
                        $this->_adminLink(),
                        $start,
                        $end,
                        session_id(),
        ...
        $tpl->assign(
            'ueQ',
            isset($_REQUEST['q']) ? urlencode($_REQUEST['q']) : '',
        );
    ...
    function _smtpPage() {
        ...
        // subnet rules
        //
        elseif ($_REQUEST['do'] == 'subnetRules') {
            // save?
            if (
                isset($_REQUEST['save']) &&
                ...
                foreach ($_REQUEST['subnets'] as $key => $val) {
                    if (
                    ...
                    } elseif (isset($val['delete'])) {
                        $db->Query(
                            'DELETE FROM {pre}bms_subnets WHERE id=?',
                            $key,
                        );
                    } elseif (
                        $key > 0 &&
                        ...
                        $db->Query(
                            'UPDATE {pre}bms_subnets SET ip=?,mask=?,classification=? WHERE id=?',
                            $val['ip'],
                            $val['mask'],
                            $val['classification'],
                            $key,
            ...
            $subnets = [];
            $res = $db->Query(
                'SELECT id,ip,mask,classification FROM {pre}bms_subnets ORDER BY ip,mask',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $subnets[$row['id']] = [
                    'id' => $row['id'],
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.subnetrules.tpl'),
            );
        }
        ...
        elseif ($_REQUEST['do'] == 'dnsblRules') {
            ...
            if (
                ...
                foreach ($_REQUEST['dnsbls'] as $key => $val) {
                    if ($key == 0 && strlen(trim($val['host'])) > 4) {
                        $db->Query(
                            'INSERT INTO {pre}bms_dnsbl(host,classification,`type`) VALUES(?,?,?)',
                            $val['host'],
                            $val['classification'],
                            $val['type'],
                        );
                        ...
                        $db->Query(
                            'DELETE FROM {pre}bms_dnsbl WHERE id=?',
                            $key,
                        );
                    } elseif ($key > 0 && strlen(trim($val['host'])) > 4) {
                        $db->Query(
                            'UPDATE {pre}bms_dnsbl SET host=?,classification=?,`type`=? WHERE id=?',
                            $val['host'],
                            $val['classification'],
                            $val['type'],
                            $key,
            ...
            $dnsbls = [];
            $res = $db->Query(
                'SELECT id,host,classification,`type` FROM {pre}bms_dnsbl ORDER BY host',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $dnsbls[$row['id']] = [
                    'id' => $row['id'],
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.dnsblrules.tpl'),
            );
        }
        ...
        // greylist
        //
        elseif ($_REQUEST['do'] == 'greylist') {
            // delete?
            if (isset($_REQUEST['delete']) && is_array($_REQUEST['delete'])) {
                $db->Query(
                    'DELETE FROM {pre}bms_greylist WHERE `id` IN ?',
                    $_REQUEST['delete'],
                );
            }
            ...
            $greylist = [];
            $res = $db->Query(
                'SELECT `id`,`ip`,`ip6`,`time`,`confirmed` FROM {pre}bms_greylist ORDER BY `time` DESC LIMIT ' .
                    $startPos .
                    ',' .
                    $perPage,
            );
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.greylist.tpl'),
            );
        }
        ...
        elseif ($_REQUEST['do'] == 'milters') {
            ...
            if (
                ...
                foreach ($_POST['milters'] as $key => $val) {
                    ...
                    if ($key == 0 && strlen(trim($val['hostname'])) > 1) {
                        $db->Query(
                            'INSERT INTO {pre}bms_milters(`title`,`hostname`,`port`,`flags`,`default_action`,`pos`) VALUES(?,?,?,?,?,?)',
                            $val['title'],
                            $val['hostname'],
                            (int) $val['port'],
                            $flags,
                        ...
                        $db->Query(
                            'DELETE FROM {pre}bms_milters WHERE `milterid`=?',
                            $key,
                        );
                    } elseif ($key > 0 && strlen(trim($val['hostname'])) > 1) {
                        $db->Query(
                            'UPDATE {pre}bms_milters SET `title`=?,`hostname`=?,`port`=?,`flags`=?,`default_action`=?,`pos`=? WHERE `milterid`=?',
                            $val['title'],
                            $val['hostname'],
                            (int) $val['port'],
                            $flags,
            ...
            $milters = [];
            $res = $db->Query(
                'SELECT `milterid`,`title`,`hostname`,`port`,`flags`,`default_action`,`pos` FROM {pre}bms_milters ORDER BY `pos` ASC',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $lastPos = $row['pos'];
    ...
    function _msgqueuePage() {
        ...
        $haveSignatureSupport =
            ($this->prefs['core_features'] & BMS_CORE_FEATURE_ALTERMIME) != 0;
        ...
        if ($_REQUEST['do'] == 'prefs') {
            ...
            if (isset($_REQUEST['flushQueue'])) {
                if ($fp = $this->_openControlChannel()) {
                    ...
                    $db->Query(
                        'UPDATE {pre}bms_queue SET `last_attempt`=0 WHERE `deleted`=0',
                    );
            ...
            if (isset($_REQUEST['clearQueue'])) {
                if ($fp = $this->_openControlChannel()) {
                    ...
                    $db->Query(
                        'UPDATE {pre}bms_queue SET `deleted`=1 WHERE `active`=0',
                    );
                    @touch('/opt/b1gmailserver/queue/state');
                }
            }
            ...
            if (isset($_REQUEST['restartQueue'])) {
                if ($fp = $this->_openControlChannel()) {
                    ...
                    PutLog(
                        'Failed to restart b1gMailServer queue service (cannot access control channel)',
                        PRIO_PLUGIN,
                        __FILE__,
                        __LINE__,
                    );
            ...
            if (isset($_REQUEST['save']) && IsPOSTRequest()) {
                $db->Query(
                    ...
                    ip2long($_REQUEST['control_addr']) !== false
                        ? $_REQUEST['control_addr']
                        : '127.0.0.1',
                ...
                if ($haveSignatureSupport) {
                    $db->Query(
                        'UPDATE {pre}bms_prefs SET outbound_add_signature=?,outbound_signature_sep=?',
            ...
            $res = $db->Query(
                'SELECT COUNT(*),SUM(size) FROM {pre}bms_queue WHERE `deleted`=0',
            );
            [$queueCount, $queueSize] = $res->FetchArray(MYSQLI_NUM);
            $res->Free();

            // get flushable entry count
            $res = $db->Query(
                'SELECT COUNT(*) FROM {pre}bms_queue WHERE `last_attempt`>=(' .
                    time() .
                    '-`attempts`*' .
                    $this->prefs['queue_retry'] .
                    ') AND `deleted`=0 AND `active`=0',
            );
            [$flushableCount] = $res->FetchArray(MYSQLI_NUM);
            $res->Free();
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.msgqueue.tpl'),
            );
        }
        ...
        elseif ($_REQUEST['do'] == 'headers') {
            if (isset($_REQUEST['save']) && isset($_POST['inbound_headers'])) {
                ...
                $db->Query(
                    'UPDATE {pre}bms_prefs SET `inbound_headers`=?,`outbound_headers`=?',
                    $inbound_headers,
                    $outbound_headers,
                );
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.msgqueue.headers.tpl'),
            );
        }
        ...
        // queue
        //
        elseif ($_REQUEST['do'] == 'deliveryRules') {
            // save?
            if (
                isset($_REQUEST['save']) &&
                isset($_POST['rules']) &&
                ...
                foreach ($_POST['rules'] as $key => $val) {
                    ...
                    if ($key == 0 && strlen(trim($val['rule'])) > 1) {
                        $db->Query(
                            'INSERT INTO {pre}bms_deliveryrules(`mail_type`,`rule_subject`,`rule`,`target`,`target_param`,`flags`,`pos`) VALUES(?,?,?,?,?,?,?)',
                            $val['mail_type'],
                            $val['rule_subject'],
                            trim($val['rule']),
                            $val['target'],
                        ...
                        $db->Query(
                            'DELETE FROM {pre}bms_deliveryrules WHERE `deliveryruleid`=?',
                            $key,
                        );
                    } elseif ($key > 0 && strlen(trim($val['rule'])) > 1) {
                        $db->Query(
                            'UPDATE {pre}bms_deliveryrules SET `mail_type`=?,`rule_subject`=?,`rule`=?,`target`=?,`target_param`=?,`flags`=?,`pos`=? WHERE `deliveryruleid`=?',
                            $val['mail_type'],
                            $val['rule_subject'],
                            trim($val['rule']),
                            $val['target'],
            ...
            $rules = [];
            $res = $db->Query(
                'SELECT `deliveryruleid`,`mail_type`,`rule_subject`,`rule`,`target`,`target_param`,`flags`,`pos` FROM {pre}bms_deliveryrules ORDER BY `pos` ASC',
            );
            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                $lastPos = $row['pos'];
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.deliveryrules.tpl'),
            );
        }
        ...
        elseif ($_REQUEST['do'] == 'queue') {
            ...
            if (isset($_REQUEST['singleAction'])) {
                if ($_REQUEST['singleAction'] == 'delete') {
                    if ($fp = $this->_openControlChannel()) {
                        ...
                        $db->Query(
                            'UPDATE {pre}bms_queue SET `deleted`=1 WHERE `id`=?',
                            $_REQUEST['singleID'],
                        );
                    }
                }
                ...
                if ($_REQUEST['massAction'] == 'delete') {
                    if ($fp = $this->_openControlChannel()) {
                        foreach ($items as $itemID) {
                            $this->_queueControlCommand(
                                $fp,
                                sprintf('DELETE_QUEUEITEM %d', $itemID),
                            );
                        }
                        $this->_closeControlChannel($fp);
                    } else {
                        $db->Query(
                            'UPDATE {pre}bms_queue SET `deleted`=1 WHERE `id` IN ?',
                            $items,
                        );
                    }
                }
            ...
            // delete by attribute?
            if (
                isset($_REQUEST['delByAttr']) &&
                isset($_POST['item_attribute'])
            ) {
                $deletedCount = 0;

                $res = $db->Query(
                    'SELECT * FROM {pre}bms_queue WHERE `id`=?',
                    $_REQUEST['delByAttr'],
                );
                if ($res->RowCount() == 1) {
                    ...
                    if (isset($itemRow[$_POST['item_attribute']])) {
                        if ($fp = $this->_openControlChannel()) {
                            $res = $db->Query(
                                'SELECT `id` FROM {pre}bms_queue WHERE `' .
                                    $_POST['item_attribute'] .
                                    '`=?',
                            ...
                            while ($row = $res->FetchArray(MYSQLI_ASSOC)) {
                                $this->_queueControlCommand(
                                    $fp,
                                    sprintf('DELETE_QUEUEITEM %d', $row['id']),
                                );
                                $deletedCount++;
                            }
                            $res->Free();
                            ...
                            $db->Query(
                                'UPDATE {pre}bms_queue SET `deleted`=1 WHERE `' .
                                    $_POST['item_attribute'] .
                                    '`=?',
                                $itemRow[$_POST['item_attribute']],
                            );
                ...
                $tpl->assign(
                    'msg',
                    sprintf($lang_admin['bms_deletedmsg'], $deletedCount),
                );
            }
            ...
            if (isset($_REQUEST['filter'])) {
                ...
                $queryTypes =
                    count($typeIDs) > 0 ? implode(',', $typeIDs) : '0';
                $queryAdd = ' AND type IN(' . $queryTypes . ') ';
                $types = is_array($_REQUEST['types']) ? $_REQUEST['types'] : [];
                ...
                if (
                    ...
                    if (
                        strlen($query) == 8 &&
                        preg_match('/^[abcdefABCDEF0-9]+$/', $query)
                    ) {
                ...
                if (
                    isset($_REQUEST['use_start']) &&
                    $_REQUEST['use_start'] == 'yes' &&
                ...
                if (
                    isset($_REQUEST['use_end']) &&
                    $_REQUEST['use_end'] == 'yes' &&
            ...
            $queue = [];
            $res = $db->Query(
                'SELECT id,`date`,`type`,`from`,`to`,`size`,last_attempt,attempts,`active` FROM {pre}bms_queue WHERE `deleted`=0 ' .
                    $queryAdd .
                    'ORDER BY `' .
                    $sortBy .
                    '` ' .
        ...
        // queue item details
        //
        elseif ($_REQUEST['do'] == 'showQueueItem' && isset($_REQUEST['id'])) {
            // fetch
            $res = $db->Query(
                'SELECT id,`type`,`date`,`size`,`from`,`to`,`attempts`,`last_attempt`,`last_status`,`smtp_user`,`b1gmail_user`,`last_status_info` FROM {pre}bms_queue WHERE id=?',
                (int) $_REQUEST['id'],
            );
            if ($res->RowCount() == 0) {
                $tpl->assign('msgIcon', 'error32');
            ...
            if ($item['smtp_user'] > 0) {
                $user = _new('BMUser', [$item['smtp_user']]);
                $userRow = $user->Fetch();
                $item['smtp_user_mail'] = $userRow['email'];
            }
            if ($item['b1gmail_user'] > 0) {
                $user = _new('BMUser', [$item['b1gmail_user']]);
                $userRow = $user->Fetch();
            ...
            $tpl->assign(
                'page',
                $this->_templatePath('bms.admin.queue.item.tpl'),
            );
        }
        ...
        //
        elseif (
            $_REQUEST['do'] == 'downloadQueueItem' &&
            isset($_REQUEST['id'])
        ) {
            if ($fp = $this->_openControlChannel()) {
                $response = $this->_queueControlCommand(
                    $fp,
                    sprintf('GET_QUEUEITEM %d', $_REQUEST['id']),
                );
                if (trim($response) == '+OK') {
                    header('Pragma: public');
                    ...
                    header(
                        sprintf(
                            'Content-Disposition: attachment; filename=queue-%08x.eml',
                            $_REQUEST['id'],
                        ),
                    );
        ...
        //
        elseif (
            $_REQUEST['do'] == 'getQueueItemHeaders' &&
            isset($_REQUEST['id'])
        ) {
            if ($fp = $this->_openControlChannel()) {
                $response = $this->_queueControlCommand(
    ...
    function FileHandler($file, $action) {
        ...
        if (
            ...
            $file == 'index.php' &&
            stripos($_SERVER['REQUEST_URI'], '/mail/config-v1.1.xml') !== false
        ) {
            if (!isset($this->prefs) || !is_array($this->prefs)) {
                $this->prefs = $this->_getPrefs();
            }
            ...
            if (
                $this->prefs['user_showlogin'] != 1 &&
                ($this->prefs['user_chosepop3folders'] != 1 ||
                    $thisGroup->_row['pop3'] != 'yes') &&
                ($this->prefs['user_choseimaplimit'] != 1 ||
                    $thisGroup->_row['imap'] != 'yes')
            ) {
                return;
            }
            ...
            $GLOBALS['prefsImages']['bms_userarea'] =
                'plugins/templates/images/bms_userarea48.png';
            $GLOBALS['prefsIcons']['bms_userarea'] =
                'plugins/templates/images/bms_userarea16.png';
        }
    }
    ...
    function _adOutput($tplFile, $userName) {
        ...
        if (
            !empty($userName) &&
            ($userID = BMUser::GetID($userName, true)) > 0
        ) {
    ...
    function UserPrefsPageHandler($action) {
        ...
        if (
            $thisGroup->_row['pop3'] == 'yes' &&
            $this->prefs['user_chosepop3folders']
        ) {
            if (isset($_POST['do']) && $_POST['do'] == 'savePOP3Folders') {
                ...
                $tpl->assign(
                    'backLink',
                    'prefs.php?action=bms_userarea&sid=' . session_id(),
                );
        ...
        if (
            $thisGroup->_row['imap'] == 'yes' &&
            $this->prefs['user_choseimaplimit']
        ) {
