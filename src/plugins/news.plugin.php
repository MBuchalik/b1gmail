...
class NewsPlugin extends BMPlugin {
    ...
    function OnReadLang(
        ...
        if ($lang == 'deutsch') {
            ...
            $lang_user['news_nonews'] =
                'Es konnten keine News gefunden werden.';
            $lang_user['news_text'] =
                'Hier finden Sie aktuelle Informationen und Ank&uuml;ndigungen rund um unserem Dienst.';
        } else {
            $lang_admin['news_news'] = 'News';
        ...
        if ($_REQUEST['action'] == 'news') {
            ...
            if (!isset($_REQUEST['do'])) {
                if (isset($_REQUEST['add'])) {
                    ...
                    $db->Query(
                        'INSERT INTO {pre}news(`title`,`date`,`loggedin`,`groups`,`text`) VALUES(?,?,?,?,?)',
                        $_REQUEST['title'],
                        time(),
                        $_REQUEST['loggedin'],
                        $groups,
                    ...
                    $db->Query(
                        'DELETE FROM {pre}news WHERE `newsid`=?',
                        (int) $_REQUEST['delete'],
                    );
                }
                ...
                $news = [];
                $res = $db->Query(
                    'SELECT `newsid`,`title`,`date`,`loggedin` FROM {pre}news ORDER BY `newsid` DESC',
                );
            ...
            elseif ($_REQUEST['do'] == 'edit' && isset($_REQUEST['id'])) {
                if (isset($_REQUEST['save'])) {
                    ...
                    $db->Query(
                        'UPDATE {pre}news SET `title`=?,`loggedin`=?,`groups`=?,`text`=? WHERE `newsid`=?',
                        $_REQUEST['title'],
                        $_REQUEST['loggedin'],
                        $groups,
                        $_REQUEST['text'],
                ...
                $news = [];
                $res = $db->Query(
                    'SELECT `newsid`,`title`,`text`,`loggedin`,`groups` FROM {pre}news WHERE `newsid`=?',
                    (int) $_REQUEST['id'],
                );
                if ($res->RowCount() != 1) {
                    exit();
                ...
                $tpl->assign(
                    'page',
                    $this->_templatePath('news.admin.edit.tpl'),
                );
            }
        }
    }
    ...
    function _getNews(
        ...
        $result = [];
        $res = $db->Query(
            'SELECT `newsid`,`date`,`title`,`text` FROM {pre}news WHERE (`loggedin`=? OR `loggedin`=?) AND (`loggedin`=? OR `groups`=? OR `groups`=? OR `groups` LIKE ? OR `groups` LIKE ? OR `groups` LIKE ?) ORDER BY `' .
                $sortField .
                '` ' .
                $sortDirection,
            $loggedin ? 'li' : 'nli',
...
class NewsWidget extends BMPlugin {
    ...
    function renderWidget() {
        ...
        $tpl->assign(
            'bmwidget_news_news',
            NewsPlugin::_getNews(true, $groupRow['id']),
        );
        return true;
    }
}
