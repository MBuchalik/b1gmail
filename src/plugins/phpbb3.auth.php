...
class phpBB3AuthPlugin extends BMPlugin {
    ...
    function Install() {
        ...
        // create prefs table
        $db->Query(
            'CREATE TABLE {pre}phpbb3_plugin_prefs(enableAuth tinyint(4) NOT NULL DEFAULT 0, mysqlHost varchar(128) NOT NULL, mysqlUser varchar(128) NOT NULL, mysqlPass varchar(128) NOT NULL, mysqlDB varchar(128) NOT NULL, mysqlPrefix varchar(128) NOT NULL, userDomain varchar(128) NOT NULL)',
        );

        // insert initial row
        [$domain] = explode(':', $bm_prefs['domains']);
        $db->Query(
            'REPLACE INTO {pre}phpbb3_plugin_prefs(enableAuth, mysqlHost, mysqlUser, mysqlPass, mysqlDB, mysqlPrefix, userDomain) VALUES' .
                '(?,?,?,?,?,?,?)',
            0,
            'localhost',
            'phpbb3-user',
    ...
    function OnAuthenticate(
        ...
        // our domain?
        if (
            strtolower($userDomain) != strtolower($phpbb3_prefs['userDomain'])
        ) {
            return false;
        }
        ...
        $mysql = @mysqli_connect(
            $phpbb3_prefs['mysqlHost'],
            $phpbb3_prefs['mysqlUser'],
            $phpbb3_prefs['mysqlPass'],
            true,
        );
        if ($mysql) {
            if (mysqli_select_db($phpbb3_prefs['mysqlDB'], $mysql)) {
                ...
                // search user
                $res = $phpbb3DB->Query(
                    'SELECT `user_id`,`user_password`,`user_email` FROM ' .
                        $phpbb3_prefs['mysqlPrefix'] .
                        'users WHERE `username`=? AND (`user_type`=0 OR `user_type`=3)',
                    $userName,
                );
                if ($res->RowCount() == 0) {
                    return false;
                ...
                if (
                    ...
                    if (BMUser::GetID($myUserName) == 0) {
                        PutLog(
                            sprintf(
                                'Creating b1gMail user for phpBB3 user <%s> (%d)',
                                $userName,
                                $row['user_id'],
                            ),
                            PRIO_PLUGIN,
            ...
            PutLog(
                'MySQL connection to phpBB3 db failed',
                PRIO_PLUGIN,
                __FILE__,
                __LINE__,
            );
    ...
    function _prefsPage() {
        ...
        if (isset($_REQUEST['do']) && $_REQUEST['do'] == 'save') {
            $db->Query(
                'UPDATE {pre}phpbb3_plugin_prefs SET enableAuth=?,mysqlHost=?,mysqlUser=?,mysqlPass=?,mysqlDB=?,mysqlPrefix=?,userDomain=?',
        ...
        $tpl->assign(
            'page',
            $this->_templatePath('phpbb3auth.plugin.prefs.tpl'),
        );
    }
    ...
    function phpbb_check_hash($password, $hash) {
        ...
        if (strlen($hash) == 34) {
            return $this->_hash_crypt_private($password, $hash, $itoa64) ===
                $hash
                ? true
                : false;
        }
